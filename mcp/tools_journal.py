...
def tool_get_journal_insights(args):
    ...
    for item in items:
        ...
        if ownership:
            ownership_scores.append(float(ownership))

        social = item.get("enriched_social_quality")
        if social:
            social_entries.append(social)

        if item.get("enriched_flow"):
            flow_count += 1
        if item.get("enriched_alcohol"):
            alcohol_days += 1

        quote = item.get("enriched_notable_quote")
        if quote:
            quotes.append({"date": date, "quote": quote})

    def rank_items(items_list, top_n=8):
        counts = {}
        for i in items_list:
            counts[i] = counts.get(i, 0) + 1
        return [{"item": k, "count": v} for k, v in sorted(counts.items(), key=lambda x: -x[1])[:top_n]]
        ...
        if subj_sleep and obj_sleep:
            if subj_sleep <= 2 and obj_sleep >= 80:
                divergences.append(
                    {
                        "date": date,
                        "type": "sleep_misperception_negative",
                        "subjective": subj_sleep,
                        "objective": obj_sleep,
                ...
                divergences.append(
                    {
                        "date": date,
                        "type": "sleep_misperception_positive",
                        "subjective": subj_sleep,
                        "objective": obj_sleep,
        ...
        if subj_stress and obj_recovery:
            if subj_stress >= 4 and obj_recovery >= 80:
                divergences.append(
                    {
                        "date": date,
                        "type": "psychological_not_physiological",
                        "subjective_stress": subj_stress,
                        "whoop_recovery": obj_recovery,
