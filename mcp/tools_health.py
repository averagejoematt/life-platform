...
def tool_get_health_dashboard(args):
    ...
    whoop_today = next(
        (w for w in sorted(whoop_recent, key=lambda x: x.get("date", ""), reverse=True) if w.get("recovery_score") is not None), None
    )
    if whoop_today:
    ...
    if whoop_30d:
        ...
        if hrv_vals:
            half = len(hrv_vals) // 2
            hrv_trend = (
                "improving" if sum(hrv_vals[half:]) / len(hrv_vals[half:]) > sum(hrv_vals[:half]) / len(hrv_vals[:half]) else "declining"
            )
            trends["hrv_30d"] = {"avg": round(sum(hrv_vals) / len(hrv_vals), 1), "trend": hrv_trend, "n_days": len(hrv_vals)}
        if rhr_vals:
            half = len(rhr_vals) // 2
            rhr_trend = (
                "improving" if sum(rhr_vals[half:]) / len(rhr_vals[half:]) < sum(rhr_vals[:half]) / len(rhr_vals[:half]) else "declining"
            )
...
def tool_get_body_composition_trend(args):
    ...
    for item in sorted(items, key=lambda x: x.get("date", "")):
        ...
        if mm is not None:
            pt["muscle_mass_lbs"] = round(float(mm), 1)
        if bm is not None:
            pt["bone_mass_lbs"] = round(float(bm), 1)
        if visc is not None:
            pt["visceral_fat_index"] = round(float(visc), 1)
        series.append(pt)
    ...
    if has_composition:
        ...
        if first_comp and last_comp and first_comp["date"] != last_comp["date"]:
            ...
            fat_change = (
                round(last_comp["fat_mass_lbs"] - first_comp["fat_mass_lbs"], 1)
                if "fat_mass_lbs" in last_comp and "fat_mass_lbs" in first_comp
                else None
            )
            lean_change = (
                round(last_comp["lean_mass_lbs"] - first_comp["lean_mass_lbs"], 1)
                if "lean_mass_lbs" in last_comp and "lean_mass_lbs" in first_comp
                else None
            )
            ...
            if fat_change is not None and wt_change != 0:
                ...
                if pct_fat_of_loss < 60:
                    summary["composition_alert"] = (
                        f"⚠️ Only {pct_fat_of_loss}% of weight lost is fat. Increase protein intake and resistance training to protect lean mass."
                    )
                else:
    ...
    if len(lean_pts) >= 2:
        ...
        if 7 <= days_span <= 28:  # reasonable window
            ...
            if weekly_rate < -0.5:
                lean_velocity["alert"] = (
                    f"\u26a0\ufe0f Losing {abs(weekly_rate)} lbs lean mass/week \u2014 increase protein and resistance training volume."
                )
...
def tool_get_body_composition_snapshot(args):
    ...
    if scan_date:
        ...
        if not scan:
            return {"error": f"No scan for {scan_date}", "available": [s.get("scan_date") for s in scans]}
    else:
        scan = scans[-1]
    ...
    if ffmi:
        if ffmi >= 25:
            ffmi_class = "exceptional (near natural limit)"
        elif ffmi >= 22:
            ffmi_class = "advanced"
        elif ffmi >= 20:
    ...
    result = {
        ...
        "derived_metrics": {
            "ffmi": ffmi,
            "ffmi_normalized": ffmi_norm,
            "ffmi_classification": ffmi_class,
    ...
    if posture:
        ...
        if captures:
            ...
            flags = []
            if avg_sh > 2.0:
                flags.append("Forward shoulder posture — possible upper-cross syndrome")
            if avg_hip > 2.5:
                flags.append("Forward hip — possible anterior pelvic tilt")
            result["posture_summary"] = {
                "avg_shoulder_forward_in": avg_sh,
                "avg_hip_forward_in": avg_hip,
    ...
    try:
        ...
        if withings:
            ...
            result["withings_current"] = {
                "date": lw.get("date"),
                "weight_lb": lw.get("weight_lbs"),
                "body_fat_pct": lw.get("body_fat_pct"),
                "weight_delta_since_dexa": (
                    round((lw.get("weight_lbs") or 0) - (bc.get("weight_lb") or 0), 1) if lw.get("weight_lbs") else None
                ),
...
def tool_get_health_risk_profile(args):
    ...
    if not domain or domain == "cardiovascular":
        ...
        if draws:
            ...
            ldl = _get_bm(bms, "ldl_c")
            hdl = _get_bm(bms, "hdl")
            tg = _get_bm(bms, "triglycerides")
            tc = _get_bm(bms, "cholesterol_total")
            apob = _get_bm(bms, "apolipoprotein_b")
            crp = _get_bm(bms, "crp_hs")
            ...
            if isinstance(hdl, (int, float)):
                ...
                cv["factors"].append({"marker": "HDL", "value": hdl, "unit": "mg/dL", "risk": "optimal" if hdl >= 50 else "low"})
        ...
        if cv_genes:
            cv["genetic_factors"] = [
                {"gene": s["gene"], "genotype": s.get("genotype"), "risk_level": s.get("risk_level"), "summary": s.get("summary")}
                for s in cv_genes
            ]
            ...
            if whoop:
                ...
                if hrvs:
                    cv["factors"].append(
                        {
                            "marker": "30d avg HRV",
                            "value": round(sum(hrvs) / len(hrvs), 1),
                            "unit": "ms",
                            "note": "Higher = better CV health",
                        }
                    )
        except Exception:
            pass
                ...
                cv["factors"].append(
                    {
                        ...
                        "note": (
                            "Age-extrapolated — validated for 40-79"
                            if any("extrapolated" in str(c) for c in ascvd_caveats)
                            else "Within validated age range"
                        ),
    ...
    if not domain or domain == "metabolic":
        ...
        if draws:
            bms = draws[-1].get("biomarkers", {})
            glu = _get_bm(bms, "glucose")
            a1c = _get_bm(bms, "hba1c")
            ...
            if isinstance(glu, (int, float)):
                ...
                glu_trend = [
                    {"date": d.get("draw_date"), "value": _get_bm(d.get("biomarkers", {}), "glucose")}
                    for d in draws
                    if isinstance(_get_bm(d.get("biomarkers", {}), "glucose"), (int, float))
                ]
                if len(glu_trend) >= 2:
        ...
        if fto or irs:
            ...
            if fto:
                ...
                met["genetic_factors"].append(
                    {
                        "cluster": "FTO obesity variants",
                        "total": len(fto),
                        "unfavorable": unfav,
                        "implication": "Exercise + protein + PUFA mitigate risk",
                    }
                )
            for s in irs:
                met["genetic_factors"].append(
        ...
        if draws:
            ...
            if a1c_vals:
                lon["factors"].append(
                    {
                        "marker": "HbA1c range",
                        "value": f"{min(a1c_vals)}-{max(a1c_vals)}%",
                    ...
                    lon["factors"].append(
                        {
                            ...
                            "value": round(sum(hrvs) / len(hrvs), 1),
                            "unit": "ms",
                            "note": "Higher HRV correlates with longevity",
                        }
                    )
        except Exception:
            pass
...
def tool_get_next_lab_priorities(args):
        ...
        recs.append(
            {
                ...
                "action": "Target 50-80 ng/mL with D3+K2",
                "cadence": "quarterly",
            }
        )
    ...
    if slco:
        recs.append(
            {
                "test": "CK + liver enzymes (pre-statin baseline)",
                "priority": "medium",
    ...
    if draws:
        ...
        if ldl_flags >= 2:
            recs.append(
                {
                    "test": "NMR LipoProfile (advanced lipid panel)",
                    "priority": "high",
    ...
    recs.append(
        {
            "test": "CMP + CBC + HbA1c + lipids",
            "priority": "routine",
            "reason": "Baseline monitoring",
            "cadence": "annually",
            "last_tested": latest_date,
        }
    )
    ...
    return {
        ...
        "latest_draw": latest_date,
        "total_historical_draws": len(draws),
        "genome_snps_analyzed": len(genome_snps),
...
def tool_get_day_type_analysis(args):
    ...
    if rest_hrv and hard_hrv:
        ...
        if diff > 10:
            insights.append(
                f"HRV drops {diff} ms on hard vs rest days — significant recovery impact. Prioritize sleep after hard sessions."
            )
        elif diff > 5:
        ...
        try:
            ...
            if len(strava_data) >= 7:
                ...
                if len(weeks_sorted) >= 4:
                    ...
                    volume_trend = (
                        "increasing" if second_avg > first_avg * 1.1 else ("decreasing" if second_avg < first_avg * 0.9 else "stable")
                    )
                    ...
                    result["fitness"] = {
                        ...
                        "attia_assessment": (
                            "Meeting Zone 2 target" if z2_pct >= 90 else f"Zone 2 at {z2_pct}% of target — increase easy cardio"
                        ),
                    }
                else:
    ...
    if domain in ("all", "recovery"):
        try:
            ...
            for item in whoop_normd:
                ...
                if rec:
                    recovery_vals.append(float(rec))
                if eff:
                    sleep_eff_vals.append(float(eff))
            ...
            if len(hrv_vals) >= 14:
                ...
                hrv_trend = (
                    "improving"
                    if sum(second_half) / len(second_half) > sum(first_half) / len(first_half) * 1.05
                    else ("declining" if sum(second_half) / len(second_half) < sum(first_half) / len(first_half) * 0.95 else "stable")
                )
                recovery_result["hrv"] = {
            ...
            if len(rhr_vals) >= 14:
                ...
                rhr_trend = (
                    "improving"
                    if sum(second_half) / len(second_half) < sum(first_half) / len(first_half) * 0.97
                    else ("worsening" if sum(second_half) / len(second_half) > sum(first_half) / len(first_half) * 1.03 else "stable")
                )
                recovery_result["rhr"] = {
            ...
            if len(sleep_eff_vals) >= 14:
                ...
                eff_trend = (
                    "improving"
                    if sum(second_half) / len(second_half) > sum(first_half) / len(first_half) + 1
                    else ("declining" if sum(second_half) / len(second_half) < sum(first_half) / len(first_half) - 1 else "stable")
                )
        ...
        try:
            ...
            if len(glucose_vals) >= 7:
                ...
                glucose_trend = (
                    "improving"
                    if sum(second_half) / len(second_half) < sum(first_half) / len(first_half) - 1
                    else ("worsening" if sum(second_half) / len(second_half) > sum(first_half) / len(first_half) + 1 else "stable")
                )
