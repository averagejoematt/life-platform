...
.gyc-fig { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
...
@media (min-width: 720px) { .gyc-chartwrap { grid-template-columns: minmax(0, 320px) minmax(0, 1fr); align-items: start; } }
.gyc-chart { position: relative; min-width: 0; }
    ...
    return {
        ...
        "matthew": {
            ...
            "provenance_line": (
                ...
                f"payload, fetched {mprov['fetched_at']}. {mprov['honest_note']}"
            ),
        ...
        "example": {
            "name": example["name"],
            "provenance_line": (
                f"SYNTHETIC — not real data and not anyone's real coach. {eprov['why']} "
                "It is here so the page has something to render before you paste your own."
            ),
