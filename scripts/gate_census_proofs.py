...
# ── #3669: the source-registry partition-coverage guard ──────────────────────────────
#
# Four mutations, all in the REAL tracked tree (no copy, no synthetic fixture), each one a
# state main has actually been in — and each md5-verified changed before the verdict was
# read, so "the mutation didn't apply" and "the guard can't fail" could not be confused.
STRUCTURAL_HAND_PROOFS.update(
    {
        "structural::test_source_registry_coverage_3669.py": {
            "gate_name": "test_source_registry_coverage_3669.py",
            "command": "python3 -m pytest tests/test_source_registry_coverage_3669.py -q   # 36 tests; baseline 36 passed",
            "mutation": (
                "M1 (box 1, the pre-#3669 state of main): `lambdas/web/site_api_status.py` put `labs` back into "
                "the serving-layer literal and flipped the splice order so the hardcode beats the registry — "
                '`_DUE_MONTHS_UNREGISTERED = {"labs": 3, …}` / `DUE_MONTHS = {**due_months(), '
                "**_DUE_MONTHS_UNREGISTERED}`. That IS the drift the issue reports: two homes for one cadence, "
                "serving layer winning. "
                "M2 (box 3, the planted unregistered partition the acceptance names): the `dexa` entry deleted "
                "from `UNREGISTERED_PARTITIONS` in `lambdas/ingestion/source_registry.py`, leaving a live, "
                "captured-class partition with no disposition of any kind. "
                "M3 (issue 3571): macrofactor's `capture_channel: None` + `capture_channel_reason` deleted — "
                "the LITERAL state of main before this PR, the specimen 3571 was filed on. "
                "M4 (the denominator): `deploy/generated/pk_family_census.json` truncated to 3 families / 12 "
                "items — the vacuous-scan trap, i.e. the artifact that would turn the whole file green by "
                "erasing what it grades."
            ),
            "observed": (
                "Watched 2026-09-17, each mutation applied and reverted one at a time with md5 recorded on both "
                "sides. BASELINE 36 passed (site_api_status.py 0af2110c…, source_registry.py 424ed21d…, "
                "pk_family_census.json a9a24043…). "
                "M1 (0af2110c… -> a1ac2684…): 2 failed — test_the_status_panel_and_the_registry_cannot_disagree "
                "('assert 3 == 6') and test_the_serving_layer_no_longer_restates_a_registered_cadence ('labs is "
                "back in the serving-layer literal'). "
                "M2 (424ed21d… -> b0daf1a0…): 3 failed — test_the_set_is_empty named it by name ('1 live SOURCE# "
                "partition(s) have NO disposition: [\\'dexa\\']'), and the planted-partition control reported "
                "['__planted_unregistered_partition_3669__', 'dexa'] rather than the plant alone. "
                "M3 (424ed21d… -> 62af2a58…): 2 failed — test_every_manual_source_answers printed 'macrofactor: "
                "no capture_channel key at all (reads identically to nobody having looked)'. "
                "M4 (a9a24043… -> 0d5e2e07…): 4 failed — 'only 3 pk families — a truncated scan, not the live "
                "table', 'whoop absent from the census', and the stale-exemption check fired too ('dexa is not "
                "live — the exemption describes nothing'). "
                "Every revert restored the ORIGINAL md5 exactly and returned 36 passed. Pytest-level evidence, "
                "not a live CI observation."
            ),
            "scope": (
                "The live leg grades a COMMITTED SNAPSHOT (deploy/generated/pk_family_census.json, captured "
                "2026-09-17 by a read-only scan through lambdas/experiment/pk_census.py — CI has no AWS "
                "credentials). It therefore says nothing about a partition created AFTER that capture until the "
                "artifact is refreshed; MAX_CENSUS_AGE_DAYS=180 is what eventually says so out loud, and the "
                "nightly qa_smoke census still watches the taxonomy-totality half live. It also says nothing "
                "about a genuinely ingested source MISCLASSIFIED into phase_taxonomy's EXPERIMENT_SCOPED / "
                "SYSTEM_STATE classes, which are auto-dispositioned here — that source would be wiped by the "
                "next experiment reset, a louder failure than this file. Finally, the capture-channel half "
                "asserts that a manual source ANSWERS the question, never which answer is right: whether "
                "'dropbox' joins the #746 channel set is an owner ruling, open on issue 3571."
            ),
            "proved_on": "2026-09-17",
        }
    }
