...
# ─────────────────────────────────────────────────────────────────────────────
# Flag-precision samples (#2639 box 3). A risk flag from `_static_source_flags` (below,
# ~836-851) is a SYNTACTIC lead, not a verdict — this is the hand-adjudicated false-
# positive rate on the two flags large enough that the report used to call them
# "unmeasured". Same idiom as `Proof.proved_on` above: `sampled_on` is an ISO date so a
# stale sample is visible as stale, and `n_flagged` is the flag's population AT SAMPLE
# TIME so `_render_error_bars` can compare it to the live count and say so if they drift,
# rather than silently trusting a number that no longer matches the source.
# ─────────────────────────────────────────────────────────────────────────────
...
FLAG_PRECISION: dict[str, FlagPrecisionSample] = {
    # 13-of-38 and 10-of-27, deterministic every-3rd sample, adjudicated against this
    # module's own flag definitions (`_static_source_flags`) rather than judgement calls.
    # Three of the sampled hits were real true positives (#2639's third comment):
