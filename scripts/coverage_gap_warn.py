...
WHAT IT DOES:
  Parses the `line-rate` attribute off the <coverage> root of a Cobertura
  coverage.xml (produced by `pytest --cov ... --cov-report=xml`), compares
  measured line coverage to the enforced floor, and emits a GitHub Actions
  `::warning::` annotation when the gap exceeds a threshold (default 10 points) —
  so a human is reminded to ratchet the floor up. It NEVER fails the build:
  every error path (missing file, unparseable XML, missing attribute) is
  fail-open — it prints a note and exits 0. The floor enforcement itself stays
  the job of `--cov-fail-under`; this is advisory only.
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(description="Warn when the CI coverage floor lags measured coverage (advisory, fail-open).")
    ...
    if message:
        ...
        print(
            f"coverage_gap_warn: floor {args.floor:.0f}% vs measured {measured_pct:.1f}% within {args.gap_threshold:.0f}pt threshold; OK."
        )
    return 0
