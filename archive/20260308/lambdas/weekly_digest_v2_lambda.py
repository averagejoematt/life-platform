"""
Weekly Digest Lambda — v4.3.0 (Board Centralization)
...
v4.3.0: Board prompt dynamically built from s3://matthew-life-platform/config/board_of_directors.json
        Falls back to hardcoded _FALLBACK_BOARD_PROMPT if S3 config unavailable.
...
try:
    ...
    logger.warning("[weekly] board_loader not available — using fallback prompts")
...
def _build_weekly_prompt_from_config():
    """Build the weekly board prompt dynamically from S3 board config.

    Returns the prompt template string (with {grade_summary} and {data_json} placeholders),
    or None if config unavailable.
    """
    if not _HAS_BOARD_LOADER:
        return None

    config = board_loader.load_board(s3, DASHBOARD_BUCKET)
    if not config:
        return None

    members = board_loader.get_feature_members(config, "weekly_digest")
    if not members:
        logger.warning("[weekly] No members configured for 'weekly_digest' feature")
        return None
    ...
    for mid, member, feat_cfg in members:
        ...
        focus = feat_cfg.get("prompt_focus", "Provide your weekly analysis.")
    ...
    prompt = f"""You are the coordinating intelligence for Matthew's Weekly Health Board of Advisors.
...
Be honest. Be a coach, not a cheerleader."""
    ...
    logger.info("[weekly] Built prompt from config with %d board members", len(members))
    return prompt


# Fallback prompt (original hardcoded version, used if S3 config unavailable)
_FALLBACK_BOARD_PROMPT = """You are the coordinating intelligence for Matthew's Weekly Health Board of Advisors.
...
def call_haiku(data, profile, api_key):
    ...
    prompt_template = _build_weekly_prompt_from_config()
    if prompt_template:
        print("[INFO] Using config-driven weekly board prompt")
    else:
        print("[INFO] Using fallback hardcoded weekly board prompt")
        prompt_template = _FALLBACK_BOARD_PROMPT
...
def lambda_handler(event, context):
    print("[INFO] Weekly Digest v4.3.0 (Board Centralization) starting...")
    result = gather_all()
