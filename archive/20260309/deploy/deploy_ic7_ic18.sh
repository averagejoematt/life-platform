...
echo "=== Life Platform v2.89.0 Deploy: IC-18 hypothesis-engine (steps 3-4 only) ==="
...
bash "${PROJECT_ROOT}/deploy/deploy_lambda.sh" daily-brief \
    "${LAMBDA_DIR}/daily_brief_lambda.py" \
    --extra-files \
    "${LAMBDA_DIR}/ai_calls.py" \
    "${LAMBDA_DIR}/html_builder.py" \
    "${LAMBDA_DIR}/output_writers.py" \
    "${LAMBDA_DIR}/board_loader.py" \
    "${LAMBDA_DIR}/scoring_engine.py" \
    "${LAMBDA_DIR}/character_engine.py" \
    "${LAMBDA_DIR}/insight_writer.py"
echo "    Waiting 10s..."
sleep 10
