...
def email_chronicle_approve() -> list[iam.PolicyStatement]:
    ...
    #2977: this role SAID "no ai-keys — content was pre-generated" long after the
    publish path grew three write-side effects the grants never followed: the #1384
    recall hook (Titan embed via Bedrock + a put_item into the recall partition), the
    Phase-3 recap commit (put_item), and the #405 share kit (PutObject under
    generated/moments/). All three failed AccessDenied on the 2026-08-21 auto-publish
    sweep — fail-soft, so the 2026-08-18 installment published with a silent hole in
    the recall corpus (the third recurrence of the class: #2705, #2858). The grants
    below are the root-cause fix; the loud half is the `recall-index-failed-*` alarm
    ...
    return [
        iam.PolicyStatement(
            ...
            # PutItem (#2977): the recall-corpus row (recall_indexer.index_document) and
            # the on-publish recap commit (_commit_recap) are both put_item writes.
        ...
        iam.PolicyStatement(
            sid="S3Write",
            actions=["s3:PutObject"],
            # generated/moments/* (#2977): the #405 share kit lands at
            # chronicle_share_kit.kit_s3_key = generated/moments/share-kits/<slug>/kit.json.
            resources=_s3("blog/*", "site/journal/*", "generated/journal/*", "generated/moments/*"),
        ),
        # #2977: the #1384 recall hook embeds the published installment via Titan
        ...
        iam.PolicyStatement(
            ...
            # without the read the guard fails open and a paused tier would still spend.
            actions=["ssm:GetParameter"],
            resources=[f"arn:aws:ssm:{REGION}:{ACCT}:parameter/life-platform/budget-tier"],
        ),
        iam.PolicyStatement(
