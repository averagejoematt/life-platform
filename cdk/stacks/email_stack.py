...
class EmailStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str,
        ...
        create_platform_lambda(self, "MonthlyDigest", function_name="monthly-digest", handler="monthly_digest_lambda.lambda_handler", source_file="lambdas/monthly_digest_lambda.py", schedule="cron(0 16 ? * 1#1 *)", timeout_seconds=120, memory_mb=256, custom_policies=rp.email_monthly_digest(), **shared)

        create_platform_lambda(self, "NutritionReview", function_name="nutrition-review", handler="nutrition_review_lambda.lambda_handler", source_file="lambdas/nutrition_review_lambda.py", schedule="cron(0 17 ? * SAT *)", timeout_seconds=120, memory_mb=256, custom_policies=rp.email_nutrition_review(), **shared)

        create_platform_lambda(self, "WednesdayChronicle", function_name="wednesday-chronicle", handler="wednesday_chronicle_lambda.lambda_handler", source_file="lambdas/wednesday_chronicle_lambda.py", schedule="cron(0 15 ? * WED *)", timeout_seconds=120, memory_mb=256, custom_policies=rp.email_wednesday_chronicle(), **shared)

        create_platform_lambda(self, "WeeklyPlate", function_name="weekly-plate", handler="weekly_plate_lambda.lambda_handler", source_file="lambdas/weekly_plate_lambda.py", schedule="cron(0 2 ? * SAT *)", timeout_seconds=120, memory_mb=512, custom_policies=rp.email_weekly_plate(), **shared)

        create_platform_lambda(self, "MondayCompass", function_name="monday-compass", handler="monday_compass_lambda.lambda_handler", source_file="lambdas/monday_compass_lambda.py", schedule="cron(0 15 ? * MON *)", timeout_seconds=120, memory_mb=512, custom_policies=rp.email_monday_compass(), **shared)

        create_platform_lambda(self, "BrittanyWeeklyEmail", function_name="brittany-weekly-email", handler="brittany_email_lambda.lambda_handler", source_file="lambdas/brittany_email_lambda.py", schedule="cron(30 17 ? * 1 *)", timeout_seconds=90, memory_mb=256, custom_policies=rp.email_brittany(), **shared)
