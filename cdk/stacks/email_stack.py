...
Lambdas (8):
  ...
  partner-weekly-email   cron(30 17 ? * 1 *)     — Sunday 9:30 AM PT
...
#       wednesday-chronicle weekly-plate monday-compass partner-weekly-email; do
...
ROLE_ARNS = {
    ...
    "partner":        _role("life-platform-email-role"),     # verified 2026-03-09
}
...
class EmailStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str,
        ...
        # ── Two shared dicts ──────────────────────────────────────────────────────────────
        # partner-weekly-email uses life-platform-email-role (shared, pre-SEC-1, no SQS perm).
        ...
        # ══════════════════════════════════════════════════════════════
        # 8. partner-weekly-email — Sunday 9:30 AM PT
        # Partner accountability email — sent after Matthew's weekly digest
        # ══════════════════════════════════════════════════════════════
        create_platform_lambda(
            self, "PartnerWeeklyEmail",
            function_name="partner-weekly-email",
            handler="partner_email_lambda.lambda_handler",
            source_file="lambdas/partner_email_lambda.py",
            schedule="cron(30 17 ? * 1 *)",
            timeout_seconds=90,
            memory_mb=256,
            existing_role_arn=ROLE_ARNS["partner"],
