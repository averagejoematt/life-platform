...
def lambda_handler(event, context):
    wake_date = event.get("date") or (
        datetime.now(timezone.utc) - timedelta(days=1)
    ).strftime("%Y-%m-%d")

    print(f"Eight Sleep ingestion — wake_date={wake_date}")
    ...
    try:
        result = ingest_day(wake_date, secret)
    except urllib.error.HTTPError as e:
        if e.code == 401:
    ...
    return {
        ...
        "body": json.dumps({
            ...
            "deep_pct":              result.get("deep_pct"),
            "rem_pct":               result.get("rem_pct"),
            "sleep_onset_hour":      result.get("sleep_onset_hour"),
            "sleep_midpoint_hour":   result.get("sleep_midpoint_hour"),
            "hrv_avg":               result.get("hrv_avg"),
        }, default=str),
    }
