...
# ── Core ingestion ─────────────────────────────────────────────────────────────
def ingest_day(target_date: str, secret: dict) -> dict:
...
def lambda_handler(event, context):
    ...
    print(f"Garmin ingestion — date={target_date}")
    ...
    if not result:
        return {
            "statusCode": 204,
            "body": json.dumps({"message": f"No Garmin data for {target_date}"}),
        }
    ...
    return {
        ...
        "body": json.dumps({
            "date":               target_date,
            "resting_heart_rate": result.get("resting_heart_rate"),
            "hrv_last_night":     result.get("hrv_last_night"),
            "hrv_status":         result.get("hrv_status"),
            "avg_stress":         result.get("avg_stress"),
            "body_battery_end":   result.get("body_battery_end"),
            "body_battery_high":  result.get("body_battery_high"),
            "avg_respiration":    result.get("avg_respiration"),
            "steps":              result.get("steps"),
            "fields_written":     len(result),
        }, default=str),
    }
