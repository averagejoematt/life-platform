...
def test_genesis_empty_returns_zero_with_warning():
    with patch.object(vf, "load_config", return_value=_cfg()), patch.object(vf, "_query_range", side_effect=_fake_query({})):
    ...
    strava = [
        {
            ...
            "activities": [{"sport_type": "Walk", "distance_miles": 3.0}, {"sport_type": "Run", "distance_miles": 2.0}],
        },
        {"total_distance_miles": 4.0, "activities": [{"sport_type": "VirtualRide", "distance_miles": 4.0}]},
    ]
    with (
        patch.object(vf, "load_config", return_value=_cfg(rate_per_mile=1.0)),
...
def test_additive_hevy_meters_to_miles():
    strava = [{"total_distance_miles": 2.0, "activities": [{"sport_type": "Walk", "distance_miles": 2.0}]}]
    hevy = [{"exercises": [{"sets": [{"distance_m": 1609.34}, {"distance_m": 3218.68}]}]}]
    with (
        patch.object(vf, "load_config", return_value=_cfg(extra_sources=["hevy"])),
        patch.object(vf, "_query_range", side_effect=_fake_query({"strava": strava, "hevy": hevy})),
    ):
        out = vf.compute_vacation_fund(start_date="2026-06-01", end_date="2026-06-08")
...
def test_additive_macrofactor_yards_to_miles():
    ...
    with (
        patch.object(vf, "load_config", return_value=_cfg(extra_sources=["macrofactor_export"])),
        patch.object(vf, "_query_range", side_effect=_fake_query({"macrofactor_workouts": mf})),
    ):
        out = vf.compute_vacation_fund(start_date="2026-06-01", end_date="2026-06-08")
...
def test_rate_and_manual_adjustment_apply():
    ...
    with (
        patch.object(vf, "load_config", return_value=_cfg(rate_per_mile=2.0, manual_adjustment_usd=50.0)),
        patch.object(vf, "_query_range", side_effect=_fake_query({"strava": strava})),
    ):
        out = vf.compute_vacation_fund(start_date="2026-06-01", end_date="2026-06-08")
...
def test_sport_type_filter_restricts_and_skips_extras():
    strava = [
        {
            "total_distance_miles": 7.0,
            "activities": [{"sport_type": "Run", "distance_miles": 5.0}, {"sport_type": "Walk", "distance_miles": 2.0}],
        }
    ]
    hevy = [{"exercises": [{"sets": [{"distance_m": 1609.34}]}]}]
    with (
        patch.object(vf, "load_config", return_value=_cfg(included_sport_types=["Run"], extra_sources=["hevy"])),
        patch.object(vf, "_query_range", side_effect=_fake_query({"strava": strava, "hevy": hevy})),
    ):
        out = vf.compute_vacation_fund(start_date="2026-06-01", end_date="2026-06-08")
...
def test_start_end_override_beats_config_default():
    ...
    with patch.object(vf, "load_config", return_value=_cfg(start_date="2020-01-01")), patch.object(vf, "_query_range", side_effect=_q):
