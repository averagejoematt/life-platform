    ...
    return {
        "3601968B": [   # db_bench_press_flat
            {"date": "2026-05-24",
             ...
             "top_weight_kg": 60.0},
            {"date": "2026-05-17",
             "sets": [{"weight_kg": 57.5, "reps": 8},
                      {"weight_kg": 57.5, "reps": 8}],
             "top_weight_kg": 57.5},
        ],
        "6A6C31A5": [   # lat_pulldown
            {"date": "2026-05-24",
             "sets": [{"weight_kg": 45.0, "reps": 12}],
             "top_weight_kg": 45.0},
        ],
        # one-session-only lift
        "DEADBEEF": [
            {"date": "2026-05-24",
             "sets": [{"weight_kg": 20.0, "reps": 10}],
             "top_weight_kg": 20.0},
        ],
    }
...
def test_render_cue_rounds_weight_to_half_kg():
    facts = {
        "sessions_count": 1, "last_date": "2026-05-24",
        "last_top_weight_kg": 57.49, "last_reps_list": [8, 8, 7],
    }
...
def test_load_recent_history_skips_legacy_aggregates():
    ...
    items = [
        {"pk": "USER#matthew#SOURCE#hevy", "sk": "DATE#2026-05-24",
         "workouts": [{"title": "old aggregate"}]},
        _ddb_item("2026-05-24", [{"template_id": "X",
                                  "sets": [{"weight_kg": Decimal("50"), "reps": Decimal("10")}]}]),
    ]
...
def test_load_recent_history_orders_sessions_most_recent_first():
    items = [
        _ddb_item("2026-05-10", [{"template_id": "X",
                                  "sets": [{"weight_kg": Decimal("40"), "reps": Decimal("10")}]}]),
        _ddb_item("2026-05-24", [{"template_id": "X",
                                  "sets": [{"weight_kg": Decimal("50"), "reps": Decimal("10")}]}]),
    ]
...
def test_load_recent_history_decimals_become_floats_for_arithmetic():
    items = [
        _ddb_item("2026-05-24", [{"template_id": "X",
                                  "sets": [{"weight_kg": Decimal("57.5"), "reps": Decimal("8")}]}]),
    ]
...
def test_anti_hallucination_render_quotes_only_source_numbers():
    ...
    for template_id in index:
        ...
        assert not leaked, (
            f"Template {template_id}: numeric tokens {leaked} not in source facts. "
            f"Cue={cue!r}; allowed={sorted(allowed)}"
        )
