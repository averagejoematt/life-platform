...
#
# The two PT-evening pins that used to live here (quality_gate_event's own
# `date.today()`-vs-Pacific fallback, and the ai_calls.py:1316 -> :812 ->
# cycle_gate_params chain) are GENERALIZED into the standing #2813 sweep —
# tests/test_pt_day_contract_sweep_2813.py's
# `test_registered_entry_agrees_with_pacific_at_a_pt_evening_instant[ai.quality_gate_contract::quality_gate_event]`
# and `test_quality_gate_wire_chain_agrees_at_a_pt_evening_instant` (which goes
# one step further: it drives the REAL `coach_quality_gate._number_grounding_report`
# gate, not just `cycle_gate_params` directly, and asserts on the resulting
# clean/ungrounded VERDICT rather than only the date string). #2813's whole
# point is that a producer/gate day-default pin should not accrete as one file
# per incident — kept here would have made this the second such file after
# #2675's.
