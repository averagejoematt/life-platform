...
def test_i2_source_file_syntax_valid(rel_path):
    ...
    try:
        ...
        pytest.fail(f"I2 FAIL: {rel_path} has a syntax error: {e}\n" f"  Fix the syntax before deploying.")
...
def test_i3_handler_signature(rel_path):
    ...
    assert params[0] == "event", f"I3 FAIL: {rel_path} lambda_handler first param is '{params[0]}', expected 'event'."
    assert params[1] == "context", f"I3 FAIL: {rel_path} lambda_handler second param is '{params[1]}', expected 'context'."
    ...
    for dirpath, _dirs, filenames in os.walk(LAMBDAS_DIR):
        for fname in filenames:
            if fname.endswith("_lambda.py") or fname == "weather_handler.py":
                candidate_files.add(os.path.relpath(os.path.join(dirpath, fname), ROOT))
...
def test_i6_mcp_server_handler():
    ...
    assert os.path.exists(full_path), f"I6 FAIL: MCP server entry point '{_MCP_SOURCE}' not found on disk."
    ...
    func = _find_lambda_handler(tree)
    assert func is not None, f"I6 FAIL: {_MCP_SOURCE} has no lambda_handler function."

    params = _handler_param_names(func)
    assert len(params) == 2 and params[0] == "event" and params[1] == "context", (
        f"I6 FAIL: {_MCP_SOURCE} lambda_handler signature is ({', '.join(params)}), " f"expected (event, context)."
    )
