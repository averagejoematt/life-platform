...
WHOLE platform" while the real ceiling is $215 base (ADR-133, #2801) / $252 in surge mode — the
...
Two guards, both offline:
  1. Truth test: with SSM mocked, the receipt's budget_ceiling_usd == the breakdown's
     ceiling (and falls back to the $215 base — never $75 — when the read fails).
...
# The base ceiling, DERIVED from the governor (via the #2898 registry that
# check_doc_facts itself reads) rather than hand-typed. Three consecutive
# amendments red this file on stale literals (#2895 at $85, #2801 at $150);
# #2896 converted one assertion to derive and it has not gone stale since.
def _governor_base_ceiling() -> float:
    import importlib.util
    from pathlib import Path

    src = Path(__file__).resolve().parent.parent / "scripts" / "budget_ceilings.py"
    spec = importlib.util.spec_from_file_location("_test_budget_ceilings", src)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return float(mod.read_family().base)


_BASE = _governor_base_ceiling()
...
def test_receipt_ceiling_equals_breakdown_ceiling(monkeypatch):
    ...
    assert f"${_BASE:.0f}" in body["note"] and "$100" in body["note"]
...
def test_receipt_falls_back_to_the_base_never_75_on_ssm_failure(monkeypatch):
    """A breakdown-read blip degrades to the ADR-133 base ($215 since #2801) — never the retired $75."""
    ssm = _FakeSSM(
        {"/life-platform/budget-tier": "0"},
    ...
    body = _payload(sad.handle_inference_receipt())
    assert body["budget_ceiling_usd"] == _BASE
...
def test_source_scan_flags_planted_defect_but_not_fixed_or_historical():
    ...
    # an ALLOWED value (the current base / surge pair) — must pass. Derived, not
    # re-pinned: this sample went stale on the $85->$150 and $150->$215 amendments.
    ok = d / "ok.py"
    ok.write_text(f'    return {{"budget_ceiling_usd": {_BASE:.0f}}}  # base\n')
