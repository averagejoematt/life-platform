...
def test_check_wired_into_qa_smoke_lambda():
    ...
    in check_steps() — the same contract check_content_cadence's wiring test
    pins (#1972)."""
    ...
    assert (
        '("subscriber_promise_truth", check_subscriber_promise_truth)' in src
    ), "check_subscriber_promise_truth is not in the check_steps() run list"
