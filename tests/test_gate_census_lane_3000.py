...
BASELINE_TOTAL_GATES = 652  # re-measured 2026-09-18 (#3514), on the tree REBASED onto e9d6f354f and with every new file COMMITTED. Both conditions are load-bearing and the second is the one that bit: gate_census enumerates the repo through git's own tracked-file listing, so an untracked new test file is invisible to it — an earlier run here reported a clean count and CI contradicted it the moment agent_commit.sh tracked the file. 'After the tree is complete' is not enough; it has to be after the tree is complete AND committed. Confirmed by id-set diff between two `gate_census.py --json` dumps, one from a separate worktree at origin/main and one from this branch, never by arithmetic: 651 -> 652, exactly {structural::test_phase_provenance_3514.py} enters and {} leaves, arriving PROVEN, so live UNPROVEN stays at 537. Prior note: re-measured 2026-09-17 (#3785 boxes 1+2) by running scripts/gate_census.py on THIS BRANCH's own committed tree and, separately, on a disposable `git archive` extraction of the merge-base (8774df695) — never by arithmetic: 650 -> 651 total, unproven UNCHANGED at 537, verdicts {can-fail (proven) 105, unproven 537, not-applicable 6, attempted-unproven 3}. Confirmed by id-set diff between the two --json dumps: exactly {guard::deploy/config_ownership_audit.py} enters, {} leaves. The entrant is the config/ OWNERSHIP registry — every `config/**.json` ruled generated-vs-hand-owned, with the not-uploadable classes held out of the twin set that site-deploy syncs (that sync is what actually pushed the June-1 hevy_template_index over the live catalogue three times; the issue had assumed an ad-hoc `aws s3 cp`). It arrives PROVEN via a GUARD_PROOFS record: four real-tree mutations, each plant's own md5 compared before/after so a no-op plant could not read as green, and M2 caught this guard's FIRST producer-identity leg passing as a substring read satisfied by the docstring that names the key — #3856's shape, four days later, in the file written by someone who had just read it. The deleted `config/hevy_template_index.json` was data, not a gate, so nothing leaves. Previous note follows. re-measured 2026-09-17 (SECOND measurement of this branch, after its gate-shaped additions were complete — the first read 649 and went stale within the same PR when the wrap-battery wiring landed a second entrant; measuring once and then adding a gate is the mistake, and CI caught it). Measured on THIS BRANCH's own tree and, separately, on a disposable `git archive origin/main` extraction of the merge-base (854d3df75) — never by arithmetic: 648 -> 650 total, unproven UNCHANGED at 537, verdicts {can-fail (proven) 104, unproven 537, not-applicable 6, attempted-unproven 3}. Confirmed by id-set diff between the two --json dumps: exactly {guard::scripts/check_merge_commit_closures.py, qa::lambdas/operational/qa_smoke_lambda.py::check_pk_family_census} enter, {} leaves. BOTH arrive PROVEN, so live UNPROVEN does not move. (1) the #3860 NIGHTLY ADR-077 totality census — the same derivation restart_pipeline Step [0] has always run (experiment.pk_census.unresolved_families), now also run by qa_smoke so an unclassified pk family is reported the day after it appears instead of at the moment an operator needs a reset; that gap IS #3860 (SOURCE#recap_cards went unclassified for ten days because the only instrument that could see it lived in deploy/, which is never staged into the Lambda bundle). Proven via a QA_PROOFS record: three arms — clean passes, a planted unclassified family FAILS naming it, a zero-item scan FAILS rather than certifying (the vacuous-scan trap) — plus a live run returning 'all 100 live pk families classify'. (2) #3863's detector D, the post-merge closing-set backstop, proven on the NATURAL EXPERIMENT rather than a plant: pointed at d681aecc6, the real 2026-09-17 merge that retired an owner-gated issue from a --body-file squash body, it reports committed={#3715,#3861} declared={#3861} and names the effect closed-by-this-commit. Previous note follows. re-measured 2026-09-16 by running scripts/gate_census.py on THIS BRANCH's own tree and, separately, on a disposable `git archive origin/main` extraction of the merge-base (854d3df75) — never by arithmetic: 648 -> 649 total, unproven UNCHANGED at 537, verdicts {can-fail (proven) 103, unproven 537, not-applicable 6, attempted-unproven 3}. Confirmed by id-set diff between the two --json dumps: exactly {qa::lambdas/operational/qa_smoke_lambda.py::check_pk_family_census} enters, {} leaves. The one entrant is #3860's NIGHTLY ADR-077 totality census — the same derivation restart_pipeline Step [0] has always run (experiment.pk_census.unresolved_families), now also run by qa_smoke so an unclassified pk family is reported the day after it appears instead of at the moment an operator needs a reset. That gap is what #3860 IS: SOURCE#recap_cards was created 2026-09-06 and went unclassified for ten days because the only instrument that could see it lived in deploy/, which is never staged into the Lambda bundle. PROVEN via a new QA_PROOFS record in scripts/gate_census_proofs.py: three arms against a paginated fake table — (A) two classified families, passed=True (negative control); (B) a planted USER#matthew#SOURCE#__planted_unclassified__ row, WARN naming the family BY NAME; (C) a zero-item scan, WARN refusing to certify totality rather than falling through to the OK line (the vacuous-scan trap) — plus one run against the LIVE table returning 'all 100 live pk families classify'. Because the entrant carries a verdict it does not move live UNPROVEN (537, unchanged). Previous note follows. re-measured 2026-09-16 by running scripts/gate_census.py on THIS BRANCH's own tree (#3785 box 3, atop main at b0d0194c1 — that merge-base measured in a disposable `git archive` extraction, never by arithmetic; this branch's own tree measured in place): 647 -> 648 total. The one entrant is guard::deploy/config_provenance_audit.py — the generated-`config/`-artifact provenance dead-man, which grades ONE live object's own `_built_at` against the clock rather than against a peer, because the existing comparison gate was green for NINE runs while the live index was clobbered (live 789 and the committed twin 789 AGREED). PROVEN via a new GUARD_PROOFS record in scripts/gate_census_proofs.py: two real-tree mutations run against LIVE S3, each watched red and reverted — M1 repointed the producer's INDEX_KEY at a non-existent object (exit 1, and the inverse arm simultaneously flagged the real object as 'ageing unwatched', which was not planned); M2 removed the config key from the EventBridge rule's description (exit 1, `cadence UNDERIVABLE` — the refusal-to-guess arm); reverted, exit 0. Its no-stamp arm carries a REPLAY of the real incident bytes from S3 version history rather than a synthetic case. M3, offline, caught the guard's own ratchet PASSING when it should have failed — the enrolment stamp leg was a substring read and `_built_at` survived on a comment line; it reads the AST now. Confirmed by id-set diff between the two --json dumps: exactly {guard::deploy/config_provenance_audit.py} enters, {} leaves. Because the entrant carries a verdict it does not move live UNPROVEN. Previous note follows. re-measured 2026-09-15 by running scripts/gate_census.py on THIS BRANCH's own tree, rebased a SECOND time (#3025, PR #3797, atop main at 46cd04289 — the merge-base measured in a disposable `git archive` extraction, never by arithmetic): 643 -> 644 total, proven UNCHANGED at 98, unproven 536 -> 537. The one entrant is the SECOND pytest step in pr-checks.yml's full-suite job (the single 'Full unit suite' step became a PARALLEL pass and a SERIAL pass), ledgered in tests/gate_census_unproven_residue.py with the reason a CI pytest step cannot be mutation-proved without pushing a deliberately failing test to the branch trying to land it. THE 643 IN THE NOTE BELOW WAS CORRECT WHEN WRITTEN AND WENT STALE UNDER THIS BRANCH: #3807 landed its own entrant (structural::test_judge_verdict_retry_3688.py) while this PR sat green, and both PRs had measured 643 from a shared base of 642. Two correct measurements, one shared base, and only the first to land may keep its number — which is the whole reason this queue is serialized. Confirmed by id-set diff between the two --json dumps: exactly {ci::pr-checks.yml::full-suite::5} enters, {} leaves. Previous note follows. re-measured 2026-09-15 by running scripts/gate_census.py on THIS BRANCH's own rebased tree (#3688, atop main at 14e8aaa93 — that merge-base commit measured in a disposable `git archive` extraction, never by arithmetic; this branch's own tree measured in place): 642 -> 643 total, 97 -> 98 proven, unproven UNCHANGED at 536. The one entrant is structural::test_judge_verdict_retry_3688.py — the SET guard over AI-judge call sites that decide on a truncated model reply — PROVEN via a MutationSpec (ARMED 1/1) + STRUCTURAL_PROOFS record in scripts/gate_census_mutations.py: M1 planted an unregistered judge under lambdas/operational/ (1 failed of 32, reverted 32 passed); M2/M2b re-planted it under mcp/ and in SUBSCRIPT form under scripts/ (1 failed each — the subscript form is the one the FIRST DRAFT of the pattern missed); M3/M3b are negative controls, the identical idiom as a leading and as a trailing comment (32 passed, no cry-wolf); M4b stripped truncation handling from the covered site in tests/visual_ai_qa.py (3 failed — the Set test naming the file plus two behavioural tests). THE MUTATION RUN WIDENED THE DETECTOR IT WAS PROVING: M4 rewrote the covered site to the membership form, semantically identical, and the first-draft pattern reported it as a phantom NEW judge while the real one sat untouched. Confirmed by id-set diff between the two --json dumps: exactly {structural::test_judge_verdict_retry_3688.py} enters, {} leaves. Because the entrant carries a verdict it does not move live UNPROVEN (536, unchanged). NOTE for whoever closes #3688: this gate judges SOURCE SHAPE only — whether a live Visual QA run actually retries a truncated verdict is an observation no offline gate can make, so that issue closes on a run, not on this test. Previous note follows. re-measured 2026-09-15 by running scripts/gate_census.py on THIS BRANCH's own rebased tree (#3804, PR #3811, atop main at 4896e5bd4 — that merge-base commit measured via a disposable `git archive origin/main` export into a scratch dir, never by arithmetic; this branch's own tree measured in place, already fully committed): 641 -> 642 total, 96 -> 97 proven, unproven UNCHANGED at 536. The one entrant is guard::scripts/check_repo_level_git_ops.py — the repository-level git-operations SET guard (#3804's own acceptance box 2) — PROVEN via a new GUARD_PROOFS record in scripts/gate_census_proofs.py: the real prohibition sentence in .claude/agents/worktree-implementer.md's item 3b was softened (the reason sentence right after it left untouched — the realistic drift mode) and watched RED live against the actual tracked file (python3 -m pytest tests/test_repo_level_git_ops_3804.py -q: 2 failed, 18 passed; CLI exit 1), then reverted via `git checkout --` and watched GREEN (20 passed; CLI exit 0). That run found a real bug in the checker's own first cut — a too-coarse blank-line-only paragraph split let an unrelated bullet's "never" satisfy a completely different bullet's missing prohibition — fixed in the same PR before this proof was recorded. Confirmed by id-set diff between the two `--json` dumps: exactly {guard::scripts/check_repo_level_git_ops.py} enters, {} leaves. Because the entrant carries a verdict it does not move live UNPROVEN (536, unchanged). Previous note follows. re-measured 2026-09-15 by running scripts/gate_census.py on THIS BRANCH's own rebased tree (#3662, atop main at a08a326d4 — that merge-base commit measured via a disposable `git worktree add --detach` checkout, never by arithmetic; this branch's own tree measured in place with `git add -A` applied): 640 -> 641 total, 95 -> 96 proven, unproven UNCHANGED at 536. The one entrant is guard::scripts/check_ingestion_hardcoded_literals.py — the #3662 generalized AST sweep over lambdas/ingestion/ for a field hard-coded to a literal while the same file reads a same-named column elsewhere — PROVEN via a new GUARD_PROOFS record in scripts/gate_census_proofs.py: two real-tree mutations, each watched RED live against the actual tracked files (not a copy) and then restored and watched green. M1: reverted measurements_ingestion_lambda.py's fixed `"measured_by": measured_by,` write to the old unconditional `"measured_by": "partner",` literal — the guard printed `measurements_ingestion_lambda.py: field 'measured_by' is hard-coded to 'partner' but the file also reads a column named 'measured_by'`, exit 1; reverted (git diff --stat clean), exit 0. M2: deleted the EXEMPTIONS["habitify_lambda.py"]["source"] entry from the guard script itself — the guard printed the `source`/`supplements` finding it normally suppresses, exit 1; reverted, exit 0. Confirmed by id-set diff between the two `--json` dumps: exactly {guard::scripts/check_ingestion_hardcoded_literals.py} enters, {} leaves. Because the entrant carries a verdict it does not move live UNPROVEN (536, unchanged). Previous note follows. re-measured 2026-09-14 by running scripts/gate_census.py on THIS BRANCH's own tree (#3642/#3799, atop main at 30cfcfdbe — that merge-base commit measured via `git archive` into a scratch tree; this branch's own tree measured in place with `git add -A` applied), never by arithmetic: 639 -> 640 total, 94 -> 95 proven, unproven UNCHANGED at 536. The one entrant is guard::scripts/check_no_verify_sites.py — the `--no-verify` commit-bypass SET guard, #3642's fourth acceptance box — PROVEN via a new GUARD_PROOFS record in scripts/gate_census_proofs.py: two real-tree mutations, each watched RED live against the actual tracked files (not a copy) and then restored and watched green. M1: moved `.claude/settings.json`'s `Bash(git commit --no-verify:*)` entry from the `ask` list to `allow` — the guard printed `POSTURE ESCALATION: ... found in .claude/settings.json's 'allow' list, not 'ask'`, exit 1; reverted (git diff --stat clean), exit 0. M2: blanked `--no-verify` off deploy/agent_commit.sh's own commit line — the guard printed `expected exactly 1 EXECUTION site, found 0`, exit 1; reverted (git diff --stat clean), exit 0. Confirmed by id-set diff between the two `--json` dumps: exactly {guard::scripts/check_no_verify_sites.py} enters, {} leaves, 0 orphan_proofs. Because the entrant carries a verdict it does not move live UNPROVEN (536, unchanged — the committed ceiling below still has headroom, not moved). CORRECTION folded into the same PR: the entrant's OWN issue (#3642) initially mis-described `.claude/settings.json`'s bypass entries as an unattended permission "grant"; live `python3 -c "import json; print(json.load(open('.claude/settings.json'))['permissions'].keys())"`-style inspection showed all four matching entries sit in `ask` (prompt-every-time), not `allow` — the guard and this record are written to the corrected, verified reading, and issue #3798 (filed on the same wrong premise) was closed not-planned as a result. Previous note follows. re-measured 2026-09-13 by running scripts/gate_census.py on THIS BRANCH's own tree (#3746, atop main at 6ec34dce4) with `git add -A` applied, never by arithmetic: 638 -> 639 total, 91 -> 92 proven, unproven UNCHANGED at 538. The one entrant is guard::lambdas/content/recap_gate.py — the daily recap card's publish gate — PROVEN via a new GUARD_PROOFS record in scripts/gate_census_proofs.py: three defects planted one at a time in the real module (step-1 fail-closed return replaced by an empty vocabulary; screen_items replaced by set(); the step-3 PrivacyViolation handler returning CLEARED instead of HELD), each watched RED and restored, 11 passed again after the third. Worth noting from that record: M1 did not fail the way the plant intended — with the explicit refusal gone, privacy_guard independently requires the same vocabulary and raises uncaught, so deleting step 1 produces a failed invocation rather than a card. Because the entrant carries a verdict it does not move live UNPROVEN. Previous note follows. re-measured 2026-09-07 by running scripts/gate_census.py on THIS BRANCH's own tree (#3690, atop main at ba1cb95aa), never by arithmetic: 637 -> 638 total, 90 -> 91 proven, unproven UNCHANGED at 538. The one entrant is structural::test_platform_stats_honesty_3690.py — PROVEN via STRUCTURAL_HAND_PROOFS in scripts/gate_census_proofs.py, and notably with NO synthetic mutation: the armed condition was the live production defect itself (/api/platform_stats serving review_grade "A" against a review that awarded A to zero of 17 lenses, site_pages 77 vs 93, active_secrets 21 vs 28). Restoring those three literals reds 4 of 6; correcting them passes 6. Because the entrant carries a verdict it does not move live UNPROVEN. Previous note follows. re-measured 2026-09-07 by running scripts/gate_census.py on THIS BRANCH's own tree (#3678, atop main at 533e226fc), never by arithmetic: 636 -> 637 total, 89 -> 90 proven, unproven UNCHANGED at 538 (derived from tests/gate_census_unproven_residue.py, not touched). The one entrant is guard::scripts/check_job_timeout_headroom.py (#3678's own timeout-minutes derivation guard) — PROVEN via a new GUARD_PROOFS record in scripts/gate_census_proofs.py: watched RED live against the real repo's then-unmodified pr-checks.yml (`timeout-minutes: 15` vs. measured p95 14.87min x 1.2 = 17.84min required), then OK against the same live measurement after the ceiling was raised to 18 in this same PR — plus the offline synthetic-fixture twin in tests/test_check_job_timeout_headroom_3678.py (test_reds_on_todays_stale_15_minute_ceiling_positive_control / test_ok_on_the_re_derived_18_minute_ceiling_negative_control). Confirmed by id-set diff against the pre-#3678 tree: exactly that one id enters, none leaves — scripts/ci_job_timeouts.py (a pure registry, no gate-shaped nonzero-exit CLI) and the ci_run_verdicts.py/check_main_green.py/check_ci_warnings.py/wait_pr_green.sh extensions (all inside the ALREADY-CENSUSED ci_run_verdicts.py::classify_cancelled_run gate, not new files matching the guard-name heuristic) add no new gate. Atop 636 (re-measured 2026-09-07 by running scripts/gate_census.py on the MERGED tree (main at 139a3d501, after #3656/#3657/#3658/#3664/#3672/#3674 + that PR's own batch), never by arithmetic: 634 -> 636 total, 88 -> 89 proven, unproven 537 -> 538. Coordinator-measured on main first (634/88/537/6, self-consistent — #3674 added no gates), so that branch's own two entrants were the WHOLE delta, confirmed by id-set diff with the tree fully staged: exactly {structural::test_a11y_ledger_3548.py, guard::scripts/playwright_gated_tests.py} enter, none leaves. structural::test_a11y_ledger_3548.py is PROVEN via STRUCTURAL_HAND_PROOFS in scripts/gate_census_proofs.py (site/coaching/read/index.html's `<div data-dx-read>` reverted in place to the pre-#3548 `<article data-dx-read>` — the real retired shape all 18 dx-read hosts carried — watched RED: 1 of 11 failed naming the file; REVERTED: 11 passed), so it does not move live UNPROVEN. guard::scripts/playwright_gated_tests.py arrives UNPROVEN by design and is ledgered in tests/gate_census_unproven_residue.py with the reason (2026-09-07): it matched the guard-name heuristic incidentally (`_gate` inside "...gated_tests.py") and its `sys.exit(main())` is syntactically indistinguishable from a real nonzero-exit gate, but `main()` always `return 0` — it is a NAMED-SKIP REPORTER (#3640's own acceptance criteria: a loud line naming each playwright-gated CI skip, never a blocking gate, since every CI run carries 5 such skips and failing on them would red every push unconditionally) — fabricating a mutation to "prove" a fail-path it must not have would be dishonest, so it is left unproven rather than gamed. Net: live UNPROVEN 537 -> 538 (+1), and the ledger itself also RETIRED two stale entries that PR's own CI run named as ratchet-down-available — registry::tests/test_module_size_guard.py::BASELINE::cdk/stacks/monitoring_stack.py and ::lambdas/coach/coach_narrative_orchestrator.py, neither of which exists in the live census any more (deleted, not merely unmatched) — so tests/gate_census_unproven_residue.py's own line count moved 540 -> 538 (+1 new, -2 stale), landing exactly on the measured live unproven count.) Atop 634 (re-measured 2026-09-06 by running scripts/gate_census.py on the MERGED tree (main after #3658/#3645 + this PR), never by arithmetic: 625 -> 634 total, 79 -> 88 proven, unproven 537 -> 537. The nine entrants are the nine rows of `tests/test_token_contrast.py::DERIVED_OPACITY_EXEMPT` — the exemption set of #3544's DERIVED recede scan, which parses every `opacity: <1` rule in every shipped stylesheet instead of listing six selectors by name (the hand list missed `.ndots-more`, and the class regrew and rolled the site back three times). ALL NINE are PROVEN in BOTH directions via REGISTRY_PROOFS in scripts/gate_census_proofs.py (delete the row -> the measured half reds naming that selector in all three palette blocks; strip the opacity out of the CSS -> the stale-exemption guard reds naming it), so live UNPROVEN does not move. The +9/+9/0 delta was verified against main's own 625/79/537 measurement, not assumed. Atop 625 re-measured 2026-09-06 on the merged tree (main after #3656's rollback fix + this PR's #3642/#3653/#3594/#3641/#3645 batch), +8 2026-09-06 (#3645: the widened tests/test_no_tool_attribution_3005.py sweep-1 mention pattern makes eight ban-STATING tracked files new ALLOWLIST entrants — ALL EIGHT PROVEN via REGISTRY_PROOFS in scripts/gate_census_proofs.py, load-bearing direction only per that block's own header (delete the entry -> the guard reds naming exactly that file; reverted -> exit 0), so live UNPROVEN does not move, 537 -> 537. Measured by id-set diff with the tree `git add`-ed: exactly eight ids enter and none leaves), atop 617 (re-measured 2026-09-06 on the merged tree (main after #3648/#3649 + this PR's proven role-family entrant), +4 2026-09-06 (#3520: `deploy/` joins tests/test_cast_roster_consistency's PROMPT_LITERAL_DIRS, and the four PROMPT_LITERAL_ALLOWLIST entries that widening required each enter as a registry gate — ALL FOUR PROVEN in both directions via REGISTRY_PROOFS in scripts/gate_census_proofs.py (delete the entry -> the guard reds naming that file; plant a non-allowlisted off-cast name in the same file -> it still reds), so live UNPROVEN does not move, 537 -> 537. Measured by id-set diff with the tree `git add`-ed: exactly four ids enter and none leaves), atop 612 +1 2026-09-06 (#3570's scripts/check_raw_zone_drift.py — a new guard-family script, measured on the tree with `git add -A` applied by scripts/gate_census.py; PROVEN via GUARD_PROOFS against the REAL pre-fix origin/main registry, watched red then clean live against S3), atop 611 (re-measured 2026-09-06 on the merged tree: main after #3637 + the #3639 tip + this PR's eight proven RECEDE_TEXT_RULES entrants), +2 2026-09-06 (#3559/#3518's two entrants, both PROVEN — structural::test_reader_input_prefix_3559.py via a MutationSpec ARMED 1/1, guard::lambdas/ai/plan_facts_gate.py via a GUARD_PROOFS record; re-measured on the merged tree by scripts/gate_census.py: 603 / 57 proven / 537 unproven), atop 601 +2 2026-09-06 (#3515/#3567's check_proof_freshness guard + the sitemap structural test, both PROVEN via STRUCTURAL_HAND_PROOFS — re-measured on the merged tree by scripts/gate_census.py), atop 599 +1 2026-09-06 (#3529/#3531/#3534's two derived mirror registries, PROVEN, net of #3568's +1 — re-measured on the merged tree by scripts/gate_census.py, not by arithmetic), atop 598 +1 2026-09-06 (#3568's sending-vocabulary census, PROVEN — 4 real-tree mutations each red their own assertion), atop 597 (#3503's repo-wide AlarmTypes sweep, atop 596, atop 595, atop 594)))
