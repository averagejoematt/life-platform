...
BASELINE_TOTAL_GATES = 651  # re-measured 2026-09-17 (#3511). Measured TWICE and never by arithmetic: on this branch's own tree (`python3 scripts/gate_census.py --json`) -> 651, and on a disposable `git archive <merge-base 8774df695>` extraction -> 650. Verdicts on the branch {can-fail (proven) 105, unproven 537, not-applicable 6, attempted-unproven 3}; unproven UNCHANGED at 537. Confirmed by id-set diff between the two --json dumps: exactly {guard::deploy/prereg_provenance_gate.py} enters, {} leaves, and it arrives PROVEN — the #3511 pre-genesis prediction provenance contract (a season PREDICTION# row presenting as pre-genesis must be in the frozen pre-registration; from genesis onward, every sealed id must be live in the season). Its GUARD_PROOFS record carries four mutations planted one at a time in the real tracked module (write-instant clause disarmed; unstamped rows treated as out-of-season; the shapeless-artifact vacuity guard softened to `set()`; the mirror clause deleted), each reverted before the next, plus the live run on which it found a real defect: 26 blocking findings against the live table, 16 of them every cycle-17 sealed bet stranded at phase=pilot. The previous line's own lesson still applies and was heeded here — measure AFTER the gate-shaped additions are complete, not before. This branch's first census run was taken with the new files still UNTRACKED and read 650 (clean); the census walks tracked files, so the entrant only appeared once it was committed, and CI is what said so.
