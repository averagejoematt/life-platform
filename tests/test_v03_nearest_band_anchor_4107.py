...
def _completed(n: int) -> list[dict]:
    """#4110: `n` loaded Hevy sessions on consecutive days from the block start — the week is
    the session sequence's (completed // 3 + 1), not a calendar week."""
    import datetime as _dt

    start = _dt.date.fromisoformat("2026-09-24")
    out = []
    for i in range(n):
...
def test_the_detraining_discount_applies_only_to_anchors_28_days_older_than_block_1(anchor_date, expected):
    assert program_structure.SESSION_SEQUENCE["block_start"] == "2026-09-24"
