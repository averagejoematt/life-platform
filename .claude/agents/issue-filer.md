...
- **Class-first filing (CONVENTIONS §10, 2026-08-23):** before filing a standalone issue, check for an open class owner — an epic or UMBRELLA story whose class the finding instantiates (`gh issue list --label type:epic --state open` plus open UMBRELLA-titled stories). An instance of an open class becomes a checkbox added to the owner's acceptance (carried verbatim — folded findings are invisible to `#NNNN`-greps) or a comment there, not a new issue. A standalone filing requires a novel class with verified evidence, or a P1/P2 whose fix shares no lane with any owner's. Per-symptom filings are the churn engine: measured 2026-08-23, 312 filed / 311 closed in 14 days while the board held ~50.
...
- **`closure:live-proof` — stamp it when the subject is an INSTRUMENT** (#3595): an alarm, gate, sweep, judge, ledger, scheduled job or fail-soft write. The label is what tells the closing side that this issue closes on its first non-degraded LIVE output (`**Live proof:** <UTC instant> — <where>`), never on the merge, and it is what arms the closure contract's `no-live-proof` block against a `Fixes #N` aimed at it (the shipping PR writes `Refs #N`) — so write the `## Outcome` sentence as the output someone will paste, not as a merge. Registry: `scripts/closure_contract.py`.
