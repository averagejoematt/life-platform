...
**Verified:** 2026-07-18 (night) — **Later-drain finish + human-gated queue cleared + cycle-8 reset to Sunday.** Matthew granted edits/merges/deploys mid-session and dispositioned every gated item. **Shipped+deployed+verified (8):** #1251/#1252/#1244 (PR #1317 cockpit trio — yesterday-scope, carry-forward markers, self-hiding cycle beat; passed gating visual-QA), #1240 (PR #1318 site_api_data split 4,184→~2,740, router parity 111→111, all 15 endpoints 200 live), #1255 (PR #1316 `allow("chronicle")` + AST no-hardcoded-tier guard), #1227 (PR #1359 + live user-named `DetectStackResourceDrift` grant, capability `simulate`→allowed; dead-capability escalation), #916 (PR #1361 cookie 30→90d, MCP boot 401), #1265 (chronicle lambdas deployed; held draft already reset-wiped), #1257-docstring (PR #1362, Refs). **Matthew ran the AWS deletes** (#1257 2 orphan rules, #1228 us-west-2 email-subscriber twin+alarm) — all verified gone. **#1266 healed:** 8,427 cycle stamps re-derived from CONTENT dates vs `CYCLE_GENESES`, re-scan 0 mismatched (near-miss: first dry-run used `tombstoned_at` = reset time — never infer generation from it). **Cycle-8 reset LIVE:** `restart_pipeline.py --genesis 2026-07-19 --override-weight-lbs 315.65 --sync-site --apply`, all hard gates PASS (40/40 rendered, semantic, reader-truth), SSM cycle=8, committed `37e09f04`; **Sunday+ run `deploy/restart_verify.py`**; prereg seed/publish left attended. **Un-red main #1416:** `.carry-tag` raw font-size passed visual-QA but redded Unit Tests via `test_css_tokens` (unit guard ≠ render gate — memory `reference_css_token_guard_vs_visual_qa`); fs-ok sanction, exact CI invocation 5,388 passed locally. #1114 v2 portrait sketches published (artifact 98729e35…, ranking v2c▸v2d▸v2b) — **Matthew picks frame+stroke next session**. NB the frontier wrap suggested absorbing #1251/#1252/#1244 — they shipped here instead. Still gated: #1243/#748/#1187/#1029; frontier backlog #1363–#1415 untouched. DLQ still 1 Withings msg (cosmetic). Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-18_FrontierPlan.md`.
