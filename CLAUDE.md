...
**Verified:** 2026-07-11 overnight ("/uplevel + full sweep — bugs, features, quality, doc drift, AI content, throughline, database + a deliberate character-math session + the Claude-self-assessment meta-question; sweep → backlog issues" → blanket approval, executed while Matthew slept). **13 PRs merged+deployed+live (#944, #979–#991), 34 issues filed (#945–#978), 11 closed same night incl. #930.** The machine: 13-lens Workflow survey (70 findings) → 12-batch adversarial verify (**67/70 confirmed** — evidence-seeded lenses beat the ~50% FP history) → ADR-099 filing → 10 parallel worktree-implementers → serial reconcile-merge → fleet 95/95 + site-api + 2 green site-deploys. Launch-eve protection shipped: privacy purge (genome identifiers out of repo tests/registry/beats, live-verified), reset-singleton guards (`singleton_visible()` — week 1 no longer narrated under cycle-4 "setback"), pilot-phase Day-1 chaining, pre-start contracts on 7 endpoints, countdown static-content sweep (home hero 315-hardcode → live-bound), habitify notes 412-since-ship, phase taxonomy made TOTAL (protects Sunday's re-run), AI trio (few-shot allow-list poisoning, AI_UNAVAILABLE sentinel, BoD stale-identity), character gate bugs (up-gate vs unboosted target + monotone XP buffer — sim: steady-good L49→L71, inversion gone). Lane B: `docs/engines/CHARACTER_MATH_AUDIT_2026-07.md` + `scripts/character_sim_year.py` (420-day trajectories; epic #956, CRITICAL #957 climb-during-darkness still open — model decision). Lane C: `docs/reviews/CLAUDE_PLATFORM_UNDERSTANDING_2026-07.md` + `/platform-review` skill + `issue-filer` agent. Suite 4607/0 green; live checks all pass. Gotchas: never chain past `git rebase` · `gh pr merge --delete-branch` from a worktree tangles primary HEAD · dual-module-identity green-solo/red-in-suite · no-TTL config caches need container recycle + CF invalidation · grep privacy scans false-positive on JSON keys. **Matthew Sunday: weigh-in → pipeline re-run (now also wipes the 23 pre-genesis singletons + re-stamps prologue cycles); then `fix_prologue_cycle_and_subscribe_ttl.py --apply`; decide #955 (presence across genesis) + #957.** Full narrative: `handovers/HANDOVER_LATEST.md`.
