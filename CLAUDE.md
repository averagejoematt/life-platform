...
**Verified:** 2026-07-21 — **The AJM brand mark lands — favicon, app icons, header (Claude Design 3a).** The site had never had a logo (bare 11px ember square + a May-19 PNG-mislabeled-`.ico`). PR #1638 (MERGED `0e5ac940`, DEPLOYED green, live) lands the full-gauge dial mark. **Source of truth = `scripts/build_brand_assets.py`** — three hand-authored masters in `site/assets/marks/` (single-A / AJM-monogram / lockup), everything else generated + `--check`-reproducible; assets pulled via the DesignSync tool. Swapped at **existing stable paths → zero HTML edits**, so all 134 pages pick it up; header markup untouched, height unchanged at 53px both themes. **Two divergences from the approved plan, both deliberate:** (1) the header uses the **single-A, not the monogram** — at the real 20px "AJM" collapses to texture (A/B-rendered), and the tab icon + header mark are now the same mark; (2) `--brand-mark` is a **token in `tokens.css`, not a rule keyed to `[data-theme="light"]`** — light mode activates on TWO legs (OS media query AND explicit toggle), and the attribute-only rule I'd planned would've shipped a dark mark on light paper for OS-light readers. **The one real defect in the delivered assets:** letters were live `<text font-family='IBM Plex Mono'>` with an empty `<defs>` (the doc's "embedded 13KB subset" claim is false) — as an `<img>`/favicon/crawler-fetch they'd fall back to platform monospace; the script **outlines the text** against the site's own self-hosted Plex Mono Medium (fontTools), verified pixel-identical to a live-font render, `grep -c '<text'`=0 on every generated file. Also added the **maskable PWA icon** the manifest never had. Gotchas: Playwright screenshot rejects a `.ico` path (shoot `.png`, rename); `design_sync_bundle` forbids absolute `url(/…)` so the mark URL is **relative** (`url(../marks/…)` — a CSS url() resolves against the stylesheet, correct at every page depth AND bundle-portable); `tokens.css` has two light legs a test asserts identical so the token went in all three declarations. Suite **6364 passed**, black/flake8/ruff/pii clean; live curl confirms favicon=new 32px A-dial + marks/manifest 200; deploy run 29869883945 all-green, no rollback. **Parallel session active throughout** — confirmed zero file overlap with `issue-1624`/`1621`/etc. and main's newer commits (#1635/#1636) don't touch `site/`. Build beat `2026-07-21-the-brand-mark`. Next-picks: **#1639** head-chrome drift (61/82 pages ship no manifest/theme-color; give `v4_apply_chrome.py` `<head>` ownership) · **#1640** brand mark on the 13 OG cards via `card_engine` · owner: confirm `lockup-*.svg` is the epic-#1619 banner asset or prune. **Live budget: tier 2** (reader narratives paused; July-only ceiling $115/$135 self-reverts 2026-08-01, ADR-133). Full narrative: `handovers/HANDOVER_LATEST.md`.
