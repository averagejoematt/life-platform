...
**Verified:** 2026-07-06 (R22 Consultancy Review — "consultancy-grade deep-dive: bugs/arch/security/tech-debt/modernization/how-we-use-Claude/Fable-opportunities; red-team; file all findings with ranking + model") — **47 verified findings filed as #779–#825** (14-lens discovery Workflow → dedup vs 61 open issues + §13b → adversarial red-team verify; 54 raw → 47 unique). Charter `docs/reviews/REVIEW_PROMPT_R22_CONSULTANCY.md`, closure `docs/reviews/REVIEW_2026-07-06_R22.md`. Each issue's `model:` label = plan-of-attack routing (sonnet=mechanical · opus=front-end/refactor · fable=security/arch/honesty/tooling). §13b in `generate_review_bundle.py` now carries the R22 table so R23 can't re-flag. **⚠️ ONE CRITICAL LIVE security exposure handled OUT-OF-BAND** (public repo + open MCP `/token` = unauth read/write of all private health data): detail is PRIVATE in memory `security-r22-mcp-token-exposure` (NOT committed — repo is public), public tracking terse as #779/#780; **remediate before disclosure.** No deploys/feature-code this session (read-only + issue filing + docs). Prior R21 line archived in `handovers/`. **PRIOR (R21 Batch 1+2):** 8 issues shipped+merged+deployed+LIVE across #774–#778. Model call: stayed on **Opus** as driver, fanned the mechanical `model:sonnet` items to Sonnet worktree subagents. **Shipped:** **#757/#754/#752** (PR #774) — R21 perimeter: **ADR-128** no standing LLM Council, **ADR-129** remediation `auto`→`shadow` (SSM flipped live; re-verified 0 merged auto-fix-safe PRs), **ADR-130** GitHub Pages **disabled** (`gh api DELETE`, 404 confirmed) + committed the R21 review record. **#756** (PR #775) — deleted the parked `hevy-webhook` FunctionURL (`cdk LifePlatformIngestion`; ResourceNotFound confirmed + reconciled ingestion to layer **v118**). **#758** (PR #776) — gated the PERMA/Seligman citation garnish on real `n` (`perma_context` ≥14 days; `cdk LifePlatformMcp`, boots 401). **#727** (PR #777, **ADR n/a**) — **scientific-liveness heartbeat**: `coach-prediction-evaluator` emits `LifePlatform/Predictions` `DecidedCount`/`GradableCount`/`DaysSinceLastDecided` (999-if-never marker); new `grading-stalled` alarm (≥14 for 2 daily periods, BREACHING) fires on the current all-pending state; `alarm_count` 109→110; verified live emit `{decided:0, gradable:127, days:999}`. **#729/#730** (PR #778) — **static-render the proof surfaces**: `scripts/v4_proof.py` bakes `<noscript>` scorecard honest-empty-state (`Evaluator live since 2026-06-14 · 309 pending · 0 graded yet · as of {date}`, ADR-104) + a dated chronicle post list into served HTML at build time (live API + `proof_snapshot.json` fallback, never fabricates; JS still enhances). Site deployed + **curl-verified in prod** (build `db889804`). **⚠️ main's ONLY red is PRE-EXISTING, not this session:** post-deploy `test_i2_lambda_layer_version_current` fails on `life-platform-freshness-checker` on layer **v116** (current v118) — it lives in the **deliberately-held** `LifePlatformOperational` (deploy held pending HAE reconcile / lands `coherence_semantic` tier-1). NOT deployed (out of authorized scope + another session's hold); I2 is non-gating (no rollback). Every other job green (Lint/Unit/Deploy-critical/Deploy/Visual-QA/Smoke). **To green main:** `cdk deploy LifePlatformOperational` — Matthew's call. **Gotchas:** CI auto-deploys on merge (my manual deploys were idempotent); MCP deploy = `cdk LifePlatformMcp` NOT `deploy_mcp_split.sh` (omits top-level `reading/` → boot break); doc-sync literals (`test_count`/`alarms`/`lambda_count`) drift across concurrent PRs — merge main into each branch + re-run `sync_doc_metadata --apply` before merging; GitHub squash rejects a branch with a merge commit even when main is an ancestor (linearize via `git reset --soft`). See `handovers/HANDOVER_LATEST.md`.
