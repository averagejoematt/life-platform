...
**Verified:** 2026-07-06 (#725 prediction-integrity + layer-drift reconcile + main CI green) — **shipped, merged, deployed, REAL-DATA-verified.** Matthew authorized **all deploys+merges** ("do it"). Single-threaded on `main` (no concurrent session). Started from a question about the home constellation, pivoted into R21's #1 move. **#725 (PR #761 → `6b4457de`; layer bump → `79b3b4aa`; layer v116):** the credibility engine's central defect — `intelligence_common.extract_thread_from_narrative:1537` let the LLM author `prediction_id`/`target_date`, so the ledger held 2024/2025-dated hallucinated IDs (**88 pending, 0 ever graded**), an ADR-106 violation; the daily writer `ai-expert-analyzer` was minting fresh corrupt records. New `stamp_thread_predictions()`: LLM writes claim text/confidence/metric/timeframe only; **code** stamps `pred_{today}_{semantic-slug}` (same slug regex as the already-correct `coach_state_updater.py` COACH# path) + strictly-future `target_date` (timeframe→window, default 14d) + `semantic_key`; **carry-forward** on semantic_key so daily re-emission updates one record (stable id+deadline) instead of the `pred_2025…` dup inflation; dropped the dated ORIENTATION_VOICE `{target_date}` promise (`:187`). Tests `tests/test_prediction_metadata_stamp_725.py` (9) + 85 related green. **THE VERIFICATION (the #412 drive-the-real-flow lesson):** invoked `ai-expert-analyzer {"expert":"training"}` LIVE on v116 → new `coach_thread#training#2026-07-06` = `pred_20260706_…`, `target_date 2026-07-20`, `semantic_key` set (vs the pre-fix `pred_20240621_nervous_system_fracture`/`pred_20250000_*` garbage still in older entries). **Layer-drift reconcile (the ACTUAL main-CI red, not #724):** v115 was published 2026-07-06 (PR #711/#417) but **never attached** — all 15 consumers stranded on v114; shipping #725 (a layer module) rebuilt → `cdk deploy LifePlatformCore` (publish v116) → `LifePlatformCompute LifePlatformEmail` (attach) → **16/16 on v116**; added `ai-expert-analyzer` (a real consumer MISSING from `ci/lambda_map.json`) → 16 tracked. **#724 closed** (its ruff red was already fixed by #759). **Main CI now GREEN** (run `79b3b4aa`: Lint+Unit+Deploy-critical+**Plan/layer-check** ✓; Deploy skipped pending prod approval, but the fix is live via the direct deploy). **Constellation aside (Matthew's opening Q):** design is right + code healthy — `/api/pillar_coupling` serves 15 real edges NOW; the no-lines screenshot was transient (all-dashed = `.node.quiet` uncoupled → drawConstellation got 0 edges that load); reload shows them. Latent, unfiled: a coupling fetch-fail renders identically to honest "no couplings" — small ADR-104 guard worth adding. **State at close:** `main`==origin (`79b3b4aa`), layer **v116**, 16/16 consumers current, worktrees main + `docs/uplevel-handover`; untracked `docs/reviews/R21_BACKLOG.md` (someone's review record, left alone). **Next:** **#726** void the legacy prediction partition (depends on #725, met; TWO stores — legacy `coach_thread#` corrupt vs already-clean `COACH#`; tombstone per ADR-077 + repoint MCP `get_predictions`→`COACH#`; **mutates prod data, confirm before tombstoning**; note legacy pk=`USER#matthew`, source in SK) · then **Session B epic #716**: #730 static-render proof surfaces + #733 permalink unification · budget pair #738/#737 · audit `ci/lambda_map.json` for other missing layer consumers. See `handovers/HANDOVER_LATEST.md`.
