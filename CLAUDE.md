...
Regenerates constants, deploys Core/Compute/Email (constants ship in every bundle — #781), phase-tags DDB, wipes intelligence, rolls the accountability ledger into a durable `LIFETIME#` aggregate + zeroes `TOTALS#current` (`deploy/restart_ledger_reset.py` — ADR-072/077), rebuilds character, curates the chronicle, syncs site + docs, verifies the 93-URL v4 surface (86 pages + 7 JSON endpoints, #918). Rollback: `deploy/restart_rollback.py`.
...
**Verified:** 2026-07-25 — **Backlog paydown (3 waves) + Horizons/diary feature builds + an owner-decision blitz.** Standing autonomous non-fable paydown + standing merge/deploy approval; interactive mid-session as Matthew worked the owner-dependent backlog (ELI5/options/rec per item). **Shipped — 12 issues closed, all merged + deployed, main GREEN:** waves 1–3 backlog — **#1572** Video Diary (#1732) · **#1569** Third Wall (#1733) · **#1379** Daily Fingerprint+`/data/wall/` (#1734) · **#1377** Wrong Feed (#1735) · **#1381** Theme River (#1736) · **#1393** Engagement Ladder (#1744) · **#1394** Cohort Strip (#1745, `COHORT#` grant CDK by Matthew) · **#1544** closed; feature builds — **#1705** Horizons S1 curation engine (#1746: pick model on the reading rail, source garden, fail-closed link-verify, MCP `curate_horizon`/`get_horizons`) · **#1707** Horizons S3 public `/data/horizons/` feed + grounded budget-gated retrospective (#1749, new `/api/horizons`) · **#1573** Whisper solo-transcription→diary (#1747) · **#1350** 18-mo subscriber-email purge + weekly rule (#1748, `LifePlatformOperational` CDK by Matthew). **Credentials wired:** `life-platform/youtube` (inbound live-invoked) + `life-platform/bluesky` (createSession-validated, staged for the unbuilt #1676/#1629 consumers). **Deliverables:** owner-decision worksheet + #741 "Proof, Not Promises" essay draft (artifacts). **Verified:** combined-tree full suite green each merge (6822→6924), load-bearing ACs genuinely-asserting (byte-identical SVG, header==cards, k-anonymity+partition-isolation, fail-closed verify, budget-gated retrospective), all 7 new endpoints pre-deployed live (autodeploy-race), five `deploy_all` + Matthew's 2 CDK deploys verified live. **Gotchas (→memory):** CI **ruff gate = 6 dirs** not `lambdas/ mcp/` (`reference_ruff_full_dir_set`); **`gh pr merge` ships the branch, not your reconciled integration tree** — land code-reconcile waves via local-merge/fast-forward (`reference_gh_merge_takes_branch_not_integration_tree`); the combined full-suite catches cross-cutting registry gaps (heartbeat exemption / MCP verb / tombstone term) that targeted agent suites miss; a site-deploy `--delete` race removed a newer page (theme-river 404 window, manual-sync recovery). **Docs:** DATA_GOVERNANCE + SCHEMA + DIARY_STUDIO_KIT updated by the PRs; sync reconciled (tools 72 / endpoints 128 / test_count 5359); checkers green. **Decisions:** none filed — governance decisions (license posture #1352, paging #1333, branch-protection #1662, embed-CSP #1678, complexity ledger #1666) are recorded on their issues and each ships its ADR at implementation. **Main:** green (`270271ca`; prior completed run `57c1365c` green, no red run). **Incidents:** 2 rows (P4 wave-2 `deploy_all` ruff-6-dir-scope lint failure, caught pre-deploy + fixed; P4 theme-river 404 window from the older site-deploy `--delete` race, manual-sync recovery). **Stash/hooks:** clean. **Labels:** OK. **Live: budget tier 1.** Build beat `2026-07-25-horizons-live`. **NEXT:** the recorded build queue from the owner blitz — doc-ADRs #1666/#1352/#1333/#1662; code #1330/#1345/#1570/#1407/#1623/#1613/#1336/#1388; credential-gated #1631/#1632/#1676+#1629/#1678+#1674 (need owner keys); deferred #1383/#1396/#1742. **Owner:** provision X/Instagram/billing-PAT creds, GitHub toggles (Dependabot+secret-scan #1336, branch-protection #1662 post-private-flip), #1029 (before 2026-08-20)/#1243/#741, the private flip (+ post-flip checklist). Full narrative: `handovers/HANDOVER_LATEST.md`.
