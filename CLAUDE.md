...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-147), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-08-02 (Day-7, solo opus session) — **six issues shipped, all merged, deployed and live-verified**: #1921 (PR **#1926**), #1909 (**#1929**), #1904 (**#1930**), #1923 (**#1932**), the #1927 alarm (**#1928**), plus a ci-cd concurrency salt (**#1933**); #1920 closed with its measurement and #1925 closed by filing **ADR-147**. **#1921 earned itself within the hour** — the deployed qa-smoke returned `failed: 1, failed_deploy_health: 0, failed_content_truth: 1`, and the OLD oracle would have reverted 100 Lambdas on that same `failed: 1`. **The session's lesson is that four of six stated premises were wrong and only measurement caught it:** (1) #1920 asked me to classify 30 days of verdicts — `reader_truth` had been **budget-paused 26 of those 30 days** (tier ≥1; `Check.pause()` sets `passed=True` and printed nothing, so 26 dark days looked identical to 26 green ones), making both sides of the noise argument moot; measured precision **2/10 = 20%, Wilson CI [5.7%, 51.0%], n=10**, all from one day, never enough to set policy; (2) #1909's claim that cost `status` is load-bearing for `overall` is **false** — but measuring found an unfiled defect, the cost block **vanished every month-start** (`Start == End` → CE `ValidationException`); (3) #1904 said 46 entries, it was **82 with 49 to fix**, and the off-roster names split two ways so a "not-in-the-retired-list" guard would have passed 18 of them; (4) #1923 looked like it covered `recovery_night_of`, which is a **different quantity** entirely. **Gotchas:** a run can report `Deploy: skipped` AND `success` while leaving a merged change **undeployed** (#1923 sat live-less ~1h; only downloading the zip caught it); `git checkout --ours` on a rebase conflict takes main's WHOLE file and would have silently deleted a new helper; **I chained past a failed `git rebase --continue`**, amended the wrong commit and force-pushed it (recovered via `--abort`, by luck); I **hand-listed an allowlist** in the #1904 guard and got it wrong — the exact drift class I was fixing; every PR collides on the `test_count` literal, so merge one and rebase the next; doc-sync stamps **UTC** and rolled ~10 docs mid-session. **Verified:** 8,230 tests; every guard negative-tested by breaking the fix; bundle boot with the repo OFF `sys.path`; deployed bytes inspected twice; `cdk diff` read before deploying (it caught my helper adding `DatapointsToAlarm` to ~30 untouched alarms). **Incidents:** 4 rows — **Whoop ingestion DEAD since 08-01 12:00Z (OAuth 401, auth breaker latched; #1934, P2 data gap, needs Matthew's interactive re-auth: `python3 setup/setup_whoop_auth.py --backfill`)**, the 26-day cutoff-1 band outage, the third phantom concurrency wedge, a visual-QA false positive (#1931). **Backlog:** Now live at 6 actionable; hygiene 0 violations over 71 open. **Stash/hooks:** clean. Build beat `2026-08-02-the-check-that-had-been-switched-off-for-26-days`. **NEXT session: #1934 (Whoop re-auth) is Matthew's and blocks a growing data gap · then the Fable delta review, which ONLY Fable may run** (banked 14/17 lenses since 07-28, due on/after 2026-08-02, then file the ~55 held findings in ONE pass) · #1931 · #1922 · #1927's **measurement half only** — the ceiling number is Matthew's and at $85 the simulation reaches tier 3 (all Bedrock off) around **2026-08-21** · #1892/#1893 · #1896/#1897 · #1919. Full narrative: `handovers/HANDOVER_LATEST.md`.
