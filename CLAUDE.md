...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/constants.py` (currently **2026-07-13**, cycle 6 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-07-18 — paid down the **`Next` /fullreview milestone, slice 1** via two disjoint-file `worktree-implementer` fan-outs (Matthew: "move to the `Next` milestone… you do all the merges… Deploys stay mine"). **7 PRs merged, 7 issues auto-closed; 1 PR HELD:** #1270/#1231 (tier-alert `_TIER_LABELS` → ADR-125 ladder, was falsely claiming tier-2 pauses /api/ask), #1271/#1236 (/deploy skill Mode 1 → CONVENTIONS §2 pointer + site-api owner Operational→Serve + 2 tombstones), #1272/#1220 (chronicle weekday↔date deterministic grounding guard + calendar-facts into Elena's packet), #1273/#1230 (inference-receipt ceiling derived from `/life-platform/budget-breakdown`, drops hardcoded $75, fail-closed to $85, + source-literal ceiling gate), #1274/#1206 (coverage floor 25→40 + self-reminding gap `::warning::` — **live on main, Unit Tests confirmed green under 40**), #1276/#1209 (bare door URLs 301-normalize to trailing-slash in the `v4-redirects` edge fn, generator+artifact), #1277/#1235 (CLAUDE.md/SCHEMA.md experiment-anchor fix cycle5/2026-07-12→cycle6/2026-07-13 + AST discoverers that self-heal every reset + `currently`-bound anchor gate). Each carried a **non-vacuous** regression guard + full offline suite green; I verified every diff before merge; every merge CLEAN (file-disjoint batches); `test_count` drift auto-reconciled by the repo bot (aligned via `reset --hard origin/main`); all agent worktrees removed. Then Matthew granted deploy authority ("yes merge, you handle all deploys") — **merged #1275 and DEPLOYED + verified everything live:** `deploy_fleet.sh` (95 fns, 0 failed → #1273/#1272/#1270/#1265/#1268 + site-api code); CloudFront `/api/*` invalidate (verified `inference_receipt` ceiling=**100** no-$75, `state_of_matthew` available=**false**); `cdk deploy LifePlatformCompute LifePlatformServe` (**#1263** both `PutMetricData` grants UPDATE_COMPLETE → clears the R8-ST6 Plan red); `publish-function v4-redirects` (**#1209** — `/data`·`/cockpit`·`/story` now 301→200, `/now/`→`/cockpit/` preserved); `site-deploy.yml` SUCCESS for #1275 CSS; smoke `82 passed/0 failed` after a 404-check follow-up (`60aef367` — the #1209 normalizer now 301s bare paths, so the check asserts the trailing-slash form). **#1263 honest note:** the grant restored `DaysSinceLastDecided` emission (evaluator invoked: 200, no AccessDenied); the `grading-stalled` alarm now reflects the TRUE value (8d, genuine cycle-6 post-reset maturation) not missing-data — it clears when a cycle-6 prediction decides, not via deploy. **Left for Matthew (NOT deploys):** #1266 DDB cycle re-stamp (history heal) + #1265 Elena held-draft regen (editorial). **Next = 30 stories;** the `check_doc_facts.py` cluster (#1232/#1205) must be SERIALIZED (they + #1230/#1235 all edit that file). Budget tier 1. Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-18_NowMilestoneFanout.md`.
