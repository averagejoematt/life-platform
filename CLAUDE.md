...
**Verified:** 2026-07-06 (R22 build & backlog paydown — "start building and paying down our backlog; do #779/#780 first out-of-band; I approve edits/merges/deploys this session; do your next picks then write docs+handover") — **7 Now-milestone issues shipped** (#779, #782, #783, #784, #785, #786, #787). PRs #826→#827→#828 (SEC-01), #829 (#782+#783), #830 (#784+#785), #831 (#786+#787). **🔒 #779 (SEC-01) — CRITICAL MCP `/token` unauth exposure CLOSED, deployed (`cdk LifePlatformMcp`), live-verified:** real OAuth 2.1 — `/authorize` issues single-use PKCE-bound codes (DDB `OAUTH#`, 10-min TTL, redirect allowlist), `/token` only exchanges a server-issued code (PKCE S256 verified, redirect matched) before minting the bearer. Unbound/forged/replayed/wrong-verifier → 400; full PKCE exchange still authorizes all 143 tools (connector intact). **Two LIVE-ONLY bugs that green unit tests missed** (three-round fix): the MCP role's `DeleteItem` is deliberately LeadingKeys-scoped away from the OAuth partition → consume via conditional `UpdateItem`; and `consumed` is a **DynamoDB reserved word** (alias via ExpressionAttributeNames) — **validate DDB-shaped changes against real DynamoDB before trusting the FakeDdb suite.** **#783 (BUG-01)** deployed (`cdk LifePlatformCompute`) + live-verified: `latest_weight` got avatar_weight's 14d→30d fallback so the ADR-104 grounding gate covers weight during routine weigh-in gaps (2026-07-05 record now writes latest_weight 300.77). **#786/#787 (CONTENT)** deployed (site, build `7d73cf3a`) + live-verified: recap "where we are now" and each coach's read now carry their own as-of date so stale/staggered content isn't presented as the live "now" — front-end only (APIs already returned the dates). **#782 (DEBT-02)** `personal_baselines.py` added to `build_layer.sh` + `lambda_map.json` (closes #697 recurrence; preventive). **#784/#785 (CLAUDE)** — **Bash(*) removed; deploy/merge/push now machine-enforced** via committed `.claude/settings.json` `ask`-rules (fully durable next session); git pre-commit now runs black+ruff (`scripts/install_hooks.sh`). **⚠️ STILL OPEN — #780 (SEC-02) rotation, NEEDS MATTHEW:** rotate the mcp-api-key (invalidates the possibly-leaked bearer; **breaks the local Claude Desktop bridge if still used — OPEN Q: does Matthew still use it?**) + rotate the Function URL (breaks claude.ai until he reconnects) + stop committing the URL. Detail in **private** memory `security-r22-mcp-token-exposure`. **Gotchas:** MCP deploy=`cdk LifePlatformMcp`; Compute shared-asset ripples ~15 lambdas (same-hash-for-all = one code delta); site modules content-hashed (verify the served hashed URL). Next picks: #781 (ARCH-01 layer-drift root cause), #788/#789 UX, #790 COST, #791 FABLE. See `handovers/HANDOVER_LATEST.md`.
