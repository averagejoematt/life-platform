...
- `docs/ARCHITECTURE.md` — full system design, ~101 Lambdas (CDK-defined; canonical count via `sync_doc_metadata.py`), 9 CDK stacks, data flows
...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-149), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-08-09 (gates-paid-for-themselves session, Opus driving, fully autonomous; 2026-08-08 20:13Z → 2026-08-09 ~00:00Z) — **27 PRs merged + deployed, 5 issues closed, 22 filed.** The plan called Phase 0 (move the post-merge-only gates into the PR fast lane) worth more than any single issue; it proved that **the same day, three times, twice against me**: the new module-size ratchet caught a **+1-line** union breach on main within the hour (#2299 × #2297, each green alone); both size gates caught my own PR pre-merge (1991→2004 — I compressed a comment that duplicated documentation rather than bump either number); and the **grounding-wiring registry**, which had run only *after* merge for its entire life, caught my `**cycle_gate_params()` refactor making the freshness class unverifiable — behaviour unchanged, *declaration* no longer checkable. **Phase 1's finding:** `BASELINE` recorded each god-module's line count as a prose note nothing compared against, and **24 of 28 entries had grown +3,565 lines** (one +879/+48%) on a registry documenting itself as shrink-only — the same class as the pipefail gate. Now enforcing; four implementers then hit it and **zero** reflex-bumped (two *lowered* their baselines by extracting siblings). **xfails 250 → 28** across 16 clusters, and the ~50 corrections are the real output — markers that named a real bug and the **wrong repair**: one remedy would have *created* a fabricated clinical number, one would have breached the live #2209 privacy gate, one accused a reader that was already correct. Real fixes behind them: absent CGM readings published as a glucose **minimum of 0.0** and averaged into a fabricated clinical warning; an unlogged nutrition day rendered **green** because `0 ≤ target`. **Three things caught me:** zsh not word-splitting unquoted params (3×, silently no-opping a revert loop AND a deploy loop), `git checkout -- ` **staging** its revert so `git diff` reads empty and looks like a missed mutation, and a CI deploy racing over mine to ship a bundle missing a merged PR — caught only because the BUNDLE was verified, since its LastModified looked *newer*. Coverage 79.02→**80.20%**. ⚠️ **MacroFactor has ingested nothing for 45 days — cycle 12 has zero nutrition data** (#2326, owner action; poller healthy, no CSV uploaded). **NEXT:** #2343 (a coach cites yesterday's real vitals as today's — a day-correspondence gap no existence-based grounding check can catch), then #2333/#2344/#2326. Full narrative: `handovers/HANDOVER_LATEST.md`.
