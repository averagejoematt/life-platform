...
- `docs/ARCHITECTURE.md` — full system design, ~90 Lambdas (CDK-defined; canonical count via `sync_doc_metadata.py`), 8 CDK stacks, data flows
...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-107), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`)
...
**Verified:** 2026-07-05 (session 15) — **the rigor trio + data-health + the parse gate, all live** (8 PRs #624–#631, all merged/deployed/live-verified; layer v109→v111). Matthew authorized all merges+deploys up front. Shipped: **#535** uncertainty everywhere (weight rate CI + honest goal-date *range* on `/api/journey`; SE-based drift z on effective-n; correlations gated to INCONCLUSIVE below `compute_confidence` MEDIUM); **#538** the calibration scoreboard (`stats_core` Brier/reliability + new `calibration_core` scorer + public `/method/calibration/` + `/api/calibration`; same numbers in the coach track-record MCP tool + `/api/coach_team`); **#532** coach commitments (tracked `COMMITMENT#` records, deterministic kept/broken grading in the evaluator, orchestrator revisits them); **#468** HAE per-datatype liveness (`/api/source_freshness` now exposes CGM dark 43d, BP/SoM never-seen — previously hidden behind one "fresh") + the DI-1.6 alert that fired **36×/72h** episode-gated to once+daily; **#377** JS parse gate on site deploys (`node --check --input-type=module`, ran live, unblocks #581). Plus two follow-ups: `deploy_site_api.sh` now pins the shared layer, and a build beat on /story/build/. **Load-bearing gotchas:** site-api is script-managed (not CDK) — a layer bump left it stale until `deploy_site_api.sh` re-attached (broke #535's CI + #538's import; NOW auto-pinned — see `[[reference_site_api_layer_manual_attach]]`); `node --check <file>` silently misses ESM syntax errors (must use `--input-type=module`); CI/CD triggers on `lambdas/mcp/tests/cdk/…` but **NOT `site/**` or `deploy/**`** (deploy those manually); CI ships code-only (cdk_only + layer changes need manual `cdk deploy`). **Empty-by-design (not bugs):** calibration `n:0` + the goal-date range stay empty until post-reset predictions resolve / ≥5 weigh-ins accrue. **Flags for Matthew:** the DI-1.6 activity alert opened a real episode on thin post-reset data (spam fixed regardless; thresholds may want tuning); GitHub Pages still enabled+public (carried, unactioned). **Watch:** Sun 07-06 / Mon 07-07 crons first-exercise the new commitment/calibration/drift paths. **Next:** #544 methods registry (unblocks #584; sizeable, not started) · #581 evidence.js split (unblocked, **do attended** — 3k-line byte-identical refactor) · self-contained data opus #484/#483 (#487/#507 need a fix-or-retire decision from Matthew). See `handovers/HANDOVER_LATEST.md`.
