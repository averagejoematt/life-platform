...
**Verified:** 2026-07-16 — session opened "what should we work on," picked #741 (publish the career artifact + measure travel); its mechanics were already live (essay, `/method/build/` cross-link, traffic-digest Travel-watch #1072), so the gap was the essay's share card — building it surfaced a **months-old production bug: every OG/social card renders as `.notdef` tofu boxes** (since HP-13 `e1adce8d`). Root cause: the bundled `bebas-neue`/`space-mono` TTFs are subsets with **NO basic-Latin glyphs** (`A`–`Z` unmapped in the cmap), invisible because visual QA checks live *pages*, never the generated PNGs. **PR #1193 OPEN (`feat/essay-og-card`), 20 card/OG tests pass, black+flake8 clean.** Fix: repointed the shared `card_engine` (ADR-114) + `reading/cover_placeholder.py` at the v5 brand fonts the live site serves — **Fraunces** (display) + **IBM Plex Mono** (mono), `woff2→ttf` from `site/assets/fonts/v4/` (deleted 3 broken TTFs, added 3 good) — one change fixes ALL cards AND rebrands them to v5; added `build_essay_org_chart` → `og-org-chart.png` + repointed the essay's `og:image`/`twitter:image`/JSON-LD; **regression guard** (`test_card_engine::test_brand_card_fonts_render_basic_latin`, PIL-only raster-vs-notdef, proven to catch the old font). **DEPLOYED + verified live:** `deploy_lambda.sh og-image-generator` + invoke → `Generated 13/13 OG images`; CDN confirmed `og-home.png` 10 KB tofu → 33 KB real glyphs, `og-org-chart.png` live+on-brand. **Merge HELD** until GitHub's artifact-storage quota resets (~6–12h): merging now triggers site-deploy, whose `visual-qa` `upload-artifact` step (`if: always()`, no `continue-on-error`) fails on the quota → **spurious auto-rollback of a healthy site** (same quota red-X'd this PR's render gate, which logged `✅ GATE PASSED`). Gotchas: artifact-quota reds *any* `upload-artifact` job repo-wide until it clears (noise, not code); the "full" legacy woff2 were ALSO broken — only the v4 hashed woff2 are real; `og-image-generator` (operational stack) is the live card producer, `life-platform-og-image` (web_stack) is not live. **Next: merge #1193 after quota reset (swaps in bespoke card, site-deploy runs clean) → beat eligible → #741 remaining = external publish (Matthew's action); follow-up = `build_builders` stale 116/59/$13 vs ~64/~94/$85; PR #1190 (no-FDA docs) also awaits Matthew; standing #1187/#1114/#1148, /fullreview ~07-18.** Budget tier 1 ($82/$85). Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-13_DocAccuracyDriftGuardrails.md`.
