...
**Verified:** 2026-07-06 (fable batch + authorized carry-overs) — TWO halves. **HALF 1 (fable batch, NO authorization → held):** all 6 `model:fable` R21 stories worked to the decision boundary; **PRs #762–#766 open, checks green, awaiting Matthew's 3 picks** — finish line (#732/PR #764, also confirm the repo-unverifiable "sixteen attempts"), first experiment (#728/PR #765 — mechanism built: REQUIRED `stopping_rule` in `experiment_design` = **layer v117 on deploy**, public artifact `generated/experiments/prereg/{id}.json`, CF behavior, scoped MCP IAM, card render; 3 candidate designs on the issue, A=sleep-window recommended), channel+posture (#745/PR #762, ADR-124 Proposed). #731 trust contract = PR #763 (**merge with/after #765**); #740 essay = PR #766; #748 hard-gated on lived weeks. **HALF 2 (Matthew: "take care of that" → SHIPPED):** **#726 CLOSED** (PR #767 → `7474cf90`, `cdk deploy LifePlatformMcp`, **prod migration executed**: 712 corrupt embedded predictions voided across 345 thread records per ADR-077 via `deploy/void_legacy_predictions_726.py`, idempotent-verified; `get_predictions`→`COACH#{coach}_coach/PREDICTION#`; **live-verified through the real MCP endpoint** — 357 canonical records, 55 already inconclusive-graded; bearer gotcha: `lp_`+HMAC, `mcp/handler.py:388`) + **#725 build beat LIVE** (PR #768, `/story/build/`, site synced, `/version.json`=`2f634e03`). **Main CI GREEN end-to-end** on the #726 run incl. Deploy + visual-QA (one red: the post-deploy layer check caught `freshness-checker` on v115 → attached v116 → re-run green). **Layer truth (supersedes "16/16 on v116"):** mcp + site-api were ALSO stranded on v115 (both fixed this session — the two special-path consumers the drift map can't see); **29 Ingestion/Operational functions remain on v115 DELIBERATELY** — harmless (v116 only changed `intelligence_common`; none import it) and `LifePlatformIngestion` carries the staged HAE change **gated on Matthew's call** — do the `ci/lambda_map.json` audit + reconcile the 29 WITH him. **Next:** Matthew's 3 picks + the batch merges (order #765→#763; #764 after the pick) · **#727** liveness heartbeat (E1 remainder) · Session B epic #716 (#730/#733) · budget pair #738/#737. Stash `session-local` + untracked `docs/reviews/R21_BACKLOG.md` left alone. See `handovers/HANDOVER_LATEST.md`.
