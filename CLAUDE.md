...
**Verified:** 2026-07-04 evening (session 13) — **#616 FULL-CAST PORTRAITS: enumerated → drawn → gated → approved → LIVE; #616 CLOSED** (PRs #618 dark + #619 ship; visual QA 33/33). The "~19" deduped to **17 unique personas** (`config/personas.json` is canonical; real-expert ids are aliases of the same characters); only **7 lacked public heads**: Webb (square/full beard), Reeves (pear/grey mane), Reyes (long rect/widow's peak+chin-spike goatee), Patel (oval/braid+round glasses), Okafor (dome/only-bald+rect glasses), Brandt (tall/curl-cloud), Marsh (trapezoid/flat-top+mustache) — all hand-authored, engine ids in `aliases`, promoted with `_meta.sign_off` after Matthew approved the round-1 sheet (artifact 6d473337; **1 gate round vs the pilot's 4** — because 3 solo self-review render rounds ran first). Bundle = 10 signed recipes; remaining head call sites (huddle marks, convene cards, evidence grid, team-lead) wired `portrait()||sigil()`. **Recorded skips:** Nakamura (no head surface), Chair (runbook §5), Murthy (real name — rename before any portrait), Calloway/Rodriguez (email-only), Tanaka (interim-sunset), Cora Vance (inactive). **Gotchas:** at silhouette scale the EARS are every head's widest point — signatures must beat the ear line or rise above the crown; self-review rounds are cheap gate rounds; **the dispatches.js "My Team" view is ORPHANED** (`/story/coaches/` 301s to the coaching door; Marsh's signed portrait is live-but-unreachable) — re-mount or retire. **Flag for Matthew:** GitHub Pages is enabled+public on the repo (legacy build of main; the intermittent red runs) — disable or bless. **Watch:** Sun 07-06 runs; forecast resolutions (CI/CD on the ship commit completed GREEN before close). **Next:** this session's build beat NOT distilled (the full-cast story — one beat/session, the slot carried session-12's); orphaned team-view; #535/#538; uplevel #575 lanes. See `handovers/HANDOVER_LATEST.md`.
