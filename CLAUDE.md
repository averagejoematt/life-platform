...
Regenerates constants, deploys Core/Compute/Email (constants ship in every bundle — #781), phase-tags DDB, wipes intelligence, rolls the accountability ledger into a durable `LIFETIME#` aggregate + zeroes `TOTALS#current` (`deploy/restart_ledger_reset.py` — ADR-072/077), rebuilds character, curates the chronicle, syncs site + docs, verifies the 86-URL v4 surface (79 pages + 7 JSON endpoints, #918). Rollback: `deploy/restart_rollback.py`.
...
**Verified:** 2026-07-23 — **Wave A shipped + deployed (7 PRs): Social Membrane inbound half live, eng-excellence hardening, coach-corrections ledger seeded.** FIRST: seeded the corrections ledger with 6 rows (`USER#matthew#SOURCE#coach_corrections`, all `status=open` — 4 stale-baseline/1 ungrounded-behavioral/1 cross-coach, from #1687 body, Matthew confirmed) + filed Coach-Correction LATER stories **#1697/#1698/#1699** (S5/S6/S7). Wave A merged+deployed: **#1700 (#1652)** root-clutter ratchet (19-dir allowlist) · **#1701 (#1673)** fail-closed sensitivity gate (`broadcast_sensitivity_gate.py`, seam `sensitivity_status=="cleared"`) — DEPLOYED Ingestion · **#1702 (#1671)** post-enrichment→coach signals (`social_enrichment_lambda.py`, membrane-before-Haiku) — DEPLOYED Ingestion (new `social-enrichment` Lambda) + daily-brief · **#1703 (#1657)** lint-waiver retirement (Matthew accepted removing 17 dead F401 imports; verified tool count 69==69) · **#1704 (#1672)** Broadcast feed `/story/broadcast/` + `/api/broadcast` — DEPLOYED site-api + site. Wave B partials merged: **#1709 (#1656)** mypy clean-set 19→124 modules, 7/14 codes enforced (Progresses; `mcp/` out) · **#1710 (#1658)** coverage floor 40→47 + up-only ratchet + 52 real tests (Progresses). Also: **#1686** decomposed → **#1705–#1708**, sourcing decision = coach web-access. Reconciles: test_count 4960→5045, lambda 96→97, endpoints 119→120. Combined-tree suite **6551/0**; Ingestion UPDATE_COMPLETE; site smoke **217/0** after recovery. **Gotchas:** new `/api/` endpoint + its page in ONE PR races the on-merge site auto-deploy → transient rollback (API must be live AT merge — [[reference_api_before_frontend_autodeploy_race]]); doc-sync reconcile can pull site/ regens into the tree (`git checkout -- site/` first); CDK classifier gate is per-command (explicit ask + unpiped + `--require-approval never`); a worktree agent died mid-response on an API error AGAIN (#1672) — resume via SendMessage, no rework. **Docs:** none beyond auto-synced counters; checkers green. **Decisions:** none as a repo ADR — #1686 web-access recorded on #1705, deferred to its own ADR at implement time. **Incidents:** 1 — the #1704 site auto-rollback (transient `/api/broadcast` 404 pre-API-deploy, recovered same session). **Main:** red — CI/CD run orphaned by a GitHub Actions dispatch stall (minutes/billing, #1544-class; CodeQL/Docs ran on the same commit) — no code cause, offline suite green, both stacks live. **Live: budget tier 1.** Build beat `2026-07-23-broadcast-feed`. **NEXT: (1)** confirm #1710 coverage floor 47 on CI 3.12 once Actions recovers (one-line drop if it reds — #1658); **(2)** GitHub Actions minutes/billing is stuck (owner); **(3) #1655** CI-composition (last big-bang, only once CI healthy); **(4) #1620** outbound social (after any site sweep); **(5)** gate:owner **#1662/#1666/#1650** (stop for sign-off); **(6) #1656** mypy remaining (`mcp/` + 7 codes); **(7)** #1686 open decisions + activate YouTube secret (unblocks the dormant Social Membrane path). Full narrative: `handovers/HANDOVER_LATEST.md`.
