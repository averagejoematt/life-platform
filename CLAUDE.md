...
**Verified:** 2026-07-05 (#478 wrap session, after the opus batch) — **Todoist real filters + pagination + decision-fatigue re-eval shipped end-to-end** (PR #696 → `main` `f79e7ec4`, ADR-122). The v1-API `/tasks` endpoint silently IGNORES a `filter` param, so overdue/due-today were both ≈ the whole active list (stored records: `overdue==due_today==270`, `active==200` page-cap); fixed to `GET /api/v1/tasks/filter?query=<filter>` (required param is `query`, verified live) in both `todoist_lambda` and `mcp/tools_todoist`, active fetch now paginates past 200. Decision-fatigue (BS-MP3) now measures `overdue+due_today` (the decision-pressure set), not `active+overdue` (double-counted + backlog-dominated, so `>15` fired every day even after the endpoint fix; `active+overdue=454` on clean data); the "15" relabelled **provisional** with the ADR-105 personal-variance migration documented. 56 poisoned records (2026-05-10→07-05) flagged `snapshot_unreliable=true` (AC-3, `scripts/flag_todoist_unreliable_snapshots.py`). **Deployed + verified (Matthew authorized "all merges and deploys"):** todoist-data-ingestion (boots clean), life-platform-mcp (imports clean, 401 auth gate). site-api PLATFORM_STATS metadata bump NOT deployed (concurrent site-api work in the tree; cosmetic, syncs next site-api deploy). **Deploy gotcha (codified):** the `.claude/commands/deploy.md` MCP recipe omitted `lambdas/reading/` → re-broke the `No module named 'reading'` crash; rebuilt the complete bundle (`mcp_server.py`+`mcp/`+`reading/`) and **fixed the recipe** (memory `reference_mcp_bundle_needs_reading`). **🔴 Handed to a separate session — pre-existing prod outage #697:** the #543 deploy omitted `personal_baselines.py` from `deploy/build_layer.sh`, so shared layer **v113 lacks it** and **daily-insight-compute + daily-metrics-compute + personal-baselines-compute fail every run** since 2026-07-05 morning (`No module named 'personal_baselines'`; `stats_core`/`phase_filter` are in the layer, which masked it). #478's daily-insight code is deployed but blocked. Fix = canonical layer republish (add to build_layer.sh → v114 → bump `SHARED_LAYER_VERSION` → redeploy Compute consumers → verify boot). **State at close:** `main` = `f79e7ec4`, #478 closed, PR #696 merged; this docs/wrap PR adds the deploy.md fix + handovers + this block. **Next:** #697 (layer outage — TOP priority, blocks daily brief intelligence) · site-ux epic tail #588/#590/#591/#593/#595 · held opus queues (#395 MCP prune; infra #416/#418/#401/#408/#411; growth #420/#405/#399) · GitHub Pages still enabled+public. See `handovers/HANDOVER_LATEST.md`.
