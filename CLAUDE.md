...
**Verified:** 2026-07-06 (Matthew's three picks + the R21 merge train — "yes do it all") — **all decided, merged, deployed, LIVE-verified.** **The picks:** finish line = **A** (185 lbs held 90 consecutive days; "sixteen attempts" CONFIRMED by Matthew) · first experiment = **B, Matthew's override of the A recommendation** (daily zone-2 floor ≥30min → recovery_score +5, 14d baseline/3d washout/28d, ACWR>1.3×3d abort guard) · fulfillment channel = **A primary + C floor**, posture confirmed → **ADR-124 ACCEPTED**. **Merge train (order held):** #765 `8953a6a3` → #763 `e4bde673` → #764 `81fe3065` → #762 `d3b80cf4` (Accepted content; squash title still reads "Proposed") → #766 `97a3f614`. **Deploy:** layer **v117** published + attached — **16/16 tracked + mcp on v117** (Core → Mcp/Web (prereg IAM + CF `/experiments/prereg/*`) → Compute/Email); site-api v117 via `deploy_site_api.sh` (**the script auto-attaches the layer now** — the manual-re-attach memory is softened); MCP code ships via `cdk deploy LifePlatformMcp` (no manual zip — re-confirmed); site synced ×2. **The flagship proof:** `create_experiment` driven through the real MCP endpoint → **`daily-zone-2-floor-30-min_2026-07-06`**, pre-registered 2026-07-06T04:18:55 **before day 1**, frozen artifact live at `/experiments/prereg/daily-zone-2-floor-30-min_2026-07-06.json`, `/api/experiments` renders design+prereg URL — the registry is no longer empty. **Build beat published** (`73893ab2`, `/story/build/`). **Main CI GREEN** (one stale race: the v117-constant push fired Plan's layer check BEFORE the Compute/Email attach landed → 16 ❌ on a correct in-flight deploy → `gh run rerun --failed` → green). Issues: #728/#732/#745 CLOSED w/ decision records, #731 auto-closed, **#740 OPEN awaiting Matthew's edit pass**, **#769 NEW** (C one-tap two-scalar path, epic #718). Gotchas: `cdk diff` prints to **stderr**; pre-commit doc-sync mutates unstaged files (`site_api_common.py` test_count) — check `git status` after committing. **29 Ingestion/Operational functions still on v115 DELIBERATELY** (HAE-staged Ingestion gated on Matthew — reconcile WITH him). **Next:** #727 liveness heartbeat (E1 remainder) · Session B epic #716 (#730/#733) · budget pair #738/#737 · #740 edit pass (Matthew) · #769. Zone-2 experiment day 1 = 2026-07-06; Whoop grades it passively. Untracked `docs/reviews/R21_BACKLOG.md` left alone. See `handovers/HANDOVER_LATEST.md`.
