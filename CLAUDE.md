...
- `docs/ARCHITECTURE.md` — full system design, ~97 Lambdas (CDK-defined; canonical count via `sync_doc_metadata.py`), 9 CDK stacks, data flows
...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-139), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/constants.py` (currently **2026-07-22**, cycle 10 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
Regenerates constants, deploys Core/Compute/Email (constants ship in every bundle — #781), phase-tags DDB, wipes intelligence, rolls the accountability ledger into a durable `LIFETIME#` aggregate + zeroes `TOTALS#current` (`deploy/restart_ledger_reset.py` — ADR-072/077), rebuilds character, curates the chronicle, syncs site + docs, verifies the 89-URL v4 surface (82 pages + 7 JSON endpoints, #918). Rollback: `deploy/restart_rollback.py`.
...
**Verified:** 2026-07-25 — **CI-craft epic: composition + ratchet-guards + god-module breakup (3/4).** Continued the standing autonomous non-fable paydown (`model:sonnet`/`model:opus`), standing approval for all merges + deploys. **Shipped (all merged + deployed, main GREEN):** **#1655 (#1726)** CI-as-composition — one composite `setup-ci` action (adopted by ci-cd.yml + 13 workflows) + reusable `ci-lint.yml`/`ci-test.yml` behind a thin orchestrator (ci-cd.yml 1538→1166); deploy-gate/`environment:production`/OIDC/#749 preserved + **live-validated on main**; *also fixed a pre-existing main-red* — `pr-checks.yml` lacked `hypothesis`, red on every PR since #1716 (#1720 tail); AC "fewer lines" unmet (+75 — a local composite can't include `checkout`). **#1665 (#1728)** module-size ratchet guard (1200 ceiling, 33-file subset-tolerant baseline, exception+generated escape hatches) + mypy no-new-global-disable ratchet (survey: root-clutter #1652/coverage #1658/mypy-clean-set #1656 already existed). **#1654 (3/4 named god-modules — issue REOPENED for slice 4)** facade + cohesive siblings + `_g` `globals()` monkeypatch hand-off, route/name parity proven: **s1 #1727** site_api_data 3016→337, **s3 #1729** site_api_observatory 2826→152, **s2 #1730** wednesday_chronicle 2975→1096; **slice 4 site_api_intelligence (2460) HELD** — `mypy.ini` contention with a concurrent #1656 session. **#1731** content-policy hotfix (the incident). **Verified:** combined-tree full suite green each merge (6770→6780; test_count 5205→5215), each load-bearing AC confirmed genuinely-asserting (extracted-step identity, guards proven to bite, `EXPECTED_ROUTE_MAP` byte-identical + `__module__` preserved); final **`deploy_all`** (full fleet Deploy/Smoke/Post-deploy I1-I2-I5/Visual-QA all success, auto-rollback skipped). **Incident (P4, logged):** chronicle merge red main ~35min — the **content-policy scan (CI-only ci-lint gate, NOT in pytest)** tripped on the Elena prompt's substance-privacy guardrail moved to `chronicle_prompt.py`, breaking the path-keyed `ALLOWLIST_FILES`; #1731 allowlisted it, `deploy_all` cleared red + deployed the stranded chronicle ([[reference_content_policy_allowlist_follows_path]]). **Gotchas:** CI-only ci-lint gates beyond pytest (content-policy/black/mypy/lambda_map/doc-drift) — a green `pytest` ≠ green lint; **`deploy/approve_deployment.sh` is NOT on main** (last session's scratchpad — recreate/commit); `cancel-in-progress:false` → merges queue (no strand), deploy train serial; a `scripts/`-only hotfix doesn't trigger CI/CD → `deploy_all` clears red + deploys the stranded lambda. **Docs:** none needed — self-documenting; ENGINEERING_STANDARDS already carried the guard contract; checkers green. **Decisions:** none — under #1648/ENGINEERING_STANDARDS. **Main:** green (`3a542cb5`, check_main_green exit 0). **Incidents:** 1 row (the P4 above). **Stash/hooks:** clean (🟢 hook; `🔴 config-drift` = `email-subscriber: NOT DEPLOYED`, an intentional skeleton). **Labels:** OK (79 stories). **Live: budget tier 1.** Build beat `2026-07-25-god-module-breakup`. **NEXT: (1) #1654 slice 4** (after #1656 lands, off clean `mypy.ini`); **(2) #1653** lambdas/ packaging — **no trivially-safe slice** (bare-name imports, `constants` ~138 fan-in), CDK-deploy-gated per slice with Matthew; **(3) fan-out** ~15 sonnet/opus feature stories (3-4 at a time, usage permitting); **(4)** gate:owner sign-offs; **owner:** recreate `approve_deployment.sh`, `life-platform/youtube` secret, #1686 coach-prescription decisions, #1396 OSS repo, #1243 podcast-audio regen. Full narrative: `handovers/HANDOVER_LATEST.md`.
