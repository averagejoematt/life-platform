...
**Verified:** 2026-08-20 eve → 2026-08-21 AM PT (Opus, plan `quizzical-wandering-dawn`; boot = charter + `blast_radius.py`; **no `model:fable` issue touched — by design**) — **86 → 86 open: a net drain of ZERO** (two closed, two filed — #2931 auto-filed mid-session, #2932 by me at wrap; reporting the closures without the filings would have claimed a drain that did not happen). What changed is the composition: the only P1 is gone and a latent silent-data-loss hazard is now visible. **fable 25, unchanged.** **#1221, the only P1 and live-exploitable at boot, is closed `realized`.** The code half had already shipped — `client_ip.py` preferred `CloudFront-Viewer-Address`, a header CloudFront sets from the TCP peer and a client cannot forge — but **its preferred source never arrived**: all 21 behaviours used legacy `ForwardedValues`, which forwards only the headers it names, so every request fell through to `X-Forwarded-For`, **which this distribution forwards unchanged from the client**. `ForwardedValues` and `OriginRequestPolicy` are mutually exclusive, so the fix was a migration: new `cdk/stacks/web_cloudfront_policies.py`, six `/api/*` behaviours moved to cache-policy + origin-request-policy pairs, the other 15 deliberately left alone. **The load-bearing design rule: `CloudFront-Viewer-Address` goes in the ORIGIN-REQUEST policies and in NO cache policy** — it is unique per viewer, and `/api/*` is the only cached `/api` behaviour (0/300/3600) and the busiest read path, so in a cache key it would have shattered one cached object into one-per-client — a latency and cost regression shipped in the name of a security fix. Mutation-proved in `tests/test_cloudfront_viewer_address_policies_1221.py`; `web_stack.py` went **1084 → 1071** (the migration removed more than it added). **Proved by behaviour, not by template:** after the deploy, six POSTs with six *different* forged `X-Forwarded-For` values returned `400 400 400 429 429 429` — the bucket holds against a brand-new forged IP; before, rotation minted a fresh bucket and 429 never appeared. Re-confirmed in a second hourly window. **Box 5 was larger than filed and the code disagreed with itself:** `BOARD_RATE_LIMIT = 5` bounded *requests*, but one `board_ask` makes one Bedrock call **per persona**, the **caller** picks the list, and the rate check ran *before* the list resolved — a 7-coach roster meant **35 Haiku calls/IP/hour for 5 tokens**, while the limiter's own docstring already called board_ask *"never unmetered"*. The fix is **not** "move the check down" (that leaves the follow-up path uncharged, and a follow-up genuinely is one call): one token up front, the remaining fan-out charged once the list is final, via a new `cost=` param on the shared limiter. **Verified live at zero Bedrock cost** — a 7-cost panel against a 5 limit is refused *before* any paid call (`429`, `RateLimitHit=1`, no invocation in the window). **Stated plainly: a full panel now costs 7 of 5 hourly tokens — one panel per hour, not five.** **What I did NOT verify, and said so:** box 2's fail-closed branch is not externally reachable (the FURL returns 403, origin-secret guard armed) — unit-tested, not probed; box 4's other two limiter families are structural, not live, because arming `/api/ask` spends real Bedrock and `/api/nudge` writes real data. **Four self-inflicted failures:** I **closed #1221 by accident** by writing *"`Fixes #1221` is **not** claimed here"* in a PR body — **a negated closing keyword still closes**, the parser cannot see the negation (reopened in a minute; the trap was already in my own memory and I had briefed three workers on it that day); a production gate **sat 2.1h** and the platform **auto-filed #2931** about it with no human involved — the **second** time in two sessions, after I wrote *"sweep in a loop, not once"* into the last wrap and then swept once (draining properly found a **second** gate queued behind the first); a docstring underline failed `test_no_conflict_markers` because `THE FIX` is seven characters, making its `=======` **byte-identical to a git conflict marker**; and `DEPENDENCY_GRAPH.md` went stale again at wrap — the **sixth** #2924 instance. **Two more surfaced during the wrap itself:** a **third** fixture-not-the-wire file (`test_capture_door_idempotency_2682.py`) went red on the fail-closed flip — I had fixed exactly that in two other files hours earlier and never swept for a third — and a gate sweep returned a **false “empty”** because an unquoted `?` in `gh api ".../runs?status=waiting"` made zsh glob-fail, so the command never ran (re-ran quoted: genuinely empty). **Chasing the first one past “make the test green” found a real hazard, filed as #2932:** fail-closed is right for a *rate limiter* (one shared bucket is a safe failure) and wrong for the three *capture doors*, which key their idempotency id on the same identity and write with `attribute_not_exists(sk)` — under the sentinel two different readers submitting the same idea **collapse onto one row** and the second is silently swallowed. Latent, not live: verified on `E3S424OXQZ8NBE` that the `/api/*` behaviour serving all three doors carries the ORP; but **nothing enforces that coupling**, and the 15 behaviours still on `ForwardedValues` do not forward the header. *One helper, two callers, opposite needs from the same failure mode.* **Two premises were stale, one of them mine:** box 3's guard test did **not** encode the false premise (already rewritten) — the real wire problem was two *fixtures* supplying the caller IP only via `sourceIp`/`x-forwarded-for`, a shape production no longer produces; and #2829's rescope, read off the `cdk diff`, was **blocking #1221's own deploy** (three CREATEs for alarms that already exist) — dropping them unblocked it and shipped #2829's actual title bug, the `email-subscriber-errors` SNS routing, in the same deploy. **Deploys: TWO, both owner-approved** — `LifePlatformWeb` (02:10Z) and a deliberate fleet `deploy_all=true` (09:00Z). **4 gates rejected, none left parked.** **PRs:** #2928, #2929, #2930. **Build beat:** none — a policy migration, an identity change and a fan-out charge; nothing a reader can see. **Owner asks (4):** a Withings weigh-in (also unblocks #2809); whether to raise `BOARD_RATE_LIMIT` now that a panel costs its true price; the coach-colour call (`sleep_coach` `#818cf8`, 6.21:1 vs 4.37:1); #1738/#1571/#1677/#1631 + #2833/#2834. **Next:** #2932, #2924, #2829 (the three orphans need `cdk import`, not synth), #2921, #2918, #2919. Full narrative: `handovers/HANDOVER_LATEST.md`.
