...
**Verified:** 2026-07-28 (Day 2, solo opus session) — **#1653 SHIPPED AND LIVE: the flat `lambdas/` root is gone, 132 loose modules → 0; main GREEN (f6870489), fleet current.** PRs **#1899** (the move) + **#1900** (the engine-doc re-verify that greened main). Nine domain packages (`common` infra, `ai` inference, `experiment` apparatus, `coach`/`health`/`training`/`content`/`privacy`); 1,570 imports rewritten across 499 files by codemod; 11 verified commits; **ADR-146** records the taxonomy + the failure catalogue. **Live:** all 99 Python Lambdas redeployed 18:58–19:06Z on two bundle shapes (97 tree, 2 MCP); CI/CD green through Deploy → I1/I2/I5 → visual+AI QA → smoke, rollback skipped; `/api/*` 200s; 0 fleet errors. **The value was the failure catalogue, not the tree — pytest caught none of it:** the unit suite had been making REAL Bedrock calls (8 tests; `from ai import X` makes `_handle_fromlist` prefer the parent-package attribute, so `sys.modules` stubs are silently bypassed — order-dependent, green alone → `tests/bundle_stubs.py`); 3 config loaders silently degraded to empty registries (`dirname(dirname(__file__))/config` depth assumption → `common/repo_config.py`); 2 data-file loaders resolved to nothing INSIDE the bundle while green locally; `session_postflight`'s asset canaries would have cried wolf on every deploy; `ci/lambda_map.json` had 9 dead paths predating the work. **`scripts/verify_bundle_boot.py`** (imports every module with ONLY the bundle root on `sys.path`, i.e. `/var/task`) is the standing gate for that class; `tests/test_lambdas_packaging_guard.py` keeps the root packaged. **Gotchas:** `check_doc_index.py --strict` is CI-ONLY and red main post-merge (engine-source moves ⇒ re-verify the engine doc in-wave — 2 of 10 line citations had genuinely drifted, so it was re-verified, not date-bumped); a stranded `production` approval blocked EVERY pipeline for 15h while `check_main_green.py` read past it (#1901); CodeQL re-attributes pre-existing alerts on large diffs (all 7 byte-identical to main). **Honest limit: 17 of 99 functions have actually cold-started on the new bundle** — the other 82 first run at the 16:30–17:00Z compute chain + daily brief, and hypothesis-engine Sunday 19:00Z. **Incidents:** 1 row (the 15h stranded approval). **Backlog:** Now live at 8 actionable, all epic #1890 live-honesty defects. **Stash/hooks:** clean. **Labels:** OK. Build beat `2026-07-28-the-tree-that-hid-eight-live-bedrock-calls`. **NEXT session: #1891 first** (a real public clinician named as a pillar owner on a live page — top of the stored rank) · then the reset-leak cluster #1895/#1898/#1894 as one wave (shared root cause) · #1896/#1897 ai-integrity (#1896 is a plausible descendant of the live-Bedrock finding above) · #1892/#1893 · new: #1901, #1902 · Fable delta review due on/after 2026-08-02 — do NOT let another model finish the banked `/fullreview` partial. Full narrative: `handovers/HANDOVER_LATEST.md`.
