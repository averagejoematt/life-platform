...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/common/constants.py` (currently **2026-09-06**, cycle 17 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-09-05 (Fable 5.1 → Opus 5 → Fable 5.1, **Session V — the review baseline, the batch paydown, and the forensic RCA**; owner: "24 hours to use 100% of our allocation … close as many issues as possible … red team with experts", then "manage yourself between now and 7am … pay down … non-fable", then "deep forensic analysis … red team … Part 1 root cause, Part 2 monitoring/self-healing, Part 3 the path to A … everything should be epics and stories"). **`/review full` NEW BASELINE on Day 0 of cycle 16**: 17 rows from scratch, **no A- survived**, 13 fell, qs rose; 116 → 99 CONFIRMED (10% refuted); 84 filed (#3489–#3571); four rows independently found the night's P1 — the chronicle sweep had republished a wiped draft (#3485). **Paydown: 28 closed, 12 PRs, board 116 → 91** via class-sized batch PRs; four green PRs (#3580/#3581/#3583/#3588, 11 issues) sit on the **gate-census chain — merge ONE, rebase, full suite, repeat**. **Day 1 flipped clean at 00:00 PT** (`day_n 1, weighin_count 0`, #3478 holding). **The forensic RCA** (`docs/reviews/FORENSIC_RCA_2026-09-05.md`, PR #3591, 4 analysts → 6 red-team personas → 1 architect): the 70 were **breadth × Day 0 × instruments that read shape and count, not effect** — ~5–6 findings/lens on every review since July, 17 lenses instead of 7, ten ungraded for five weeks, measured hours after a writer undid the wipe; no pre-push hook, zero pytest in `agent_commit.sh`, ~half of main by direct push, closure in warn-mode at 9h median, 597 gates / 50 proven, 12 resets in 55 days on machinery priced "a few times a quarter" (6 of 7 P1s). **9 classes → 6 structural changes, all free but one ≈$0.01/mo leg; rent register accepted ≈$0.06/mo, ~$9/mo rejected, retirements net negative.** "All A on an identical re-run" was replaced (anchors move in-run; Day 0 ≠ Day 6) by a frozen-anchor two-instant target. Filed: epics #3592/#3593, stories #3594–#3603 (#3598 P1, #3601 gate:owner), #3528 amended. **Part 4 (from A- to A, PR #3605 b8ff4706e): cost is NOT the constraint ($2.67/mo gross closes every rent gap; two anchors price machinery negative) — the distance is cycle length (median 5 days; 30 is the knee), ~22 owner acts, four taste verdicts, in-run anchors, and un-enumerated "every" sets (538/597 gates unproven)**; filed #3606 (the ONE gate:owner decisions issue) + #3607–#3621. **Owner pre-authorized 1–22**: cycle 16 PUBLISHED + SEALED, remediation role applied CLEAN, reader-input PII redacted + versions purged, tier-3 drill (doors pause honestly), SES identity created, urgent topic confirmed; rulings 7–16 recorded (30-day minimum cycle · commitment loop KEPT · Wednesday send · protocols brute-force reset). #3598 landed (PR #3622 merged, Refs — closes on its live proof); the supersede was superseded; the reconcile script is PR #3627 (nothing applied). **Then, at ~19:00 PT, one more reset: cycle 16 → cycle 17, genesis 2026-09-06 (Sunday)** — one hour after the 30-day ruling, flagged once, run as ordered; cycle 16 lasted ONE day and closed SEALED; `--override-weight-lbs 326.2` (the 09-05 real weigh-in); the #3477 sweep aborted run 1 on the JS genesis instants (re-derived, never loosened), run 2 passed rendered · semantic · truth 8/8; the FIRST reset under #3622's writer contract (`WORK_CONTRACT` lines = #3598's live proof); cycle 17's prereg frozen (16 predictions + 2 hypotheses, sha `bd225d24…`), PUBLISHED + SEALED, predict-the-week seeded. **Day 1 flips 09-06 00:00 PT. Day-1 runlist: #3627 MERGED 1851a9c3d** (the countdown-gap sweep's CROSS_PHASE carve-out was inert — it would have tombstoned the coach relationship rows), then restart_verify → the two reconcilers in order → the provenance reconcile → the supersede on the 09-06 weigh-in. **Main is red on the IAM gate on every run since 23:56 PT** — #3573 modified the qa-smoke role's S3List in place; **owner act `bash deploy/cdk_deploy.sh LifePlatformOperational`**; the unit-test red (a composite alarm invisible to the AST discoverer — #3503 one layer up) is PR #3590. **Owner acts owed, in order:** Day-1 weigh-in → supersede (missed the 09:30 compute; the 10:00 brief used the override) · attended cycle-16 prereg publish + stamp (still UNSEALED) · the Operational CDK deploy · `setup_remediation_role.sh` · the us-east-1 log-group delete · the three RCA decisions (reset cadence, commitment loop, reader-alarm routing). Deploys owed: site-api, coach-nudge, qa-smoke, site-api-ai, cost-governor. Session U proper (Architect ritual) UNTOUCHED, 2026-09-08 — `~/.claude/plans/lovely-snacking-panda.md`. Full narrative: `handovers/HANDOVER_LATEST.md`.
