...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/common/constants.py` (currently **2026-09-04**, cycle 16 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-09-03 (Opus 5, **Session U — the Friday re-anchor**; owner instruction: "do another full platform experiment reset so it all starts on friday september 4th", then an overnight backlog grant narrowed to `model:opus` and below). **Cycle 15 → cycle 16, genesis 2026-09-04**, baseline 324.64 lbs (the 09-03 weigh-in; a future genesis needs the override). Census preflight 98/98, 50 rows re-phased, 314 intelligence rows tombstoned, ledger rolled to `LIFETIME#`, 45 open bets voided, full `cdk deploy --all`; **hard gates rendered 96/96 · semantic 8/8 · truth 8 surfaces clean** (tier 0, ran for real). Live by content: `/api/journey` and `/api/character` serve `start_date: 2026-09-04`, `day_n: 0`, `pre_start: true`. **Cycle 15 closes SEALED** — its prereg artifact + stamp still verify, so no grandfather record. **Cycle 16's claims are frozen (sha `cd8156f1…`) but NOT published — it stays unsealed until the attended publish+stamp runs** (owner chose review-first; the eve lock email was declined, skip recorded). **The reset's real lesson: `restart_pipeline.py` exited 0 and produced five gate reds, three of which recur on EVERY reset** — it runs one doc gate, CI runs twelve (filed #3477). Backlog night closed **#3474** (token alarm sat at the platform's 75th percentile, firing 25.8% [13.7%, 43.2%] of days — re-derived to 250k between Tukey 260,014 and median+3·MAD 221,743, now 9.7% [3.3%, 24.9%]) and **#3473** (the missing compute heartbeat — but the CAUSE was the `_DETECTORS` ledger naming neither compute pair, so the guard built to see the gap was blind to it; both pairs ledgered, negative-controlled two ways, paid for by extraction 1331 → 1300, live 85 → 86 resources with zero dropped). Filed **#3476** (a CI warning that is unshippable and fires on every green run). **#3390 re-anchored** to cycle 16 rather than closed. **Gotchas:** a task's "exit 0" is the wrapper's, not the command's (two cases, real exits 1 and 4); `gh run list --commit <short-sha>` returns zero runs silently; zsh does not word-split an unquoted `$var` (eleven gates falsely red); a local suite run concurrent with edits is evidence in neither direction. **Open (opus lane): #3390 (Day-1 runlist, run on/after 09-04), #3477, #3476, #3403/#2978 (~09-08), #2883 (owner); #3422/#3436/#3373/#3042 are `model:fable`, out of lane not blocked.** Session U proper (Architect ritual + time-anchored batch) is UNTOUCHED and still scheduled 2026-09-08 — `~/.claude/plans/lovely-snacking-panda.md`. Full narrative: `handovers/HANDOVER_LATEST.md`.
