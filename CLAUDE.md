...
**Verified:** 2026-07-18 (late night) — **High-value backlog paydown: 7 issues closed properly, all live.** Matthew pre-authorized all merges+deploys; selection = highest Score × timeliness from the two fresh review queues. **Shipped+merged+deployed+verified (7):** #1369 Truth Spine (PR #1421 — NEW `web/vitals_resolver.py`, ONE latest-reading resolution consumed by pulse/vitals/snapshot/stats-refresh/daily-brief; counts + hero copy derive from PLATFORM_STATS; `/api/wrong` header derives from parts; live: vitals==pulse 96/green/8.4h where snapshot had fabricated 0.0/red), #1375 Serial Restarter's Ledger (PR #1424 — `/story/attempts/` expedition log + same-day-axis overlay, cockpit "Attempt #8 — previous best: 13 days" masthead, footer sweep; 3 render-QA rounds fixed a FALSE fs-ok floor claim 5.1px→11.0px effective, PADL clipping; gating visual-QA passed), #1328 (PR #1422 — site-api concurrency 5→20 after the cap bound daily w/ 627 reader 429s/30d, Throttles alarms per serve lambda live, `account_concurrency_limit` fact + doc rewrite, CDK deployed; also fixed #1419's `_count_adrs` h2-only ping-pong 121→133), #1327 un-red main (PR #1423 — transient-aware I9 by SentTimestamp age, discovery-first shared facts, `scripts/check_main_green.py` + wrap e2 gate; b1a596fb ran GREEN end-to-end), fan-out #1335 (PR #1418 spend-attribution Period ceil), #1321 (PR #1419 ADR index h3 records, 133), #1370 (PR #1420 `not_yet_skillful` — live −0.0047 now serves 45/skilled=False). **Merge queue** via /reconcile-branch (test_count 3644→3948, zero drift left). **Gotchas:** CI deploy race (an older merge's in-flight run overwrote my manual site-api deploy — verify CodeSha256, redeploy after pipeline settles); CloudFront /api/* TTL needs viewer-path invalidation before verifying; NEW site page = FOUR registries (visual_qa+smoke+restart+PAGE_BINDINGS — the last redded a518ad3b, fixed 902ba9b8). **Attended queue:** cycle-8 prereg SEEDED (publish dry-run in `handovers/prereg_dryrun_cycle8.txt` — awaiting Matthew's OK); Sunday+ `deploy/restart_verify.py` + re-seed prereg after any pipeline re-run; public_stats platform counts self-heal at next daily-brief. Decision menu: #1319 approval-gate posture, #1114 portrait pick, #1243/#748/#1187/#1029, gate:owner #1350/#1329. Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-18_LaterDrainCycle8Reset.md`.
