...
**Verified:** 2026-09-02 (Fable 5, **Session S — the repair, the night-shift crew, and the proof of the numbers**; "read the plan and get going" — the owner-approved Session S plan run end-to-end). **Shipped: 4 PRs merged+deployed (content-verified), 3 issues closed with verdicts (1 honestly reopened), 14 filed.** Phase 1: **#3419 closed realized** — the live probe showed even the 5-persona panel timed out at the Lambda's own 30s ceiling, so PR #3439 shipped raise-to-7 + a PARALLEL persona pass (extracted to `web/site_api_board_panel.py`) + the no-burn conditional rate charge; **live proof: HTTP 200 in 17.2s, all 7 coaches answering — the first time the checkbox ever worked**. Phase 2: #3423 closed realized (reader-audience alarms escalate on FIRST red, both citation-gate legs, launch-day episode replayed red-then-green, PR #3441); #3422 closed **partial and reopened** — the lane pinned the existing #3021 janitor as the steward (PR #3440), but the live measurement found its real sweep cadence is 2–4.5 HOURS, not 15 minutes (datapoint on the issue). Phase 3: **the calculation-proof pass** — 7 Fable graders re-derived every formula family vs live data; **23/23 findings verified CONFIRMED, 0 refuted**; verdict: the arithmetic is byte-exact everywhere, the defects live in the PIPES — the 8-member whoop `#WORKOUT#` sub-record clobber class (#3442 P1, prior art recorded and fixed at one site in W26), **ACWR destroyed nightly since 08-24, 9 days dark, zero alarms** (#3443 P1 + incident row), the #2109 escape's third wave running "90-day" windows as 14 days with the Time-Affluence Meter never having scored a week (#3444), + 9 more (#3445–#3453); scorecard `docs/reviews/CALCULATION_PROOF_2026-09-02.md` (PR #3454, deliberately NOT claiming the accuracy-full calendar slot). **Gotchas:** the auto-mode classifier blocks a persistent steward loop (one-shot pass on watcher events worked 5/5 gates); an extraction trips every guard anchored on the old site (module-size, 4 source-pins, invoke census, mypy, wiring — cured structurally with WRAPPER_CHOKEPOINTS); a 1s-budget test flaked red on a loaded runner and red-mained a docs-only merge (#3455, rerun green). **Open:** #3442/#3443 the natural next picks (Now·P1, the second unmasked by the first); #3399 box 3 on the ~09-02 22:30Z sweep; #3422's cadence leg; #3448 promoted to Now; owner asks 1–11 in the handover incl. the DIL-039/040 number sharpened to 8/37 Wilson [11.4%, 37.2%] and the 09-08 Architect inputs. **Through-line: correct arithmetic flowing through broken pipes — every formula recomputed exact while the pipes fed fragments, truncated windows, and erased finished work.** Full narrative: `handovers/HANDOVER_LATEST.md`.
