...
**Verified:** 2026-08-14 eve (working the bug-bash queue) — **5 PRs merged and DEPLOYED (#2648, #2685, #2687, #2689, #2691) + owner-run `cdk deploy` for #2684; #2656/#2657/#2658/#2679 closed `realized` on live evidence; #1221 and #2654/#2655 deliberately left OPEN; main GREEN (`4b6cfd6a`).** The brief was to land two open PRs then work the ranked non-fable queue. **Every issue's filed scope was smaller than the real defect, and twice fixing only what was filed would have closed the issue with the symptom still live:** #2658's `limit=abc` was the small half — `_parallel_fetch` catches each partition error *individually*, so a total DDB outage still produced a zeroed scorecard at HTTP 200 by a path the reported fix never touches; #2679 named four capture doors, and deriving the set from `_SIMPLE_ROUTES` + AST found **eleven**. **The headline is that I shipped a security fix which disabled the control it was fixing.** #1221 was confirmed live (rotating one header buys a fresh rate-limit bucket — CloudFront forwards the client's `X-Forwarded-For` UNCHANGED here, so no hop index is safe). #2689 replaced it with `sourceIp` — un-forgeable, but the CloudFront *edge* address, which is **not stable per viewer**: measured `400 429 400 400 400 400` against a 3/hour limit, i.e. ~40 min with every IP-gated write effectively unmetered. **18,421 tests, every deploy gate, smoke and visual/AI QA were all green throughout** — the defect lives in the code↔edge relationship every fixture asserts rather than observes (the §9a class written up the day before). Caught only by re-running the live abuse probe post-deploy, out of habit. #2691 restored the order `CloudFront-Viewer-Address` → XFF last hop → `sourceIp` and pins the **order** by test; enforcement verified back to `400 400 400 429 429`. **#1221 stays open** — the bypass needs the origin request policy, which is *not* a one-line attach: CloudFront rejects `ForwardedValues` alongside `OriginRequestPolicyId` and all ~20 `/api/*` behaviours use the legacy form. **#2654/#2655 stay open at 1-of-4 acceptance boxes each** (the IAM grants are live and simulated `allowed`, but the fail-open `_scrub`, the swallowed `AccessDenied`, the missing DLQ and the still-ALARM heartbeat are untouched). **My own errors, all caught:** the `sourceIp` regression (evidence was already in #1221's body — *17 POSTs → 0×429* — and I misread it); a `$FILES` word-split that faked a "127 passed" mutation proof; a CloudFront cache hit that made a working fix look broken; four grep-derived `limit` findings that were all false; and **auto-closing #1221 with the phrase "sufficient to close #1221"** in a body that opened by disclaiming auto-close — 2nd time in 4 days, memory updated with the catching grep. Filed: #2686, #2688, #2692. Full narrative: `handovers/HANDOVER_LATEST.md`.
