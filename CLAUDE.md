...
**Verified:** 2026-07-06 (ARCH-01 session — "plan next session from memory/handover, pay down efficiently; I approve merges/deploys") — **#781 (ADR-131) SHIPPED+DEPLOYED+LIVE-VERIFIED, #791 + #792 closed** (PR #833). **The shared layer is RETIRED**: shared code ships inside every function's bundle, staged by the ONE implementation `deploy/build_bundle.py` (lambdas/ tree + food_vocabulary.json; MCP shape adds mcp_server.py + mcp/). All 8 stacks deployed (consumers first, Core last — the layer resource is orphaned-not-deleted, RETAIN); **fleet verified at ZERO `life-platform-shared-utils` references** (garth/pillow stay); MCP boots 401-healthy; site-api 200 via the new full-bundle `deploy_site_api.sh`; qa-smoke 0 failed. **New reflexes:** shared-module change → `deploy_fleet.sh` or `cdk deploy --all` (CI auto-fleet-deploys on unmapped lambdas/ changes); `deploy_lambda.sh` is always full-bundle (MCP included); the zero-reference invariant is enforced by CI plan job + `test_i2_shared_layer_retired` + session_postflight + drift_sentinel. #791 closed by scope-down: the Monday sentinel already feeds the remediation report; added the missing **doc-literals lens** (live alarm count vs PLATFORM_FACTS — expect it to flag the 110-vs-122 gap weekly until #795/#809 land). Retired incident classes: stale-layer P2 · #697 · site-api partial zips · MCP-missing-reading/ (CI's MCP step had this bug live). **Gotcha:** the doc-drift gate reds main if a test is added after `sync_doc_metadata --apply` runs — re-run it (test_count) before the final commit. **Next picks (Now):** #780 SEC-02 rotation (**NEEDS MATTHEW: still using the local Claude Desktop bridge?**) · #788+#789 (+#804) as one opus site session · #790 COST-01. Older Matthew queue: #417 re-stamp decisions, Ingestion/HAE deploy call, #740 edit pass, untracked REVIEW_BUNDLE_2026-07-06.md. See `handovers/HANDOVER_LATEST.md`.
