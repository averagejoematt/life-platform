...
**Verified:** 2026-07-12 early UTC (genesis night) — "do as many as you can this session to close status … best practice (DRs etc.) can be skipped". **4 PRs #1071–#1074 ALL merged + deployed; #1023 + #1066 closed; #741 advanced.** #1023 = Matthew's option (b): /privacy/ scopes affiliate arrangements to /gear/'s disclosed links (live-verified). #1066 = the training-block lever on /now/ + read-only `/api/routine` (counts-only fail-closed projection; deployed API-before-frontend; shows honest staged state until the first cycle-5 routine draft). #741 = travel watch in the traffic digest (per-page referrers for the career essay — which was ALREADY live via PR #899; only measurement was missing; HN submission + travel remain Matthew's). #1074 = inherited CI red root-caused: #1049's archive move broke a test's sys.path import — **ci-cd Unit Tests was red ~26h unnoticed (ci-cd red does NOT block site deploys; check its conclusions at wrap)**; post-fix run GREEN. Ops: 03:05 UTC Withings "invalid refresh_token" diagnosed transient (manual invoke healthy, genesis weigh-in safe) — **its 1 DLQ message is still queued (delete classifier-blocked) and reds I9 on the next full ci-cd run**. **Matthew queue: run the DLQ-clear `!` command (in handover), decide the essay's live `[CONFIRM:…]` draft-marker fix before HN, Sunday pipeline + prereg, wk0 spot-listen, FDA grant, genome location, #1029. Watch post-genesis: v1.6.0 character run ~17:35 UTC, board_ask p95, levers/inputs + training lever on Day-1 data. Backlog: #1029/#936/#916/#1017/#748/#741 + Later epics.** Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-11_FablePaydown2.md`.
