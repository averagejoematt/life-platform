...
**Verified:** 2026-07-21 — **Cycle-10 reset SHIPPED (genesis 2026-07-22) + non-fable paydown (4 PRs merged/deployed/live) + the Social Membrane epic filed.** Run concurrently with the eng-excellence/craft-review session (its PR #1647/#1648 carried forward below). **The cycle-10 reset is DONE, not pending** (`b206bf21`, `restart_pipeline.py --apply --override-weight-lbs 321.38` — no 07-22 weigh-in, honest last-known 07-20 baseline; clean-prologue, "Before the Numbers" kept as the genesis−6 lead-in): SSM cycle=10, constants=2026-07-22 fleet-wide, ledger→$0, live `/api/journey` `day_n:0/pre_start/days_until_start:1`. **Semantic gate 7/7** (0 poisoned rows/32 sources) = authoritative; **rendered gate's 2 fails were verified FALSE-POSITIVES** (2-day-gap reset collides the outgoing 07-20 with the legit last-data `as_of_date` + genesis−2 prequel — the literal scan can't tell them apart, semantic gate wins); **truth gate LOUD-SKIPPED at budget tier 1** (July window — AI reader-truth did NOT run, logged not glossed). **Paydown, all merged+deployed+verified live:** **#1644**(#1634 canary judge FP — persona contract derived from `persona_registry`, deterministic-authoritative, judge stays advisory), **#1645**(#1640 — AJM dial mark on all 13 OG cards via shared `card_engine.base_canvas`, 13/13 regenerated live), **#1646**(#1618 — receipts dashed governor projection to month-end, ADR-105 estimate caption, $94.6 under the $115 ceiling), **#1667**(#1639 — full head-chrome on **79** pages via single-source `v4_chrome.head_chrome()`+`--check`). Two `test_count` reconciles (4857→4866→4870). **Also filed:** **The Social Membrane** epic **#1668 + 11 stories #1669–#1679** (bidirectional social — ingest own posts→coach signal+display, not just outbound; the loop-breaker Matthew flagged = **#1670 provenance membrane**; cross-linked to outbound epic **#1619**); **#1643** (stale `daily-brief` regen name in `restart_site_copy_sync.py`). Gotchas: **small-gap reset → rendered-gate false-positives** (semantic gate is the authoritative signal — don't rollback on a literal-scan-only fail, diff the served JSON); the concurrent shared-tree race hit again (my `git add -A` swept the parallel session's staged `git mv` — harmless). **Main:** red — latest completed non-cancelled CI/CD run (`953566a2`) parked/FAILED at the manual production Deploy gate; superseding runs auto-cancelled; my automated gates (Lint/Test/Plan) green. **Live: budget tier 1.** Build beat `2026-07-21-receipts-honest-projection`. **NEXT: (1)** owner-merge **PR #1647** (eng-excellence /craft-review skill) → drain epic **#1648** (18 stories #1649–#1666) via `/uplevel`; **(2)** start Social Membrane foundation **#1669 + #1670**; **(3)** **#1620** outbound social links (after any HTML sweep). Full narrative: `handovers/HANDOVER_LATEST.md`.
