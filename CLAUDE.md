...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-144), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/constants.py` (currently **2026-07-27**, cycle 11 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-07-27 (Day 1, day session) — **14 PRs LANDED + THE 64-ISSUE CORPUS SURGERY: diary-360 chain complete, the calibration open artifact live, and the backlog now ranks itself** (suite 7,632→8,141, green every merge round; main-green repair first via failed-jobs rerun — the STS red was transient, zero deploys spent). **Diary-360 #1841–#1846 ALL shipped:** claims ledger into the ONE prediction grader (consent is-True fail-closed) · SRT vocal biomarkers (SET-only guarded writes) · diary-day intervention variable + one code-registered hypothesis · channel-divergence prereg **sealed+published+SHA-verified live** · cut→engagement loop with the Goodhart guardrail STRUCTURAL (fail-closed purpose enum + import-graph test: mcp/ and coach/ cannot reach engagement) · consent-gated `/story/diary/` shelf (site-api deployed BEFORE the site push; honest empty state live, withheld:1). **#1396:** `oss/calibration-core/` (Python↔JS bit-parity) + `/method/grade-your-coach/` LIVE (visual-QA green); repo publication = owner one-liner in PR #1874. **Epic #1863 Now+Next COMPLETE** (#1864–#1871 + #1868's backfill 226→1 violations, zero info destroyed; #1872/#1873 Later remain): ADR-099 amendment (canonical score line, ## Outcome, closure contract), `backlog_next.py` + shared contract module, /uplevel seeds from the stored rank, hygiene linter advisory, wrap gates (e7)-(e9) — **executed for the first time by this very wrap** (17 closure comments). **Infra pair:** #1858 IAM cycle-read ×7 roles + #1859 rollback residuals (us-east-1 region, fleet-push matrix). **THE ONE DEPENDENCY: main is code-green but R8-ST6 reds Plan until the IAM deploy — today's lambda changes are STRANDED-undeployed (MCP/evaluator/daily-metrics/correlation/hypothesis/youtube; live fleet healthy on yesterday's code). Sequence: owner runs `bash deploy/cdk_deploy.sh LifePlatformCompute LifePlatformEmail` → dispatch deploy_all + approve.** Site+site-api NOT stranded (live current). **Day-1 watches all healthy** (docket honest-empty under tier-2 pause; dossier first render 0-withheld; #1840 AC4 both halves; remediation cron fired 16:48Z — late, not missed). **Gotchas (→ memory):** an undeployed IAM merge strands every later CI deploy; never wholesale-capture API schemas on a freshly-reset platform. **Incidents:** 1 row (budget tier 1→2, by-design). **Backlog:** Now = #1741/#1840/#1653 by stored rank; Next exhausted-honest. **Stash/hooks:** clean. **Labels:** OK. Build beat `2026-07-27-grade-your-own-coach`. **NEXT:** owner CDK one-liner (load-bearing) → deploy_all; OSS repo publish; #1872/#1873 later; Day-2 watches (Tue character sheet + brief cites 321.09 clean); queue seeds via `python3 scripts/backlog_next.py`. Full narrative: `handovers/HANDOVER_LATEST.md`.
