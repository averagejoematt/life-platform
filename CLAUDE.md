...
- `docs/ARCHITECTURE.md` — full system design, ~99 Lambdas (CDK-defined; canonical count via `sync_doc_metadata.py`), 9 CDK stacks, data flows
...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-146), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/common/constants.py` (currently **2026-07-27**, cycle 11 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
Regenerates constants, deploys Core/Compute/Email (constants ship in every bundle — #781), phase-tags DDB, wipes intelligence, rolls the accountability ledger into a durable `LIFETIME#` aggregate + zeroes `TOTALS#current` (`deploy/restart_ledger_reset.py` — ADR-072/077), rebuilds character, curates the chronicle, syncs site + docs, verifies the 95-URL v4 surface (88 pages + 7 JSON endpoints, #918). Rollback: `deploy/restart_rollback.py`.
...
**Verified:** 2026-08-01 (Day-5/6, solo opus session) — **the deploy-machinery block SHIPPED AND LIVE: #1911, #1907, #1908.** PRs **#1915** (CI machinery, no deploy) + **#1916** (handler + IAM), split deliberately because an IAM/`role_policies` merge reds the R8-ST6 Plan gate by design. **#1911's root cause overturned its own hypothesis:** the issue guessed a cold container; CloudWatch at the ORIGIN showed both incidents ran 11,628ms and 14,934ms with **InitDuration absent** — no cold start, just ~62 *sequential* CloudWatch calls that stretch to 11–15s under throttling. Fixed with `GetMetricData` (one round trip); threads rejected on #1527's evidence. **Every issue was again a class, not an instance:** #1911's `set -e` + `$(curl …)` trap was **12** call sites (guard is a *derived* scan, negative-tested by injecting a new one); #1908's issue text would have made things **worse if followed literally** — the ci-lint copy it proposed deleting is load-bearing for code-pushes-that-drift-docs, so coverage MOVED (docs-ci gained `lambdas/** mcp/** config/** cdk/** tests/**`) rather than disappearing. **The best find was a silent one:** `check_doc_index.py` skips the #973 drift gate on a shallow clone and docs-ci checked out at depth 1 — it had been running `--strict` with its drift half **disabled**, reporting a green it never earned; now `fetch-depth: 0`, and it caught real drift on its first run. **Gotchas:** the doc-sync stamp is **UTC**, so "same-day commit+merge" really has a **17:00 PDT deadline** — I committed at 17:05 and reddened main + the PR with 8 stale literals; **YAML anchors don't work in GitHub Actions** (caught pre-push); `command curl` bypasses shell-function stubs (defeated every unit test). **Verified:** 8,165 tests (sole failure = the documented low-activity `test_i16`, not a regression); **deployed bytes inspected, not inferred** (`_batch_daily` present, serial `_sum` gone); deploy order held (IAM 6:02:30 → fn 6:02:40); `/api/inference_receipt` 200 with `today_in == month_in` — exactly right on Aug 1; smoke 186/0 twice. **ADR-133 ceiling reverted $115 → $85 on schedule** (live: 85.0, tier 0). **Honest caveats:** the 0.22–0.36s latency is **partly confounded** by the UTC month rollover — the real proof is the call-count contract; and `smoke_test_site.sh` runs ONLY from `site-deploy.yml`, so the smoke fix hasn't executed in production CI yet. **Incidents:** 1 row (100 Lambdas auto-rolled-back by a *pre-existing* qa-smoke failure). **NEXT session: #1917 FIRST — it is a live blocker**, `/api/vitals` publishes `hrv_30d_avg` (n=5) + `weight_delta_30d` labelled "30d" on Day 5, failing qa-smoke's reader_truth, so **every merge now deploys → fails smoke → auto-rolls-back**; then approve the stranded `7408e326` deploy (only after #1917) · #1909 (now verifiable across the real $85 boundary) · #1904/#1892/#1893/#1896/#1897 · Fable delta review on/after 2026-08-02. Full narrative: `handovers/HANDOVER_LATEST.md`. PRs **#1903 / #1910 / #1912 / #1913**, plus **#1906 / #1914** (two engine-doc re-verifies that greened main). **#1891:** `/method/game/` no longer names Dr. Peter Attia — a real, non-consenting clinician — as a pillar owner; the durable half is that **nothing coupled the coach roster to that page**, now two fail-closed render-time guards (`assert_cast_current` + the page passing `privacy_guard`, which being *generated* it had never passed). **The class-level framing paid every time — each issue was filed as an instance and was really more:** #1895's reported leak was **1 of 4** (two live, incl. `/api/ask` feeding a tombstoned snapshot **into the model's prompt**, and `/api/pillar_coupling` with NO phase filter — 118/122 records tombstoned yet the home constellation published `r=-0.87, n=47, p=0.0, significant` over a wiped cycle); #1898's 3 stale 190 g literals were **4** (a hardcoded `character_engine` fallback that fails silently) and `target_grams` is a SCORING target, so protein now grades against the sealed 170 g prereg — a real behaviour change. **Why #1895's class recurred 3× (#946/#1085/#1197):** the invariant existed, but every guard was a hand-written test for a reader someone REMEMBERED — nothing asserted completeness, so #1654's module split escaped. Fixed with a derived **closure test** that AST-scans `lambdas/web/`; it immediately found an instance I'd missed by reading. **Gotchas:** the site smoke's `exit 28` is **deterministic, not transient** — I mislabelled it twice before the timestamps lined up (same endpoint, same 10s boundary, two days running; it's `/api/inference_receipt`, ~15× its neighbours' latency) and it auto-reverted correct merged work **twice**, once re-exposing the clinician's name ~15 min (**#1911**); a docs-only merge during a rollback recovery silently strands the deploy with every run green (**#1907**); a docs-only fix can't clear the CI gate it fixes (**#1908**); the engine-doc gate fired 3× in two days and each re-verify found REAL drift (`PILLAR_COMPUTERS` moved `:1684-1693 → :1690-1697`) — **two independent overnight traps** (squash re-dates the commit; doc-sync stamps today's date), so **prefer same-day commit+merge**. Both module size gates fired on #1894 and both were mine — split cohesively (`weight_truth_qa` 1196→1166, `weight_recency`) rather than grandfathered. **Corrected 3 of my own diagnoses mid-flight** before they hardened into lore. **#1653 residual CLOSED:** 5 coach functions cold-started clean on the packaged bundle, all 9 coaches named in live output. **Verified:** 8,124 tests / 0 failed; bundle boot 293/0; `privacy_guard` over the LIVE bytes → zero violations. **Incidents:** 2 rows (both spurious site auto-rollbacks). **Backlog:** Now live at 9 actionable. **Stash/hooks:** clean. Build beat `2026-07-30-the-four-that-were-really-eleven`. **NEXT session: the deploy-machinery block #1911 → #1907 → #1908 as one wave** — it compounds and now taxes every change · then **#1909** (`/api/status` publicly claims 627% of a hardcoded $15 budget; verify across the **2026-08-01** $115→$85 revert, which will jump the tier) · #1904/#1905 · #1892/#1893/#1896/#1897 · Fable delta review on/after 2026-08-02 — do NOT let another model finish the banked `/fullreview` partial. Full narrative: `handovers/HANDOVER_LATEST.md`.
