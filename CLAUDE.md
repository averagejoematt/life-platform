...
**Verified:** 2026-07-22 — **The Coach Correction Loop foundation SHIPPED + DEPLOYED (epic #1687, 4 stories + 1 main-red fix, all merged; both stacks live).** Merged: **#1694 (#1691)** baseline-freshness gate — deterministic `grounded_generation.baseline_freshness_findings` for the reset-window stale-baseline class ("315 lbs"/"Day 1" during pre-start), advisory-wired in `ai_calls.py` + ⚠️ flag in `ai_review_pack_lambda.py`, 07-20 regression replay — **DEPLOYED** (LifePlatformEmail); **#1692 (#1689)** corrections ledger `lambdas/coach_corrections.py` (pk `…#coach_corrections`/sk `CORRECTION#…`, `ERROR_CLASSES`+`other`, CROSS_PHASE) — inert; **#1695 (#1688)** Hybrid ranker `lambdas/review_pack_ranker.py` (`numbered_entries` + deterministic heuristics reusing the baseline gate + a Haiku critic at budget tier ≤1 → made ai-review-pack an AI surface, added bedrock+SSM grant) — **DEPLOYED**; **#1696 (#1690)** BOTH feedback channels — MCP `log_coach_correction` + email-reply `#N` parser via shared `coach_correction_resolver` — **DEPLOYED** (LifePlatformEmail+Mcp); **#1693** youtube `capture_channel` main-red fix (#1682 follow-up, a scheduled RSS pull must carry none) — **DEPLOYED**. Reconciles: test_count 4887→4960, mcp_tools 68→69. **The flywheel is live: ranked pack → correct by `#N` (MCP tool or email reply) → class-tagged ledger row.** Gotchas: **a merge/audit can leave a registry misconfig a NON-gating test catches** (youtube capture_channel, ADR-117 flagged main but didn't block); **a worktree agent can die mid-response on an API error — resume via SendMessage, no rework** (#1688); **CDK deploy is classifier-gated — run UNPIPED, use `-- --require-approval never` for TTY-less IAM deploys** ([[reference_cdk_deploy_classifier_and_approval]]); zsh `$VAR:l` ate a git path (brace it). **Docs:** SCHEMA/PHASE_TAXONOMY + MCP catalog regen (in-PR/hook). **Decisions:** none needed — HYBRID + BOTH pre-locked in epic #1687; baseline gate under ADR-104/105; critic budget policy under ADR-125. **Incidents:** none. **Main:** in-flight (batch-push supersession); Plan reds by design on the #1695/#1696 role_policies IAM gate (R8-ST6, clears on prod approval); offline suite green (6465 merged-preview); both stacks UPDATE_COMPLETE. **Live: budget tier 1.** Build beat `2026-07-22-coach-correction-loop`. **NEXT: (1) SEED the ledger** with last session's #1–4 findings (via the now-deployed `log_coach_correction` tool next session, or a `write_correction` seed script — Matthew confirms rows; #1687); (2) Coach Correction Loop **LATER** stories S5/S6/S7 (unfiled, `/plan` — #1687); (3) Social Membrane Next **#1671/#1672/#1673**; (4) eng-excellence **#1657/#1656/#1658/#1655/#1652** + gate:owner **#1662/#1666/#1650**; (5) **#1620** outbound social (after any site/head sweep); (6) **#1686** Coach's Prescription (`/plan`); activate YouTube (owner secret). Full narrative: `handovers/HANDOVER_LATEST.md`.
