...
**Verified:** 2026-07-04 (session 7) — **The data-source health review's Now milestone is PAID DOWN TO ZERO**: the Now-remainder batch (#473/#474/#477/#480/#481/#482/#497 + #518) is MERGED + DEPLOYED + LIVE-VERIFIED (PRs #523/#524, layer **v101**). Habitify finalizes yesterday (`refresh_trailing_days=1` — live: yesterday's frozen `pending_count=61` → 0 on one invoke); supplement bridge merges around manual logs; validator specs match written names + `whoop_workout` mini-schema (ends ~1,500 false warnings/fortnight); eightsleep 401-path persists the fresh token (live drill: run 1 persisted, forced cold start, run 2 no 401 — 18 password-grants/day dead) + framework secret-writeback retries once then ERRORs; every standalone writer stamps phase via public `phase_for_date()`; measurements re-armed end-to-end (S3 notification MERGED into bucket config + CDK permission; multi-row CSVs; date-rank session numbers — live drill: upload → trigger → record with phase, then cleaned); **#474 DECIDED: XML path RETIRED** (ADR-103 row, lambda+role deleted, backfill hard-guarded); **#497 DECIDED: garmin cron DISABLED** (ADR-074 updated; no Garmin rules live); #518 REQUIRED_SECRETS audited — **pipeline-health-check `failed:0` for the first time since May 25**. Gotchas: bucket notifications are replace-not-merge (get→append→put, backup first); deleting a lambda means pruning `ci/lambda_map.json` + the handler/wiring/ddb test lists; `imports/*` has an explicit DeleteObject deny (drill files are permanent). **area:data Now = EMPTY** — next data work is on Next/Later (journal remainder in #464, whoop webhooks A-8, unified-sleep A-2, TDEE chain B-1). Watch: `slo-source-freshness` 7-day window from 07-04; eightsleep relogin logs ~0/day expected (#481 AC, 7-day check). See `handovers/HANDOVER_LATEST.md`.
