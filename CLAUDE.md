...
- `docs/ARCHITECTURE.md` — full system design, ~106 Lambdas (CDK-defined; canonical count via `sync_doc_metadata.py`), 10 CDK stacks, data flows
...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-156), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-09-26 (Fable 5.1, **Session AV — the site night: a site a friend can read**; 21:30 PT → ~01:10 PT, attended to ~22:15 PT then autonomous). The owner re-scoped a planning session: *"I have not used it in the 3 weeks … friends overwhelmed with the layout, the content, what to do, and even some of the words"* — the whole Fable budget to the website; merge authority on site PRs granted, the pillars call delegated, boards standing in for the interview. **Method:** four read-only audit lanes (newcomer/friend with 60 screenshots · the subject's morning screen from served JSON · vocabulary census + coach-vs-coach · the #4163 triage) → driver-verified → a Fable red-team panel (Product Board + Matthew/friend/mother/an editor) with a dissent register → Opus/Sonnet build lanes → every PR rendered locally with saved live JSON. **Shipped, merged AND deployed under epic #4182:** #4195 the front door (the friends' read under the claim; reverses the 07-19 dial pick) · #4197 the chronicle opens on its first sentence · #4198/#4201 the vocabulary registry + shrink-only guard + the static-reach ratchet · #4200 the cockpit's three questions, the level demoted · #4199 the coaching door's one read per day, chosen by a stated rule, the weekly call labelled weekly · #4203/#4204 the gate fixes-forward. Docs: `docs/SITE_TRANSFORMATION_V6.md` (the end-to-end plan; §7 = the owner's morning list), ADR-156. **Engine PRs open, not merged (outside the grant):** #4192 open_actions · #4193 the genesis-bounded rate · #4194 coach-vs-coach/engine QA legs + the #4180 trend classifier · #4196 the tool-call residue guard. **Truth defects found live:** two loss rates across the genesis (#4184), a coach's "six days without logs" on 20/20 logged (#4185), `open_actions` empty with 9 pending (#4187), a raw tool-call tag served in `/api/decisions` (#4190), the chronicle's `[Weight:` line (#4191), 24 orphan drafts on an expired citation (#4183). **Incidents:** the visual gate red twice on the new pages and DECLINED rollback both times, correctly — one check-side false red (#4202 → #4203), one real a11y link red (→ #4204). **Gotchas:** `safe_merge.sh` does not exist; `lane_worktree.py` nests when run from a worktree; `gh pr checks` can't see a queued run; every new `BASELINE` entry is a census gate; never write "rebase" in a lane brief. **Owner acts:** the §7 list (door labels · home headline · the level name · the page cap · glucose · the subscriber counter · merge+deploy the four engine PRs · the DDB scrub + chronicle bracket · photos #3761 · the memory backup sync).