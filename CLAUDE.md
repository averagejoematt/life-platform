...
**Verified:** 2026-07-05 (session 16) — **the data-integrity opus quartet: 3 of 4 shipped** (5 PRs #633/#663/#649/#664/#666, all merged/deployed/verified; layer stayed v111). Matthew authorized all merges+deploys up front. ⚠️ **A CONCURRENT SESSION ran in parallel** and merged a large Now/Next slice (#378/#379/#382/#383/#389/#419/#470/#579/#580 + more — a11y, perf budget, weather-in-registry, dual-plane guard, drift gate, coverage ratchet); that work is live but **in its own wrap, not here**. Shipped this session: **#484** resurrected the dead TDEE/deficit chain (field-name mismatch — ingest wrote `expenditure_kcal`, readers wanted `tdee_kcal`; converged all readers + a labeled Mifflin estimate fallback; `/api/nutrition_overview` live: `tdee 3623 / estimate_mifflin / deficit 2116`); **#483** HAE webhook — `validate_fields` gate on every merge, unit-aware water/weight/distance, cgm-source by cadence (fixes the UTC-truncated partial-day mislabel), BP-loop break fix; **#476** Notion — `last_edited_time` edit sync, stable page-id SKs, deletion reconcile, raw S3 archive. **Load-bearing gotchas:** HAE has NO shared layer + bundles siblings via `from_asset` → marked `cdk_only`, ships via `cdk deploy LifePlatformIngestion` (a single-file hot-deploy would strip siblings — #382 hazard); the `test_count` doc-drift trap **compounds** across stacked same-base PRs (re-sync after each merge); import-time-frozen module globals make tests order-fragile (`monkeypatch` the global, don't trust env); CI Deploy is gated behind Unit Tests (red test → Deploy skipped → deploy manually). **Flags for Matthew:** **Notion API is 401** (expired token, pre-existing) — #476 code is deployed+clean but its archive/reconcile data-path is unverified live until the token is refreshed; two Claudes on one repo made main churny (coordinate); the I22 site-version lag is red until the next static-site deploy (the concurrent site work clears it); GitHub Pages still enabled+public. **Next:** **#581 evidence.js split (the 4th opus item, DEFERRED — do attended)** — a 3k-line byte-identical refactor that collides with the concurrent session's live site-ux work; do it in a worktree once that settles · #544 methods registry (unblocks #584) · refresh the Notion token then confirm #476's S3 archive. See `handovers/HANDOVER_LATEST.md`.
