...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-137), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-07-20 (night) — **THE CYCLE-9 RESET, verified by behavior for the first time + Day-3 drain (13 PRs).** Matthew unblocked mid-session ("315lb for now") → `restart_pipeline --genesis 2026-07-20 --override-weight-lbs 315 --with-preregistration --apply` ran clean (rendered PASS, semantic PASS 0-poisoned/32-sources, truth SKIPPED honestly at tier-1; the frozen-prereg protection correctly forced a deliberate cycle-8 archive before re-seed). Prereg published + sealed (908fa45a… live at /experiments/prereg/genesis-2026-07-20.json), predict-week live (re-run Monday → W30), the eve-only lock email SENT — tonight WAS the eve. **#1559 SHIPPED first and proved it**: the behavioral harness (4 legs + synthetic HAE round-trip) ran twice pre-reset against cycle-8 (caught 4 of its own bugs + 4 true dark sources) then post-reset as its first customer: cycle=9 ✓, Day-0 Level-1 sheet + replay_verified ✓, 80/80 URLs ✓, synthetic dedup EXACT + verified cleanup ✓. Its static gate RED on 12 unguarded pillar chain-reads in 4 email lambdas → all fixed + standing CI gate (Monday Compass would have crashed on the Day-1 sheet). **#1544 SOLVED**: GitHub job annotations say payments failed / spending limit — owner billing fix needed; all 13 merges rode local full suites + manual deploys. Merged+deployed: #1560/#1579/#1562/#1561/#1585/#1580/#1565/#1583/#1584/#1581/#1582/#1587/#1588 (adversarial review BLOCKED 2 on real MAJORs pre-merge — memory-starvation trap, false 90d retention). **Monday-morning DONE (10:45 PT):** real weigh-in 321.38 → full same-genesis re-anchor (cycle stayed 9, gates PASS, prereg re-landed verbatim — seal 908fa45a… held), predict-week W30, restart_verify 11/12 (the 12th = tomorrow's 16:30 UTC sheet, by design), site == 4718e7b9. NEW true positive: the canary's FIRST 3×/wk run found itself blind — 5×403 from site-api-ai, readers verified fine → #1589. Dark sources for Matthew: hevy since 6/25 (lifts logged?), notion 5/25, strava 7/14. Billing STILL broken. Full narrative: `handovers/HANDOVER_LATEST.md`.
