...
**Verified:** 2026-07-19 (evening) — **Day-2 drain: 12 issues closed honestly (104→92 open), all SHIPPED + live-verified.** Flagship: **#1410 THE GHOST** — pure-Python BSTS-lite counterfactual (lambdas/bsts_lite.py: local-level Kalman + OLS on FROZEN control specs, deterministic q-grid ML), spec frozen at pre-registration, effect = observed − ghost with the full-covariance CI or a STATED refusal (MAPE gate); card renders the served widening band; methods_registry fp bcde99d1a787 live in /api/methods (PR #1551, guards 8-red pre-fix). **QA depth:** /qa manifest modes + qa_audit coverage-drift + the SSM qa-level dial (#1449/#1450/#1452, PR #1557 — param created, IAM deployed); static-page structural smoke (#1429, smoke now 166); weekly advisory WebKit mobile sweep (#1434, first fire Tue 21:37 UTC); advisory workflows now file/auto-close `auto-filed` issues (#1447); Monday green report DEPLOYED for tomorrow's 16:00 UTC email (#1446). **GitHub-side sentinel (#1320 + #1544 detector, PR #1558):** GET-only posture asserts + main-push run-liveness (guard-red recovered the exact six #1544 shas); live drift found: vulnerability alerts DISABLED + the #1319 gate — 2 standing weekly lines until owner acts. **Docs truth:** COST_TRACKER from live CE (real floor ~$36–43/mo, ADR-133 amendment: tier-1 residence accepted, 45d freshness gate), Day-1 docs vs live crons (Garmin PAUSED taught honestly, >60d stamp advisory naming 8 stale docs) (#1354/#1348). **Utility pages de-templated** (#1473, Litmus 10/10). **The radar rescue:** Matthew missed the constellation — promoted to first beat under the loop (~3,570px, /#pillars), live-verified after healing a superseded-skip deploy race (the standing reflex worked; INCIDENT_LOG row). **In flight at wrap:** #1433 a11y PR (agent finishing its day-one baseline — next session's first merge). Monday-vs-Sunday caught: prompt said Monday, clock said Sunday — W30 rebuild + ops-email glance deferred to actual Monday; felt-probe verify tonight. **Decision menu (Matthew):** #1544 owner legs, GH_POSTURE_TOKEN PAT (Admin:read+Actions:read+Contents:read), Dependabot alerts toggle, #1319 fork, SNS click, #1114 pick, #1350 sign (email-subscriber NOT DEPLOYED until then — expected), #1329 rotate, Withings weigh-in. Full narrative: `handovers/HANDOVER_LATEST.md`.
