...
**Verified:** 2026-09-01 (Opus 5, **Session P — the Day-1-eve bug drain**; plan under the September posture — "opus or below only, no fable" — then the owner steer "paying down as many bugs and open issues"). **Shipped:** main un-redded at boot (`f82c44f0d`); 4 PRs merged; 3 issues CLOSED — **#3378** (PR #3397: ci-cd.yml's push `paths:` filter REMOVED, so every push to main mints a real verdict; option (b) chosen once the cost was MEASURED at ~39 min wall / $0 on a public repo, with no added deploy or AI spend), **#3396** (PR #3398: `restart_verify.py` check 20 — the SERVED genesis must equal the staged one, live-proven), **#3401** (PR #3402, owner-approved deploy: R7's night floor bounded by `min(genesis-1, today-1)`). Plus **#3374 R2 of 3** (PR #3400: a month growing >1.15x must name a `driver:`; the threshold is derived — the founding incident is 1.158 and just trips it). **3 filed on measurement:** #3399 (the reader-truth judge labelled a self-withdrawn finding `basis:impossibility` and it GATED; `basis:"withdrawn"` emitted 0 times in 35 findings), #3401, #3403 (unit-test budget now sits at the MEAN — 1942s vs 1950s, n=4). **Gotchas:** a filed issue's mechanism is a HYPOTHESIS — #3396's was disproven in 3 min (`describe-stack-events` is the decisive instrument) and FOUR hypotheses died with it; the divergence is still unexplained and recorded as such. The CI-warnings gate passes by DEGRADE after a lease rejection; `gh api runs?per_page=10` TRUNCATES the lease enumeration (one waited 1.1h unseen); a lit alarm's citation may contain no closed `#N` anywhere, notes included. **Open (September = owner USES the platform; bug-fix-only on Opus, Fable out — no credits):** #3390 Day-1 runlist, pre-flighted (escapees = 3, named by partition); #3403; #3399; #3374 R1+R3; #3395 (designed in full on the issue, deliberately not shipped on launch eve); #3369. Full narrative: `handovers/HANDOVER_LATEST.md`.
