...
**Verified:** 2026-07-06 (the fable batch — all 6 `model:fable` R21 stories in one session) — **5 PRs OPEN, NOTHING merged or deployed; no authorization was given this session.** Directed session ("all model:fable issues, efficiently without sacrificing quality"). **PR #762** (#745): ADR-124 Proposed — fulfillment capture-channel memo (A StateOfMind→HAE + C two-scalar-ritual floor recommended; B rejected) + bad-week publication posture decided BEFORE data flows. **PR #763** (#731, `Fixes`): /method/ opens with the static trust contract (`.trust-doc`+`.prose`, ten sections, the #725 incident disclosed; Playwright JS-off/on + 390px verified) — **merge with/after #765** (describes its pre-registration mechanism). **PR #764** (#732, **HOLD**): protagonist hero fold (who/sixteen-attempts/315→185/finish line "185 held 90d or failed") — needs Matthew's finish-line pick + confirmation of the R21-sourced "sixteen" (not repo-verifiable). **PR #765** (#728): pre-registration pipeline — `stopping_rule` REQUIRED in `experiment_design` (**layer module → v117 on deploy**), public timestamped artifact `generated/experiments/prereg/{id}.json` (fail-soft honest), CF `/experiments/prereg/*` behavior, scoped MCP IAM, `/api/experiments` passthrough + card render; 3 candidate designs commented on #728 (A sleep-window recommended); synth-verified in templates, sweep 106 green. **PR #766** (#740): the org-chart essay draft + talk outline + venues (failures-forward; `[CONFIRM]` flags where self-reported; the "case-insensitive worktree" framing was NOT verifiable — essay uses the documented squash-inheritance mechanism). **#748**: hard-gated (needs #745 pick + ≥4 flowing weeks incl. a rough patch) — status commented. All PR checks green (the #408 render+accuracy gate passed on site PRs). **The batched ask (4 items) is in `handovers/HANDOVER_LATEST.md`**: finish-line pick · first-experiment pick · channel pick+posture confirm · merges & the deploy train (layer v117 per CONVENTIONS §1, cdk Web/Mcp, MCP zip w/ `lambdas/reading/`, `deploy_site_api.sh`, site sync). Pre-existing `stash@{0} "session-local"` + untracked `docs/reviews/R21_BACKLOG.md` left alone. **Next after the ask:** #726 legacy-prediction tombstone (mutates prod data — confirm first) · Session B epic #716 (#730 static proof render, #733 permalinks) · budget pair #738/#737.
