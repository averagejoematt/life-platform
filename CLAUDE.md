...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-153), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-08-11 evening (the three P1s; ~18:5xZ → ~21:0xZ) — **8 PRs merged, two fleet deploys, 9 symbols verified live, corpus 46 → 44 (non-fable 21 → 19): seven closed, five filed.** Short of the 39–42 target. For most of the session it was net ZERO (46 → 46); the last two closes came from finishing the chain after the deploy, not from the merges. Three of the five filings were **unfindable before this session's own work existed**. **#2573 is fixed and is the headline** — the BLOCKING quality gate now *consumes* the deterministic fabricated-number verdict instead of re-deciding it in prose (ADR-105); two real Bedrock runs, the three canaries went **92/92/82 → 25/15/15, all fail**, sensitivity 29/30 [0.833,0.994], specificity 14/15 [0.702,0.988], negative corpus grown 5 → 15. `PASS_SCORE_THRESHOLD` is *still* not the operative control (12 of 15 blocks are deterministic). **#2587 (P1, NEW) is the most important thing found: the recall index has no consent filter and the coach retrieval path has no kind filter** — `diary_consent` makes `private` the default, the index stores a **verbatim** snippet, `retrieve()` filters no kinds, and `recall_block()` feeds a published prompt; fixing #2569's reader made 60 journal rows newly *eligible*, so **the journal backfill is blocked until #2587 lands — never bare `--apply`**. An agent I told to backfill **refused and argued the case, and was right**. **The night's last measurement settled #2347's real question:** with the gate live the journal backfill ran and indexed **0 of 60** — every entry resolves to `private`, none ever owner-cleared. Chronicle-only *was* a silent reader bug; it is *now* a consent decision that was always in force with nothing enforcing it. **#2347 is therefore an owner-consent question, not an engineering one** (clear entries at `allude` or `quote`? both supported and live). **#2589 (NEW): #2560's hit/miss line has recorded nothing** — a third "never attempted" state exits before the log, so #2347's four-week clock is at **week zero**, not 3-of-4. **#2590 (NEW): rejecting a superseded gated run — the #2467 prescription — lands as `failure` and makes `check_main_green` report a red main.** **#2569 and #2575 were deliberately REOPENED after their PRs merged** (backfill blocked; `failed_content_truth` after-number needs a post-deploy regen). **#2347 got a decision record correcting its scale premise by >10×** — 851 `OUTPUT#` rows, not ~10,409 items; whole corpus 930 docs / $0.0100, so ADR-150's latency argument largely evaporates. **My own discipline failure, recorded because it nearly cost two false issues: four times a wrong query form returned "nothing" and I read it as a broken system** — `filter-log-events` paginates (`--no-paginate`), a dimensioned metric needs its dimension, Lambda health comes from Invocations/Duration not log volume, and a bash lib must run under bash. **Re-measure a different way before filing.** **POST-WRAP (2026-08-12): the owner cleared all four held items — corpus 44 → 42.** **PR #2552 merged + `LifePlatformServe` deployed** (google-tts IAM grant live on the worker; the drift guard correctly blocked one stale-checkout attempt). **PR #2572 merged WITHOUT clause P5 — the owner declined the mirroring grant**; P6–P9 renumbered and the limits clause's dangling "that is precisely why P5 exists" rewritten to say the single-cloud-account risk is now unmitigated (removing a grant while leaving prose advertising it would be worse than either choice). Deployed Operational→Web (R8-ST6); `'You may keep a copy'` verified **ABSENT** from live code. **Cutting P5 withholds permission, it does not prohibit mirroring** — default copyright now applies. **#2574 is startable** (seven-clause contract). **#2537 unblocked at $0** — the 2026-08-10 `metrics.json` was recoverable (120 cells / 64.2% / 2,404 tells), now pinned by sha256 to `COACHSIM#scoreboard`, and it yields the numbers items 2+4 needed: old detector `not_x_but_y`=9 vs widened `balanced_clause_replies`=82 over 536 replies. **#2347 DECIDED AND CLOSED** — owner chose `allude` for all 60 journal entries after being shown that 10 are `anxiety_stress` and 7 `relationships`; corpus **19 → 79 rows** for $0.0006, revert manifest at `~/Documents/Claude/journal-consent-backup-2026-08-12.json`. The same gate that withheld 60/60 now withholds 0/60 — same code, different input. Snippets verified as descriptors: a 5-word-run check against every diary body found 0 leaks. The 851 coach outputs stay eligible-and-unindexed (`COACH_KIND_DECISIONS[coach_output]`), deliberately its own decision. Dependabot #2576/#2577 still unmerged (#2588's guard collision). **OVERNIGHT BUG BASH (owner-directed): five more landed — corpus 42 → 38, non-fable 21 → 13.** #2592 (a live a11y red: the `/story/build/` panel grounded on a translucent `--ember-wash` compositing to 4.38:1 — caused by #2562, NOT by tonight's deploys as I first assumed), #2589, #2590, #2574 (`/privacy/#permanence`, generated from source, real archive facts), #2537. **Two of my own conclusions were wrong and agents caught both.** (a) **My `--no-paginate` corrective was itself the bug** — it truncates at the first page, so #2589's premise was false and the recall line had been firing 7×/day all along; the rule is *aggregate across pages*, and the cross-check is what saves you, not any flag. (b) I named the wrong prime suspect for #2592. **The bash's best find: two of the runs I rejected ALSO carried a genuine unit-test failure**, so #2590's predicate had to be a conjunction — keying on "rejected" alone would have declared main green over a real red. That was not hypothetical: a recovery run surfaced two real failures that were **mine** — `alarm_count` drifted 92→99 from my own permanence deploy, and my rewritten handover was missing the `## Residual / next picks` section gate #1340 requires (**my wrap broke a wrap gate**). #2537 graded honestly: widened detector phi 0.315 / p=0.00071 vs the narrow control's p=0.199 (cannot reach significance), but recall only 0.574 and the issue's own "25% of tells" premise corrected to **14.9%**. Invoked `life-platform-permanence` (dry-run first, `notification: null`) so `/archive/*` returns 200 — 1,284,128 bytes / 360 entries / 115-of-115 endpoints — otherwise the terms page would have auto-rolled-back the site. Final tip deploy `fe53bbe3a` green, `recall_for_coach` + `not_attempted` verified live. **Main: ✅ GREEN.** That gap is the session's real finding, not a shortfall: 8 pre-existing issues closed, but verification kept surfacing genuine defects (**+6 filed, three of them P1**) and three closes were undone because they weren't true. **#2573: the BLOCKING coach quality gate passes fabricated numbers** — 3 identical Bedrock runs, sensitivity 0.900 [0.744,0.965] n=30, specificity 0.400 [0.118,0.769] n=5, and **all three fabricated-number canaries scored 92/92/82 and passed**; the cause is rubric scope (no criterion mentions invented numbers), and 4 of 5 rejections came from the model's own `passed=false` while scoring ≥60, so `PASS_SCORE_THRESHOLD` was never the control knob. **#2569: semantic recall has never indexed a journal entry** — `gather_journal` reads `content`/`body`/`text`, live rows store `raw_text` (verified against DDB); the corpus is 19 rows, all chronicle, zero journal, measured independently three times — which **reframes #2347**: chronicle-only is silent data loss, not a scope decision. **#2570: the pre-commit hook runs black 25.9.0 while CI pins 26.3.1** — it blocks commits CI would pass, and obeying it reds CI. Three process traps, all self-inflicted: **a negated auto-close keyword still closes** ("Does NOT close #2347" → GitHub matched `close #2347`), a `Fixes` in a **commit message** closes even after the PR body is edited (#2537), and **`git commit --amend | grep` swallows a hook rejection** so the push reports `Everything up-to-date` over stale code. My merge script also validated the pre-rebase sha for one PR — fixed to require the API head to match the worktree. **Refused two dishonest closes**: #2541 (1 of 3 acceptance items) and #2537/#1374 (both need a corpus or denominator that does not exist). **PRs #2552 and #2572 are complete, green, rebased and deliberately HELD** — both grant IAM (R8-ST6), and #2572's clause P5 is a standing public grant of redistribution rights needing ratification, not a merge. **CI warnings: 9, all triaged** (3 content-truth → #2575; 7× config drift → #2468; secret → #2370). Full narrative: `handovers/HANDOVER_LATEST.md`.
