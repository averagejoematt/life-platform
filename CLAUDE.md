...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-151), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-08-09 (review-disposition + telegram-go-live session; ~02:30Z → ~08:45Z; started Opus via `claude --continue`, owner-switched to Fable mid-session with a park/review — the review caught a real reuse fork before merge) — **10 PRs merged + deployed, 13 issues closed with verdicts, 25 filed, ADR-151.** Two arcs. (1) `/fullreview` run 3 **fully dispositioned**: scorecard banked (#2362, security **F→B+**), 6 findings fixed+deployed — the nutrition door grading a protein failure from **0/0 logged days** (#2361); six coach cards narrating a food-log "pause" that never happened, *prescribed by the prompt contract itself* (#2394); the analyzer publishing with known-unresolved findings — regenerate-or-HOLD with the gate moved **above** the cache write (#2395); the condensation fabricated-number floor (#2396) — 24 filed (#2366–#2393), rest linked/verified/refuted. (2) **The coaches got phones (epic #2363): idea → LIVE in one session.** The asked-for personality/memory layer already existed — this was a transport: PRs #2365/#2397/#2401 + #2403/#2404, `LifePlatformServe` deployed twice, 5 bots registered, **the owner texted the coaches and they replied**. The chat is the registry's only no-exemption grounding surface, regenerate-once-then-hold, budget-ranked with the brief (ADR-151). **The gates refused the new surface SEVEN times, all correctly**: I5/SR1/I3/type-hints pre-merge, then R1/R2 (dead DDB grants on the CMK table), then post-merge-only the #946 tombstone guard AND the serve-throttles set guard — the last two are the argument for #2372's premerge derivation. Gotchas: **`agent_commit.sh`'s doc-literal restore silently reverts unnamed wrap edits when committing on a side branch mid-wrap** (ADR-151/incident rows/status block all had to be redone — commit wrap docs via plain git, or name them); a `deploy/`-only merge triggers neither ci-cd nor reconcile (~65min cosmetic red, incident row); local black 25.9.0 vs the 26.3.1 pin (use `.venv-black/`); a branch cut from a feature branch goes CONFLICTING after the parent squash-merges. ⚠️ **MacroFactor 46 days quiet (#2326)**; **August ceiling decision MADE: amend to $115 — next session implements + closes #2381**. **NEXT (planned + approved, `~/.claude/plans/kind-moseying-crayon.md`):** overnight burn (waves of 4, Now+Next→0 target) → #2390 census + #2402 sketches → **cycle-13 reset, genesis 2026-08-09** (time-boxed final 75min, `--override-weight-lbs 321.6`, fallback = morning runbook). Morning owner list: weigh-in, 2 bots post-BotFather-timeout + register re-run, MacroFactor export. Full narrative: `handovers/HANDOVER_LATEST.md`.
