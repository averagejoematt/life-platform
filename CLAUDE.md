...
**Verified:** 2026-08-20 (Thu afternoon/eve PT, Opus, plan `quizzical-wandering-dawn`; boot = charter + `blast_radius.py`; **no `model:fable` issue touched — by design**, the plan was built from the 59 workable non-fable issues) — **89 → 86 open; fable 25, unchanged.** Shipped the **#2799 deploy-plane cluster**: **#2830** (`scripts/assert_pr_green.py` — an empty *or incomplete* rollup is not green; parses both the `statusCheckRollup` and `gh pr checks --json bucket` dialects), **#2826** (`head_coverage()` on `deploy-wedge-watch.yml`'s 15-min cron, three-way exit so it cannot fail dark), **#2920** (bundled-config deploy triggers **derived** from `build_bundle.py`), **#2831** (API-before-frontend pre-merge check, replayed against #1704) — all four now rows in **`docs/CONVENTIONS.md` §9**, landed as ONE consolidated commit because four workers editing that file would collide. **I chose these four because I had personally hit three of them the session before, not knowing they were filed.** **Phase 0 caught two wrong premises before any worker built on them:** `head_coverage` called a *path-filter skip* a swallowed push (`Docs CI` ran on that sha) — wired to a cron unchanged it would have paged on every docs commit; and **#2920's premise, filed by me 3h earlier, was wrong** — `config/**` was ALREADY in `paths:`, so my stated fix was a no-op. The real defect: `ci-cd.yml:650` gave `config/food_vocabulary.json` a hand-written fleet trigger and **forgot `personas.json`**. **The derivation then found 15 files nobody knew about** — `--print-bundled-config-paths` returns **17 paths** incl. every `config/coaches/*.json`, all shipping in every bundle with no trigger. **I shipped a live false positive and caught it 20 min later:** #2925's cron called every `chore(reconcile)` commit a SWALLOWED PUSH — a `GITHUB_TOKEN` push never dispatches (anti-recursion) *and* the commit touches `lambdas/**`, so it hit the partial-swallow branch. A reconcile follows **every merge**, so it would have paged forever. Fixed in **#2927** (`ZR_BOT_PUSH_NO_DISPATCH`), verified on merged main, with a guard that a *human* zero-run push still pages. **The wrap itself found two more:** a production gate **parked 2.6h** past the #1901 threshold (I had swept twice and still missed it — *sweep in a loop, not once*), and main genuinely **red** because #2831's new gate lacked `if: always()` — **the gate built to stop silent failures could itself be silently masked** (#749). Both fixed. **Filed #2924, now at five instances:** the 168-test 'structural set' every brief uses as a pre-merge proxy is **not in CONVENTIONS.md, not a script, not a workflow** — oral tradition, underived, and green through all five of the last two sessions' main-reds. **Deploys:** none — every merge is CI-logic-only, so a redeploy is a no-op; **4 production gates rejected, none left waiting.** **Owner asks (4):** a Withings weigh-in; the one-line coach-colour call (`sleep_coach` `#818cf8` on a11y grounds, 6.21:1 vs 4.37:1); #1738/#1571/#1677/#1631 + #2833/#2834; promote the #2831 check from advisory to required. **Next:** #2924, #2829 (CDK owns **one** of six us-east-1 alarms — route that, defer the `cdk import` three), #2921, #1221 (the live P1: 21/21 behaviours still on legacy `ForwardedValues`). Full narrative: `handovers/HANDOVER_LATEST.md`.
