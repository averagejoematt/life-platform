...
**Verified:** 2026-07-05 (session 14) — **fable queue at ZERO + session-13 residuals shipped** (PR #622, squash `49182230`, live-verified 8/8). All 36 `model:fable` issues CLOSED; every open story is opus/sonnet by design — a next fable session triages fresh fable-tier stories from the epics (#525–#528/#575) or runs `/uplevel`. Shipped: **the CC-10 "My Team" collective read re-mounted** as the lead entry of the coaching door's Team section (Marsh's signed portrait reachable again; huddle click-through); **deep-link bug fixed** — `/coaching/coaches/` used start key `"coaches"` (not in BYKEY) so it silently fell to The Read and dropped `#coach` links (podcast guest bylines + popover "full page →" + old 301s all landed wrong; now starts at team, ids = persona ids); **dispatches.js dead coach machinery deleted** (−107 lines); **session-13 build beat live** on /story/build/. **Gotchas:** the site's service worker BYPASSES Playwright `page.route` mocks — local render QA needs `service_workers="block"`; motion.js scroll-reveals make full-page screenshots show blank space where DOM content exists (verify by DOM count, not pixels). **Flag for Matthew (unactioned):** GitHub Pages enabled+public on the repo — disable or bless. **Watch:** Sun 07-06 runs (hypothesis engine, panelcast v2, inter-coach SKIPS); Mon 07-07 data-recon; forecast resolutions since 07-05. **Next:** session-14 beat slot open (team-view resurrection is the candidate); Now milestone #577–#581 (sonnet/opus); Nakamura/Murthy/Cora portraits conditional; mouth-frames undriven. See `handovers/HANDOVER_LATEST.md`.
