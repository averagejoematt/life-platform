...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/common/constants.py` (currently **2026-09-01**, cycle 15 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-09-01 (Opus 5, **Session Q — Day 1, the launch-day P1**; open ask — "what should we work on today, i have a day off" — answered as *subject first, then I build*, with a standing deploy grant for bug fixes). **Shipped, merged AND deployed AND verified live: 2 PRs, 2 issues closed, 3 filed.** **#3413 (P1)**: `/api/board_ask` was serving **504s to real readers for ~34h across launch day**. Filed as a bad timeout; measurement made it worse twice — `coach-quality-gate`'s p50 is **10434ms (7d, n=211)** / 16371ms (48h), so the 10s client cap sat below the callee's **median in both windows**, and 7 days of live board traffic gave **8 attempts → 6 skipped, 2 timeouts, 0 verdicts** with `BoardQualityGateFired` empty for all of August. The gate had never once worked on that surface, so it was REMOVED from the reader path (PR #3415), not retuned; ADR-108's scope narrowed. Live: **33.9s → 12.8s http 200**, verified by shipped zip CONTENT, canary `board_grounded:status: OK — 200`, `ai-canary-overall` ALARM→OK 12:20:36 PT. **#3412** (PR #3416): the alarm-citation gate read `DATE#2026-08-24` as issue #2026 — and that false red was **masking a true one** (`#3410` really was dead). **Filed:** #3414 (async verdict — deliberately not bolted onto a P1), #3417 (the "scale is weight-only" premise is false since 08-16; the `lean_mass_floor_lbs: 155` hard stop has no instrument — **hold the handles**), #3418 (six `Pending owner cdk deploy` warnings returned 1 day after #3365 verified them cleared). **Gotchas:** my own guard was VACUOUS and only the mutation proof caught it (token-joining split the multi-token needle); `gh run list --commit <SHORT_SHA>` returns **0 silently**, identical to a real swallow, and the `-f head_sha=` form in memory 404s always — use `?head_sha=$FULL`; "approve the tip, reject ancestors" earned itself when the reconcile commit carried a `platform_counts.py` change the P1's sha lacked. **Open:** #3390 at 23/24 (blocked on a real weigh-in — the scale did not register upstream either); the **7-persona board case is UNMEASURED** (rate limiter; synthetic probes declined to protect Day-1 coach memory); #3403, #3369, #3374, #2883, #1407. **Not filed, owner call:** `coach-quality-gate` exceeds its own 30s ceiling on **4%** of invocations, so ~4% of daily-brief coach sections ship silently ungated. **Through-line: three instruments whose green came from being unable to fail.** Full narrative: `handovers/HANDOVER_LATEST.md`.
