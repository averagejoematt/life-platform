...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-133), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
3. **Serve/Compute**:
   ...
   - **Compute Lambdas** (5) — run before the 17:00 UTC brief daily: `character-sheet`, `adaptive-mode`, `daily-metrics-compute`, `daily-insight-compute`, `hypothesis-engine`; store pre-computed results to DynamoDB
...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/constants.py` (currently **2026-07-12**, cycle 5 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-07-10/11 overnight ("another reset, genesis Sunday 2026-07-12 — audit the content first: 2 weeks of manual silence, do the coaches sound accurate? character should be sad" → blanket green light → "nuke it NOW, countdown to Sunday" → done). **8 PRs merged+deployed+live (#917–#922, #939, #941) and the CYCLE-5 RESET EXECUTED** — the site is LIVE in pre-start countdown ("T−2 days") until Sunday's weigh-in. Truth audit (47-agent workflow, 34 verified findings, 4 critical): AI coaches mostly named the stall; the deterministic layer papered over it and the character LEVELED UP 8→13 during 14 silent days (up-gate scale bug). Shipped: privacy hotfix (live genotype leak + real-person recommenders — #920), presence severity ladder + acknowledgment gate in every narrative prompt + habitify gap-0 bug (#921), character atrophy/XP-debt/mood/dormant-UI (#919), 13 dead-source honesty fixes (#922), reset-protocol clean sweep (#918 — wipe-coverage crash, fail-fast, media archival, cycle bookkeeping), coach check-in MCP loop (#917, 64 tools), countdown state (#939), reset-aware tests (#941). Reset detail: cycle SSM=5, ~2.2k tombstoned (nothing deleted), lead-ins upleveled+re-dated as Prologue I/II via new `restart_leadin_pages.py`, verify 40/40, site-deploy+smoke+visual-QA green. **Presentation rule (Matthew): only `/data/cycles/` knows it's another attempt — no reset article, EP0 podcast = pure introduction.** Gotchas: DDB reserved word `hidden` in resurrect · pipeline resume loses outgoing genesis (pass `--old-genesis`) · lambda-invoke base64+raw flag = 254 · deploy gates needed pre_start awareness (2 auto-rollbacks) · wiki session collision (#926 reverted 3 docs, restored; string PRs in beats.json). **Matthew Sunday: weigh in → re-run the pipeline command (re-anchors baseline); + the 3-line eightsleep dupe delete (classifier-blocked).** Open: #942 (sentinel pre-start grace — ALARMS harmlessly until Sunday), EP0 week-1, #917 injection follow-ups. Full narrative: `handovers/HANDOVER_LATEST.md`.
