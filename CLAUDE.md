...
**Verified:** 2026-07-05 (#589 the honest freshness pulse, solo) — **shipped, merged, deployed, live-verified.** Matthew had already authorized edits/merges/deploys for the session. New reusable primitive: `tokens.css` §12c + `motion.js`'s `wireFreshness()` (kept beside, not merged into, the existing `data-cpts` chart-wiring per the issue's ask, so it stays merge-clean with the still-open #590 branch) — any `data-fresh-ts`+`data-fresh-window` element pulses ONLY while inside its own real freshness window (thresholds now also returned by `/api/source_freshness`/`/api/last_sync`, sourced from `source_registry.py`), falling through to the existing motionless `.pv-stale` past it; reduced-motion keeps color, drops the keyframe. **Adopted on:** the cockpit sync line (retired the flat 45-min `SYNC_FRESH_MIN` guess for each source's own `stale_hours`), the `/data/` pipeline board, and — its first real use — the `.provenance` design-system kit (documented in DESIGN_SYSTEM_V5 for weeks, never actually wired to a page until now). Unit test (`tests/test_freshness_pulse_589.py`) extracts and drives the ACTUAL shipped `freshWindowOk()` via Node (sentinel-fenced in `motion.js`), not a Python re-implementation. **Deployed:** 1 site-api (`deploy_site_api.sh`, layer already v114, no re-attach) + 1 static-site sync; both verified live (new API fields present, `version.json`==HEAD, **smoke 67/67**), plus a live-browser DOM check confirming real per-source pulse/stale states against production data. **No layer bump, no CDK.** Tests green throughout (3620 passed). **Gotchas:** grepping for the `.provenance` byline pattern found zero adopters anywhere despite being fully speced with example markup — a documented design-system primitive isn't an adopted one; running `gh pr merge --delete-branch` from inside a worktree that had the feature branch checked out switched THAT worktree onto `main` post-merge, which then blocked the primary repo directory from also checking out `main` — deployed from the worktree instead (it matched `origin/main` HEAD exactly) rather than force anything in the primary directory. **State at close:** `main` clean == origin, #589 closed (PR #705). **Not mine, not resolved:** the primary shared working directory was found mid-session on `feat/408-pr-render-accuracy-gate` with uncommitted changes — a concurrent agent's in-progress #408 work, left completely untouched (did all #589 work in an isolated worktree instead). **GitHub Pages still enabled+public (carried, unactioned).** **Next:** **#590** home cinematic (constellation v2 + waveform onto `data-cpts`) is still queued, deferred by Matthew's call in the prior session — see `handovers/HANDOVER_2026-07-05_MotionEpicWrap.md` for its full scope + the open pillar-correlation data question (unchanged this session). Then **#408** (in progress concurrently, see above) · **#409** batch-price content AI (layer bump). See `handovers/HANDOVER_LATEST.md`.
