...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-136), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-07-18 (evening) — **Big-architecture paydown: 4 issues closed properly, all live** (parallel plan-only design session same night — its block superseded here, narrative in `HANDOVER_2026-07-18_DesignPartnerPlan.md`, backlog #1460–#1475). Matthew pre-authorized merges+deploys. **Shipped+merged+deployed+verified:** #1371 cold-start-as-armed-instrument (PR #1457 — NEW `lambdas/experiment_gates.py` ONE arming-threshold registry imported by the correlation/hypothesis engines + coupling floor; gates+measured current_n in shaped-empty /api/correlations+hypotheses; carried/carried_from_cycle provenance + experiment anchor on /api/source_freshness; RESET-MANUFACTURED-GAPS no-scold clause in the phase block; `.warmup` hollow-mark grammar on 2 zero-states; live: gates serve 0/10 honest pre-genesis), #1405 private intake ledger (PR #1458 — intake_count tap → SOURCE#private_intake never evening_ritual; `intake_response.py` n_eff/p-on-n_eff/block-bootstrap CI, dose bins arm at 15 nonzero; MCP log/get tools + brief line; privacy contract test pins the boundary BOTH ways), #1403 flourishing daylighted (PR #1459 — `flourishing.py` daily provenance-stamped PERMA projection; zero-LLM backfill ran: 18 rows live in DDB; MCP get_flourishing_trend; flourishing row = PRIMARY Relationships input, Mind values_alignment config-gated, S3 config uploaded+verified), fan-out #1322 (PR #1456 — deploy README/RUNBOOK rewritten vs live scripts, MCP-zip recipe tombstoned, deploy-docs gate). Also healed public_stats platform counts live (121/26/62→64/20/94; brief had run pre-deploy). **Un-red mid-wrap:** #1405's dynamic write pk redded the orphan-partition gate (masked locally by `pytest -x`) — fix 28ee812a inlines both write literals in the update_item call; **28ee812a ran GREEN end-to-end + fleet-deployed** (MCP bundle AST-verified: 67 tools). **Gotchas:** orphan gate needs inline write literals; `config/character_sheet.json` feeds the GENERATED /method/game/ page — regenerate same-commit; parallel-session staged `git mv` rode into my commit (check staged state). **Attended queue:** Sunday `restart_verify.py` + brief-writes-64/20/94 check; prereg publish awaits OK; decision menu: #1319 posture (blocks #1338), SSM cycle-param IAM line for site-api, #1114, #1243/#748/#1187/#1029, gate:owner #1350/#1329. Next picks: #1409 (time-sensitive weekly probes), #1426 QA manifest, #1395, #1376, design #1462→#1464+#1469. Full narrative: `handovers/HANDOVER_LATEST.md`.
