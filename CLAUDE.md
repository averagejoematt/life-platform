...
**Verified:** 2026-07-04 (session 2, part 2) — **The Fable LATER batch (#410 #403 #404 #406 #407 #413 #398) SHIPPED + DEPLOYED LIVE + verified** (PR #455 merged; 4 stacks + site-api + site synced; live proof in `handovers/HANDOVER_LATEST.md`: last_sync strip real stamps, explain probe grounded on r=0.843, week-moment minted w/ own card, fold in-frame both breakpoints, between-chronicle dry_run assembled 1 real finding — send dark behind EXTERNAL_EMAILS_ENABLED). **NEXT FABLE SESSION queued: the data-source health review /plan (brief in memory `project_data_source_health_review.md`).** One commit per issue on `feat/fable-later-batch`: #410 bounded coach compression (windowed reads + honest rollups + budget guard; replays the 52/39 failure), #403 `/api/explain` server-grounded explainers (allowlisted surface names only; injection closed by construction), #404 permalinked moments + per-moment OG cards (`/moments/*`; recap · board answers · graded predictions; share buttons), #406 the cockpit sync strip (`/api/last_sync`, real ingested_at stamps, earned glow ≤45min), #407 the loop teaser under the H1 (in-fold verified 1440×900 + 390×844) + the constellation /100 legend, #413 the zero-storage 'where would you land' mirror (Playwright-proved zero non-GET during interaction), #398 the between-chronicle subscriber note (zero AI, content-hash dedup, no tracking; **ships dark behind EXTERNAL_EMAILS_ENABLED=false** — flipping it is Matthew's call). Suite 2,712 pass (same 5 pre-existing fails); synth clean. — *Earlier this session:* the Fable NEXT batch is LIVE (layer v97, postflight 🟢, PRs #453/#454 merged). Shadow-sweep re-measure vs 11/112 still pending.
