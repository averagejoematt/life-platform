...
**Verified:** 2026-07-10 ("build the wiki to a 10/10 bar; if AI powered down, humans continue from docs alone; make it stay true" + blanket merge/deploy green light). **The Engineering Wiki program: 10 PRs merged+deployed** (#923/#924/#925/#926/#927/#928/#929 corpus + #932/#937/#938 grading loop). Wiki home = `docs/README.md` (Diátaxis+role index, 100% registry); new keystones: CONTINUITY (8 AI-state surfaces + memory export), AWS_ACCESS, ACCOUNTS (registrar NameCheap — **domain renews 2026-08-20**), SITE_AUTHORING, `docs/engines/`×5, SCHEMA Key-Family Catalog. **Drift machinery live:** `docs-ci.yml` + ci-cd gates (literal-sync w/ zero-match hardening · dead-links · tombstones · index/headers · 180d blocking freshness · ADR-index currency), wrap step (e) doc-sweep gate, PR-template Docs checklist — CONVENTIONS §8 is the contract. **5-persona adversarial panel graded 6.9 → ≈7.5/10** (re-graded once; 24+9 verified defects fixed — daily brief is 17:00 UTC not 11AM, concurrency is 100 not 10, NO fleet ingestion alarm exists by design, `deploy/setup_whoop_auth.py` exists in deploy/ not setup/). Gotchas: write docs to CURRENT truth never intended truth (my "repo is private" claim was a false security assertion — hotfixed #937) · describe-alarms silently omits nonexistent names · GitHub mergeability cache races force-pushes (~20s). **Gap to 9 = 3 Matthew actions: flip repo PRIVATE (coaching/ biometrics exposed NOW), fill ACCOUNTS estate rows, enable Identity Center (SSO lane then finishes).** Issues filed: #930 (restart KeyError bug) #933–#936. Full narrative: `handovers/HANDOVER_LATEST.md`.
