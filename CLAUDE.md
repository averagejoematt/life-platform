...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-142), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-07-26 — **opus+sonnet paydown (7) + fable reconcile wave (6) + 2 CI-gate fixes = 13 issues closed, all merged + deployed, main GREEN.** Standing autonomous non-fable paydown + standing approval; mid-session launched a **parallel fable session** (isolated OPEN-PR paydown) and reconciled+merged+deployed its 6 shippable PRs here. **My 7:** **#1352** LICENSES.md + advisory pip-licenses (#1750) · **#1330** restore strava check + IAM-parity test (#1751; `LifePlatformOperational` CDK by Matthew) · **#1345** smoke-oracle fail-open + PARSE_ERROR test, extracted `deploy/lib/smoke_oracle_decision.py` (#1752) · **#1574** coach diary reactions (`/api/diary_reactions` + grounding + render; trigger→#1756) (#1754) · **#1385** whole-life chronicle (1M+1hr cached archive into chronicle+SoM, folded into grounding allow-list, Structured Outputs) (#1755) · **#1336** SCA gaps (pillow/lameenc manifests + pip_audit RED-on-unscanned + gitleaks) (#1757) · **#1706** Prescription follow-up hook (question/hand-off via coach check-in queue) (#1765). **Fable reconcile (6, `/reconcile-branch` one queue):** #1626 MILESTONE# ledger · #1630 ADR-140 · #1471 art layer · #1567 journal-publish · #1627 spiral breaker (unwired) · #1743 video-diary spike. **2 CI fixes (would break every deploy):** gitleaks **push-only** (`6ff5e80d` — full-history scan tripped the historical Firebase key on dispatch); **#1345 smoke-test job had NO checkout** (`44e7bc5e` — the extracted script was missing → smoke failed on a healthy deploy → **spurious partial rollback 50/97**, recovered by re-`deploy_all`). **Verified:** combined full suite each merge (6924→6987→**7041**); all 6-dir+wiki gates; two deploys (wave `deploy_all` + fable change-detection push) Deploy/Smoke/Post-deploy/Visual-QA GREEN, no rollback; MCP boots clean (200); #1336 manifests uploaded to `config/requirements/`. **Gotchas (→memory):** gitleaks scans full-history on non-push events (`reference_gitleaks_push_only`); an extracted CI-step script needs a checkout in EVERY calling job (`reference_ci_extracted_script_needs_checkout`); a spurious smoke fail triggers a partial fleet rollback (no-`previous.zip` lambdas fail the revert → mixed state, functional if additive); mypy "source found twice" from a dual `from lambdas import X`/`import X` → use `importlib`; fable-branch reconcile is conflict-free when each branch reverted its doc-sync literals. **Docs:** LICENSES.md + README index + ADR-140 + 5 engine-doc re-verifications (unblocked check_doc_index --strict after #1336's fetch-depth:0); sync reconciled (test_count 5475 / adrs 138 / adr_max 140); checkers green. **Decisions:** none newly filed by me (ADR-140 via fable #1630). **Main:** green (`44e7bc5e` latest completed; fable-wave `3eb04d07` finishing Visual-QA, Deploy/Smoke already green). **Incidents:** 1 row (P3 false-positive smoke fail → partial rollback 50/97 → re-deploy recovery). **Stash/hooks:** clean. **Labels:** OK (61 stories). **Live: budget tier 1.** Build beat `2026-07-26-thirteen-closed`. **NEXT:** **#1768** portrait PR OPEN for your ADR-106 approval; #1756 (diary trigger + sk-fix); #1708 (needs #1756 + design); #1650 (deferred); doc-ADRs #1666/#1333/#1662; infra #1658/#1653/#1654/#1656. **Owner:** #1768 approval, X/Instagram/billing-PAT creds, GitHub toggles (#1336; Dependabot now auto-merging), private flip (#1662 after), #1029 (before 2026-08-20)/#741, LICENSES.md §5 ratify. Full narrative: `handovers/HANDOVER_LATEST.md`.
