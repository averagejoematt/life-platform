...
**Verified:** 2026-07-05 (opus batch #2) — **13 model:opus issues, all merged/deployed/verified, layer v113→v114.** Matthew authorized all edits/merges/deploys up front. Shipped as 3 coherent lanes via 10 parallel worktree agents (ADRs 114–121 pre-reserved to avoid collision): **distribution/share-cards** (**#595** the one code-drawn card engine `lambdas/web/card_engine.py`, 12 daily cards byte-identical/test-pinned, ADR-114 · **#420** character card + linkable `/data/character/` · **#405** per-chronicle share kit in the approval email · **#593** portraits travel — coach OG cards/email PNG/episode art, adopts the engine via a thin `_Engine` adapter + `portrait_raster.render_recipe` · **#399** read-only Agent Activity feed `/story/agents/` + `/api/agent_activity`, renders existing artifacts only, privacy-gated); **harden/budget** (**#402** DLQ escalation ADR-115 — content-hash stable-id `SYSTEM#dlq-ledger`, delete-only-after-2xx, pages `life-platform-alerts` · **#411** CloudWatch audit ADR-116 — deleted 18 orphan alarms 136→118 via `deploy/cloudwatch_retire_orphans.sh`, refused the 48 per-lambda alarms since only 2/49 lambdas have a DLQ · **#416**+**#418** ADR-117 — deploy-critical test lane (`@pytest.mark.deploy_critical`, `plan` needs it not the full suite) + site/layer rollback wiring; **fixed a latent bug: the Lambda auto-rollback `if` was missing `always()` so it had never fired**); **data honesty** (**#508** Whoop-webhook verdict KEEP POLLING ADR-119 · **#489** Eight Sleep dead-temp RETIRED ADR-118, tool 144→143 · **#507** State of Mind KEPT ADR-121 — found+fixed **8 consumers reading an empty `state_of_mind` partition ingestion never writes** (data lands on `apple_health` as `som_*`), read-side only, honest empty states so it flows when Matthew starts logging). **#401** OIDC codify-half ADR-120 (roles+provider → `infra/iam/` + `deploy/verify_oidc_iam.py`, 0 live IAM mutation; tighten staged as **#687**). Full suite green (3578 passed; only reds pre-existing live-state i3/i9/i14). **Deployed:** shared layer **v114** (build→Core publish→verify→bump→consumers), CDK Compute/Email/Mcp/Operational/Monitoring (2 adopted alarms)/**Ingestion** (was 3 versions behind at 111→114), `cloudwatch_retire_orphans.sh --apply`, site-api (`/api/agent_activity` 200), static site (build hash == HEAD `1576e52f`, smoke 67/67, visual QA 33 pass/1 unrelated Protocols data-sparsity FP/10 transient Bedrock base64 warns). **Load-bearing gotchas (durable in memory):** **macOS case-insensitive path twin let 3 parallel agents leak edits into the shared main tree** — pushed PR branch is truth, preserve stranded main-tree work before cleaning (`reference_worktree_case_insensitive_pollution`); the pre-commit hook regenerates counter files but only stages ARCHITECTURE.md → disable it during a stacked-merge run and regenerate counts once at the end; a concurrent session landed **#478/ADR-122** on main mid-session (merged it in). **State at close:** `main` clean == origin (`1576e52f`), all gates green, all 13 issues + PRs closed, worktrees pruned; **GitHub Pages still enabled+public (carried, unactioned).** **Next (batch #3, do fresh):** cleanly-doable = **#408** QA-left on site PRs · **#409** batch-price content AI (**touches `bedrock_client`/`ai_calls` = layer modules → another layer bump + full consumer redeploy**) · motion trio **#588/#590/#591**. Need Matthew's input = instrument-depth **#412/#415/#417/#421/#422/#475** (external endpoints / template↔movement map / week-of-data). Attended = **#395** MCP prune (destructive). Plus **#687** OIDC tighten (watched live CI run). See `handovers/HANDOVER_LATEST.md`.
