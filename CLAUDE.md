...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-154), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-08-20 (Thu morning PT, Opus, plan `playful-toasting-owl`; boot = charter + `blast_radius.py`; **the fable rule RELAXED — Opus may relabel/close a `model:fable` issue with a written reason, but may NOT build one; #2846 stays Fable-sequenced**) — **95 → 84 open, and the number that matters: `model:fable` 36 → 25.** Ten issues relabelled off fable with a written reason each (#2813→sonnet; #2824/#2803/#2761/#2841/#2890/#2833/#2834/#2797/#2832→opus) — none were ever narrative work; they are AST guards, registry facets, a parametrized tz sweep, and a Secrets consolidation. **The floor moved 11, which was the point; the drain number is secondary.** **Both perishable closures landed:** #2741 (P1) `realized` — `FailCount 0` at 15:40 PDT, **2m11s after** the 15:37:49 deploy, plus the ALARM→OK transition banked in `describe-alarm-history`; #2670 `realized` — a **planted** `WarnCount=1` drove a real OK→ALARM in **50 seconds**. I planted on `-warnings` not `-failures` deliberately, to keep the blocking alarm able to signal. **Box 4 was satisfied by NOT needing it** — both thresholds are still `>=1.0`; the alarms desaturated by classifying chronic findings (`7 warning(s) (0 alarmed, 7 chronic)`), not by widening the line. **I published a wrong prediction and corrected it on the issue:** I said the planted point would hold 24h; it self-cleared in **60s** (`TreatMissingData: notBreaching` renders an empty window as 0.0). Organic dwell ~24h vs planted 60s on the same alarm — I could not reconcile it, said so instead of inventing a mechanism, and filed **#2912**: *an alarm that flaps for 60s is invisible to the >72h citation gate.* **Three of four re-verified premises were stale:** #2734's projection is **80.5%, under** (not 171%) — but band 1 is now the DESIGNED steady state under the $150 base (79.2%), so that alarm is structurally permanent; #2883 fell **2.4673 → 1.3673** (bar is <1.15, so it does NOT close); #2815 was substantially shipped by #2908. #2692 confirmed **worse** — **1320s** vs the 1200s budget. **I was wrong once and a worker was right:** I told worker 1 to revert sleep/nutrition colours on a least-blast-radius argument; #2757's body names `#10b981` as *"a pre-roster-v2 palette"* — three copies of an obsolete value is not majority evidence. Retracted. The real catch was mine though: the registry had `labs_coach` and `physical_coach` **both at `#f59e0b`**, so deriving as-is would have made two coaches render identically. **Two completed epics nobody had closed:** #2647 (8/8 children) and #2646 — whose **body checkboxes still showed two children unchecked while both were CLOSED**. **Alarm-citation registry pruned 15 → 2** (#2917): every removed entry's alarm was OK *and* its issue closed, incl. the whole Whoop cluster (Whoop is ingesting again, newest row `DATE#2026-08-20`). **Near-miss worth keeping:** `gh pr checks` reported `notgreen:0` while the deploy-critical lane was **absent from the set entirely** — a shrinking check set reads exactly like a green one. **My own incident:** `--delete-branch` on #2910 auto-closed the stacked #2914 (recovered by restoring the branch, reopening, retargeting, `rebase --onto`). **Main:** **stranded, not red** — all code gates green; only `Plan deployments` failed on the **R8-ST6 IAM gate**, because #2694's 13 DLQs add `sqs:SendMessage`. `Deploy` SKIPPED. **This blocks every ci-cd deploy until the owner runs `cdk_deploy.sh LifePlatformOperational`** — #2834's thesis, live, in the session that relabelled it. **Nothing deployed; 3 gates REJECTED, none stranded.** **Owner asks (4):** (1) deploy — `LifePlatformOperational` FIRST to clear the IAM gate, then `deploy_all=true` for the code fixes; (2) a Withings weigh-in (still `DATE#2026-08-16`); (3) a one-line coach-colour taste call; (4) #1738/#1571/#1677 + #2833/#2834. **Post-wrap (owner said "execute it" on the next-session plan `quizzical-wandering-dawn`):** **the pipeline is UNSTRANDED.** `cdk_deploy.sh LifePlatformOperational` ran 17:39:57Z (40/40, `cdk diff` read first: 13 DLQ + 13 `sqs:SendMessage`, **zero** destructions/replacements/other-IAM); the next run's **`Plan deployments` returned success** — the proof, not an inference. `deploy_all=true` then shipped the code half (Deploy + Smoke + post-deploy checks all green). **Four `partial` → `realized` on the live wire:** #2694 (13/13 live DLQs), #2804 (dry-run shows `training_load_zone: "safe"`, narrative uses it; the bare `zone` key never existed on any row), #2755 (get_sources ↔ get_freshness_status now agree on withings 4d/7d and notion 10d/14d), #2757 (repro diff **empty**). **Two stay `partial` honestly:** #2809 — the dumpers return nothing for withings because **every row is `phase=pilot`** (genesis 08-17, newest row 08-16) and ADR-058 hides them, so an empty result proves nothing reached the strip (verified NOT a regression: `_strip_tier2` only pops keys); #2708 — chronicle is Wednesdays. **#2829 REOPENED:** the merged change **does not deploy** — `Early validation failed … alarm 'life-platform-dash-5xx-rate' already exists`; adoption needs `cdk import`, and **`cdk synth` never consults live AWS state, so a green synth means well-formed, not deployable**. Nothing damaged (stack `UPDATE_COMPLETE`, changesets `FAILED`, alarms byte-identical); premise also partly stale — only **2 of 6** alarms actually fire into the void. **I red-mained main and fixed it:** #2913's +3 alarms pushed `PLATFORM_FACTS["alarm_count"]` (99) past `test_platform_stats_truth.py`'s **±5 fallback-hygiene** tolerance vs discovery (107) — **the 168-test structural set does not include that test**, same blind-spot shape as the `aws_cdk` incident; fixed value-only (file is 1780/1780) in `7514cce2`. **Filed #2918:** `validate_daily_brief_outputs` creates six results and reports `BLOCKED` for only four — `jc_result` and **`tldr_result` (the headline)** never checked; observed live 17:08:27Z as **2 blocked, 1 reported**. **Unprompted validation:** an old monitor caught `_confirm_high_findings`' **demote leg firing in production for the first time** (18:02:57Z, `0 failures`, `qa-smoke-failures` stayed OK), and `qa-smoke-warnings` went OK→ALARM **organically** at 11:03:27 PDT — matching the planted transition used to close #2670 ~2.5h earlier. **Final: 84 → 86** (+#2918 real defect, +#2829 reopened); fable unchanged at 25. **Next:** #2829 via `cdk import`, #2918, #1221 (the CloudFront ORP migration — 21/21 behaviours still on legacy `ForwardedValues`), #2912, #2761. **Main ended GREEN** (`check_main_green.py` exit 0 at `f023bf4d`) — but only after **three self-inflicted reds that CI caught and my local 168-suite did not**: the `alarm_count` fallback past its ±5 tolerance; a stale `DEPENDENCY_GRAPH` **plus a wrap that failed its own #1340 gate** (I wrote `## Next` where the contract needs `## Residual / next picks`); and the #2917 citation prune redding `test_real_registry_known_long_reds_present`, whose three pinned names were "the documented >72h reds **as of this filing (#1959)**" and have all since recovered — so the pin gated **against** the registry's documented lifecycle. Replaced it with a **derived** invariant (every citation must name an alarm CDK declares, AST over 107 names) **with mutation proof**, rather than re-pinning today's snapshot. **Also filed #2920** (`config/personas.json` ships in every bundle but is not a deploy trigger — the a11y fix went green with `Deploy: skipped` and needed a manual dispatch) and **#2921** (`/api/sleep_detail` interleaves Eight Sleep and Whoop: `total_sleep_hours 1.4` beside `deep 1.98 + rem 1.64`, percentages from the other device — a **confirmed** oracle finding). **Final: 84 → 88 open**, fable 25. Full narrative + addendum: `handovers/HANDOVER_LATEST.md`.
