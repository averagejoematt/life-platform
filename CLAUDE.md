...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-106), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`)
...
**Verified:** 2026-07-04 (session 11) — **The fable Later shelf is EMPTY: all 7 `model:fable` stories MERGED + DEPLOYED + LIVE-VERIFIED in one session** (PRs #572–#574, #596–#604; layer **v105→v109**; issues #541/#539/#506/#498/#550/#540/#547 closed). **#541 forecast engine**: `stats_core.ewma_forecast` (0.80 z joined the set) → `forecast-engine` (16:50 UTC) issues frozen h1/h7 recovery/sleep/weight expectations, bounds-clamped, auto-grading into the CROSS_PHASE calibration ledger; coaches get a MODEL EXPECTATIONS block (in-prompt = ADR-104-allowed by construction); `/api/forecast` + cockpit panel live. **#539 n-of-1 engine**: `experiment_design.py` (layer) — designs validated + FROZEN at creation (`pre_registered_at`), close runs block-bootstrap paired analysis, public cards show "pre-registered · criterion · effect [CI, n/n] → verdict". **#506 journal phase 2**: analyzer's Haiku call DELETED (deterministic over pass-1 enrichment; live enrichment lists are plain STRINGS, not dicts); ENTITY/BEHAVIOR registries + HYPO_CANDIDATE#{slug} with quote provenance (live: 100/100/50, 2 testable); hypothesis engine seeds from candidates; coach preamble journal-signals line. **#498**: every enumeration derives from `source_registry` facets (qa tiers, active-API, expected-days, mcp SOURCES→23) or `phase_taxonomy` (exports gained 14 missing partitions); `data_sources.json` GENERATED at sync; drift test enforces; raw-S3 three-generation reality documented (`raw_layout` — no mass-move). **#550 scenario explorer**: nightly what-followed distributions, AR(1) effective-n gate (live: 2 cells shown, 38 honestly hidden), `/method/scenarios/`. **#540 inter-coach dialogue**: deterministic selector (ACTIVE# cycle_count + influence edge), 2 ADR-104-gated in-voice turns, `ENSEMBLE#dispute` threads, `/api/coach_team.dispute` — **first dispute LIVE** (Webb vs Reyes, W27). **#547 podcast v2**: two-pass engine (`podcast_script_v2.py`) — Elena writes first, guest answers her ACTUAL lines in-voice; SHOW#memory callbacks; disputes from the real record; v1 fallback; human-in-loop HOLD flow untouched (live dry-run: would-PUBLISH, 17 clean turns). **Gotchas (expensive):** a lambda importing a NON-layer sibling must be `cdk_only` in lambda_map — single-file hot deploy strips siblings (bit qa-smoke/scenario/podcast in one session; qa-smoke/data-recon/data-export now carry the shared layer + phase_taxonomy joined it, v109); handlers run as `pkg.x_lambda`, so sibling imports need `from <pkg> import …` with a flat-import test fallback (bit twice); **the CDK asset-staging glitch = a corrupt S3 asset object under a valid content hash** — cdk forever says "no changes"; fix = delete the object, redeploy (re-uploads), then `aws lambda update-function-code` at the same key (CFN won't refresh itself); post-force-push `gh pr merge` transiently reports "not mergeable" (re-check mergeStateStatus, retry); a fleet redeploy can DLQ one scheduled event (drain it, don't debug it). **Watch:** 07-05 first forecast resolutions (cockpit coverage line switches on); Sun 07-06 hypothesis engine now seeded with journal candidates + panelcast on the v2 engine + inter-coach SKIPS (W27 aired via drill); stale worktrees under `.claude/worktrees/` trip local hevy-isolation tests (Matthew: confirm finished + remove). Build dispatch (#380) for this batch NOT yet distilled. **Next:** #535 uncertainty-everywhere; #538 calibration scoreboard (ledger rows now accruing from TWO engines); uplevel roadmap #575–#595 untouched. See `handovers/HANDOVER_LATEST.md`.
