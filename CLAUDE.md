...
**Verified:** 2026-07-05 (motion-epic #575, solo) — **4 of the 5 motion-chain issues shipped, each merged + deployed + live-verified; #590 deferred to a fresh session by Matthew's call.** Matthew authorized all edits/merges/deploys up front and answered each taste-gate in-session. Front-end-only except #591 (which added one site-api field). Shipped the dependency chain in order, one PR each: **#577** (PR #700) one `theme.js` replaces six duplicated `wireTheme()` blocks (+ keeps `<meta theme-color>` honest to an explicit theme; mind.js is orphaned but consolidated). **#578** (PR #700) the v5 kit on **every door** — `.page-hero` + `.loop-ribbon` (`Now · Data → Coaching → Protocols → Story ↺`, current door marks ember; Home/Method mark nothing); new `scripts/v4_kit.py` is the ONE ribbon source so the spine can't drift across the evidence/coaching/dispatches/methods builders; heads converted `.ev-head`/`.dx-head`→`.page-hero` (JS anchors followed); Cockpit got a slim variant, Home kept its cinematic hero (loop aligned to include Now). Ribbon-node model (`Now ·` leading) was Matthew's taste-gate pick; `/data/` screenshotted+approved before the sweep. **#588** (PR #701) motion v2 — cross-document **View Transitions** (`@view-transition: navigation auto`): the `.loop-ribbon` morphs in place (ember marker slides door→door), `.brand` pinned, content cross-fades; pure progressive enhancement, reduced-motion drops it to an instant swap; `.page-hero` pulled out of the `html.mo` opacity-reveal so the morph target isn't captured mid-fade; scroll-driven cockpit spine (`animation-timeline: scroll()`). SW needs no change (no nav-preload, clean fetch on navigate). Door-pair filmstrip taste-gated + reduced-motion verified. **#591** (PR #702) cockpit presence pass — the board read is now **signed + timestamped** ("written 6:02 AM this morning", author sigil/portrait), the #540 inter-coach dispute renders as a **thread** (position→reply→rejoinder, sigil-marked, handles 1..n turns), and **stance chips** show an HONEST held-duration — **"held since {date} · ~N weeks", never a fabricated day count** (STANCE# is weekly, ADR-104/105; Matthew chose this over the issue's literal "held 6 days"). Server side: `site_api_coach.handle_coach_team` gained a per-coach `held_since` (earliest consecutive same-stage STANCE# `as_of`; additive). **Deployed:** 3 static-site syncs (`sync_site_to_s3.sh`) + 1 site-api (`deploy_site_api.sh`, layer already v114 so no re-attach); each verified `version.json` == HEAD + **smoke 67/67**; #591 confirmed live with real data (real 3-turn dispute, 8 stance chips, "written 7:55 AM"). **No layer bump, no CDK.** Tests green throughout (385 coach/site-api/a11y on the #591 pass). **Gotchas (minor):** `sync_site_to_s3.sh` regenerates `site/feed.xml`+`site/rss.xml` → they show dirty after every deploy (date-only; `git checkout` them before the next commit); coaching/story/methods SHELLs are `.format()` not f-strings, so the ribbon is inlined once via `SHELL.replace("{ribbon}", loop_ribbon(...))` before the per-page format calls. **State at close:** `main` clean == origin (`12df246b`), 2 branches (main + pre-existing `docs/uplevel-handover`), all 4 issues + PRs closed. Session opened by cleaning the repo: **281 → 2 local branches** (pruned 125 gone-upstream + the rest after confirming 0 open PRs; verified `recovered/378` was already-shipped before deleting). **GitHub Pages still enabled+public (carried, unactioned).** **Next (do #590 fresh):** **#590** home cinematic — constellation v2 (deterministic drift seeded like sigils.js, pillar-node hover→door affordance, edges weighted by real pillar-correlation data) + waveform onto the shared `data-cpts` contract; **taste-gate: 2–3 constellation options rendered for Matthew's pick.** OPEN QUESTION: the pillar-pair correlation edge-weight data is NOT exposed in `/api/character` today (#535 was uncertainty/CIs, not pillar correlations) — scope a site-api field or a client-side derive first. Then batch #3 leftovers: **#408** QA-left · **#409** batch-price content AI (layer bump). See `handovers/HANDOVER_LATEST.md`.
