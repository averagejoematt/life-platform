...
Regenerates constants, deploys Core/Compute/Email (constants ship in every bundle — #781), phase-tags DDB, wipes intelligence, rolls the accountability ledger into a durable `LIFETIME#` aggregate + zeroes `TOTALS#current` (`deploy/restart_ledger_reset.py` — ADR-072/077), rebuilds character, curates the chronicle, syncs site + docs, verifies the 88-URL v4 surface (81 pages + 7 JSON endpoints, #918). Rollback: `deploy/restart_rollback.py`.
...
**Verified:** 2026-07-24 — **CI concurrency recovery + validated 3-PR batch shipped, deployed & live.** Session began as backlog paydown (fanned out #1697/#1656/#1620 as verified open PRs), then Matthew's "how do i clear billing" turned it into a full CI-dispatch recovery. **Root cause was NOT billing** — the repo is PUBLIC (unlimited minutes, [[project_repo_visibility]]); the `CI/CD` workflow's `concurrency` group had a phantom stuck-queue entry (`cancel-in-progress:false`) that wedged every run at pending/0-jobs, surviving cancellation + an empty group, while PR/schedule workflows ran fine + githubstatus green. **Fix `60d6652c`:** salted the group name `…-${{ github.ref }}-v2` → fresh queue (self-proving run went green). Also **deployed `LifePlatformOperational`** (Matthew ran it — the CDK classifier hard-blocks agent deploys) to clear the R8-ST6 Plan gate (the additive `InsightEmailParserRole` qa_archive grant, #1441/#1687). Then merged the batch: **#1713 (#1656)** mypy clean-set 124→161 (+all of `mcp/` via `if not TYPE_CHECKING:` guards), 10/14 codes · **#1711 (#1697)** coach corrections→prompt-memory (rolling window N=5, injected outside the cache boundary) — DEPLOYED coach-gen · **#1712 (#1620)** outbound social links + follow row — LIVE (TikTok `averagejoematt`). Reconcile `1ec3feba` test_count 5045→5057. Pre-validated combined tree **6642/0, zero conflicts**; final CI run `1ec3feba` **fully green** (Deploy/Smoke/I1I2I5/visual-QA — production Deploy owner-approved via `pending_deployments` API); site-deploy green. **Gotchas:** public-repo CI stall≠billing (diagnose: do PR/schedule workflows pass? does a fresh `workflow_dispatch` wedge with an empty group? → concurrency phantom, salt the group); CI/CD `push:` has a `paths:` filter (docs/empty commits create NO run — expected) + a `workflow_dispatch:` re-kick lever; CDK-deploy classifier hard-blocks agent deploys (owner runs via `!`); pre-commit hook sweeps doc "Last updated" dates into commits. **Docs:** none needed — captured in memory+handover, no canonical page invalidated; checkers green. **Decisions:** none — operational fixes; #1711 rolling-window on PR under epic #1687. **Incidents:** 1 — CI/CD dispatch stall (P3, >2h, concurrency phantom, no user/data impact, resolved by salt). **Main:** green (1ec3feba). **Stash/hooks:** clean (🟢 hook; `🔴 config-drift` = standing fleet-churn advisory). **Labels:** OK (91 stories). **Live: budget tier 1.** Build beat `2026-07-24-outbound-social`. **NEXT: (1) #1655** CI-composition — now UNBLOCKED (CI healthy), run ALONE; **(2) #1656** mypy remaining (4 residual codes + check_untyped_defs/warn_return_any + 9 DIRTY); **(3)** gate:owner **#1662/#1666/#1650** (code-complete then stop for sign-off); **(4)** owner: `life-platform/youtube` secret + #1686 product decisions + full-fleet CDK reconcile (clears config-drift). Full narrative: `handovers/HANDOVER_LATEST.md`.
