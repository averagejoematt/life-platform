...
- `docs/ARCHITECTURE.md` — full system design, ~95 Lambdas (CDK-defined; canonical count via `sync_doc_metadata.py`), 9 CDK stacks, data flows
...
**Verified:** 2026-07-21 — **Owner-queue drain: July ceiling window, achievements ledger, attribution, and the CDK drift cleared.** Third session of the day (after Glass-Engine, brand-mark). **#1635** (`8b412d4a`, MERGED+DEPLOYED) raised the budget ceiling to **$115 base / $135 surge for July ONLY** (`_TEMP_CEILING_WINDOW`, auto-reverts 2026-08-01, no deploy) — **$115 not the $110 first approved because the tier bands are FRACTIONS of the ceiling** (≈73/87/97%), so $110 left the $96 projection at 87.4% = still tier 2; $115 → 83.6% → tier 1. Governor invoked → **tier 2→1 live, reader narratives un-paused.** Fixed a **latent surge inversion** it exposed: `_effective_ceiling` returned the surge ceiling unconditionally, so any base >$100 made reader arrival LOWER the ceiling and tighten the guard (success-punishes-you) — now floored with `max()`, pinned by `test_surge_never_lowers_the_ceiling`; latent at any base over $100, predates the PR. AWS Budgets backstop LEFT at $85 so the overrun still signals. ADR-133 amended. **#1637** (`953566a2`, MERGED+DEPLOYED, closes #1624) — durable **achievements first-earn ledger**; `handle_achievements()` was `earned_date=today if <cond>` for all 40 badges (badges un-earned on a 2-lb water swing). Writer/reader split: new `lambdas/achievement_rules.py`, writer=`daily-metrics-compute` (**proven to boot via live invoke**), reader=`site_api_vitals` (no serving-path writes, grep-verified); ledger EXPERIMENT_SCOPED so the cycle-10 reset rebuilds it. **#1642** (`0809cc03`, MERGED+DEPLOYED, closes #1621) — site-wide **UTM attribution**; the issue's diagnosis was wrong (corrected on-issue: `source` WAS sent, `referrer` WAS captured — the real defect was the dead referrer fallback + zero UTM capture). Built `attribution.js` (site-wide sessionStorage capture) + `lambdas/utm.py` (host-only referrer), three separate signals, **canary hard short-circuit** (verified live). **CDK drift cleared — all 9 stacks** (`cdk diff --all` = 0 differences): the "pending deploy" was the LogRetention runtime bump PLUS a brand-new scheduled email-sender, **`AiReviewPack` (merged #1594, fires Sun 18:00 UTC)** — diffed first (zero deletions/replacements), deployed under explicit owner approval. Plan gate re-run = success. Also: **#1620** TikTok corrected (`averagejoematt`, gate cleared), **#1589** closed with simulated-IAM+live-canary evidence, **#1634** filed (canary judge FPs on sanctioned coach personas). Gotchas: the $110 fractional-band trap; agent self-reports ~50% wrong on their own numbers (#1637 agent off by 2.3× on badge counts, #1621 body wrong twice — grep the branch); `node --test tests/js/` with a dir arg fails, CI runs bare `node --test`; a hashed asset name (`attribution.6d69289b.js`) hides from a literal grep. **Main:** red only because `check_main_green` reads the latest COMPLETED CI/CD run (`953566a2`, pre-cdk-deploy Plan failure) — HEAD `0809cc03` has every automated gate green, run parked at the manual production Deploy gate. **Live: budget tier 1** (July window working). Build beat `2026-07-21-badges-that-stay-earned`. **NEXT SESSION = cycle-10 reset to 2026-07-22 (Wed)** — no 07-22 weigh-in yet (fallback `--override-weight-lbs 321.38`), `restart_pipeline.py --apply` runs `cdk deploy --all` = OWNER; plan in scratchpad `NEXT_SESSION_PLAN.md`. Full narrative: `handovers/HANDOVER_LATEST.md`.
