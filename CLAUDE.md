...
**Verified:** 2026-07-04 (session 10) — **The rigor chain is MERGED + DEPLOYED + LIVE-VERIFIED in one session**: **ADR-105 the rigor bar** (PR #567) → **#529 stats_core** (PR #568, layer **v105**) → **#530 hypothesis engine v2** (PR #570 — supersedes #569; gotcha below). New shared `stats_core.py` (stdlib-only, seeded/deterministic): one `pearson_r`, erf p-value on **fractional n**, AR(1)/Bartlett effective sample size (clamped conservative), moving-block-bootstrap CIs, `bootstrap_mean_diff_ci`, `cohens_d`, BH-FDR — weekly-correlation/tools_training/mcp-helpers/digest_utils migrated, duplicate math deleted; **p-values now on n_eff** (live: habit_pct_vs_day_grade r=.88 n=20→n_eff 7.1 still sig; tier0_streak n=20→17.1 no longer sig; 1/23 FDR-sig = the honest count). Hypothesis engine v2: creation must emit a **frozen machine-checkable `test_spec`** (pre-registration; `SPEC_METRICS` must stay in sync with `build_data_narrative`), checks are **pure Python** (arm split → effect + block-bootstrap CI → supported/contradicted/inconclusive), confirmed = supported + full window, Haiku narrates resolutions only (weekly per-hypothesis Haiku verdicts DELETED), every resolution → **`SOURCE#calibration` ledger (CROSS_PHASE** in phase_taxonomy — survives resets). Live drill: 3 v2 hypotheses stored with specs, 2 rejected by the gate, 5 legacy v1 skipped (age out via hard expiry); `/api/hypotheses` + evidence.js serve spec/effect/CI/"pre-registered". Gotchas: **`gh pr merge --delete-branch` on a stacked base CLOSES the child PR unrecoverably** (can't retarget/reopen — rebase onto main + fresh PR); **full CI gates don't run at PR level** (only Dependabot validate — the local FAKE-creds suite is the real gate); new layer modules need `ci/lambda_map.json` entries as `lambdas/<name>.py`; weekly-correlation's Decimal conversion is top-level-only — new pair fields must be FLAT keys. Fleet on v105; main CI green end-to-end incl. visual-QA (run 28718898507). **Watch:** Sun 07-06 first scheduled v2 engine run (v1-skip logs; no resolutions before ~07-11) + weekly-correlation FDR count sits lower (the correction, not a bug); Wed 07-09 chronicle (grounded hints + Elena's promises). **Next:** #535 uncertainty-everywhere (tools_correlation's n=5 HARMFUL labels deliberately left for it); #538 calibration scoreboard once ledger rows exist; fable Later flagships (#541/#540/#506/#539/#547/#550/#498). See `handovers/HANDOVER_LATEST.md`.
