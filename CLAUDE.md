...
**Verified:** 2026-08-17 (evening, Fable, plan `system-model-and-matured-boxes`) — **4 PRs merged + deployed; the system model is LIVE**: **#2865/#2845+#2805 — `model/platform_model.json` + `docs/DEPENDENCY_GRAPH.md` are GENERATED** (`scripts/generate_platform_model.py` from CDK AST + `source_registry` + the ADR-077 census; 104 lambdas/644 edges/105 partitions; 99.4% sampled ground truth; mutation-proven drift gate `tests/test_platform_model_drift.py`; queries `scripts/blast_radius.py --touches/--feeds`) — **session boot is now charter + model**; #2839 closed (subsumed), #2866 filed (the model exposed check_doc_facts' cron map mis-attributing whoop's recovery cron). Fan-out: #2867/#2860 daily-brief in-flight lease (deployed, lock symbols byte-verified live), #2868/#2859 (`/archive/*` CloudFront behavior had NO function association — #1805 class; cdk-deployed, all `/archive/v1/*` 301s live-verified), #2669 already-fixed (PR #2795, Wednesday 08-19 = live box). **Unplanned: main redded 3× on vote tests** — they patched `_challenge_catalog_cache` but not the TTL stamp; stale stamp + CI FAKE creds → 503, local real creds silently served the REAL catalog (fixture-must-be-the-wire); #2869 pins the stamp, proven both directions; deploy/smoke stayed green throughout. **Gotcha:** a tests-only merge SKIPS Deploy — #2867's guard would have sat undeployed behind a green run; check the Deploy job ran, then direct-deploy + symbol-verify. **Main:** green (`46f79a222`, final run 32096134923 fully green). **Owner asks pending:** #2836 September base (due 09-01), Whoop re-auth (not done — no rows past 08-16; then #2085 latch-clear + backfill). **Next:** 08-18 boxes (#2668 3/3 at 17:00Z, #2858 sweep 18:30Z, #2735 age-out ≤21:20Z), #2669 Wednesday box, #2866, #2670 threshold shape, #2846/#2847 kernel builds, genesis weigh-in supersede when Withings syncs. Full narrative: `handovers/HANDOVER_LATEST.md`.
