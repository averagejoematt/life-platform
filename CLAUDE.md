...
- `docs/ARCHITECTURE.md` — full system design, ~105 Lambdas (CDK-defined; canonical count via `sync_doc_metadata.py`), 10 CDK stacks, data flows
...
Regenerates constants, deploys Core/Compute/Email (constants ship in every bundle — #781), phase-tags DDB, wipes intelligence, rolls the accountability ledger into a durable `LIFETIME#` aggregate + zeroes `TOTALS#current` (`deploy/restart_ledger_reset.py` — ADR-072/077), rebuilds character, curates the chronicle, syncs site + docs, verifies the 97-URL v4 surface (90 pages + 7 JSON endpoints, #918). Rollback: `deploy/restart_rollback.py`.
...
**Verified:** 2026-09-21 (Fable 5.1, **Session AQ — the flip that waited on a ruling, three lanes that landed, and the prompt that still stalls deploys**; attended-autonomous 12:20 PT → ~18:40 PT under standing merge + deploy authority). **54 → 53 open (50 non-Roadmap)** against a ≤30 target: 3 closed on live proof (#3563 #3900 #3615), 2 filed (#4022 close-on-first-live-output on epic #3592; #4025 the dated-citation false positive), 4 PRs merged (#4023 #4026 #4027 #4028), 2 leases rejected by name, zero blanket, 0 epics closed on evidence (14 have open children; the four zero-child Later epics carry folded unshipped scope — owner's call). **The #3753 ruling the brief said was posted was never posted, so v0.2 stays `ACTIVE = False`; the flip is DRAFT PR #4029 (premerge 11442 green, blast radius measured against the live JSON grid: PPL six days, set ceiling 25 → 18, minutes 75 → 60) — merge → fleet → `plan_next_session` reads ACTIVE → close #3753/#3755 = the beat.** **Shipped and live:** the nutrition critics (#3754 boxes 3–4; first output: 5 of 6 days below the 1,700 kcal floor, 7-day mean 1,400, protein 136 g — `change → 2000 kcal`, not `veto`, because the redlines are PROPOSED) · the voice sampler across cycles + 11 pages stop advertising the empty podcast feed (#3615 closed) · `cross_surface:vitals` judges a dated coach citation against its own day (#4025) · both lit alarms re-cited against live causes (#4026). **Open, green, not merged:** #4024 (#3760 private photo viewer — a separate `progress-viewer` Lambda in `LifePlatformWeb`; needs the owner's secret, two CDK deploys, a census re-merge that reads 544 vs 540). **Deploys:** fleet `bash deploy/deploy_fleet.sh` from **`d33b37d0`** — 106/0, five functions read back `d33b37d0 · commit · dirty=false`; no CDK. **The permission layer still stalls deploys under the new prefix allow rules** — two bare deploys sat ~57 and ~33 min; this session runs in auto mode, whose classifier also refused the #3900 two-key DynamoDB stamp; owner told: leave auto mode or accept one fleet deploy per checkpoint. **Main:** pending — every completed run since the base failed on the Deploy job alone (rejected leases); `d33b37d01`'s run in flight at the wrap. **POST-WRAP (19:15–20:30 PT, owner present — see the handover addendum):** the owner ruled **no further resets** (cycle 17 terminal; reset-gated boxes close on REHEARSAL proof — plan Step 1b), asked for and **APPROVED v0.3** (six-persona red team on his 2024–25 record: 3.5 lb/wk to 275 with DXA gates, 3–4 full-body sessions with loads that HOLD, floor 1,800, 13 h walking, landing from 240, 26-wk maintenance; `s3://…/config/coaching/TRAINING_PROGRAM_v0.3.md` + `_redteam.md`; its machine-twin lane in flight in worktree `issue-3753-v03-active-flip`; draft #4029 superseded), ruled all 15 Later calls (5 closed, 3 promoted, 7 kept incl. the authorised #3915 strip), closed the four zero-child Later epics with scope quoted (+ re-filed #4034 #4035), authorised the photo-viewer secret (created 20:09 PT). Filed #4030 (get_exercise_history phase filter; PR #4033 open) + #4031 #4032 #4036. **Count 49 / 46 at 20:30 PT.** Next session: merge #4033 → the v0.3 flip PR → ONE fleet deploy → `plan_next_session` ACTIVE → close #3753/#3755 = the beat → #4036 lane → Step 1b → #4024. Owner acts left: baseline booking (§13.2), the named human (§13.3), photos this week, the mode switch (auto → default).
