...
**Verified:** 2026-08-18 (daytime, Opus, plan `opus-boxes-and-hardening`; boot = charter + model, not prose) — **4 PRs merged + deployed, 4 issues closed, 1 filed**. **#2874/#2819 (headline): the handled-5xx detector** — `emit_handled_5xx()` fires from ALL THREE response builders (`_error`, `_envelope`, and site_api_ai's own `_error`) + alarm `site-api-handled-5xx` (Sum≥5/15min → digest, live/OK). **The load-bearing finding: `_emit_route_log` is NOT a chokepoint** — it sits on the `ROUTES.get(path)` tail and **27 routes return before it**, so the metric there would have covered a subset while reading complete (filed as #2876). Proven beyond unit tests: emitter symbol-verified in both deployed bundles, and the EMF shape published to a throwaway namespace materialized BOTH dimension sets (Sum=1.0) — a malformed `_aws` block yields no metric *silently*. Fan-out: **#2873/#2642** (the worker corrected the brief — the bucket is `from_bucket_name`, so CDK CANNOT own its lifecycle; the fix was live-but-unwritten and `put-bucket-lifecycle-configuration` replaces the set wholesale — one re-run from reversion; 96.87→37.72 GB), **#2875/#2866** (cron map derives from the #2845 model; whoop's 3 real schedules), **#2877/#2643** (Eight Sleep 08-09 settled by DIRECT LIVE VENDOR PROBE = branch 2; new opt-in `record_gap_exhausted_absence`; backfill applied, `InteriorGapCount` 1.0→0.0). **Boxes: #2858 confirmed** (verified against corpus data, since a qa-smoke pass is SILENT); **#2668 is 2 of 3, NOT closed — the plan's premise was off by one** (the `parsed clean` line was added BY the fix, so 08-13/15/16 were silent successes; run 3 is 08-19). **#1221 analyzed, deliberately not shipped** (all six `/api/*` behaviours are on legacy `ForwardedValues`, which CloudFront forbids alongside an origin-request-policy → migrating all six, public-cache blast radius, owner call). **Gotchas:** local `black` 25.9.0 vs pinned 26.3.1 rewrote 20 unrelated files (#2570 class — use `agent_commit.sh`); a merge produced **ZERO CI runs** (swallowed-push, incident row added, recovered by manual dispatch); a watcher aimed at a nonexistent log group would have reported absence as a pass. **Main:** green (`9e3659a1`). **Owner asks (3, all open):** #2836 September base — measured burn ~$4.71/day means the $85 auto-revert lands **tier 3 hard cutoff** early in September; Whoop re-auth (no rows past 08-16); a Withings weigh-in (baseline still the 321.01 override). **Next:** #2668 close 08-19, #2735 age-out check, #2669 Wednesday box, #2876, #2692, #2846/#2847 kernel builds. Full narrative: `handovers/HANDOVER_LATEST.md`.
