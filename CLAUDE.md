...
**Verified:** 2026-07-05 (#415 Whoop reconciliation + #417 routine branch model — epic #348, two *Later* stories) — **shipped, merged, deployed, live-verified.** Matthew authorized edit/merge/deploy in-session. Built by **two worktree-isolated background agents in parallel** — the correct call, because a concurrent **#412 `feat/412-training-truth`** session shared the primary tree; isolation prevented any #590/#408-style squash-stomp. **#415 (PR #710, TR-07, NO layer bump):** generalized the Strava DI-2 provider-diff reconciler to Whoop — `whoop_lambda._reconcile` (`{"reconcile":true}`, EventBridge `WhoopReconciliation` 18:20 UTC) pulls trailing-14d sleeps+workouts from the Whoop API, diffs vs the store, emits `MissingActivityCount{Source=whoop}`; **read-only, dedup-aware (±120s twins), fails open**; alarms `ingest-reconciliation-whoop`+heartbeat. Opt-in `provider_reconcile` facet in `source_registry.py`. **ADR-123 = Garmin deliberately NOT reconciled** (paused ADR-074 + rate-capped 4x/day). Deployed `LifePlatformIngestion`+`LifePlatformMonitoring`. **First live run caught a REAL gap** — a Whoop workout the API held but the store missed (`7a62677b…`, 2026-07-05; the silent-drop class the story targets; trailing-refresh should heal). **#417 (PR #711, TR-04/05, LAYER v114→115):** `routine_ir.RoutineBranch`+`RoutineSpec.branches` first-class; `hevy_compiler.render_branches_note()` renders the branch menu into notes (branchless routine → "" → byte-identical push); `routine_generator.emit_branch_model()` folds ideal/floor/re-entry into ONE branch-carrying primary; cron pushes that. New `hevy-restamp` lambda re-orders/re-flags recommended by recovery band — **never adds/removes a branch (asserted), fails open, SHIPS DISABLED** at rule (`enabled=False`) + SSM `/life-platform/hevy/restamp_enabled`=false. Layer sequence: build → `cdk deploy LifePlatformCore` (published v115, verified) → `LifePlatformOperational`+`LifePlatformMcp` (ONLY stacks that deserialize routines; rest stay on v114 — old-layer deserialize crashes on a `branches` field). Verified: restamp+cron on v115, rule DISABLED, MCP boots 401, restamp smoke=`{"status":"noop"}`. **⚠ TWO decisions await Matthew before ENABLING the re-stamp (it's disabled, safe to hold):** (1) TIMING — rule is 12:45 UTC but Whoop recovery refreshes 17:30 UTC, so it'd re-stamp on stale recovery; move recovery earlier or restamp later, then flip SSM+rule. (2) BRANCH PUSH FORMAT — currently base exercises + menu-in-notes; alt = push the recommended branch's own exercises (one-line compiler change). No "harder" branch generated today (add-load gated). **State at close:** `main`==origin (`543beef6`), both issues CLOSED, layer v115, the two agent worktrees removed (#412's worktree + `uplevel-handover` remain). Build-log beat published (#380). **⚠ Concurrent #412 session live in the shared tree — this status block is single-writer; #412's close may replace it.** **GitHub Pages still enabled+public (carried, unactioned).** **Next:** enable the re-stamp after the timing decision · watch `ingest-reconciliation-whoop` (real drop if the missing workout persists) · #409 batch-price content AI (layer, attended) · #395 MCP prune (attended). See `handovers/HANDOVER_LATEST.md`.
