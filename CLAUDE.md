...
**Verified:** 2026-07-05 (session 12) — **Coach portraits: gate → approval → LIVE; the fable queue is EMPTY again** (#585/#586/#587 closed; PRs #606–#615; also: session-11 worktrees removed, I22 CI rerun green, build-dispatch beat for the forecast batch live). **#585**: ADR-106 (AI may sketch, only code ships, only Matthew approves; photoreal NO-GO; kill = 2 failed rounds) + `docs/design/PORTRAIT_RUNBOOK.md`. **#586**: `portraits.js` + recipe schema (`config/portraits/`, 13 fixed layers) — **the sign-off gate is structural**: only recipes with `_meta.sign_off` bundle into the generated `portrait_data.js`; `portrait(c) || sigil(c)` at coach-heads/popovers; shipped dark, byte-identical. **#587**: the taste gate ran 4 live rounds on one artifact ("they look the same" → distinct constructions; "less sketch" → duotone; "identifiable without the name, like animated-film characters" → **shape language**: distinct geometric base + owned tone palette + one signature feature + the **silhouette litmus**; approved → variant A). Engine grew a validated per-recipe `palette` + per-element `tone` (PR #612). Live: Elena (chronicle byline), Park + Chen (coaching heads, popovers), disclosure line on /coaching/, aria disclosure everywhere. **Gotchas:** runtime coach objects carry **V2 engine ids** (`sleep_coach`/`training_coach`), not board ids (`lisa_park`) — first deploy silently sigil-fell-back; recipes now carry `aliases` → bundler emits a collision-checked ALIASES map (PR #615) — **any new V2-coach recipe needs its engine id in `aliases`**; style ADRs written before the first contact sheet WILL be amended (the amendment path worked: load-bearing rules untouched); the silhouette litmus (solid shapes, no faces) is the cheap recognisability test — now mandatory per runbook. **Watch:** Sun 07-06 scheduled runs (journal-seeded hypothesis engine, panelcast v2, inter-coach SKIPS); forecast resolutions accruing since 07-05. **Next: ⭐ #616 FULL-CAST PORTRAITS — Matthew's explicit close-of-session direction ("the same visuals for all of the coaches"; ~19 sigil-only personas, batched contact sheets through his gate)**; this session's own build beat (the portraits story) not yet distilled (one beat/session — #606 used it); #535/#538; uplevel #575 lanes. See `handovers/HANDOVER_LATEST.md`.
