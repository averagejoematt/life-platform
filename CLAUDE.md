...
**Verified:** 2026-07-18 — ran a **live end-to-end experiment reset to genesis Saturday 2026-07-18 (cycle 6→7, baseline 315.65 lbs)** as a drill to prove the pipeline still resets in ONE command and the site's Day-1 pre-start behavior is correct; Matthew granted merge + deploy authority INCLUDING `restart_pipeline.py --apply` and its live-DDB steps. Command: `restart_pipeline.py --genesis 2026-07-18 --override-weight-lbs 315.65 --apply --sync-site` (override needed — genesis was +1d, no genesis-day weigh-in; used the honest latest Withings reading). Site verified in **PRE-START COUNTDOWN** (`pre_start=true`, `day_n=0`, character zeroed Level 1, "1 day to go" home hero, "STAGED" cockpit; weight deferred to Day-1 weigh-in per ADR-104). **The drill surfaced TWO "one-command" gaps — both fixed + guarded in PR #1287 (MERGED `6ff76722`):** (1) `/journal/posts.json` served a `"tombstone": true` marker — `restart_leadin_pages` OWNS that manifest and runs early, but `restart_site_copy_sync.ORPHAN_S3_FILES` re-tombstoned it later; fix = drop the owned keys + make leadin_pages the unconditional owner (empty `{"posts": []}` on zero installments). The rendered gate caught it (39/40) while the semantic gate PASSED (vacuous on empty `posts[]`). (2) the reset never ran `sync_doc_metadata.py`, leaving genesis/cycle doc literals (SCHEMA.md, CLAUDE.md) stale — a manual `--apply` that, skipped, reds `test_platform_stats_truth`; fix = folded it in as the final sub-script. Non-vacuous guards: `test_reset_orphan_list_excludes_owned_keys`, `test_build_sub_scripts_sequence`. Re-ran with the fix → **clean exit 0**, all gates pass, `fix_prologue` hook ran. **Verified:** rendered 40/40, semantic 7/7, truth gate honest-SKIP (budget tier 1), smoke **82/0**, render-QA PASS both surfaces, offline suite `-m "not integration"` **5253/0** (the one full-run fail is `test_i16`, integration-marked/CI-excluded, a known UTC-vs-PT genesis-boundary flake). **Post-merge watch:** CI/CD runs on push-to-main (Lint/Test/Plan on `6ff76722`, pre-verified green locally); Site-deploy on `6ff76722` (site already live-correct from the reset's `--sync-site`; superseded-skip reflex if a reconcile commit lands → manual `sync_site_to_s3.sh`). New reflex + the reset trap now in memory: [[reference-reset-pipeline-owned-manifest-clobber]], [[project-monday-reset]] (cycle 7). **Left for Matthew (NOT deploys):** #1266 DDB cycle re-stamp + #1265 Elena held-draft regen (reset did NOT supersede). **SECONDARY held by design:** Next milestone = 19 stories; SERIALIZE the `check_doc_facts.py` cluster (#1232/#1205). Budget tier 1. Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-18_NextSlice2.md`.
