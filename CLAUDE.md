...
**Verified:** 2026-07-05 (#590 home constellation v2 — motion epic #575 now **5/5 COMPLETE**, solo w/ 2 background agents) — **shipped, merged, deployed, live-verified.** Matthew authorized all edits/merges/deploys up front. The home hero constellation now draws **real measured pillar co-movement**, not hand-drawn edges. **Backend (additive, read-only, no layer bump):** new **`/api/pillar_coupling`** (`site_api_data.handle_pillar_coupling`) — trailing-60d pairwise Pearson of each pillar's daily `raw_score` via `stats_core`, **masking non-real days** (`coverage_hold`/`data_coverage<=0` dropped so a floored/held score can't fake correlation across the manual-log gap); every edge carries its own `n`, thin/flat pairs omitted not faked (ADR-104/105); cached 1h. `/api/character` now also surfaces the already-persisted `active_effects`. **Frontend (`story.js`/`story.css`/`tokens.css`/`index.html`):** ember-solid edge = pillars *rise & fall together*, dashed-ink = *trade off* (single-accent identity held — no new hue); width ∝ |r|, opacity ∝ significance; a pillar with no measured coupling (relationships, flat 50) has **no edges** + renders `.node.quiet` (absence stated). Deterministic seeded drift (`fnv1a→mulberry32`, `cn-drift` keyframe) replaces uniform `mo-breathe`; hover→door affordance (`domainIcon` in a `foreignObject` + "open →"); designed-coupling overlay lights `.edge.effect` when an engine `cross_pillar_effect` fires (rare). **Waveform migrated onto the shared `data-cpts` contract (#582):** bespoke tooltip gone; bars live in a fresh inner **`.wave-cpts[data-cpts]`** node so `motion.js`'s MutationObserver wires ONE focus-dot/tooltip/keyboard path (setting `data-cpts` on the pre-existing wrap would NOT wire — only *added* nodes get scanned). **The data question (was open):** no inter-pillar correlation existed (#535 was uncertainty, not pillar corr); Matthew chose Option 2 = build it honestly. De-risk showed the coupling is rich — strongest are NEGATIVE trade-offs (sleep↔nutrition −0.84, movement↔nutrition −0.75 live), robust to excluding the quiet week. Guardrail: **no forecasting** coupling from stale data. **Taste-gate:** 3 variants rendered as real screenshots in an Artifact; Matthew picked **A** (relational web); B/C stripped. **Deployed:** 1 site-api (`deploy_site_api.sh`, layer v114, no re-attach) + 1 static-site (`sync_site_to_s3.sh`); `version.json`==HEAD (`2a7545a2`), **smoke 67/67**, live browser check = 15 edges w/ real scores, no h-overflow desktop+mobile, reduced-motion static, 0 console errors. **231 site-api tests green. No layer bump, no CDK, no MCP.** **⚠️ The #408 mixup (worktree lesson, reinforced):** built #590 in the SHARED primary working dir while a concurrent session was also using it → the #706 squash **silently dragged that session's #408 work (=PR #703) onto main** under my commit (test/CI-only, no prod impact, but unreviewed + mislabeled). **Reverted clean via PR #707** (0 diff vs pre-#590 base). Lesson: a live concurrent session sharing the primary dir means `git checkout -b` can inherit their committed work → it rides onto your squash. **Use a worktree for your OWN work when another session is live.** **State at close:** `main` == origin (`a29313ba` — after my revert, the concurrent session merged #408 properly via its own PR #703 on top, so #408 legitimately landed and the duplicate #704 was closed; my `/api/pillar_coupling` + constellation are unaffected — separate files), worktrees pruned to 2 (main + `docs/uplevel-handover`). **#408 is DONE — no action needed.** **GitHub Pages still enabled+public (carried, unactioned).** **Next:** **#409** batch-price content AI (LAYER bump, attended) · **#395** MCP prune (attended) · **#687** OIDC (watched CI) · **#583** chart-contract-v2 batch-2 (extends the same `data-cpts`/motion.js path #590 touched — do aware). See `handovers/HANDOVER_LATEST.md`.
