...
**Verified:** 2026-08-18 (afternoon, Opus, plan `swirling-singing-crown`; boot = charter + model) — **2 PRs merged + deployed, 1 issue closed, 1 filed, 1 worker PR held**. **#2879/#2878 (headline): the publish path was un-blocked without a weigh-in** — the vitals smoke carve-out keyed on `day_n <= 1` as a proxy for "no weigh-in yet"; the proxy expired on Day 2 while the condition held, so every `site/**` merge deployed → failed smoke → auto-rolled-back. It now reads the real predicate from `/api/source_freshness` (a **different module on an un-genesis-clamped query**, so the two must agree — that independence is what keeps it able to fail). Mutation-proven BOTH directions on the block extracted verbatim: a weigh-in present in DDB but absent from the payload still FAILs; unreadable freshness fails closed. Live: deploy+smoke+Visual/AI-QA green, rollback skipped, `/version.json` == HEAD, morning beat served (113→114). **#2880/#2741: the durable-design-copy class retired structurally** — measured FIRST from the qa-smoke log group at zero Bedrock cost (**25 of 60 runs, 42%**, across every phase, severity flipping `med`→`high` on byte-identical copy); `DURABLE_DESIGN_COPY` is the registry, `build_prompt` renders the rubric clause FROM it so the two cannot drift, and the drop fires only when every quoted span is registered. Deployed + **symbol-verified in the live bundle**, not inferred from a green run. Issue stays OPEN — box 5 needs a nightly + the observed alarm transition. **The wire taught what a retyped fixture would have missed:** live notes carry **U+2011 non-breaking hyphen** in `Day‑1` vs the rubric's ASCII (raw compare matches nothing, forever, while hand-written tests pass); a real note quotes the fragment `'starts at'`, which reddened the suite and forced two-way containment. **Plan premises corrected: it was Tuesday, not Wednesday** (#2668 run 3 and #2669's box are 08-19, unobservable), and **Whoop was already fixed** (auth 0.0→1.0 at ~08:00 PT, complete rows 08-16→18 — no latch-clear, no backfill; its alarm is a 24h `Minimum` lag, self-clearing ~08-19 08:00 PT). **Gotchas:** a watcher exiting on its iteration cap is not a run finishing; `## Next picks` silently no-ops the residual gate (it needs `Residual / next picks`) — a check reporting clean while measuring nothing. **Filed #2881** — `deploy/**` is outside ci-cd's `paths:`, so the script that can revert the public site merged with no lint and no tests, a legitimate skip indistinguishable from a swallowed push. **Main:** green. **Owner asks (3):** #2882's cost call (worst case ~$26/mo but metrics are hourly-prorated — measured 80 of 165 route metrics active, so realistically ~$12–13/mo); #2734/#2836 September base (MTD $100.82, forecast $163.42 vs an $85 ceiling auto-reverting 09-01 → tier 3, reader AI dark); a Withings weigh-in (no longer blocks deploys, only the supersede). **Next:** #2668/#2669 on 08-19, #2741 box 5, #2670 (now unblocked), #2881, #2692, #2820. Full narrative: `handovers/HANDOVER_LATEST.md`.
