...
**Verified:** 2026-08-10 (coaching brilliance + the orchestrated queue burn; ~03:00Z → ~05:0xZ) — **the coach character work became evidence-driven.** Harvested the first night of real `CHAT#` transcripts and fixed what they showed: replies ran 5–10× Matthew's message length, one stat was recited three times in 47 minutes, 12 of 22 replies closed on a question, a stock opener leaked across two voices, and a summary row had memorialized the pre-#2478 identity bug ("I'm not Lisa") into the coach's own memory. Act 1 = 3 PRs + a roadmap: **#2481** CONVERSATION RULES v2 (register matching, question budget, assistant-ism ban, off-lane conversation welcomed, persona-outranks-remembered-notes) + **real prompt caching on the chat surface** (`build_request` had been sending `system` as a plain string while the docstring claimed COST-OPT-2 — on the platform's highest-frequency AI surface) + first-person CHAT#summary + the `eli_marsh` partition fix; **#2498** experiment-aware coaches (Day N of cycle 13, each coach's own open `PREDICTION#` calls, Eli's weekly priority — and `generation_date_iso=PT` into the grounder, without which every evening reply naming the day would self-hold); **#2505** bounded initiative (referral handoffs + Eli's weekday check-in, both dark until BotFather; ≤2 unsolicited/day). **`docs/design/COACH_HUMANITY_ROADMAP.md`** = the standing 52-idea ledger for the "pass for a person" mandate; 12 stories filed #2485–#2496 under `area:coach-humanity`. Act 2 burned 19 PRs / 12 closures / 31 triage actions (open corpus 64), incl. #2370 privacy scrub (~60 files once counted honestly), #2414 PT-today (49 sites, not ~15), #2331 strava measured-zero (648/650 zero rows are trainer sessions — storing 0 would fabricate the other way), #1905, #2326, #2350, #2351, #2464, #2467. Deploy-of-record GREEN (run 31354465497); #2468 closed with evidence. Gotcha class of the night: **fail-closed must be scoped to the artifact, not the lane** (#2503's raising screen inside a reconcile generator stopped literal reconciliation platform-wide until #2510) — plus the eternal-skeleton fetch hang (#2514) and the visual-QA dark-state discriminator (#2511). Owed: owner cdk `LifePlatformServe` (**required** — #2505's IAM diff strands later CI deploys), BotFather ×3 + Max rename, `CONTENT_FILTER_JSON` secret, strava 6-row reconcile, the ratification list, and Matthew's morning texts as Act 1's acceptance bar. Full narrative: `handovers/HANDOVER_LATEST.md`.
