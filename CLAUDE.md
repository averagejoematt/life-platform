...
**Verified:** 2026-07-04 (session 6) — **The data-truth batch (#486/#488/#491/#492/#495/#496) is MERGED + DEPLOYED + LIVE-VERIFIED** (PRs #520/#521, layer **v100**): ONE shared `weight_trend.latest_weight()` (withings backscan + 7-day apple window) replaces the three divergent current-weight resolutions and `/data/physical/` now renders **"LATEST · JUN 26"** instead of "today" for the 8-day-old weigh-in (M-5 live-verified); `/api/last_sync` carries withings; readiness stores + serves its ACTUAL inputs as `readiness_components` (cockpit shows recovery/sleep/hrv-trend/tsb, never the day-grade set); sleep_detail substitutes only the recovery trio + carries `recovery_night_of` (UI captions the splice); character engine reads `blood_glucose_time_in_range_pct` + never-scorable `body_fat_trajectory` removed (metabolic coverage now honest — live 0.15 from real absence, 1.0 pinned in tests; S3 config `config/matthew/character_sheet.json` updated — it's runtime-mutable, ALWAYS diff live vs repo before upload); nutrition_review's fully-dead `extract_cgm` repointed; hypothesis_engine reads real fields + whoop onset window skips `#WORKOUT#` sub-records; **strava out of `DECLARED_PAUSED_SOURCES`** — pipeline health check counts it again (live: paused=0), freshness says `behavioral-stale` not "paused". Gotcha: deployed-zip greps must use nested entry paths (`compute/…`, `mcp/…`) — a root-level grep returns 0 and mimics the asset-staging glitch. **7 area:data Now stories remain** (#473/#474-opus/#477/#480/#481/#482/#497); #518 (dropbox secret misfire) still open; watch `slo-source-freshness` 7-day window from 07-04. See `handovers/HANDOVER_LATEST.md`.
