...
**Verified:** 2026-07-13 — an external repo assessment ("documentation volume exceeding documentation truth" — 4 drift examples + the meta-point *governance is expressed but not achieved*) drove a full doc-accuracy + guardrails pass. **PR #1189 OPEN (4 commits, awaiting Matthew's merge; branch `docs/accuracy-and-drift-guardrails`), full offline suite 5106 passed / 0 fail, all six doc gates green.** **Phase 1 accuracy (`aba73513`):** fixed every confirmed drift — `ARCHITECTURE **Tools:** 127→64` (self-contradicted its own ruled header; the phrasing had no sync rule), `TESTING 1,217→3,646`, **35 shared-layer docstrings** across `lambdas/`+`mcp/`, essay `.md`+live-HTML (8→9 stacks, ~140→~64 tools, $75→$85), repo description $75→$85; `BACKLOG` $75 left intact (frozen archive, truthful history). **Phase 2 guardrails (`8952ec01`) — the structural fix:** new **`scripts/check_doc_facts.py`** generalized stale-number gate (knows ground truth from `sync_doc_metadata`'s discoverers, fails on a stale count/budget in ANY phrasing — precision-first: forward-only, glue-guarded, lambda_count unpoliced, ledger/historical exempt; first run caught a real "~125 MCP tools"); **tombstone scan extended to source** with one broad `shared (?:Lambda )?layer` rule (the 3 narrow ones missed "part of the shared layer" entirely); `test_count` + 2 RULES into sync. Both wired into docs-ci + ci-cd, documented in CONVENTIONS §8. **Phase 3 (`8bdc46da`):** untracked 22 generated `docs/restart/_*` reports (~1 MB churn). **Phase 4 (`0eb9d0bc`):** `.config.json` regen from `life-platform/mcp-api-key` + FDA grant → `NEW_MACHINE_BOOTSTRAP` §3b/§3c; reconciled stale "#1026 not landed" across DR docs (it landed `48f635e3`, memory leg live) + fixed a wrong datadrops S3 path (`uploads/…` 30d-expires → top-level `datadrops-archive/`). **5th finding (infra disclosure) mooted** — repo went PRIVATE, sweep found zero credential VALUES. **Gotcha lessons:** a false-positive gate gets disabled (34→2 hits by tightening); always prove a new gate catches a planted violation (vacuous-scan trap). **Next: merge #1189 (Matthew); optional follow-up = archive 7 REVIEW_BUNDLE_* snapshots; Matthew laptop = FDA grant + remove dead may25-pivot.plist; standing #1187/#1114/#741/#1148, /fullreview relaunch ~07-18.** Budget tier 1 ($82/$85). Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-13_GreenMainPrereg.md`.
