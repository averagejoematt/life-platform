...
**Verified:** 2026-07-11 mobile review ("mobile experience isn't as good as the website — full review + a plan in epics/issues an architect would endorse; get it into git and wrap"). **Review-only: 5 live-site sweep agents (55+ pages @390/360px, measured geometry) + architecture review → 5 epics + 19 stories + 3 accuracy issues FILED #997–#1023; zero code shipped by design.** Two root-caused criticals for Day-1 readers: #1002 scroll-reveal never fires on tall sections (`motion.js:260` threshold 0.1 vs ~22,600px sections — challenges/experiments backlogs invisible, challenges blank on desktop too) and #1003 app-bar overflow on every page (`.doors a` 0,1,1 `!important` resurrects the hidden `.nav-follow` 0,1,0; un-flexed `<button>` toggle unreachable at 360px). Plan keeps the one-responsive-codebase decision; epics: A launch-critical (Now #1002–#1005), B foundation — breakpoint tokens/app-bar rebuild/table primitive/chrome partial/44px pass/DS-v5 mobile spec (Next), C mobile in the pre-merge CI gate (Next), D wayfinding (Later), E perf+PWA (Later); accuracy #1021–#1023 (timeline Day-1 contradiction, stale $75 ceiling, privacy-vs-gear affiliate). Gotchas: two `!important` rules still fight on specificity; IO `threshold:0.1` is a time bomb on unbounded-height sections. **Next session: Epic A #997 first (small, root-caused, every mobile reader hits both on Day 1). Matthew Sunday queue unchanged: weigh-in → pipeline re-run → `fix_prologue_cycle_and_subscribe_ttl.py --apply` → `seed_genesis_preregistration.py --apply` + `publish_genesis_preregistration.py --apply`.** Full narrative: `handovers/HANDOVER_LATEST.md`; prior session (4/5 Now issues shipped): `handovers/HANDOVER_2026-07-11_Now-paydown.md`.
