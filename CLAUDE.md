...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/common/constants.py` (currently **2026-09-05**, cycle 16 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-09-04 (Opus 5, **Session U — the Friday re-anchor, then the drain past midnight**; owner: "do another full platform experiment reset so it all starts on friday september 4th", then an overnight backlog grant narrowed to `model:opus` and below, then "fix them then"). **Cycle 15 → cycle 16, genesis 2026-09-05 (Saturday)**, baseline 324.64 lbs; gates rendered 96/96 · semantic 8/8 · truth clean. The reset first landed on **Friday 2026-09-04 by owner typo** and the Friday flip worked unattended at PT midnight (`/api/journey` day_n 0 → 1); caught ~1h later and **re-anchored IN PLACE to the intended Saturday** — `--no-close-cycle` plus a corrected `CYCLE_GENESES[16]`, so no 1-day phantom cycle enters the registry `prereg_seal_gate.py` iterates (closing cycle 16 unsealed would have needed a permanent grandfather record). Nothing of record was lost: no weigh-in, no brief, no published prereg. **The #3477 sweep proved itself on that re-run** — it ABORTED the pipeline twice rather than exit 0, catching the JS fixtures and a `docs/MCP_TOOL_CATALOG.md` clobber where `restart_docs_update` appended a staler duplicate of a section the canonical generator already owns (the #1287 class, 3rd specimen — duplicate writer removed). Cycle 15 closed SEALED; **cycle 16 is FROZEN but UNPUBLISHED and stays unsealed until the attended publish runs (cycle 13's precedent is a grandfather record)**. **Closed #3474** (token alarm sat at the platform's 75th percentile, 25.8% [13.7%, 43.2%] of days — re-derived to 250k between Tukey 260,014 and median+3·MAD 221,743), **#3473** (missing compute heartbeat; the CAUSE was the `_DETECTORS` ledger naming neither compute pair), **#3477** (the reset ran ONE doc gate to CI's twelve — and the filed mechanism was WRONG: #2986's freshness hold classified the genesis anchor as a cosmetic re-stamp, so the one rule that converges it could never fire), **#3476** (a CI warning no deploy can clear — fixed WITHOUT widening `TOLERATED_NON_IAM`, a ratchet that may only shrink). **Filed #3478** (Day 1 serves a weigh-in that never happened). **#3390 worked to its limit, deliberately NOT closed** — restart_verify 23/2, both remaining failures owner-only; en route its own #2116 composite check turned out to have **never once run** (`describe_alarms` without `AlarmTypes`), silently skipping four assertions. **I broke main TWICE, both times by running targeted tests instead of the full suite** (5 checks, then 4 ratchets — the latter tripped by comments I wrote explaining the fix); **both repaired and CI-verified, main GREEN at eb777b5b**. **A CI/CD `cancelled` rollup hid real failures three times tonight — never read it as a timeout.** Through-line: six instruments unable to fail or not meaning what they said, including my own first wire-guard. **Open: #3390 (owner acts), #3478, #3403/#2978 (~09-08), #2883 (owner); #3422/#3436/#3373/#3042 are `model:fable`, out of lane.** Session U proper (Architect ritual) is UNTOUCHED, still 2026-09-08 — `~/.claude/plans/lovely-snacking-panda.md`. Full narrative: `handovers/HANDOVER_LATEST.md`.
