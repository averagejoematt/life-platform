...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-134), why things are the way they are; **ADR-103 = the complexity-posture ledger** (load-bearing / portfolio / retire-candidate per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
**Verified:** 2026-07-12 ~20:15 UTC (genesis evening) — backlog blitz under full autonomy ("maximize paydown, hardest first, Fable"): **17 issues + 7 epics closed, 21 PRs #1145–#1165 ALL merged + deployed + live-verified** (3 agent waves + serial merge queue + 2 driver drills). Review epics R1–R5/R7/R9 CLOSED; every unblocked review story shipped: protocols hypothesis grammar ×3 (#1116/#1117/#1118), immersive coach bios + Eli Marsh live at lead tier (#1113/#1112), per-timeframe coach narratives — Week lens finally consumes `/api/state_of_matthew`, month rollup, daily one-liner honestly attributed (#1115), cockpit instrument strip (#1106), habits dot strip (#1107), /data/physical/ cadences (#1119), build-log why-it-mattered ×40 beats — field now validator-required (#1120), chronicle audio date-keyed — fixed a LIVE wrong-audio state + dead IAM permission (#1121), gamification trio: /method/game/ engine-derived rulebook + fingerprint tripwire, level-up drivers persisted at fire (engine v1.6.1), /data/badges/ 0-of-40 honest wall (#1124/#1125/#1126). **#936 DR drill: Scenario-2 swap-back was broken as written** (env-repoint → implicitDeny, proven by IAM simulation; 11 hardcoded table literals) — fixed to ONE `TABLE_NAME=<restored> cdk deploy --all`, **measured RTO ~15 min** vs asserted 30 (PR #1153). **#1094 reset drill: the reader-truth gate's first full run correctly caught 2 real Day-1 lies** (home "every day, including the dips" over a 1-dot chart; cockpit scope caps) — fixed #1156/#1159/#1160, re-verified; drill contract now a REQUIRED RUNBOOK pre-reset step. New reflexes: `node --check` misses function-body const redeclares (verify via real `import()`); CloudFront caches API 404s 300s → invalidate a new route's viewer path post-deploy or the site smoke auto-rolls-back (bit twice). Post-genesis watch all green (phase-aware coach 17:00, weigh-in 314 lbs, character honest Day-1). Final: smoke 76/76, zero open PRs, tree clean. **Matthew queue: review the 18 drafted supplement hypotheses (#1148) + authored coach trait scores (`lambdas/coach_traits.py`); #1123 wk0 listen (holds epic #1082), #1114 portraits (holds #1080), #1029, #741. Deferred w/ reason: #916 (no observed friction), #1017 (needs data).** Full narrative: `handovers/HANDOVER_LATEST.md`; prior: `handovers/HANDOVER_2026-07-12_ReviewRemediation.md`.
