...
**Verified:** 2026-07-06 (opus backlog batch → layer **v118**; "do all the appropriate open opus issues" + "I approve you to do all merges and deploys") — **3 issues merged + deployed + LIVE-verified.** **Shipped:** **#737** (PR #770 `d6f3420f`, **ADR-125**) budget-guard re-banded by **audience** — internal/dev AI (ensemble, coherence_semantic [was unlisted→defaulted tier-3, a real inversion], chronicle_editor) pauses tier 1; reader narrative (coach_narrative **1→2**, state_of_matthew 1→2, chronicle) tier 2; irreducible reader (website_ai, daily_brief_ai) tier 3 · **#738** (PR #771 `f4122316`, **ADR-126**) hash-and-reuse coach generation briefs — new layer module `generation_cache.py` fingerprints the semantic brief, reuses the last gate-passed output on an exact match, skips Sonnet+gates; honesty invariant = any semantic change (incl. `gap_days`) busts the hash; metric `GenerationSkippedUnchanged` · **#733** (PR #772 `dd8d51a5`) per-post **share affordance** (`dx-share-btn` → native share / clipboard-copy the crawlable permalink; ~80% of #733 was pre-done). **Deploy:** layer **v117→v118** (`build_layer.sh` → `deploy/cdk_deploy.sh LifePlatformCore` → verify 118 → Compute/Email/Mcp/Web → `deploy_site_api.sh` auto-attach); site synced. Spot-verified daily-brief/coach-orchestrator/coach-memoir/mcp/site-api on **118**. **Deploy-verify catch:** #733's sitemap AC was NOT live — `v4_build_sitemap.py` existed but was never wired into `sync_site_to_s3.sh`, so `sitemap.xml` drifted post-less; fixed (`e85198c9`) + wired the builder into the sync (4 post URLs + chronicle noscript dated list now live — also covers #730's chronicle half). **3 post-merge CI reds, all fixed → main GREEN:** ruff isort (ran black+flake8 not ruff), LV4 lambda_map drift (new layer module needs `ci/lambda_map.json` too, not just `build_layer.sh`), coach_memoir tier-1→2 test. Gotchas: merge-train #770→#771 conflicts at `DECISIONS.md` EOF (ADR-125 then 126); pre-commit doc-sync leaves `site_api_common.py` dirty every commit (`git add`+`--amend`). **Ingestion+Operational stay v115 DELIBERATELY** (HAE gate) → `coherence_sentinel`'s new tier-1 pause not active there yet (benign); `site-api-ai` on v115 pre-existing (website_ai cutoff unchanged, no impact). **#730 SCOPED+deferred** (scorecard half needs a daily-gen Lambda + CloudFront route + deploy, attended — plan on the issue). Untouched opus: Attended #755/#750/#687, gated #746, large features #734/#409/#422/#421/#742/#753/#395/#749/#475. **Next:** deploy held Operational→v118 on HAE reconcile (lands coherence_semantic tier-1) · watch `GenerationSkippedUnchanged` · optional #380 build beat for this batch (not done — Matthew's call). Untracked `docs/reviews/R21_BACKLOG.md` left alone. See `handovers/HANDOVER_LATEST.md`.
