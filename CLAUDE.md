...
- `docs/DECISIONS.md` — ADRs (ADR-001 through ADR-148), why things are the way they are; **ADR-103/144 = the complexity-posture standard — the maintained ledger is `docs/PROPORTIONALITY.md`** (posture + rent + demote trigger per subsystem — consult it before adding or removing machinery); **ADR-104 = honest numbers everywhere** (behavioral-absence semantics in the character engine + the grounded-generation gate on every AI narrative surface); **ADR-105 = the rigor bar** (uncertainty + n on every statistical claim, every forecast graded, deterministic computation before any LLM verdict, thresholds from personal variance); **ADR-106 = coach portraits** (AI may sketch, only code ships, only Matthew approves — `docs/design/PORTRAIT_RUNBOOK.md`); **ADR-107 = the coverage floor + mypy tier-2** (story #419); **ADR-108 = coach quality gate promoted advisory → blocking** (N-06 #390 — measured 30d re-eval on real CloudWatch verdicts before flipping, regenerate-or-hold in `ai_calls._enforce_quality_gate`)
...
Experiment is anchored by `EXPERIMENT_START_DATE` in `lambdas/common/constants.py` (currently **2026-08-03**, cycle 12 — a future genesis is sanctioned: the site runs a pre-start countdown until Day 1, #931/#939). Re-anchoring is one idempotent command:
...
**Verified:** 2026-08-03 (solo Fable, 2026-08-02 evening → 08-03 afternoon, Matthew AT the keyboard — driven by the pre-prepared plan `swift-clearing-dawn`: un-strand deploys with owner actions requested LIVE, paydown waves, then the cycle-12 reset) — **all four phases closed. Cycle 12 is LIVE and SEALED same-day**: genesis 2026-08-03, baseline 321.6 (real weigh-in), prereg published + hash-verified end-to-end (artifact sha == stamp == `/api/calibration.prereg_seal`), predict-the-week live (W32), rendered verify 95/95, semantic 0 poisoned rows, `restart_verify` 16/17 (the 17th is structurally n/a on a same-day genesis — no pre-genesis gap window exists; the #1979 completion gate merged this session ran live for the first time: 12/12 cycles sealed-or-grandfathered). **19 PRs merged closing 16 issues** (#2061–#2083 range; waves of ≤5 worktree-implementers on their own model labels, driver-only merge queue), all **deployed and gate-verified** — six production-gate approvals flushed the stranded queue, the 7 merged-dark site-api halves went live (content_cadence 200, prereg_seal, character_stats agreement), the #1893 correction landed in full (public voided 273 → **1,708** after #2072 fixed the 1MB-page truncation the reconcile exposed — lifetime Brier n 33→50). Owner reconciles all applied with live review: 1,435+69 prereg voids, 382 countdown stamps (3 flagged rows approved per-row), the config-twin first sync (which caught `podcast_series_bible.json` NEWER in S3 — backported, never blind-applied), #1896's full chain (tombstone → regen → rebake, closed). The #1951 kill-switch is LIFTED (first live Wednesday send Aug 5 — owner reviews it). **Reconcile-ordering lesson: the countdown reconcile creates prereg orphans — run the void reconcile AFTER it** (the reset's invariant caught 69 and named its own cure). Three same-day-genesis defects found+fixed live (eve-framed prereg essay ×2 through one seam, wall-clock-free tests; the #1986 guard firing on honest genesis-week emptiness). Whoop died AGAIN overnight (rotation lost to a 502-no-retry, 400 unclassified — #2076 shipped the durable fixes) and recovery exposed **#2085: re-auth doesn't clear the breaker latch**. Phase 4 resolved free: the banked Fable delta had ALREADY completed 2026-08-02 (concurrent session) — my own plan + the memory index were the stale premise; fixed. **Main: green (`ab8b3673`).** **Build beat:** 2026-08-03-cycle-12-sealed-same-day. **NEXT:** #2080/#2081 (genesis-blind brief staleness + anomaly baseline — bite Day 1, top of Now), #2084 (challenges_catalog source-of-truth), #2085 (breaker latch), #2079's box-4 CloudWatch check after the next coach run; owner: 3 CodeQL dismissals + PR #2012 purge + review the Aug-5 first send. Full narrative: `handovers/HANDOVER_LATEST.md`.
