...
**Verified:** 2026-07-06 (#583 chart contract v2 + #412 training-truth + main-CI health) — **all shipped, merged, deployed, verified.** Matthew authorized all edits/merges/deploys ("close out as much as we can"). A **concurrent session was live** the whole time (it had just shipped #415/#417); I isolated every change in a throwaway worktree off the latest `origin/main` — clean 3-way merges, zero stomping (no #590/#408-style tangle). **#583 (PR #709, `9f9f2f30`, static-site only):** completed the `data-cpts` interaction contract 24/24 — `motion.js` now has ONE shared readout (`makeReadout`+`wireGrammar`) with two hit-test strategies: 2-D Euclidean (`data-cpts-hit="xy"`) for radials (`ring`/`pillarRing` via new `pillarRingCpts`/`radarChart`/`autonomicQuadrant`) and a reflow-robust `data-cells` DOM path for `heatStrip`/`mealWindowRibbon`/`habitsEffortMap`; batch-1 axis path behavior-preserved + regression-guarded. Verified via a Playwright harness then LIVE on `/data/{character,vitals,habits,nutrition}/`, smoke 67/67, 0 console errors, both themes. **#412 (PR #712 + fix PR #714, `4ab48797`, NO layer bump):** wired the built-but-uncalled `adherence_calc.calculate_adherence` into Hevy ingestion — `derive_adherence` embeds pushed-vs-performed adherence in the workout record (exact-match via `hevy_routine_id` immune to the UTC-date bug; Pacific-day fallback via new `pacific_time.pacific_date_of`; ad_hoc/ambiguous omit all pct per ADR-104), surfaced via `tools_hevy._slim_workout`→`get_workout_detail`; IAM S3-read grant for the catalog+cache; `cdk deploy LifePlatformIngestion LifePlatformMcp`. **THE LESSON: drive the real flow, not just tests** — unit tests green + deploy healthy, but re-ingesting the real 2026-06-25 workout showed a deflated `overall_pct=47.8%` (4 `tmpl:<id>` raw-template movements mis-counted missing); the tier-0 `_ir_movement_to_template` fix (#714) restored it to an honest 100.0. Real-data check forced by rewinding the `INGESTION_STATE#hevy` `since_iso` cursor + invoking `hevy-backfill` (idempotent; cursor auto-advances back). **Main-CI health (PRs #759 `610f787b` + #760 `c67c5602`, test/lint only):** main-push CI had been red at the **Lint gate** (masking Test) — `ruff --fix`'d an import-sort in `site_api_lambda.py` + fixed a stale `test_home_fold` assertion left by my own #590 caption rewrite; greening Lint then unmasked 4 portrait/card tests that imported PIL at module scope without a guard (CI test lanes run Pillow-free by design) → added `pytest.importorskip("PIL")` per the `test_card_engine` convention. **Main CI now GREEN through Lint+Deploy-critical+Unit Tests** (first time since the pre-#408 era; Deploy waits for prod approval as designed). **State at close:** `main`==origin (`c67c5602`), all this-session issues CLOSED, layer v115, worktrees pruned to 2 (main + `docs/uplevel-handover`). **Carried from the #415/#417 close (Matthew's calls):** re-stamp #417 SHIPS DISABLED pending 2 decisions (timing: rule 12:45 UTC vs Whoop recovery 17:30 UTC; branch push format) — flip SSM `/life-platform/hevy/restamp_enabled`+rule after · watch `ingest-reconciliation-whoop` (a real dropped Whoop workout `7a62677b…` should self-heal) · **GitHub Pages still enabled+public**. **Next:** #409 batch-price content AI (layer, attended) · #395 MCP prune (attended/destructive) · #687 OIDC (watched IAM) · #475 Hevy edit/delete (adjacent to #412) · #552 State-of-Matthew brief. See `handovers/HANDOVER_LATEST.md`.
