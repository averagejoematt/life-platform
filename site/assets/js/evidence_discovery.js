...
export async function renderExperiments(d) {
  ...
  const libCard = (x) => {
    ...
    // #1983 — for a no-direct-study entry the prose IS the absence ("No direct study
    // verified — trade book: …"), so printing it as `src:` beside the "no direct study"
    // marker stutters. Show the marker, and let its tooltip carry the reason.
    const noStudy = x.citation_status === "no-direct-study";
    const meta = [x.pillar, x.difficulty, !noStudy && x.evidence_citation && "src: " + x.evidence_citation]
      .filter(Boolean)
      .map(esc)
      .join("  ·  ");
