...
export async function renderHabits(d) {
  ...
  // Tier-0 %, cut-start (Aug 17) ringed. Replaces the old green/amber/red 7-day grid (rainbow +
  // red, both off-brand) with the ONE-ember heat scale. Reuses heatStrip (compact mode).
  const grid = _histAll.length
    ? sec("90-day adherence heatmap", heatStrip(_histAll, { valueKey: "tier0_pct", unit: "%", max: 100, compact: true, cutDate: "2026-08-17", label: "Daily Tier-0 adherence", caption: "Each square is a day · ember intensity = the non-negotiables held · the ringed square is the cut starting Aug 17 · 90-day history predates the cut." }))
    : "";
