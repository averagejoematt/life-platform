...
function renderSleep(d) { const s = d.sleep_detail || {}; if (!Object.values(s).some(has)) return empty("No sleep data yet — score, stages, HRV and recovery appear here nightly."); return figs([s.sleep_score != null && fig(fmt(s.sleep_score), "sleep score"), s.total_sleep_hours != null && fig(fmt(s.total_sleep_hours, 1), "hours"), s.sleep_efficiency != null && fig(fmt(s.sleep_efficiency) + "%", "efficiency"), s.recovery_score != null && fig(fmt(s.recovery_score), "recovery"), s.hrv != null && fig(fmt(s.hrv), "hrv ms")]) + sec("Stages & physiology", kvtable({ deep_sleep_hours: s.deep_sleep_hours, rem_sleep_hours: s.rem_sleep_hours, whoop_quality: s.whoop_quality, bed_temp_f: s.bed_temp_f })) + sec("Sleep-score trend", lineChart(d.sleep_trend || [], { valueKey: "sleep_score", label: "Sleep score", emptyMsg: "The sleep-score trend fills in nightly." })) + note("Correlative — last night and the recent trend."); }
...
async function renderChallenges(d) { const cur = await tryJSON("/api/current_challenge"); const cc = cur && cur.current_challenge; const list = d.challenges || []; const sm = d.summary || {}; const banner = cc && cc.challenge ? `<div class="rd-obs"><p class="rd-primary">${esc(cc.challenge)}</p>${cc.detail ? `<p class="rd-why">${esc(cc.detail)}</p>` : ""}<p class="rd-meta label">day ${esc(cc.days_complete ?? 0)} of ${esc(cc.days_total ?? "—")}</p></div>` : ""; const head = figs([fig(sm.total ?? list.length, "total"), sm.active != null && fig(sm.active, "active"), sm.completed != null && fig(sm.completed, "completed")]); const cards = list.length ? `<div class="rd-cards">${list.slice(0, 40).map((c) => { const done = !!c.completed_at; const active = !done && !!c.activated_at; return `<article class="rd-card"><header class="rd-cardhead"><h3 class="rd-cardname">${esc(ttl(c.challenge_id || "Challenge"))}</h3><span class="rd-badge ${active ? "rd-badge-live" : ""}">${done ? "completed" : active ? "active" : "candidate"}</span></header><p class="rd-meta label">${[c.character_xp_awarded != null && c.character_xp_awarded + " XP", c.badge_earned && "🏅 badge"].filter(Boolean).join("  ·  ")}</p></article>`; }).join("")}</div>` : empty("No challenges yet — they'll appear here as you take them on."); return banner + head + cards + note("An N=1 instrument — reader participation is deferred."); }
...
function renderExperiments(d) { const xs = (d.experiments || []).slice().sort((a, b) => (/(active|running)/i.test(a.status || "") ? 0 : 1) - (/(active|running)/i.test(b.status || "") ? 0 : 1)); if (!xs.length) return empty("No experiments running yet."); return figs([fig(xs.length, "experiments")]) + `<div class="rd-cards">${xs.map((x) => { const done = /complete|done|ended|closed/i.test(x.status || ""); const verdict = x.hypothesis_confirmed === true ? "confirmed" : x.hypothesis_confirmed === false ? "not confirmed" : (x.outcome || x.status || "running"); return `<article class="rd-card"><header class="rd-cardhead"><h3 class="rd-cardname">${esc(x.name)}</h3><span class="rd-badge ${done ? "" : "rd-badge-live"}">${esc(x.status || "")}</span></header>${x.hypothesis ? `<p class="rd-why"><span class="label">hypothesis</span> ${esc(x.hypothesis)}</p>` : ""}${x.result_summary && !isBad(x.result_summary) ? `<p class="rd-line">${esc(x.result_summary)}</p>` : ""}<p class="rd-meta label">${[verdict, x.grade && "grade " + esc(x.grade)].filter(Boolean).map(esc).join("  ·  ")}</p></article>`; }).join("")}</div>` + note("N=1 instrument exposed as proof."); }
async function renderHabits(d) { const dows = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]; const a = d.day_of_week_avgs || []; const mx = Math.max(1, ...a); const reg = await tryJSON("/api/habit_registry"); const habits = (reg && reg.habits) || []; const head = figs([fig(d.current_streak ?? 0, "day streak"), fig(d.days_tracked ?? 0, "days tracked"), habits.length ? fig(habits.length, "habits tracked") : ""]); const dow = a.length ? sec("Adherence by day of week", `<div class="hb-chart">${a.map((v, i) => `<div class="hb-col"><span class="hb-bar" style="height:${Math.max(4, (v / mx) * 100)}%"></span><span class="hb-day label">${dows[i] || ""}</span></div>`).join("")}</div>`) : ""; const list = habits.length ? sec(`All habits (${habits.length})`, `<table class="rd-tbl"><thead><tr><th>habit</th><th>frequency</th><th>expected impact</th><th>weight</th></tr></thead><tbody>${habits.map((h) => `<tr><td class="rd-name">${esc(h.name)}</td><td class="num">${esc(h.target_frequency || "—")}</td><td>${esc(h.expected_impact || "")}</td><td class="num rd-range">${fmt(h.scoring_weight)}</td></tr>`).join("")}</tbody></table>`) : ""; return head + dow + list + note("The full habit registry — the discipline layer behind Consistency."); }
