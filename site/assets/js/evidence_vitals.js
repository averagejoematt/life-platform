...
export function vitalsCaptureBacklog(p) {
  ...
  const cards = [
    `<div class="cap-card"><h3 class="cap-h">Blood pressure <span class="cap-tag">needs a cuff</span></h3><p class="rd-meta label">The most valuable missing daily vital for a heavy man mid-cut — and the one that should improve visibly. A morning cuff reading would add a BP trend here. Not captured yet.</p></div>`,
    `<div class="cap-card"><h3 class="cap-h">Hourly habit history <span class="cap-tag">upgrades the glyphs</span></h3><p class="rd-meta label">The glyph row shows "X of N today"; with hourly habit-completion history it would become "vs your average by this hour" — a real pace benchmark. Until that exists, the honest X-of-N stands (no faked hourly baseline).</p></div>`,
    `<div class="cap-card"><h3 class="cap-h">Continuous intraday HR <span class="cap-tag">needs capture</span></h3><p class="rd-meta label">Walking heart rate now ships above (per-walk averages from Strava). A <em>continuous</em> intraday curve — heart rate second-by-second through the day — still isn't in the feed; Garmin/Whoop store only summaries and zone-seconds, so we don't draw one.</p></div>`,
    `<div class="cap-card"><h3 class="cap-h">Subjective energy / mood <span class="cap-tag">needs capture</span></h3><p class="rd-meta label">A morning 1–5 "how do you actually feel" check-in — the ground-truth overlay the wearables miss, graded against the readiness read.</p></div>`,
    `<div class="cap-card"><h3 class="cap-h">Cross-metric relationships <span class="cap-tag">unlocks ${wks != null && wks > 0 ? "in ~" + wks + " days" : "at 2 weeks"}</span></h3><p class="rd-meta label">Which vitals move together — sleep→recovery, strain→next-day HRV — stays <strong>withheld</strong> until ~2 weeks of overlapping days (day ${dayNum != null ? dayNum : "?"} now). At n≈10 it's noise, not signal; when it opens it reuses the self-policing correlation board from Sleep — direction-only first, a coefficient only once the window is honest. No chip is drawn early.</p></div>`,
  ];
