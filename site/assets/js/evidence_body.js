...
export const PHYS_GENESIS = "2026-09-05";
...
/* #3523 (ADR-104) — "This week — daily movement", as a PURE builder so the three
   states that matter (a pre-genesis week, Day 1, a week with real data) can be
   pinned by tests/js instead of only existing inside an async render.

   The defect it closes: `weekly_physical_summary`'s query window is clamped to
   EXPERIMENT_START by `_experiment_date`, so a calendar day before genesis was never
   asked about — yet the handler built its 7-day array unconditionally and stamped
   `total_active_minutes: round(0)` on every one of them. `fmt` returns "—" for null
   but "0" for 0, so a single absence was encoded TWO ways in the same row: steps "—"
   beside active-min "0". The table then read "Sat — 0 · Sun — 0 · …" — seven days of
   measured stillness that were never measured at all. The API now sends null for a
   pre-genesis day (that is the real fix; the front end cannot tell 0-the-measurement
   from 0-the-fabrication), and this caption says WHY the week opens with dashes. */
export function movementWeekBody(days) {
  const rows = (days || []).map((x) => `<tr><td class="rd-name">${esc(x.day_of_week || x.date)}</td><td class="num">${fmt(x.steps)}</td><td class="num">${fmt(x.total_active_minutes)}</td></tr>`).join("");
  const pre = (days || []).filter((x) => x && x.pre_genesis).length;
  const note = pre
    ? `<p class="rd-meta label">${pre} of these ${(days || []).length} days fall before Day 1 — outside the experiment window, so they read as absence in every column, never as zero minutes of movement.</p>`
    : "";
