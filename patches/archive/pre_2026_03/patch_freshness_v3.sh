#!/bin/bash
# patch_freshness_v3.sh — Update apple_health thresholds for webhook cadence
# Changes: apple_health 504h→12h (was manual XML export, now 4h webhook push)
# Also adds gait/energy to impacts list, updates description
set -euo pipefail

FUNCTION_NAME="life-platform-freshness-checker"
REGION="us-west-2"

echo "=== Patch Freshness Checker v3: Apple Health webhook thresholds ==="
...
"""
Data Completeness Alerting v3
Monitors all automated data sources for gaps that could corrupt trend analysis.

v3 changes:
- apple_health threshold: 504h/720h → 12h/24h (now webhook-driven via Health Auto Export)
- Updated apple_health impacts for new gait, energy, CGM tools
...
SOURCES = {
    ...
    'apple_health': {
        ...
        'description': 'Steps, gait metrics, energy expenditure, CGM glucose, HR/HRV cross-ref',
        'stale_hours': 12,        # Webhook pushes every 4h — 12h = 3 missed cycles
        'critical_hours': 24,     # Full day missed = likely app/auth failure
        'impacts': [
            'get_field_stats (steps, active calories)',
            'gait analysis (walking speed, step length, asymmetry, double support)',
            'energy balance (TDEE from active + basal calories)',
            'CGM glucose tracking (blood_glucose_*, time_in_range)',
            'anomaly-detector (steps, gait metrics)',
            'daily brief (step count, glucose insights)',
            'device_agreement (Apple Watch HR/HRV cross-reference)',
        ],
    },
    'eightsleep': {
        'name': 'Eight Sleep',
...
<div style="max-width:640px;margin:0 auto;padding:24px;">
    <div style="background:linear-gradient(135deg,{summary_color},{'#991b1b' if critical else '#d97706'});border-radius:12px;padding:24px;margin-bottom:24px;">
        ...
        <p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">
            {len(critical)} critical, {len(stale) - len(critical)} warning — {len(fresh)}/{len(results)} sources healthy
        </p>
    </div>
    ...
    <div style="text-align:center;color:#9ca3af;font-size:12px;padding:16px;">
        ...
        Life Platform Data Completeness Monitor v3
    </div>
</div>
</body>
</html>'''
...
zip -j /tmp/freshness_v3.zip lambda_function.py

echo "Deploying..."
aws lambda update-function-code \
    --function-name "$FUNCTION_NAME" \
    --zip-file fileb:///tmp/freshness_v3.zip \
    --region "$REGION" > /dev/null

aws lambda wait function-updated --function-name "$FUNCTION_NAME" --region "$REGION"

echo "✓ Freshness checker v3 deployed"
echo ""
echo "Changes:"
echo "  • apple_health: 504h/720h → 12h/24h (webhook-driven, was manual XML)"
echo "  • apple_health impacts: added gait, energy, CGM, device_agreement"
echo "  • Description updated to reflect webhook source"
echo ""
echo "Test: aws lambda invoke --function-name $FUNCTION_NAME /tmp/freshness_test.json --region $REGION && cat /tmp/freshness_test.json | python3 -m json.tool"
