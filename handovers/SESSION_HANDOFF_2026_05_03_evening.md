...
| 3 | Partner weekly email | Already off via `EXTERNAL_EMAILS_ENABLED=false` kill switch. Same switch covers chronicle-email-sender + weekly-signal. No action. |
...
2. **Partner does not appear by name or by relationship-detail.** If she appears at all, "the person closest to him" — and only if the piece needs her, which it doesn't.
