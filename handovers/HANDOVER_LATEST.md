...
**Date:** 2026-03-09  
**Session:** IC-4 + IC-5 + Partner email fix  
**Version:** v3.3.8
...
### Partner Weekly Email ✅ LIVE (QA mode — sending to awsdev)
**Lambda:** `partner-weekly-email`

Fixed two blockers:
1. `ANTHROPIC_SECRET` default was `"life-platform/api-keys"` → corrected to `"life-platform/ai-keys"`
2. Lambda was missing 3 env vars entirely: `EMAIL_SENDER`, `PARTNER_EMAIL`, `ANTHROPIC_SECRET`

All 5 Board sections now rendering: `lede`, `rodriguez`, `conti`, `murthy`, `chair`  
Runtime: ~29s (within 90s timeout)  
Sending to: `awsdev@mattsusername.com` — swap to his partner's real address when QA complete
...
```
9:30 AM PT  partner-weekly-email
...
|------|----------|--------|
| Partner email — swap to real address | 🟡 Medium | After QA cycles at awsdev |
...
- `partner-weekly-email` was deployed with CDK but missing 3 env vars — always verify env vars after CDK deploy
