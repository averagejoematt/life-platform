...
- **Main:** **stranded, not green** — `check_main_green.py` reports R8-ST6 Plan-red at the session start and the condition is unchanged: the owner's `cdk_deploy.sh LifePlatformServe` has still not run, so every ci-cd deploy strands. #2527 adds a second CDK-owned resource to that same stack. **#2468 deliberately left open** (drift still present; do not re-close it by grepping a job log — use annotations or `check_ci_warnings.py`).
...
- **Owner ③** BotFather trio + Max rename — unchanged, still blocks Eli's check-in and all Vale/Brooks referrals.
- **Owner ④** `CONTENT_FILTER_JSON` secret · **⑤** the strava 6-row reconcile · **⑥** portraits.
