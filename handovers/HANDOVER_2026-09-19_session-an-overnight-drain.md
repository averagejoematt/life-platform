...
**Main:** red — FOUR causes in sequence tonight, each fixed forward: `test_config_ownership_3785` from `f2540c84f` to `064ca8f3a` (cured by #3969); the #3609 ISO-parse registry gate from `7667991df` (#3975's hand-rolled fromisoformat, cured by #3981 at `7d0eefa49`); `test_residual_queue_gate_1340` on this handover's own section name from `5650ac83f` (renamed at `ec12a7ec4`); then the #3609 gate AGAIN from `adb8a4805` — #3965 (my own #3772 PR) carried two `date.fromisoformat` day-key parses in `lambdas/training/routine_repo.py`, which #3977's merged gate caught on the next main run — **fix-forward PR #3983 (`shift_day_key`) is armed; the next session's Step 0 confirms it merged, rejects its lease by name, and reads main green.**
