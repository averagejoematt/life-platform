    ...
    try:
        _cw.put_metric_data(
            ...
            MetricData=[
                ...
                {"MetricName": "AnthropicAPIFailure", "Value": 1, "Unit": "Count"},  # #3260: fleet-wide twin (see common/retry_utils)
            ],
        )
    except Exception as e:
...
def lambda_handler(event, context):
    ...
    predictions_made = extraction.get("predictions_made", [])
    _gradable_n = 0  # SS-06: track directional/point (gradable) vs qualitative for the run metric
    _qualitative_n = 0
