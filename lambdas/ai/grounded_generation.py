...
pieces that previously lived apart:
  ...
  4. Cycle-freshness   — baseline_freshness_findings() (#1691, epic #1687) catches
                         ...
                         Takes the cycle constants as params so it stays pure. Lives
                         in ai/baseline_freshness.py since #3154 (this file was at its
                         §2 ceiling) along with experiment_span_findings() (#1897) and
                         absence_span_findings() (#2756); all three re-exported here.
...
# Moved to ai/baseline_freshness.py 2026-08-25 (#3154), the same §2-ceiling reason
# as ai/behavior_logs.py (#2056) above and ai/regen_discard_telemetry.py (#3086):
# this module was sitting at its 1,200-line hard ceiling with zero headroom, and
# the #3154 determiner-variant regex fix (the "started THIS experiment at" gap —
# see that module's _BASELINE_FRAMING_RE comment) had nowhere to land. Re-exported
# here so no caller's import path changes.
try:
    from ai.baseline_freshness import (  # noqa: F401
        absence_span_findings,
        baseline_freshness_findings,
        experiment_span_findings,
    )
except ImportError:  # pragma: no cover — flat/layer bundle layout
    from baseline_freshness import (  # type: ignore[no-redef]  # noqa: F401
        absence_span_findings,
        baseline_freshness_findings,
        experiment_span_findings,
    )
