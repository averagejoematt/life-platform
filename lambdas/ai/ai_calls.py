...
- If `last_food_log_date` is PRESENT, ground the day-count in it and the real `gap_days` — e.g. "it's been four days since you logged a meal"; cite the real number, never round it up for drama. If `last_food_log_date` is NULL, nothing has been logged this cycle and `gap_days` measures the cycle window (#955), NOT a pause: say "nothing logged yet this cycle", and NEVER say logging "paused"/"went silent N days ago" or date a transition — no such event happened (#2382).
