...
def _section_error_html(section_name, error):
    ...
    return (
        '<div style="background:#fef2f2;border-left:3px solid #fca5a5;'
...
# build_html below is a thin orchestrator that calls these in order. Each helper is a
# verbatim move of its section group; the daily-brief golden + equivalence harness pin
...
def _brief_header(brief_mode, data, day_grade_score, grade, tldr_guidance, vacation_fund, banner_color, day_label):
    ...
    try:
        ...
        out += _section_error_html("Adaptive Mode Banner", _e)

    # --- Dark gradient header ---
    out += (
        '<div style="background:linear-gradient(135deg,' + banner_color + ",#16213e,#0f3460);"
        'padding:32px 24px 24px;border-bottom:1px solid #2d2d5e;">'
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;">'
        "<div>"
    )

    grade_color = {"A": "#22c55e", "B": "#84cc16", "C": "#f59e0b", "D": "#f97316", "F": "#ef4444"}.get(grade, "#94a3b8")
    ...
    if day_grade_score is not None:
        out += (
            ...
            '<p style="color:' + grade_color + ';font-size:36px;font-weight:800;margin:0;line-height:1;">' + grade + "</p>"
    ...
    try:
        ...
        out += _section_error_html("Vacation Fund", _vf_e)

    return out


def _brief_character(character_sheet, protocol_recs, triggered_rewards):
    """Section group of the daily brief (extracted from build_html, ADR-pending).
    ...
    try:
        if character_sheet:
            ...
            tier_bg = {"Foundation": "#1c1917", "Momentum": "#1e3a5f", "Discipline": "#2e1065", "Mastery": "#451a03", "Elite": "#450a0a"}
            tc = tier_colors.get(cs_tier, "#78716c")
            tb = tier_bg.get(cs_tier, "#1c1917")

            out += (
                '<div style="background:' + tb + ";border-left:3px solid " + tc + ";"
            ...
            # Level events
            if cs_events:
                out += '<div style="padding:8px 24px;background:rgba(0,0,0,0.2);">'
                for ev in cs_events:
                    ev_type = ev.get("type", "")
                    ...
                    if "character" in ev_type:
                        ...
                        out += (
                            '<p style="color:#a78bfa;font-size:11px;margin:2px 0;">🎖 '
                            + pillar
                            + " Tier: "
                            + str(old)
                            + " → "
            ...
            # Active effects
            if cs_effects:
                out += '<div style="padding:4px 24px 8px;background:rgba(0,0,0,0.1);">'
            ...
            if triggered_rewards:
                out += '<div style="padding:8px 24px;background:rgba(251,191,36,0.1);border-top:1px solid rgba(251,191,36,0.2);">'
                for rw in triggered_rewards:
                    out += (
                        '<p style="color:#fbbf24;font-size:12px;margin:2px 0;font-weight:600;">'
            ...
            if protocol_recs:
                ...
                for rec in protocol_recs:
                    ...
                    out += '<p style="color:#f87171;font-size:11px;margin:2px 0;">' + arrow + " " + pillar_name + ":</p>"
                    for proto in protos:
                        if isinstance(proto, dict):
                            out += (
                                '<p style="color:#9ca3af;font-size:10px;margin:0 0 2px 12px;">• ' + str(proto.get("name", proto)) + "</p>"
                            )
                        else:
                            out += '<p style="color:#9ca3af;font-size:10px;margin:0 0 2px 12px;">• ' + str(proto) + "</p>"
                out += "</div>"

            out += "<!-- /S:character_sheet --></div>"

    except Exception as _e:
        out += _section_error_html("Character Sheet", _e)

    return out


def _brief_scorecards(component_details, component_scores, data, mvp_streak, profile, readiness_colour, vice_streaks):
    """Section group of the daily brief (extracted from build_html, ADR-pending).
    ...
    try:
        ...
        if t0_data:
            ...
            if vice_streaks:
                ...
                if vice_parts:
                    out += '<p style="color:#22c55e;font-size:10px;margin:4px 0 0;">🚫 ' + " · ".join(vice_parts) + "</p>"
            out += "</div>"
        ...
        if sleep_dur or sleep_score_val:
            out += '<div style="margin-top:10px;padding-top:10px;border-top:1px solid #2d2d5e;">'
        ...
        out += _section_error_html("Scorecard", _e)
    ...
    try:
        ...
        out += _section_error_html("Essential Seven", _e)
    ...
    try:
        ...
        out += (
            '<div style="background:#1e293b;padding:16px 24px;border-bottom:1px solid #2d2d5e;">'
            '<div style="display:flex;align-items:center;gap:12px;">'
            '<div style="background:rgba(0,0,0,0.3);border-radius:50%;width:48px;height:48px;'
            'display:flex;align-items:center;justify-content:center;font-size:24px;">' + r_emojis.get(readiness_colour, "⚪") + "</div>"
            '<div><p style="color:#94a3b8;font-size:10px;margin:0;font-weight:700;">READINESS SIGNAL</p>'
            '<p style="color:' + r_col + ';font-size:20px;font-weight:700;margin:2px 0;">' + r_labels.get(readiness_colour, "—") + "</p>"
        ...
        if recovery or strain or rhr:
            out += '<div style="margin-top:10px;display:flex;gap:16px;">'
            for r_label, r_val in [
                ("Recovery", str(round(recovery)) + "%" if recovery else "—"),
                ("Strain", str(round(strain, 1)) if strain else "—"),
                ("RHR", str(round(rhr)) + " bpm" if rhr else "—"),
            ]:
                out += (
                    '<div><p style="color:#e2e8f0;font-size:14px;font-weight:600;margin:0;">' + r_val + "</p>"
        ...
        out += _section_error_html("Readiness", _e)

    return out


def _brief_training_body(data, full_streak, mvp_streak, profile, training_nutrition):
    """Section group of the daily brief (extracted from build_html, ADR-pending).
    ...
    try:
        ...
        if activities:
            for act in activities:
                ...
                if stat_parts:
                    out += '<p style="color:#94a3b8;font-size:11px;margin:4px 0 0;">' + " · ".join(stat_parts) + "</p>"
                out += "</div>"

        elif data.get("garmin"):
            garmin = data.get("garmin")
            steps = safe_float(garmin, "steps")
            out += '<p style="color:#64748b;font-size:12px;margin:0;">No structured workouts. Steps: ' + fmt_num(steps) + "</p>"
        else:
            out += '<p style="color:#64748b;font-size:12px;margin:0;">No training data for yesterday.</p>'
        ...
        try:
            ...
            if acwr_val is not None:
                ...
                out += (
                    '<p style="color:#475569;font-size:10px;margin:4px 0 0;">'
                    'ACWR: <span style="color:'
                    + acwr_color
                    + ';font-weight:600;">'
                    + str(round(_av, 2))
                ...
                if acwr_alert and acwr_reason:
                    out += (
                        ...
                        '<p style="color:#fca5a5;font-size:11px;margin:2px 0 0;line-height:1.5;">' + acwr_reason + "</p></div>"
                    )
        except Exception:
            pass
        ...
        out += _section_error_html("Training Report", _e)
    ...
    try:
        ...
        if cals is not None:
            ...
            if fiber:
                out += '<p style="color:#475569;font-size:10px;margin:0;">Fiber: ' + str(round(fiber)) + "g</p>"
        else:
            out += '<p style="color:#64748b;font-size:12px;margin:0;">No nutrition data logged yesterday.</p>'

        nutrition_text = (training_nutrition or {}).get("nutrition", "")
        if nutrition_text:
            out += (
                '<div style="background:#16213e;border-left:3px solid #22c55e;border-radius:0 8px 8px 0;'
                'padding:10px 14px;margin-top:10px;">'
        ...
        out += _section_error_html("Nutrition Report", _e)
    ...
    try:
        ...
        if registry:
            # Tier 0 first
            for tier_label, tier_num, tier_color in [
                ("TIER 0 — NON-NEGOTIABLE", 0, "#ef4444"),
                ("TIER 1 — HIGH PRIORITY", 1, "#f59e0b"),
                ("TIER 2 — GOOD TO DO", 2, "#64748b"),
            ]:
                tier_habits = [(n, m) for n, m in registry.items() if m.get("tier", 2) == tier_num and m.get("status") == "active"]
                if not tier_habits:
                    continue
                ...
                for h_name, meta in sorted(tier_habits, key=lambda x: x[0]):
                    ...
                    out += (
                        ...
                        '<div><p style="color:' + ("#e2e8f0" if completed else "#94a3b8") + ';font-size:12px;margin:0;">' + h_name + "</p>"
                    )
                    if why and not completed:
                        out += '<p style="color:#475569;font-size:10px;margin:0;">' + why[:80] + "</p>"
                    out += "</div></div>"
            ...
            for sg, misses in synergy_misses.items():
                ...
                if len(misses) >= total * 0.5 and total >= 3:
                    out += '<p style="color:#f59e0b;font-size:10px;margin:6px 0 0;">' "⚠ Synergy alert: " + sg + " stack mostly missed</p>"
        elif h_map:
            done_count = sum(1 for v in h_map.values() if v and float(v) >= 1)
            total_count = len(h_map)
            out += (
                '<p style="color:#94a3b8;font-size:13px;margin:0;">' + str(done_count) + " / " + str(total_count) + " habits completed</p>"
            )
        else:
            out += '<p style="color:#64748b;font-size:12px;margin:0;">No habit data for yesterday.</p>'

        out += "</div><!-- /S:habits -->"
    except Exception as _e:
        out += _section_error_html("Habits Deep-Dive", _e)

    # --- Supplements ---
    try:
        out += '<div style="background:#1e293b;padding:16px 24px;border-bottom:1px solid #2d2d5e;">'
        out += '<p style="color:#64748b;font-size:10px;margin:0 0 8px;font-weight:700;letter-spacing:1px;">SUPPLEMENTS</p>'
        ...
        if supps_today:
            ...
            out += '<p style="color:#475569;font-size:11px;margin:0;">No supplement data for yesterday.</p>'
        out += "</div>"
    except Exception as _e:
        out += _section_error_html("Supplements", _e)
    ...
    try:
        ...
        out += _section_error_html("CGM Spotlight", _e)
    ...
    try:
        ...
        if ws or sl:
            gait_items = []
            if ws:
                gait_items.append(("Speed", str(round(ws, 2)) + " mph", "#22c55e" if ws >= 3.0 else "#f59e0b" if ws >= 2.5 else "#ef4444"))
            if sl:
                gait_items.append(("Step Length", str(round(sl, 1)) + '"', "#22c55e" if sl >= 27 else "#f59e0b"))
            if asym:
                gait_items.append(("Asymmetry", str(round(asym, 1)) + "%", "#22c55e" if asym < 3 else "#f59e0b" if asym < 5 else "#ef4444"))
            if dbl:
            ...
            out += '<p style="color:#64748b;font-size:11px;margin:0;">No gait data available.</p>'
        out += "</div>"
    except Exception as _e:
        out += _section_error_html("Gait & Mobility", _e)

    # --- Habit Streaks ---
    try:
        out += "<!-- S:habit_streaks -->"
        out += (
            '<div style="background:#16213e;padding:16px 24px;border-bottom:1px solid #2d2d5e;">'
        ...
        if mvp_streak > 0 or full_streak > 0:
            out += '<div style="display:flex;gap:16px;">'
            if mvp_streak > 0:
                out += (
                    '<div style="text-align:center;">'
            ...
            out += '<p style="color:#475569;font-size:11px;margin:0;">No active streak. Start today.</p>'

        out += "</div><!-- /S:habit_streaks -->"
    except Exception as _e:
        out += _section_error_html("Habit Streaks", _e)
...
def _brief_lifestyle(data, profile, tldr_guidance):
    ...
    try:
        ...
        if weather:
            ...
            if temp_hi:
                out += (
                    '<div><p style="color:#f59e0b;font-size:18px;font-weight:700;margin:0;">'
                    + str(round(temp_hi))
                    + "°/"
                    + (str(round(temp_lo)) if temp_lo else "—")
                    + "°F</p>"
        ...
        out += _section_error_html("Weather", _e)

    # --- Blood Pressure ---
    try:
        out += "<!-- S:blood_pressure -->"
        ...
        if bp_data:
            ...
            if bp_class:
                out += (
                    ...
                    + bp_class
                    + '</p><p style="color:#475569;font-size:9px;margin:0;">AHA Class</p></div>'
                )
            if pulse_val:
                out += (
                    '<div><p style="color:#94a3b8;font-size:13px;margin:0;">'
                    + str(round(pulse_val))
                    + ' bpm</p><p style="color:#475569;font-size:9px;margin:0;">Pulse</p></div>'
                )
            if readings > 1:
                out += '<div><p style="color:#475569;font-size:11px;margin:0;">' + str(readings) + " readings avg</p></div>"
            out += "</div></div>"
        out += "<!-- /S:blood_pressure -->"
    except Exception as _e:
        out += _section_error_html("Blood Pressure", _e)
    ...
    try:
        ...
        if todoist:
            ...
            out += (
                ...
                + load_label
                + '</p><p style="color:#475569;font-size:9px;margin:0;">LOAD SIGNAL</p></div>'
                "</div>"
            )
            ...
            if by_project:
                ...
                if top_proj:
                    out += '<p style="color:#475569;font-size:10px;margin:8px 0 4px;">Yesterday — '
                    out += " · ".join(f'<span style="color:#94a3b8;">{p}</span> {c}' for p, c in top_proj)
                    out += "</p>"

            out += "</div>"
        out += "<!-- /S:task_load -->"
    except Exception as _e:
        out += _section_error_html("Task Load", _e)
    ...
    try:
        ...
        if latest_weight:
            ...
            out += (
                ...
                '<p style="color:#e2e8f0;font-size:20px;font-weight:700;margin:0;">' + str(round(latest_weight, 1)) + " lbs</p>"
            )
            ...
            if week_ago_weight:
                ...
                out += '<p style="color:' + delta_color + ';font-size:14px;font-weight:600;margin:0;">' + delta_str + " lbs vs 7d ago</p>"
            out += "</div>"

            out += (
                '<div style="background:#374151;border-radius:6px;height:8px;margin-bottom:6px;">'
            ...
            if phase_name:
                out += (
                    ...
                    + phase_name
                    + "</strong>"
                    + (" · target " + str(round(phase_end)) + " lbs" if phase_end else "")
                    + "</p>"
                )
        else:
            out += '<p style="color:#64748b;font-size:12px;margin:0;">No weight data recorded recently.</p>'

        out += "</div><!-- /S:weight_phase -->"
    except Exception as _e:
        out += _section_error_html("Weight Phase", _e)
    ...
    try:
        ...
        out += _section_error_html("Guidance", _e)
...
def _brief_journal_coaches(
    ...
    try:
        ...
        if journal:
            ...
            for j_label, j_val in [("Mood", mood), ("Energy", energy), ("Stress", stress)]:
                if j_val is not None:
                    ...
                    out += (
                        '<div style="text-align:center;">'
                        '<p style="color:' + j_color + ';font-size:20px;font-weight:700;margin:0;">' + str(round(j_val, 1)) + "/5</p>"
        ...
        out += _section_error_html("Journal Pulse", _e)
    ...
    try:
        ...
        if journal_coach_text:
            ...
            if reflection:
                out += (
                    '<p style="color:#e2e8f0;font-size:13px;line-height:1.6;margin:0 0 8px;font-style:italic;">' '"' + reflection + '"</p>'
                )
            if tactic:
                out += (
                    '<div style="background:#1e293b;border-left:3px solid #8b5cf6;border-radius:0 8px 8px 0;'
                    'padding:8px 12px;">'
    ...
    try:
        ...
        out += _section_error_html("Sleep Coach V2", _e)

    # --- Nutrition Coach V2 (Coach Intelligence Pipeline) ---
    try:
        out += "<!-- S:nutrition_coach_v2 -->"
        ...
        out += _section_error_html("Nutrition Coach V2", _e)

    # --- Training Coach V2 (Coach Intelligence Pipeline) ---
    try:
        out += "<!-- S:training_coach_v2 -->"
        ...
        out += _section_error_html("Training Coach V2", _e)
    ...
    for _sec_id, _sec_text, _sec_title, _sec_color in _v2_coaches:
        try:
            ...
            if _sec_text:
                ...
                out += (
                    f'<div style="background:#1e293b;padding:20px 24px;border-bottom:1px solid #2d2d5e;">'
                    ...
                    f'padding:12px 16px;">' + _body + "</div></div>"
                )
            out += f"<!-- /S:{_sec_id} -->"
        except Exception as _e:
            out += _section_error_html(_sec_id, _e)
    ...
    try:
        ...
        out += _section_error_html("Board of Directors", _e)
    ...
    try:
        if weekly_habit_review:
            out += _render_weekly_habit_review(weekly_habit_review)
    except Exception as _whr_e:
        out += _section_error_html("Weekly Habit Review", _whr_e)
...
def _brief_footer(compute_age_msg, compute_stale, data, date_str):
    ...
    if compute_stale:
        out += '<div style="background:#fffbeb;padding:6px 24px;border-top:1px solid #fde68a;margin-top:8px;">'
        out += '<p style="color:#92400e;font-size:9px;margin:0;text-align:center;">'
        out += (
            "&#9888;&#65039; Compute data "
            + (compute_age_msg or "unavailable")
...
def build_html(
    ...
    html += _brief_lifestyle(data, profile, tldr_guidance)
    html += _brief_journal_coaches(
        bod_insight,
        data,
        explorer_coach_v2_text,
        glucose_coach_v2_text,
