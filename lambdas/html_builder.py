...
def build_html(
    ...
    if tldr_text:
        html += '<p style="color:#cbd5e1;font-size:13px;margin:4px 0 0;font-style:italic;">' + tldr_text + "</p>"
        ...
        if registry:
            ...
            for sg, misses in synergy_misses.items():
                ...
                if len(misses) >= total * 0.5 and total >= 3:
                    html += '<p style="color:#f59e0b;font-size:10px;margin:6px 0 0;">' "⚠ Synergy alert: " + sg + " stack mostly missed</p>"
        elif h_map:
    ...
    try:
        ...
        if bp_data:
            ...
            if readings > 1:
                html += '<div><p style="color:#475569;font-size:11px;margin:0;">' + str(readings) + " readings avg</p></div>"
            html += "</div></div>"
        html += "<!-- /S:blood_pressure -->"
    except Exception as _e:
    ...
    try:
        ...
        if latest_weight:
            ...
            if week_ago_weight:
                ...
                html += '<p style="color:' + delta_color + ';font-size:14px;font-weight:600;margin:0;">' + delta_str + " lbs vs 7d ago</p>"
            html += "</div>"
        ...
        if anomaly and anomaly.get("has_anomalies"):
            ...
            if alerts:
                ...
                for alert in alerts[:3]:
                    ...
                    html += '<p style="color:#fca5a5;font-size:12px;margin:0 0 4px;">' "<strong>" + metric + ":</strong> " + msg + "</p>"
                html += "</div>"
        html += "<!-- /S:anomaly -->"
    except Exception as _e:
