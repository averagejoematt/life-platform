...
# ── E. DETAIL — lives in recap_detail.py (the 1000-line ceiling, #1665) ───────
#: The second card's public names resolve from `web.recap_detail` lazily, so the two
#: modules can import each other's chrome without a circular import at load time.
_DETAIL_EXPORTS = {
    "detail_plan",
    "detail_bands",
    "detail_caption",
    "DETAIL_MIN_BANDS",
    "DETAIL_SLOTS",
    "DETAIL_EXERCISE_ROWS",
    "DETAIL_ABSENCE_NOTES",
}


def __getattr__(name: str):
    if name in _DETAIL_EXPORTS:
        from web import recap_detail

        return getattr(recap_detail, name)
    raise AttributeError(name)


def detail(facts, *, date_label: str, weight_series=None, grade_series=None):
    """The second card of the day — see `recap_detail.detail`."""
    from web import recap_detail

    return recap_detail.detail(facts, date_label=date_label, weight_series=weight_series, grade_series=grade_series)
