...
# ceiling forced the move (#1665/#2610) and the as-of layer paid for it. The names
# are re-exported here VERBATIM because the canary imports `_ask_fetch_context`
# from this module and the behavior suite drives `ai._ask_fetch_context` /
# patches `ai.table` (which the sibling resolves through this module at call time).
from web.site_api_ai_context import (  # noqa: F401
    _ask_fetch_computed_reads,
    _ask_fetch_context,
    _board_facts_block,
    _latest_item,
    _soft_block,
    age_annotation as _age_annotation,
)
...
CURRENT DATA:
  Weight: {ctx.get('weight_lbs', '?')} lbs{_w_ann} (started {ctx.get('start_weight', EXPERIMENT_BASELINE_WEIGHT_LBS)}, goal {ctx.get('goal_weight', 185)})
  HRV: {ctx.get('hrv_ms', '?')} ms{_v_ann}
  RHR: {ctx.get('rhr_bpm', '?')} bpm{_v_ann}
  Whoop recovery score: {ctx.get('recovery_pct', '?')}%{_v_ann} (whole-body readiness from HRV/RHR/sleep — NOT a pillar score, and NOT specific to any pillar)
  Sleep: {ctx.get('sleep_hours', '?')} hours{_v_ann}
...
- A metric line carrying "(as of <date>)" is from THAT day, not today — say so when you cite it ("his last weigh-in, August 11th, was…"), and NEVER present a reading marked STALE as current.
