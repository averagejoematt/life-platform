...
def lambda_handler(event, context):
    try:
        ...
        resp = table.query(
            KeyConditionExpression=Key("pk").eq(journal_pk) & Key("sk").between(f"DATE#{start_date}#journal", f"DATE#{today}#journal#~"),
        )
        ...
        for entry in entries:
            ...
            existing = table.get_item(Key={"pk": CACHE_PK, "sk": f"DATE#{date_str}"}).get("Item")
            if existing:
                skipped_existing += 1
                continue
