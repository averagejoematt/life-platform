...
def build_data_inventory() -> dict:
    ...
    for label, partition in _INVENTORY_SOURCES:
        ...
        try:
            ...
            base = {"KeyConditionExpression": Key("pk").eq(pk) & Key("sk").between(f"DATE#{d90}", f"DATE#{today}~")}
            if partition == "whoop":
                resp = table.query(**with_phase_filter(dict(base, ProjectionExpression="sk"), include_pilot=cross_phase))
                count = sum(1 for i in resp.get("Items", []) if is_day_row(i))
            else:
                resp = table.query(**with_phase_filter(dict(base, Select="COUNT"), include_pilot=cross_phase))
