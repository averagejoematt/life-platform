...
class Check:
    ...
    def warn(self, msg="", chronic=False):
        ...
        drift branch — the receipt is a snapshot and the engine keeps moving
        on a platform that redeploys routinely, so "changed since a receipt
        was written" is the norm, not a rare deliberate-change event;
        measured 8 of 8 consecutive nightly runs), and (b) known-recurring
