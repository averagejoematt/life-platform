...
EXPECTED_HANDLERS["partner-weekly-email"]="partner_email_lambda.lambda_handler"
...
aws events list-rules \
    ...
    --query 'Rules[].{Name:Name,State:State,Schedule:ScheduleExpression}' \
    --output table --no-cli-pager 2>/dev/null | \
    grep -E "daily-brief|weekly-digest|monthly-digest|nutrition|chronicle|weekly-plate|monday|partner|RULE_NAME|──" || \
