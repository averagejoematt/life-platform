...
## Native dependencies (Garmin, Pillow, lameenc)

Third-party packages with compiled parts ship as **dependency layers** (`garth-layer`,
`pillow-layer`, `lameenc-layer` — ARNs pinned in `cdk/stacks/constants.py`), never inside
the code bundle. Those layers are the platform's entire third-party surface: the Lambda
*source* tree is stdlib-only.

**Use `deploy/build_lambda_layer.py` — do not hand-roll the pip line (#2099).** Until #2099
there was no build tooling for pillow/garth at all, which is how `pillow-layer:1` accrued
25 live CVEs with no fix path: editing `lambdas/requirements/*.txt` deploys nothing
(#1778/#1780), so Dependabot's PRs were structurally inert.

```bash
python3 deploy/build_lambda_layer.py --list            # registry: pins, versions, consumers
python3 deploy/build_lambda_layer.py --check-manifest  # offline drift gate
python3 deploy/build_lambda_layer.py build pillow      # deterministic zip + build record
```

The script pins the target versions, cross-installs Linux wheels for the right
runtime/arch (`python3.12` / `x86_64`, read from the live functions), prints every
distribution it packaged, and writes a byte-reproducible zip. It **never publishes** — the
publish + `cdk deploy` is an owner step, printed at the end of each build.

Two traps it exists to remove:

- **Platform tags rot.** The old snippet here hardcoded `manylinux2014_x86_64`. Pillow 12.x
  no longer publishes a manylinux2014 wheel (it moved to `manylinux_2_27/_2_28`), so that
  line resolves *nothing* for the CVE-clean Pillow. Each spec now carries a tag list.
- **The manifest is derived, not authored.** `lambdas/requirements/{pillow,garmin,lameenc}.txt`
  are generated by `render_manifest()` from the recorded live-layer contents; hand-edit them
  and `tests/test_layer_build_manifest.py` goes red. After a publish + `cdk deploy`, run
  `--promote <layer> --from-build <build.json> --layer-version <N>` to re-derive them.

The manifests list the **full transitive closure**, not just the top-level pins: before
#2099 `garmin.txt` pinned 2 of the 14 packages in `garth-layer:2`, so pip-audit never saw
the `idna` / `urllib3` advisories sitting inside the deployed layer.
