...
PLATFORM_FACTS = {
    ...
    "secret_count": 26,  # live-verified 2026-08-09 via `aws secretsmanager list-secrets` (not auto-discovered — update after secret add/delete)
    ...
    "alarm_count": 92,  # fallback: auto-discovered from cdk/stacks/*.py when parseable (#795, _auto_discover_alarm_count); 113→65 on #790 (ADR-116); 65→67 on #809 (site-api-ai-errors + recursive-loop adopted into CDK); 67→69 on #1229 (alert-digest Errors + queue-age alarms); 69→71 on #1328 (serve-stack Throttles alarms); 71→77 refreshed to discovery on #1455 (drift >5; includes compute-outputs-missing + compute-outputs-heartbeat); 77→86 refreshed to discovery on #1960 (drift >5; includes the 5 per-source ingest-auth-unhealthy-* alarms); 86→92 refreshed to discovery on the 2026-08-03 merge wave (whoop auth alarm #2068, canary stored-state alarms #2065, kill-switch skip alarms #2067)
