...
PLATFORM_FACTS = {
    ...
    "alarm_count": 129,  # fallback: auto-discovered from cdk/stacks/*.py when parseable (#795, _auto_discover_alarm_count); 113→65 on #790 (ADR-116); 65→67 on #809 (site-api-ai-errors + recursive-loop adopted into CDK); 67→69 on #1229 (alert-digest Errors + queue-age alarms); 69→71 on #1328 (serve-stack Throttles alarms); 71→77 refreshed to discovery on #1455 (drift >5; includes compute-outputs-missing + compute-outputs-heartbeat); 77→86 refreshed to discovery on #1960 (drift >5; includes the 5 per-source ingest-auth-unhealthy-* alarms); 86→92 refreshed to discovery on the 2026-08-03 merge wave (whoop auth alarm #2068, canary stored-state alarms #2065, kill-switch skip alarms #2067); 92→99 refreshed to discovery on the 2026-08-11 permanence merge (#1400/PR #2572 — the continuity-watch + public-archive alarms landed by the LifePlatformOperational cdk deploy); 99→107 refreshed to discovery on the 2026-08-20 us-east-1 reconcile (#2829/PR #2913 — dash-5xx-rate, dash-total-errors, cf-auth-errors declared in cdk/stacks/web_alarms.py; +3 crossed the ±5 fallback-hygiene tolerance and redded main, the class this test's docstring warns about); 107→113 refreshed to discovery on #3037 (the #2977 recall silence alarms + approve-path pair crossed the ±5 tolerance); 113→120 refreshed to discovery on #3563 (the two fail-soft write-denial silence alarms chronicle-status-write-failed + freshness-sentinel-write-failed took the drift from exactly 5 to 7 — the ±5 tolerance is a fallback-hygiene bound, not a per-PR bump, and this is the crossing); 120→127 refreshed to discovery on #3506 (the two dead-men canary-no-invocations-1h + board-verdict-silence-7d took the drift to 7 — main was already sitting at EXACTLY the ±5 bound, so any alarm-adding PR crosses it, and this is that PR); 127→129 refreshed to discovery on 2026-09-21 (two new alarms from recent PRs; main drift detection #809 refresh cadence)
    ...
    for doc, pattern, replacement_template in RULES:
        ...
        if new != current:
            ...
            if old_match:
                ...
                # #3477: ONLY for a rule that actually STAMPS freshness — applied to every
                # rule it swallowed semantic date FACTS too. See tests/test_semantic_date_not_a_freshness_stamp_3477.py
