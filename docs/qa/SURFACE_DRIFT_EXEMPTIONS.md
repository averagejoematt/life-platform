...
- 2026-08-07 | route | /api/membrane | #1679 bidirectional membrane dashboard (epic #1668 S11) — new read-only route ships in the same PR with a dated _exemptions.json capture-failed entry (route not deployed yet, no live shape to snapshot); post-deploy the driver runs deploy/capture_api_schemas.py, commits the real baseline, and drops the JSON exemption
- 2026-08-06 | route | /api/social_context | #1674 contextual social embeds — new route ships in the same PR with a dated _exemptions.json capture-failed entry (route not deployed yet, no live shape to snapshot); post-deploy the driver runs deploy/capture_api_schemas.py, commits the real baseline, and drops the JSON exemption
