...
- 2026-08-02 | route | /api/content_cadence | #1972 content cadence — same landing-order constraint as the #1404 entry: capture is wholesale-only against live and the deploy queue is stranded (merge-only session); ships with a dated `_exemptions.json` capture-failed entry; post-deploy run deploy/capture_api_schemas.py, commit the baseline, drop the JSON exemption
