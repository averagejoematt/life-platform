...
7 email Lambdas (daily-brief at 17:00 UTC (10 AM PDT), weekly digests, chronicle, etc.)
  ...
  + site-stats-refresh (writes public_stats.json)
    │
    ▼
MCP Lambda (72 tools) ← Claude Desktop + claude.ai + mobile via remote MCP
site-api Lambda (~129 endpoints, primarily read-only — ADR-037) ← averagejoematt.com
```
...
| **MCP Lambda** | 72 tools across 25 domain modules in `mcp/` | The interface Claude uses to query data |
...
| Term | Meaning |
|------|---------|
| **MCP** | Model Context Protocol — Claude's native tool interface. The MCP Lambda exposes 72 tools that Claude calls to query health data. |
