...
**Schedule:** Sunday 10:00 AM PT (`cron(0 17 ? * SUN *)` UTC) — after weekly-digest (Sun 9am) and hypothesis-engine (Sun 12pm)
**Memory:** 512 MB
**Timeout:** 120s
...
def lambda_handler(event, context):
    ...
    raw = call_anthropic(prompt, api_key, max_tokens=600, system=system)
    
    # 6. Parse three paragraphs
    ...
    # 7. Infer ai_tone from paragraph content
    ai_tone = infer_tone(ai_present, ai_lookback, ai_focus)
...
**`infer_tone()` — simple keyword heuristic:**
```python
def infer_tone(present, lookback, focus):
    combined = f"{present} {lookback} {focus}".lower()
    urgent_words = ["concerning", "significant drop", "needs immediate", "warning", "critical"]
    affirming_words = ["strong week", "consistent", "above baseline", "trend is positive", "improvement"]
    cautionary_words = ["slipping", "missed", "below", "deficit", "trend suggests"]
    if any(w in combined for w in urgent_words): return "urgent"
    if any(w in combined for w in cautionary_words): return "cautionary"
    if any(w in combined for w in affirming_words): return "affirming"
    return "mixed"
```
...
Add both entries before the closing `}` on line 2647. Current tool count: 112. After: 114. `EXPECTED_MAX_TOOLS = 115` — still within bounds. **No change to test file needed.**
...
**Pattern:** Follow `handle_achievements()` at line 2876 exactly.
...
#### Step 2: Register route in ROUTES dict (line 5541+)
...
Note: this route uses a query parameter (`?week=`). It needs to be called with `event` passed through. Check that the `lambda_handler` passes `event` to handlers that need it — follow the pattern for `/api/correlations` (line 2076) which also accepts `event`.
...
| `lambdas/site_api_lambda.py` | Add `handle_field_notes()`, register `/api/field_notes` route | 2 |
...
|---|---|---|---|
| MCP tool function pattern | `mcp/tools_decisions.py` | `tool_log_decision` | Follow exactly |
| MCP TOOLS dict entry | `mcp/registry.py` | Lines 2024–2050 | Same structure |
| TOOLS closing brace | `mcp/registry.py` | Line 2647 | Add entries before this |
| tools_lifestyle append point | `mcp/tools_lifestyle.py` | After `tool_get_ruck_log` | Last function |
| ISO week update_item pattern | `mcp/tools_decisions.py` | `tool_update_decision_outcome` | update_item pattern |
| `call_anthropic` signature | `lambdas/ai_calls.py` | Line 1095 | `(prompt, api_key, max_tokens, system)` |
| Secrets fetch pattern | `lambdas/weekly_digest_lambda.py` | Top of lambda_handler | `life-platform/ai-keys` |
| Batch DDB query pattern | `lambdas/weekly_digest_lambda.py` | Data gather functions | `sk BETWEEN` pattern |
| site_api handler pattern | `lambdas/site_api_lambda.py` | `handle_achievements()` line 2876 | |
| Event/query param pattern | `lambdas/site_api_lambda.py` | `handle_correlations()` line 2076 | Takes `event` param |
| ROUTES dict | `lambdas/site_api_lambda.py` | Line 5541 | Add `/api/field_notes` |
| `_ok()` / `_error()` | `lambdas/site_api_lambda.py` | Lines 274/292 | |
| `USER_PREFIX` constant | `lambdas/site_api_lambda.py` | Line 73 | |
| Chronicle page shell | `site/chronicle/index.html` | Full file | Copy page structure |
| Story nav section | `site/assets/js/components.js` | Line 29 | Insert Field Notes here |
| Story footer section | `site/assets/js/components.js` | Line 186 | Insert Field Notes here |
...
- BL-04: Field Notes — weekly AI-vs-Matthew lab notebook
  - `field-notes-generate` Lambda — Sunday 10am PT, Sonnet synthesis across all domains
  - `USER#matthew#SOURCE#field_notes` DynamoDB partition
  - `get_field_notes` + `log_field_note_response` MCP tools
  - `GET /api/field_notes` endpoint (list + entry modes)
...
All three tests must pass. Then deploy the MCP Lambda using the exact zip commands 
...
- [ ] Confirm EventBridge schedule: Sunday 10am PT after hypothesis-engine is acceptable
