...
Adding 1 tool → 113 total. Still within bounds. **No change needed to the test file.**
...
**Pattern reference:** Follow the exact same structure as `handle_achievements()` at line 2876.
...
The `ROUTES` dict starts at line 5541. Add the new route in the "Observatory pages" section near the bottom:
...
|---|---|---|---|
| MCP tool function structure | `mcp/tools_decisions.py` | `tool_log_decision` at top | Exact pattern to follow |
| MCP TOOLS dict entry structure | `mcp/registry.py` | Lines 2024–2050 | `log_decision` entry |
| TOOLS dict closing brace | `mcp/registry.py` | Line 2647 | Add new entry before `}` |
| tools_lifestyle.py end | `mcp/tools_lifestyle.py` | After `tool_get_ruck_log` | Append new tool here |
| `from tools_lifestyle import *` | `mcp/registry.py` | Line 33 | Already covers new tools in that file |
| site_api handler structure | `lambdas/site_api_lambda.py` | `handle_achievements()` line 2876 | Closest structural match |
| ROUTES dict | `lambdas/site_api_lambda.py` | Line 5541 | Add `/api/ledger` entry here |
| `_ok()` helper | `lambdas/site_api_lambda.py` | Line 274 | `_ok(data, cache_seconds=N)` |
| `USER_PREFIX` constant | `lambdas/site_api_lambda.py` | Line 73 | `f"USER#{USER_ID}#SOURCE#"` |
| `_decimal_to_float()` | `lambdas/site_api_lambda.py` | Search for `def _decimal_to_float` | Use on all DDB responses |
| S3 config read pattern | `lambdas/site_api_lambda.py` | Line 3330 | `experiment_library.json` load pattern |
| Footer Internal links array | `site/assets/js/components.js` | Line 232 | Add Snake Fund to this array |
...
When the session ends, bump to **v4.6.0** and add to `CHANGELOG.md`:

```markdown
## v4.6.0 — 2026-MM-DD
### Added
- BL-03: The Ledger / Snake Fund — achievement-linked charitable giving
  - `log_ledger_entry` MCP tool (mcp/tools_lifestyle.py)
...
from the spec — Matthew will update the real names), add tool_log_ledger_entry to 
mcp/tools_lifestyle.py following the log_decision pattern in mcp/tools_decisions.py, 
register it in mcp/registry.py before the closing brace, and run:

  python3 -m pytest tests/test_mcp_registry.py -v

All three tests (R1, R2, R3) must pass before deploying. Then deploy the MCP Lambda 
