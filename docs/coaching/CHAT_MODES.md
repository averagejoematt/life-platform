...
| **Vlog / video diary** | `.claude/commands/vlog.md` | Claude as live interviewer for the Insta360 Luna video diaries (#1571, epic #1564). Zero-setup open (context pulled, format proposed, question one in the first response), six-format library, route-the-takeaways close → Notion `Video Diary` template (`channel: video_diary` stamped by ingestion, #1572). Studio kit (Project prompt + room setups) in private S3 `config/studio/`. | Whenever the Luna comes out |
