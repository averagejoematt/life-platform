...
> **Status:** OPEN — DI-1.1 diagnosed (read-only) 2026-06-19; **DI-1.2 + DI-1.3 + DI-1.1 source-state + DI-1.4 built + tested (not deployed) 2026-06-19; DI-1.5 ADR-091 (cross-coach honesty-guard standard) written**; Dr. Chen thread correction out-of-band (Matthew). DI-1.6 (HAE failsafe — needs Matthew's Garmin→Apple/HAE config check, **+ the active_calories + steps-undercount HAE feed gaps logged in DI-1.6**) pending. (DI-1.1's remaining step is the Strava re-enable CDK deploy — Matthew.) **DI-1.1 Strava decision RESOLVED 2026-06-19: Matthew subscribed to Strava → re-enable ingestion (see DI-1.1).** **DI-1.6 (Apple HAE failsafe) added.** GARM-1 (official Garmin API) deferred — see Deferred decisions.
...
**Chain:** Garmin watch → Garmin Connect → Apple Health (workout sync) → Health Auto Export → platform ingestion. Verify these config preconditions before coding: (a) Garmin Connect → Apple Health **workout** sync is on (not just steps); (b) HAE is set to export **workouts/activities** (distance, duration, HR), not only the gapped step field (DI-1.4); (c) **the HAE export-config field gaps below are real and must be closed first — a failsafe can't be trusted while its own feed is silently incomplete.**

> **HAE feed gaps found 2026-06-19 (verify against the export config — same surface as (a)/(b)):**
> - **`active_calories` entirely missing.** `apple_health.active_calories` is `None` on **every day 6/3–6/19** in DDB. The movement tool's NEAT estimate + the `<200 active cal` sedentary clause silently degrade. Check that HAE exports **Active Energy**.
> - **Steps undercounted on watch-without-phone days** (RCA 2026-06-19). DDB 6/15 = 402 / 6/18 = 444 vs the Apple Health app's ~6,500 / ~7,800. Two compounding code causes in `health_auto_export_lambda.py` — (1) `pick_source_or_all` keeps a **single** highest-priority source for the whole day and discards the rest (the code comment at L324 admits the watch-without-phone loss); (2) **no MAX/accumulate guard on the steps write** (unlike water/caffeine's `_rd_` map), so a partial export plain-overwrites. **Possible third cause = config:** HAE may not export the **Watch** step stream at all on those days (confirm in the HAE export config). **Code fix (separate item, needs a historical backfill):** for additive activity metrics use **MAX across per-source daily sums** (kills double-count *and* undercount) + **GREATEST(stored,new)** on write; then re-derive history from the raw `raw/matthew/health_auto_export/` payloads. This undercount is why DI-1.4's "low step days" looked phone-only — they were partly an ingestion artifact.
