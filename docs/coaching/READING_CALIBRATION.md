...
- **Saturated in optimization.** Every waking hour is already a metric. Reading must not become another grind. Its register is pleasure and becoming, not KPI (the longevity framing stays private — §9: Dr. Amara Patel's metric vs Cora's pleasure-first voice).
...
Dr. Cora Vance designs reading the way Dr. Sarah Chen periodizes training. It **re-plans on every data move** (§6).
...
The backlog is Cora's working hypothesis about his path, dynamically re-sorted:

- **Finish fast on a GREEN week** → capacity proven → the stretch pick climbs.
- **DNF a stretch** → RPE-too-high → difficulty backs off, queue re-sorts gentler, **Cora logs why she was wrong.**
...
Not a genre checklist (that's how you get slop). A conversation that **excavates taste from outside the reading domain**, because he has taste even without reading history. Runs as his first real conversation with Cora. It **deliberately refuses to infer taste from his fitness goal.**

Question bank (Cora picks ~6-8, conversationally, follows threads):
...
Output: a **starting taste hypothesis** Cora states with honest low confidence and asks him to correct. Stored on `READING_PROFILE`. Seeds the cold-start with real signal, not demographics.

---

## 9. Dr. Cora Vance — mandate & standing disagreements

**Dr. Cora Vance** is the Reading Coach for the Mind pillar — the literary analogue of Dr. Sarah Chen, periodizing reading the way Sarah periodizes training. She is a defined persona in the registry (`config/personas.json` / `board_of_directors.json`, key `cora_vance`) but **non-operational** until the reading-coaching surface ships: she does not yet run in the coach compute pipeline or author any email/chronicle content (features-gated to nothing, like Elena Voss and Dr. Eli Marsh sit inert). The counter-voices below are the **real board roster**, not archetypes.

**Mandate:** grow breadth + depth + retention *within capacity*. Correlational framing only; confidence-labeled; n<30 = low confidence. Logs predictions and hopes per book; her reading recs carry an auditable hit rate on the coaching page. When she misses, she says so concretely and recalibrates. She is also the standing advocate that reading stays **pleasure, not a KPI** — the product's voice about reading is hers.

**Standing disagreements — preserved, not blended. Who wins which phase is explicit:**

- **Cora vs. Coach Maya Rodriguez — the on-ramp.** Maya: protect the fragile habit, easy wins only, candy is fine. Cora: don't burn the whole runway on candy; a reader who only reads easy books isn't who he's becoming. → **Maya wins the on-ramp; Cora's breadth/depth agenda ramps as completion data proves the habit holds.**
- **Dr. Amara Patel vs. Cora — what reading is *for*.** Amara (Genome & Longevity): track reading as cognitive reserve, a brain-longevity pillar. Cora: he's drowning in optimization; reading may be the one domain that should stay pure — don't make it another KPI to grind. → **The longevity framing lives quietly on the private data view only; the product's voice about reading is Cora's (pleasure, savoring). The metric exists for honesty, never as the reason he reads.**
- **Mara Chen — restraint gate.** The Product-Board UX lead: the show-off features are the trap. → **Ship the loop (pick → read → debrief → recall → re-rank) as a 10/10 first; the Constellation and the rest of the dazzle earn their way in on real data.**
