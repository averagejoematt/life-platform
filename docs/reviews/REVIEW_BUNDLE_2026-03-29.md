...
### Infrastructure (14)
`life-platform-freshness-checker` · `dropbox-poll` · `insight-email-parser` · `life-platform-key-rotator` · `dashboard-refresh` · `life-platform-data-export` · `life-platform-qa-smoke` · `life-platform-mcp` · `life-platform-mcp-warmer` · `dlq-consumer` · `life-platform-canary` · `data-reconciliation` · `pip-audit` · `partner-weekly-email`
...
7. **One-off deploy scripts bypassing canonical tooling** — 2 incidents (Mar 11 Partner email inline zip, **Mar 16 S3 bucket wipe**)
...
**Why it matters:** CLI-created Lambdas don't get CDK's IAM role management, EventBridge rule management, alarm creation, or layer attachment. They drift silently. The Mar 12 Todoist incident (CDK drift) and the Mar 11 Partner email incident (stale layer) were both caused by this pattern.
