...
| Security | trusted edge identity · WAF · no open critical/high findings · enabled alerts · least privilege · credential hygiene · pentest evidence | **8.0** | ✅ live: CodeQL 0 open (after this session's 6-alert re-triage), vulnerability alerts 204-enabled, edge identity fail-closed + AST-guarded + wire-proved, per-Lambda least-privilege roles, SECRETS_MAP rotation register. ❌ WAF deliberately absent (priced, compensating stack live — defensible); **no third-party penetration evidence at all** (§12 names it); **#3278 OPEN: security-log retention is 30d in two regions and NEVER-EXPIRE in five while docs say 90d**, cf-auth not CDK-owned; CodeQL regrowth sat untriaged 3–4 days — inside `drift_sentinel.check_codeql_alerts`' WEEKLY window (the dead-man exists, fails closed, and last reported 08-24 when 0-open was true; the exposure is its 7-day cadence against a claimed steady-state-0, not a missing instrument). A 9 here would be manufactured. |
...
5. **CodeQL triage cadence.** Steady-state-0 regrew to 6 open highs within 3 days of
   #3047 closing. The dead-man exists (`check_codeql_alerts`, weekly, fail-closed —
   verified: its last run, 08-24, reported 0-open when that was true) and would have
   flagged at its next window; the gap is that a WEEKLY assertion backs a
   ZERO-AT-ALL-TIMES claim, so up to 7 days of regrowth is structurally silent.
   Either tighten the cadence or restate the claim as "triaged weekly".
