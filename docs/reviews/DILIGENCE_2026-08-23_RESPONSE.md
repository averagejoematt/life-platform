...
| 020 security tests pass against a false production model | **CONFIRMED (row added 2026-08-29 — previously only named in the header fold line)** | The report's thesis proved live repeatedly after filing: Session F (08-27) found five green instruments measuring nothing; the census itself had counted ten libraries as gates on a filename substring. Remediation is the #2578 program: 565 gates inventoried by derivation, `PROVEN_CAN_FAIL` mutation registry + re-runnable harness (`scripts/gate_census_mutations.py --run` — plant the defect, watch the red, revert), the sentinel two-half bar (detect AND cannot-observe must both prove), fixture-must-be-the-wire discipline | Owned by #2578 — **CLOSED 2026-08-31 on its amended Outcome** (Session R update 2026-09-01): 588 declared gates derived from source; every gate added after 2026-08-24 entered proven under the #3000 pre-merge lane; the unproven ceiling is DOWN-ONLY (531 ≤ 541 at close); six structurally-unfailable gates carry recorded not-applicable reasons. Standing machinery: `scripts/gate_census.py` + `tests/test_gate_census_lane_3000.py` (pre-merge ratchet) + `scripts/gate_census_mutations.py` + the PROPORTIONALITY row with its demote trigger. Disposition: CONFIRMED → remediated-by-program; the recorded honest residual (installed base ~8.2% proven) is bounded by the shrink-only ratchet, and the original all-gates sentence was ruled unachievable-by-design on #3329 |
| 022 shared-code change ⇒ fleet-wide redeploy (~104 fns) | **CONFIRMED → PRICED (row added 2026-08-29 — the id was missing from this register entirely)** | True by construction: the #781 one-bundle rule stages the whole `lambdas/` tree into every function, so a `common/` edit redeploys the fleet. This is the RECORDED trade (docs/CONVENTIONS.md §1): it bought the death of the layer-skew class (functions running different shared-code versions), which had produced real incidents; fleet deploys are scripted (`build_bundle.py` + postflight content-greps, never sha-trust) | PRICED: revisit trigger = bundle size or cold-start cost crossing the budget close's line, a per-function isolation need, or commercialization. Until then one artifact = one truth is the cheaper defect class. **UPDATE 2026-09-01:** the priced-table row this disposition claims now exists below (it was missing — unbacked by the register's own bar); the PROPORTIONALITY carrier is the Deploy-guardrails row (one-bundle rule) |
...
| 023 heartbeat silent-fail | **STALE-corrected** | #1196 fixed; `test_heartbeat_completeness.py` + live liveness gauge | live assert: `diligence_verify.py::public_data_is_fresh` (DIL-023/024) — 12-PASS run 2026-09-01 |
...
| 029 stale-cycle grounding | **STALE-corrected 2026-09-01** | closed coach-correction epic; standing assurance = D4 | D4 SHIPPED: #3050 CLOSED 2026-08-25 — the standing AI-safety eval matrix carries the temporal/grounding golden families this finding names (DIL-029 is in #3050's own title). Disposition: STALE-corrected with standing assurance |
| 030 stored-text injection | **STALE-corrected 2026-09-01** | #811 closed (2026-07-07, stored-injection delimiting on board_ask); expansion coverage = D4 | D4 SHIPPED: #3050 CLOSED 2026-08-25 — the injection golden family in the standing eval matrix (DIL-030 in #3050's title). Disposition: STALE-corrected with standing assurance |
| 031 clinical operating model | **COMMERCIAL GAP** | no clinician-reviewed hazard register | D4 clinical-LITE build + PRICED (full) |
| 032 prediction grading/calibration | **CONFIRMED → FIXED** | folds into DIL-007 diagnosis | D0.4 SHIPPED: #3046 CLOSED 2026-08-23 (gradeability + scorecard honesty); DIL-032 is in `diligence_verify.py`'s live-asserted set (`calibration_reported_honestly`, 12-PASS 2026-09-01). Disposition: CONFIRMED-fixed |
...
| 038 complexity vs use | **PARTIALLY** | gate count 425-vs-490 CONFIRMED (#3000); MCP "11 of 133" WRONG (31 of 143 served; live 76) | #3000 CLOSED 2026-08-24 (the census runs in a pre-merge lane; the PROPORTIONALITY count is derived, not hand-stated) · #2578 CLOSED 2026-08-31 (amended Outcome; down-only ratchet standing). Disposition 2026-09-01: CONFIRMED-fixed on the count class; the MCP claim half stays WRONG-evidenced (31 of 143 served at filing, live 76) |
| 039/040 outcome + IA | **CONFIRMED** | product; deprioritized | PRICED / D-later |
| 041 API contract baselines | **STALE-corrected 2026-09-01** | #1436 closed 2026-07-19 | D5 contract check LIVE: `tests/api_schemas/` endpoint snapshots + `tests/test_api_schema_completeness.py`, pre-merge on every PR since #3025 put the full unit suite there. Disposition: STALE-corrected |
...
| 050 deletion propagation map | **PARTIALLY → SPLIT-disposed 2026-09-01** | archives well-documented; propagation machinery absent | Subscriber half CONFIRMED-fixed: #3044 CLOSED (deletion promise + retention behavior + tokenless unsubscribe). Owner-data propagation machinery: PRICED-accepted 2026-09-01 — dated row in the priced table below |
| 051 accessibility coverage | **STALE-corrected (automated half) · PRICED (manual audit) 2026-09-01** | WCAG audit due | Automated coverage standing: `tests/a11y_audit.py` (axe) inside the gating visual-QA lane + `test_a11y_light_theme.py` + `test_site_a11y_landmarks.py`; the color-mix contrast blind spot is a recorded discipline (prove by arithmetic; read CI's report.json). The full manual WCAG audit remains not performed: PRICED-accepted — dated row in the priced table below |
...
| No independent evidence coaching changes behavior; feature surface exceeds visible evidence | 039/040 | first graded-prediction cohort matures (≥10 graded outcomes — earliest window 2026-08-31, so this trigger is MECHANICAL, not aspirational). **TRIGGER MET 2026-08-31 (verified 2026-09-01): 10 outcomes machine-graded the day the window opened (4 confirmed/6 refuted; lifetime 37 graded, 21.6% accuracy, served publicly) — the revisit is now DUE; note the evidence delivered is forecast skill, and the finding's own claim (no independent evidence coaching changes behavior) remains open pending the owner's revisit** · any public claim that coaching caused an outcome · commercialization. *(Row added 2026-08-29 — the Fable re-grade walk found 039/040 marked "PRICED / D-later" with no row here, the exact shape this section calls itself unwilling to ship)* |
...
| Fleet-wide redeploy on any shared-code change (the #781 one-bundle rule) | 022 | bundle size or cold-start cost crossing the budget close's line · a per-function isolation need · commercialization. PROPORTIONALITY carrier: the Deploy-guardrails row (one-bundle rule). *(Row added 2026-09-01 — the register row claimed PRICED since 2026-08-29 with no table row behind it)* |
| Owner-data deletion-propagation machinery absent (subscriber half shipped, #3044) | 050 | an actual erasure obligation · a second user · commercialization · an ADR-155 consent-surface change. *(Row added 2026-09-01)* |
| No manual WCAG audit (automated axe sweep is standing in the gating visual-QA lane) | 051 | measurable reader-audience growth · any accessibility complaint · commercialization. *(Row added 2026-09-01)* |
