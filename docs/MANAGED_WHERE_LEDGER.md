...
| **GitHub `main` branch ruleset** (`main-block-force-push-and-deletion`, id `19162901`) | Blocks non-fast-forward pushes + branch deletion on `main` only — no required checks, no PR rule | GitHub rulesets aren't CDK/IaC-managed (repo-config, not AWS); created directly via `gh api` (#1325) so the reconcile bot's normal pushes and squash-merges stay unaffected | GitHub repo settings (Rules → Rulesets); one-command-ready payload + creation call in PR #1325's body | Manual today — `gh api repos/<owner>/<repo>/rulesets` (see `docs/CONVENTIONS.md`'s drift-discovery table). **No automated assertion wired yet**; #1320 (GitHub-side drift-sentinel leg, epic #1355) is the tracked story to wire this row into a GET-only CI/remediation-agent check the way the AWS rows above are |
