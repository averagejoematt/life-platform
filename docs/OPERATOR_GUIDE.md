...
The Life Platform is a personal health intelligence system. It pulls data from ~20 sources (wearables, apps, food logs, labs), stores everything in DynamoDB single-table (`life-platform`, us-west-2), and makes it queryable by Claude through 68 MCP tools. **96 Lambdas** run the ingest → compute → email pipeline daily.
