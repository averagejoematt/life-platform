...
| `mcp/` | MCP server — 81 tools across `tools_*.py` domain modules, wired in `registry.py`. |
...
| `model/` | The generated system model (#2845) — `platform_model.json`, regenerated by `scripts/generate_platform_model.py`, drift-gated by `tests/test_platform_model_drift.py`, queried via `scripts/blast_radius.py`. |
