...
6. **`live-proof-before-close`** — An INSTRUMENT — an alarm, gate, sweep, judge, ledger, scheduled job or fail-soft write — closes on its first non-degraded LIVE output, never on the merge. Its PR carries `Refs #N`, not `Fixes #N`, and names the output it will be closed on; the closing comment carries `**Live proof:** <UTC instant> — <where>`. `Fixes #N` is for product/config/doc fixes a live curl can prove after deploy. Class 3 of the 2026-09-05 forensic RCA: INT-1 (49 days), G-3 (28), OBS-1 (30+), CPO-2 (21 runs) all read CLOSED while non-functional. *Detector:* `scripts/closure_sweep.py` + `scripts/check_pr_closing_set.py` → `no-live-proof`.
7. **`close-the-shipped`** — A merged commit that names an open issue in its SUBJECT has either closed it or said why not. Detector B catches a PR closing too MUCH; this is the other direction — the fix ships, the keyword is forgotten, and the issue sits open until a human re-reads it. Session AF swept 40 open issues by hand and found 11 already fixed by a merged PR (open 7-17 days); TEN named the issue in the merge subject with no closing keyword (#3812). An instrument held for its live proof (`closure:live-proof`, or `**Closure class:** instrument`) and a `type:epic` are correctly unlinked and are NOT findings. A finding is a question for a human, never a closure. *Detector:* `scripts/check_unlinked_closures.py` → `shipped-unlinked`.
8. **`partial-is-not-a-close`** — A PR body that still carries an unchecked acceptance box (`- [ ]`) does not carry a closing keyword; and a closing keyword closes regardless of negation or tense (`docs/OPERATING_DISCIPLINE.md` §2.1, §2.5 — #2921 closed itself twice by writing "does NOT close #2921"). *Detector:* `scripts/check_pr_closing_set.py` → `partial-acceptance-close`, `negated-closing-keyword`.
9. **`validated-merge-text`** — The closing set of the text that is ACTUALLY COMMITTED is validated, not only the PR body and branch commits. Detector B's three texts are all pre-merge; `gh pr merge --squash --body-file` (or `--subject`, or the web UI's editable squash box) supplies a fourth that no pre-merge guard has seen. On 2026-09-17 that path retired an owner-gated issue because the custom body's note explaining a removed closing phrase QUOTED the phrase. Two legs: guard_bash.py refuses a supplied message on the sanctioned path, and detector D compares the merge commit's closing set against the PR BODY plus GitHub's computed set — deliberately NOT the branch commits, since a commit-only ref disagreeing with the body is the exact warn that was reasoned past. Each finding carries its MEASURED effect (did this commit actually retire that issue), so a keyword written at an already-closed issue is visibly not the same event as a live retirement. *Detector:* `scripts/check_merge_commit_closures.py` + `scripts/hooks/guard_bash.py` → `unvalidated-merge-closure`.
...
The instrument class is derived STRUCTURALLY, never from a title phrase: a `**Closure class:** instrument|product — <reason>` line in the PR body, or the `closure:live-proof` label on the issue (both decisive → block), or AST instrument sites in the PR's changed files — a fail-soft write, an EMF emitter, a CloudWatch alarm, a scheduled job, a `chronic=True` check result — which advise only. Live-proof obligations are going-forward-only from **2026-09-06**, the ADR-099 CONTRACT_SINCE shape.
...
2. **Stranded Plan — the R8-ST6 shape.** The run FAILS, but the only red job is
   ...
   **Since #2834 (2026-08-30, ADR-065 amendment) the gate is structural, not a grep.**
   `deploy/iam_additive_gate.py` reads the TEMPLATE diff (`cdk.out` vs each stack's live
   `Original` template, read-only) and returns per stack `NO-IAM-CHANGE` (Plan proceeds;
   non-IAM CDK drift is a `::warning` naming the stack, never a strand — the old grep's
   CloudFront-"Policy" false positive is gone), `ALLOW-ADDITIVE` (new `Allow` statements on
   life-platform-namespace resources only, nothing else riding along — the Deploy job
   re-evaluates at apply time, runs `cdk deploy <Stack> --app cdk.out --exclusively` from
   the evaluated assembly, proves convergence, and writes
   `s3://matthew-life-platform/remediation-log/iam-additive-gate/YYYY/MM/DD/`), or
   `OWNER-REQUIRED` (THIS stranded shape, now naming every offending statement; input the
   gate cannot parse or fetch is OWNER-REQUIRED too — exit 2, fail closed). So the strand
   only fires on the shapes the owner still deploys by hand: any `Deny` change, a
   trust-policy edit, a new/removed/changed role, a managed-policy attachment or
   permissions boundary, a resource-based policy (Lambda::Permission, bucket/queue/topic
   policy, KMS key policy), a wildcard or out-of-namespace grant, or additive IAM sharing
   its stack with Lambda-config/alarm/queue changes. The registry of the allowed shape is
   data in `deploy/iam_additive_registry.py`, ratcheted against the fenced
   ```json iam-additive-gate-baseline``` block in the ADR-065 amendment
   (`tests/test_iam_additive_gate_guards_2834.py`) — a widening lands where the owner reads.
...
- **A config the runtime reads from S3 is NOT fixed by editing the repo copy (#3671, 2026-09-07).**
  `deploy/build_bundle.py` stages a short, explicit list into `config/` at the bundle root —
  `food_vocabulary.json`, `personas.json`, `config/coaches/*` (see `bundled_extra_paths()`).
  Anything else that a Lambda loads "from `config/`" resolves the S3 fallback instead. Measured:
  a PR hand re-anchored `config/training_phases.json`'s `reset_epoch_date` and merged; the S3
  object still held the old value and the live counter never moved. **Before believing a config
  change landed, read the live object** (`aws s3 cp s3://matthew-life-platform/config/<file> -`).
  And prefer **deleting the second copy to synchronising it**: `reset_epoch_date` had drifted
  through eleven resets because ADR-077's phase taxonomy classifies DynamoDB *partitions* and has
  no jurisdiction over config *files*, so the registry that exists to stop scoped state surviving
  a reset could not see it. Deriving from `EXPERIMENT_START_DATE` — regenerated by every reset,
  shipped in every bundle (#781) — removes the drift rather than scheduling a fix for it.

- **A red `lint` on main is a hold on the whole verification chain, not a style complaint
  (#3679, 2026-09-07).** CI/CD's `test`, `Plan deployments`, `Deploy` and every post-deploy gate
  are SKIPPED behind a failing `lint`, so **a skipped `test` job is not a passing one** — read
  `gh run view <id> --json jobs` and look at the conclusions, not the rollup. Two corollaries
  measured the same day: (a) when two workflows go red together, look for **one** cause before
  assuming two — `deploy/pii_surface_guard.py --tracked` runs in *both* `CI · Lint` and
  `sync_site_to_s3.sh`, so a single tracked-JSON violation reads as two independent failures;
  (b) the session boot brief prints main's CI state on its first line — **read it at session
...
- **`pytest --timeout` is not installed here — the flag errors and a pipe still reports exit 0
  ...
  reports success and the visible output is just pytest's `rootdir:` banner. **Read the
  `N passed` / `N failed` summary line, never the exit code**, whenever the command is piped.
...
| An alarm red >72h is "cited" by an entry nobody has re-checked since it went red — a citation written the morning of the transition about the previous cause reads identical to one written after | Citation AGE predicate (#4034), folded into the alarm-citation gate, step (e10): the citation's `cause_observed`/`added` stamp must POST-DATE the StateTransitionedTimestamp (a bare date must be a later day) | `scripts/alarm_citation_age.py`; hook in `scripts/check_alarm_citations.py::main` |
| A suppressor (an alarm whose ALARM state gates a composite) stuck red withholds its composite's page forever while every sweep excludes it as "red by design" | Suppressor registry (#4034): type + window + end condition + consumed composites (contract-tested against the alarm_rule AST, and every NOT() term must be registered); sweeps exclude via `suppression_holds()` only inside the window, and escalate it past | `cdk/stacks/constants.SUPPRESSOR_REGISTRY`; `tests/test_suppressor_registry_4034.py` |
...
| A security-tier Lambda log group (canary, key-rotator, dlq-consumer, cf-auth, buddy-auth) retains for other than the `docs/DATA_GOVERNANCE.md` 90 days in ANY region — including a Lambda@Edge replica group a newly-served region just created at NEVER_EXPIRE (#3278: five regions sat that way, unpoliced) | Security log-retention declared-vs-live sweep (#3278): declared once in `cdk/stacks/constants.py`, derived by CDK for the regional functions, asserted weekly across every enabled region; the doc row is asserted against the same constant offline | `deploy/sentinel_log_retention.py`; `deploy/apply_log_retention.py --apply`; `tests/test_security_log_retention_3278.py` |
| A producer Lambda silently stops running while its exemption (or the absence of any schedule) means nothing measures it — the heartbeat ledger accepted 51 such deaths | Producer census (#4034), drift-sentinel leg: every Lambda that can write a `deploy/emf_namespace_ledger.py` namespace graded on AWS/Lambda Invocations against its cadence | `deploy/sentinel_producer_census.py`; `tests/test_producer_census_4034.py` |
| A deploy renames a metric, disables an alarm's actions or deletes a heartbeat alarm, and the ledger (which checks NAMES in source) stays green over a blind sensor | Post-deploy sensor self-tests (#4034): the set derived from the heartbeat COVERAGE registry, each doing its real read + a scratch write; a degraded verdict FAILS `deploy/post_cdk_smoke.sh` | `deploy/sensor_self_test.py`; `tests/test_sensor_self_test_4034.py` |
