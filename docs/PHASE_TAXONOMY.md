...
|---|---|---|---|
| **cross_phase** | Never tagged, never wiped | Always fully visible (`include_pilot`) | labs, dexa, genome, **supplements/meds**, **`chronicling`** (frozen pre-platform archive), subscribers, profile, durable platform memories |
| **raw_timeseries** | Never wiped (facts kept) | Current views **genesis-anchored** (date-clamped to `EXPERIMENT_START`) | whoop, withings, strava, garmin, apple_health, eightsleep, habitify, macrofactor, hevy, notion/journal, food_delivery, sick_days, **measurements**, **day_grade**, sleep_unified, state_of_mind, travel, interactions, exposures, temptations |
