...
`coach_gen_cache`, `composite_scores`, `deletion_log`, `dropbox_tracker`, `email_digest`, `email_log`, `experiment_suggestions`, `google_calendar`, `health_check`, `hevy_id_map`, `ingest_liveness`, `journal_analysis`, `pending_writes`, `personal_baselines`, `qa_hook_matrix`, `qa_predict_dark`, `routine_index`, `sleep_unified`

## 3. Consumer Edges (module → partition)

707 edges from the two-pass AST sweep (#2805 mechanism). Directions:
...
- Edge sites: 1228 total · 879 resolved · 349 dynamic (unresolvable at AST time, tagged — never guessed)
