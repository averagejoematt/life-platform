...
704 edges from the two-pass AST sweep (#2805 mechanism). Directions:
...
| `coach_thread` | intelligence_common.py, training_notes.py | tools_coach_intelligence.py |
...
| `hevy` | hevy_common.py | ai_expert_analyzer_lambda.py, daily_metrics_compute_lambda.py, recap_data.py, site_api_nutrition.py, site_api_pulse.py, site_api_training.py, tools_health.py, tools_hevy_routine.py, tools_nutrition.py, tools_strength.py, tools_training_notes.py, training_notes.py, vacation_fund.py |
...
| `board-verdict-silence-7d` | monitoring_stack | metric | digest | declaration |  |
...
| `canary-no-invocations-1h` | operational_stack | metric | digest | declaration |  |
...
| `commitments-ungraded` | monitoring_prediction_alarms | metric | digest | declaration |  |
...
| `swallowed-permission-denial` | monitoring_denial_alarms | metric | digest | declaration |  |
...
- Edge sites: 1216 total · 871 resolved · 345 dynamic (unresolvable at AST time, tagged — never guessed)
