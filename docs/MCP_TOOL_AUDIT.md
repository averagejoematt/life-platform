...
| **2026-09-13** | **82 → 83** | **0** | **#3751 addition — `plan_next_session`.** The deterministic stage of the planning engine, reachable from every client. Over chat/MCP there was no planner at all: the tools returned data and one chat turn decided the session, while the 154-line structured procedure lived only in a skill file Claude Code runs — so the app the owner happened to open decided the quality of the plan. ONE tool, not fourteen: it fixes the call SET and computes the constraint block server-side with no model in the loop. Band raised 82 → 83. |
| **2026-09-23** | **84 → 85** | **0** | **#4078 addition — `manage_pending_writes`.** Four chat coaching sessions (09-08, 09-18, 09-20, 09-21) ended by telling the owner writes were "queued pending approval"; no queue existed, so each write died with its conversation. ONE tool with four actions (enqueue / list / approve / discard) over a persisted `SOURCE#pending_writes` partition, surfaced by `get_capture_queues` and aged by a qa-smoke dead-man — not four tools, for the #395 reason. What may be queued is DERIVED (`is_write_tool` over the live registry), never listed. Band raised 84 → 85. |
| **2026-09-23** | **85 → 86** | **0** | **#4082 addition — `get_coach_session_packet`.** The owner's audit of eight chat coaching sessions (09-14 → 09-22): each opened with 10+ calls re-verifying the same inputs (per-muscle sets, last session per type, a week of MacroFactor, walking hours, streaks, readiness, block position), and the wrong calls traced to the input skipped or read over a different window than the planner. ONE read returns the packet, every field from its canonical function (no second derivation, AST-guarded in `tests/test_coach_session_packet_4082.py`) with a measured / absent / read_failed state. **Ruling: a new tool, not a `view`.** `get_training`'s views are training analytics and the packet spans nutrition, recovery and the program; `plan_next_session` is at its size ceiling and is the planner, whose inputs this is. The name carries a read verb (`get_`) — `coach_session_packet` would classify as a WRITE in `mcp.audit`. Band raised 85 → 86. **Demote trigger:** zero invocations in any 30-day window after deploy (chat is not calling it) → retire it through this ledger. |
