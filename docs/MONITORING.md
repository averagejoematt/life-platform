...
`Source=<name>`, which the per-source `ingest-auth-unhealthy-{src}` alarms read. Every
credentialed source now has per-source coverage — either an
`ingest-consecutive-failures-{src}` alarm (whoop, withings, strava, eightsleep, hevy) or
an `ingest-auth-unhealthy-{src}` one (todoist, habitify, dropbox urgent; garmin and notion
to digest — garmin's 429 auth death is the accepted, unfixable state ADR-074 de-paged, and
notion is `monitored: False`). The set is **derived, not hand-listed**: the `oauth` facet
...
_**129** CloudWatch alarms are defined in `cdk/stacks/*.py` and AST-discovered by `deploy/sync_doc_metadata.py::_auto_discover_alarm_names` (#795 counts them, #934 names them). Regenerated by `python3 deploy/sync_doc_metadata.py --apply` and gated by `--check`: add / rename / remove an alarm in CDK and this updates automatically; a name here that no stack defines reds CI._

**`email_stack.py`** (2)

- `chronicle-delivery-heartbeat`
- `weekly-signal-delivery-heartbeat`

**`ingestion_stack.py`** (2)

- `hae-webhook-errors`
- `hevy-template-index-not-rebuilt-48h`
...
- `mcp-warmer-error`
- `mcp-warmer-no-invocations-24h`
- `nightly-predraft-missing`
- `slo-mcp-availability`
- `slo-warmer-completeness`

**`monitoring_budget_alarms.py`** (1)

- `budget-tier-unreadable`

**`monitoring_compute_alarms.py`** (4)
...
**`monitoring_denial_alarms.py`** (1)

- `swallowed-permission-denial`

**`monitoring_prediction_alarms.py`** (3)

- `commitments-ungraded`
- `grading-stalled`
- `prediction-gradable-share-low`

**`monitoring_silence_alarms.py`** (7)

- `between-chronicle-scrub-failed-closed`
- `chronicle-status-write-failed`
- `expert-gate-infra-hold`
- `freshness-sentinel-write-failed`
- `recall-index-failed-chronicle-approve`
- `recall-index-failed-wednesday-chronicle`
- `telegram-coach-hold`

**`monitoring_stack.py`** (56)
...
- `ingest-auth-unhealthy-dropbox`
- `ingest-auth-unhealthy-garmin`
- `ingest-auth-unhealthy-habitify`
- `ingest-auth-unhealthy-notion`
- `ingest-auth-unhealthy-todoist`
- `ingest-auth-unhealthy-whoop`
...
- `life-platform-between-chronicle-kill-switch-skip`
- `life-platform-budget-tier-escalation`
- `life-platform-chronicle-email-sender-kill-switch-skip`
...
- `life-platform-weekly-signal-kill-switch-skip`
...
**`operational_stack.py`** (26)

- `canary-no-invocations-1h`
- `cost-metric-drift-sustained`
- `freshness-checker-errors`
- `hevy-restamp-errors`
- `hevy-routine-cron-errors`
- `ingestion-error-insight-email-parser`
...
- `life-platform-canary-subscribe-cleanup-failure`
- `life-platform-canary-subscribe-residue`
...
- `life-platform-insight-email-parser-parse-failure`
- `permanence-errors`
- `permanence-heartbeat`
- `recap-card-no-invocations-24h`

**`serve_stack.py`** (13)

- `site-api-ai-errors`
- `site-api-ai-throttles`
- `site-api-content-filter-fallback`
- `site-api-errors`
- `site-api-handled-5xx`
- `site-api-invocation-spike`
- `site-api-p95-latency-high`
- `site-api-throttles`
- `telegram-event-sweep-heartbeat`
- `telegram-webhook-errors`
- `telegram-webhook-throttles`
- `telegram-worker-errors`
- `telegram-worker-throttles`

**`web_stack.py`** (3)

- `email-subscriber-errors`
- `life-platform-og-image-errors`
- `progress-viewer-errors`
