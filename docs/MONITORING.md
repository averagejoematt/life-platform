...
_**80** CloudWatch alarms are defined in `cdk/stacks/*.py` and AST-discovered by `deploy/sync_doc_metadata.py::_auto_discover_alarm_names` (#795 counts them, #934 names them). Regenerated by `python3 deploy/sync_doc_metadata.py --apply` and gated by `--check`: add / rename / remove an alarm in CDK and this updates automatically; a name here that no stack defines reds CI._
