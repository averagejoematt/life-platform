...
**Last updated:** 2026-09-23 (v8.6.0 — 84 MCP tools, 20 data sources, 106 Lambdas, 12 cached tools)
...
| `…SOURCE#{x, instagram, tiktok}` / `DATE#<d>#<post_id>` | **closed-platform inbound social (#1677, epic #1668)** — the same fact layer as `youtube`/`bluesky`/`mastodon`, but **paste-captured**: X needs a paid API tier, Instagram/TikTok need Business/Creator Graph tokens behind app review, and the owner's 2026-08-12 decision is not to provision any of it. Each row carries `channel`, `origin` (`human`\|`platform`, classified by the same `social_provenance` classifier a fetched post runs — pasting is *not* taken as proof of human authorship, so a syndicated echo is still caught), `capture: 'paste'` (the one honest difference — a reader of the row can tell how it arrived), `post_id`, `post_type`, `date`, `url`, `text`, `embed_url`, the channel-agnostic `title`/`description`/`thumbnail_url` broadcast-card trio (#2221), `published_at`, `author`. **On `origin: human` rows only**, the #1673 fail-closed gate stamps `sensitivity_status` (`cleared`\|`held`), `sensitivity_reason` and `sensitivity_categories`. No engagement counts — likes/views are not pasted; a future token-backed poll must cast them to `Decimal` at the transform. Empty values are dropped, not stored | `lambdas/ingestion/closed_social_paste_lambda.py` via the shared SIMP-2 framework (identical path for the Lambda handler and `scripts/paste_social_post.py --ingest`) | `lambdas/privacy/social_provenance.py` exclusion predicates, `broadcast_sensitivity_gate` consumers, `/story/broadcast/` | raw_timeseries | empty (paste-only — no post pasted yet) |
| `…SOURCE#{x, instagram, tiktok}` / `PASTE#<d>#<post_id>` | **the staging inbox for the three rows above.** Deliberately the *same partition* as the ingested row, under a distinct sk prefix that sorts entirely outside the `DATE#` range — so a `DATE#` query never sees staged pastes. This staged row **is the raw record**: nothing is fetched, so there is no API response to archive, and all three sources carry `raw_layout: None` with `enable_raw_archive=False` to match. Holds exactly the strings the owner pasted: `channel`, `capture: 'paste'`, `staged_at`, plus the normalized entry (`post_id` — derived from url+text+date when not supplied — `text`, `url`, `embed_url`, `published`, `author`, `date`). All values are strings; there is no numeric field on a pasted post. Idempotent on (channel, date, post_id) | `lambdas/ingestion/social_paste_inbox.py::stage_paste` | `staged_entries()` — this partition IS the `fetch_day` source the framework reads | raw_timeseries (one classification covers both sk forms — `phase_taxonomy.py:139-144`) | empty (paste-only — no post pasted yet) |
...
| `…SOURCE#time_affluence` / `EDGE#<sunday>` | **the weekly time-affluence edge test** — one row per ISO week keyed on its Sunday, holding the week's reported numbers and the test that produced them (`lambdas/health/time_affluence.py`). RAW_TIMESERIES: it is a measured weekly reading about how the time was spent, true of that week forever, so it is kept across a reset and genesis-anchored on read rather than tombstoned | `lambdas/health/time_affluence.py` | `emails/evening_nudge_lambda.py`, `web/site_api_social_engage.py`, `content/ritual_link.py` | raw_timeseries | ✓ |
| `…SOURCE#journal_quotes` / `QUOTE#<date>#<id>` | **consent-per-line verbatim quote marks** (#1568, ADR-142) — the owner's explicit approval of an exact line of his own journal for public use, frozen at mark time with the approved text. Follows its `notion` parent: RAW_TIMESERIES, kept forever and genesis-anchored on read. Revocation is an explicit unmark, never a reset wipe — a reset must not be able to withdraw or re-grant consent | MCP `mark_journal_quote` | public quote surfaces | raw_timeseries | n/v (no line marked yet) |
...
| `…SOURCE#training_constraints` / `DISMISSAL#<site>#<YYYY-MM-DD>` | **owner dismissals of derived pain flags (#4036)** — `site` (his words, e.g. `right lower back`) + `site_key` (normalized, and what is in the sk) + `dismissed_on` + `words` (VERBATIM) + `movements`/`movement_keys` + `flag_note_date` (the flag instance dismissed) + `recorded_at`/`recorded_via`. Read at runtime beside the flag layer by `plan_engine._tripwire_states` (the row becomes `dismissed_by_owner`, **never `clear`** — an absence and an override are different facts) and `coach/critics.py::build_joints_packet` (no `pain_flag_loaded` veto on the dismissed instance). RE-ARMS by date: a note dated after the dismissal trips the flag again and marks the dismissal superseded; the one comparison lives in `training/training_context_registry.resolve_flag`. **Tier 2 owner-only** (his verbatim words about his body; no public projection). CROSS_PHASE (ADR-077 ruling 2026-09-21): a reset that wiped it would silently re-arm every site he has already said is resolved, and the input — a sentence he said once — cannot be recomputed | `mcp/tools_training_notes.py::tool_get_exercise_notes` (`action='dismiss'`) | `plan_next_session` (stage 1 tripwire + stage 2 joints critic) | cross_phase | n/v (owner-only; first live entry pending) |
| `…SOURCE#named_human_contact` / `STATE#current` | **the named-human contact path's episode de-dup / cooldown row (#4063, v0.3 §9/§13.3)** — ONE row: `episode_anchor` (the last day any engagement channel logged, or `none`) + `episode_quiet_days` + per-rung `quiet_sent_on`/`quiet_mode` and `four_week_sent_on`/`four_week_mode` (mode ∈ `preview` / `armed` / `superseded_*`) + the cross-episode cooldown pair `last_quiet_sent_on`/`last_quiet_sent_mode` + `updated_at`. Dates and modes ONLY — **the contact's identity is never written here**; it lives only in the private S3 object `config/coaching/named_human.json`, read at send time. Written only after an actual send. **Tier 2 owner-only** (a disengagement record is a behavioural fact about him; no public projection). SYSTEM_STATE (ADR-077): a reset must not wipe it, or the contact would be re-mailed for a stretch they were already told about | `coach/named_human_contact.py::run_leg` (a fail-soft leg of `evening-nudge`) | the same leg (next night's `decide`) | system_state | n/v (no row until the first preview send) |
| `…SOURCE#eyeball_estimate` / `ESTIMATE#<date>#<id8>`, `GRADE#<date>#<id8>` | **meal-photo macro estimates and their grades against MacroFactor truth** (#1390, epic #1080) — the reliability record for the model's eyeballing, not nutrition data (the module carries an isolation guard saying so). CROSS_PHASE for the same reason as `calibration` and `coach_corrections`: it measures the MODEL's accuracy across the whole cross-cycle history, and wiping it at a reset would discard the accumulating error distribution the public chart is built on | `lambdas/eyeball_calibration.py` | the eyeball accuracy chart | cross_phase | n/v (no graded probe yet) |
| `…SOURCE#recall_embeddings` / `DOC#<kind>#<date>` | semantic-recall index (#1384, epic #1080): one item per corpus doc (kind ∈ chronicle/coach_output/journal). The Titan-v2 vector rides as a base64-packed float32 **String** attribute `emb` (compact ≈1.4 KB/256-dim, sidesteps float→Decimal); each item carries its source `cycle` stamp + `artifact_pk`/`artifact_sk` (resolves the precedent to a real record) + `text_sha` (backfill idempotency) + `link`/`snippet`. **cross_phase on purpose** — cross-reset recall is the whole point, so the index survives resets and is read by a RAW query (no phase filter). NO vector DB, NO GSI (ADR-103) — re-examined against DynamoDB native vector search at GA and deliberately kept, with the measurements and a numeric revisit trigger in **ADR-150**: the small corpus loads in one Query, cosine is brute-forced in-Lambda | `lambdas/ai/recall_indexer.py` at PUBLISH (`chronicle_approve` + the direct-publish path — the corpus had no automated writer at all until then, and froze for 18 days), `deploy/backfill_recall_embeddings.py` (bulk catch-up), `bedrock_client.embed_text` | `lambdas/ai/semantic_recall.py` (coach precedent line in `ai_calls`; chronicle recall card rendered on the published post by `chronicle_render`), `lambdas/operational/recall_freshness_qa.py` (nightly: every published installment must be indexed) | cross_phase | live (17 rows at 2026-08-08; publish-time writer added) |
...
| `…SOURCE#recap_cards` / `DATE#<d>` | the daily recap card's picker record (#3741): `chosen`, every losing candidate's score, `absent_sources`, the privacy verdict, `delivered{telegram,email}`, `s3_key`, `outcome` (`sent`\|`rendered`\|`no_signal`\|`held`\|`blocked`). Written on EVERY run including the days that produce no card — 'a different card every day' is provable from this row, and 'no card' is explicable from it. The PNG lives at `recap/<d>.png` — a prefix with no CloudFront behaviour AND no anonymous read, because the card is private until the owner posts it (ADR-140 rule 5). It first shipped to `generated/recap/`, where `PublicReadGenerated` made every card world-readable at a date-derivable key; 'no CloudFront route' was asserted, and was never the property that mattered (2026-09-14, the #3559 class one prefix over). | `web/recap_card_lambda.py` | — (owner-facing; Telegram + email) | n/v |
| `…SOURCE#qa_hook_matrix` / `DATE#<d>` | the nightly hook-liveness matrix (#3615/#4015): one row per PT day keyed by `cycle_day`, the (hook × downstream artifact) liveness cells the qa-smoke `hooks:liveness_matrix` leg wrote that night. SYSTEM_STATE (ADR-077 ruling 2026-09-22): an operational instrument's own output, regenerated nightly, never evidence about the experiment — survives a reset, read genesis-anchored | `operational/hook_liveness_qa.py` (inside `qa_smoke_lambda`) | qa-smoke email, `check_alarm_citations` | n/v |
| `…SOURCE#progress_photos` / `DATE#<d>#<pose>` | the progress-photo index row (#3758): `pose` (front\|side\|back), `s3_key`, `bytes`, `sha256`, `telegram_file_unique_id`, `week_n`, `source_channel: telegram`. **Tier 2, owner-only (#3757/#3782)** — registered before the first byte could be stored, per ADR-154's ordering. The JPEG itself lives at `raw/matthew/progress_photos/<YYYY>/<MM>/<YYYY-MM-DD>-<pose>.jpg`; `raw/*` carries no anonymous `Allow` in the bucket policy (the three that exist are `site/*`, `blog/*`, `generated/*`) and no CloudFront behaviour. The DATE in the LEAF is load-bearing — the registry's first draft said `<pose>.jpg`, which under `date-tree` overwrites every week of a month with the last. Idempotent on `telegram_file_unique_id`: a re-send is a no-op, a genuinely different file for the same day and pose is a retake and replaces it. | `coach/progress_capture.py` (via `telegram-worker`) | — (owner-only; no public projection) | n/v |
...
| `…SOURCE#protocols` / `PROTOCOL#<id>` | protocol registry (the levers). **`spawned_by` is REQUIRED (#3621)** — a `HYPOTHESIS#<id>` the lever came out of, or the exact label `pre-platform / literature` meaning no platform hypothesis spawned it; `origin` keeps the prose about why the lever exists and is a separate field. Refused at the write by `experiment/protocol_levers.py::build_protocol_item`; the same predicate grades `site/config/protocols.json` (the S3 fallback `/api/protocols` serves). The named MCP writers (`create/update/retire_protocol`) were pruned by #395 and no longer exist. | `experiment/protocol_levers.py` via `deploy/seed_protocols.py` (the one write path) | site_api_protocols `/api/protocols` | ✓ |
| `…SOURCE#field_notes` / `WEEK#<iso-week>` | weekly field notes | `intelligence/field_notes_lambda.py` | chronicle, `get_field_notes` | ✓ |
| `…SOURCE#diary_reactions` / `DATE#<d>#<channel>#<entry_uid>` | one short coach reaction per thing Matthew said — a **V3-consented** Video Diary / Solo Recording entry (#1574) **or** a membrane-cleared public social post (#1675): `coach_id`/`coach_name`, `reaction`, `tone`, `theme` (the 8-way laundered public theme), `tier` (`allude`\|`quote`), `kind` (`diary`\|`social`), `entry_date`, `entry_uid`, optional cleared `quote`, and (social) `post_url`. The source record NEVER lands here — `lambdas/privacy/diary_consent.py` (diary) / `lambdas/privacy/social_consent.py` (social) reduces it to a leak-proof public context before generation. `entry_uid` = the #476/E-6 stable page suffix for a diary entry, the `post_id` for a social post, so two same-day records on one channel keep separate rows (#1756 fixed the collision) | `coach/coach_diary_reaction.py::store_reaction`, triggered inline per enriched record by `ingestion/journal_enrichment_lambda.py` (#1756) and `ingestion/social_enrichment_lambda.py` (#1675) — gate- + budget-gated, idempotent, fail-open | `/api/diary_reactions` → lab-notes | n/v (fires on the first consented diary entry / cleared post) |
...
| `PREDICTION#<id>`, `LEARNING#<date>#<slug>`, `CONFIDENCE#<subdomain>`, `COMMITMENT#<id>` | predictions + graded outcomes + per-subdomain confidence. `PREDICTION#` statuses: `pending`/`confirming` → `confirmed`/`refuted`/`inconclusive`/`expired`, **plus `observation` (#3046)** — a qualitative claim with no deterministic grading path, stamped `gradeable_by: "none"` at emission (`coach/prediction_emission.py`) and never counted as pending; gradable records carry `gradeable_by: "deterministic"` | `coach/coach_state_updater.py` (writes), `coach/coach_prediction_evaluator.py` (grades), `coach_checkin.py` | track-record pages, stance engine | ✓ (partition) |
...
| `BROADCAST_ORIGIN#<channel>` / `POST#<post_id>` | **the provenance membrane ledger (#1670, epic #1668)** — one row per post the platform syndicates OUT, so inbound ingestion can stamp its own echoes `origin: platform` and exclude them from the voice feed / enrichment / re-broadcast. SYSTEM_STATE (provenance is cross-phase truth — survives reset). Outbound writer is LIVE since #1679: `scripts/post_social.py` calls `social_provenance.record_broadcast_origin` after a successful post (fail-soft — a provenance miss warns, it never fails an already-sent post). Also read by `/api/membrane` (the #1679 dashboard's "what I said, where it went" side) | `scripts/post_social.py` via `lambdas/privacy/social_provenance.py` (write helper) | `social_provenance.classify_origin` inbound cross-ref; `web/site_api_social.handle_membrane` | system_state | empty (no post has been made through the poster yet) |
...
Two device modes (#3417, ADR-154 amendment 2026-09-01): a plain stand-on weigh writes **weight only**; a full scan (handles held) also writes the impedance-derived composition fields (fat/muscle/bone/hydration + the BodyScan 2 set below). Composition is therefore **behavioral/sparse** — present only on full-scan days (first: 2026-08-16). Absence of composition on a weigh-day means a basic weigh happened, not a device or pipeline failure (ADR-104). The `lean_mass_delta_14d`/`fat_mass_delta_14d` fields once documented here were **never written**: their computation was deleted 2026-07-04 (#486/B-3) having never produced a value, and stays deleted by recorded decision (ADR-154 amendment 2026-09-01).
...
`extracellular_water_kg`, `intracellular_water_kg`, `visceral_fat_index` (unitless), `bmr_kcal` (label inferred from device class — BodyScan publishes BMR, value is kcal-scale), `eda_feet`/`eda_left_foot`/`eda_right_foot` (electrodermal activity / nerve health), `afib_result` (0 = screening ran, not detected — an event-class medical result, never a trend stat), `metabolic_age`, and segmental body composition `{fat_free_mass,fat_mass,muscle_mass}_{torso,left_arm,right_arm,left_leg,right_leg}_kg` (15 fields; no lbs twins). **Segment mapping is magnitude-inferred, not vendor-documented** — the `position` attribute→limb map in `SEGMENT_POSITIONS` was derived from the live 2026-08-16 capture (torso carries the dominant share; the type-173 five-segment sum equals scalar `fat_free_mass_kg` exactly, which pins the semantics); an anatomically absurd future reading means the mapping, not the body, changed. **VERIFIED LIVE 2026-08-22 (#2994), and until that date these 15 fields had never been written at all**: #2794 added the parsing branch and left the `getmeas` request built from `MEAS_TYPES` alone, which does not contain 173/174/175, so the transform could never be fed them — this paragraph described a capability the platform did not have, while the transform's own tests stayed green on a fixture the real request could not produce. After the fix the `2026-08-16` row carries all 15: torso 48.76 / legs 15.48, 15.35 / arms 6.33, 6.16 kg fat-free, summing to 92.08 = the scalar, so the mapping is now confirmed through the production path rather than from an exploratory unfiltered call. `spo2_pct` **zero is never stored** (the device transmits 0 on a failed measurement — ADR-104 absence, not a reading).

**Privacy tiers (PhenoAge posture):** `vascular_age`, `metabolic_age`, and `afib_result` are **Tier-2 owner-only** — never on a public surface and never quoted into an AI narrative context. **This paragraph is no longer the source of truth (#2803):** the ruling is now structure in `lambdas/privacy/field_tiers.py`, and `tests/privacy_tier_wiring.py` AST-derives every module mentioning a Tier-2 field and requires a recorded SAW/EXCLUDED decision per pair — a public/AI-family consumer may only be `EXCLUDED`, so a new reader of one of these fields fails the build before merge. `mcp/tools_data.py`'s strip set derives from the same registry rather than restating it. Edit the registry, not this line. **Correction (#2809, 2026-08-20):** the #2782 "all current consumers are field-selective" claim was FALSE — `mcp/tools_data.py`'s generic row dumpers (`_get_latest`, `_get_daily_summary`, `tool_get_date_range`, `_find_days_filter`) returned full withings rows, Tier-2 trio included, into Claude conversation context; #2782's sweep enumerated site_api_body/rollups/ai_context + `mcp/tools_health.py` and missed `mcp/tools_data.py` entirely. Fixed by a field strip at each dumper (`TIER2_STRIP_FIELDS` in `mcp/tools_data.py`) rather than by re-litigating the sanctioned exception — MCP remains the owner-only surface, but a "dump the row" path is not the same thing as a "pick fields on purpose" path, and only the latter is the sanctioned one. Any future consumer of the withings partition must pick fields explicitly, or strip Tier-2 explicitly if it can't — see the NEW_SIGNAL_PLAYBOOK step-7 consumer table, which now names `mcp/tools_data.py` so this miss can't repeat.
...
**Data source:** Open-Meteo archive API (free, no auth, WMO-grade data), plus Open-Meteo's air-quality product for `aqi`. Ingestion path:
...
> **Historical:** an `get_weather_correlation` MCP tool once fetched and cached missing dates on demand. It was **pruned 2026-07-08** in the #395 tool audit (`docs/MCP_TOOL_AUDIT.md:157`) against 30-day EMF telemetry. The scheduled Lambda is now the only writer of this partition.
...
| `condition` | string | Human-readable sky condition (#2311), mapped from Open-Meteo's daily WMO 4677 `weather_code` via `_WMO_CONDITIONS`. **Omitted entirely when the code is not in the map** — honest absence, never a guessed condition (ADR-104) |
| `sunrise_local` | string | `HH:MM` local (America/Los_Angeles) sunrise (#2311). Omitted when the API returns no parseable timestamp |
| `sunset_local` | string | `HH:MM` local (America/Los_Angeles) sunset (#2311). Omitted when the API returns no parseable timestamp |
| `aqi` | number | **Max** hourly US AQI for the day (#2311), from Open-Meteo's separate air-quality product (`air-quality-api.open-meteo.com`, hourly `us_aqi`; same provider, no auth, one extra call). **Fail-soft:** an air-quality outage omits this field for the day and never fails the weather record; omitted when nothing was measured (ADR-104) |
| `ingested_at` | string | ISO timestamp of ingestion (Lambda-written records only) |

All four fields above are written by `transform()` in `lambdas/ingestion/weather_lambda.py` and have been live since **2026-08-09** (#2311). Per ADR-104 each is **omitted, not null-filled**, when its upstream value is absent — so a consumer must treat "key missing" as "not measured," never as zero.
...
Note: S3 raw backup at `raw/weather/YYYY/MM/YYYY-MM-DD.json` (Lambda path only) — the leaf filename is the **full date**, matching the `raw_layout` facet in `lambdas/ingestion/source_registry.py` (`prefix: raw/weather`, `scheme: date-tree`, `filename: YYYY-MM-DD.json`). This line previously read `DD.json`, the pre-SIMP-2 form. **Read the registry facet, never construct a raw key by hand** (#1256/#2286).
...
| Value         | Meaning                                                          |
|---------------|------------------------------------------------------------------|
| `experiment`  | Record dated on or after EXPERIMENT_START_DATE (currently 2026-09-06). |
