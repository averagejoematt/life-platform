...
4. **Privacy tier.** `answer_quote`/`takeaway` quote Matthew's verbatim check-in answers → **Matthew-private** (same tier as the CHECKIN# answers themselves, `docs/DATA_GOVERNANCE.md`). Public surfaces must never render `channel=conversation` learning text; `lambdas/web/site_api_coach.py` filters explicitly (track record + timeline), defense-in-depth on top of the status-vocabulary exclusion.
