...
*Last updated: 2026-09-07 (v8.6.0)*
...
**Still open** on #3671: no registry enumerates which *config* fields are experiment-anchored (`character_sheet.json` and `user_goals.json` are re-anchored by the reset, `vacation_fund.json`'s null `start_date` has never been ruled on), and the reset's report does not name the anchors it moved. This amendment closes the Hevy counter; it does not close the class.
