...
The pair is ~24 cents a run, not pennies, and the **prose** gate is the expensive half — ~13x the Lambda copy's unit and ~4x the vision gate, the opposite of where the images-are-expensive intuition pointed. **What the placement actually rests on survives intact:** per-run cost is stable to the cent across runs, so "per-DEPLOY and bounded" holds; the daily total is merge-cadence-driven ($0.90–$3.10/day over 2026-08-23..26, n=4 complete days), which is a cadence question, not a band question. Not priced here: the Sunday full-surface `--ai-qa` pass (`visual-qa.yml`, no tier cap) and the daily standalone `--reader-truth` fire. The cadence decision this opens is filed separately.
