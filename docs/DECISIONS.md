...
**Amended 2026-07-08 (ADR-133):** base ceiling raised $75 → **$85** (surge mode floats it to $100 on real reader traffic); the dollar figures below are historical.
...
| `hevy-webhook` FunctionURL lambda (parked — Hevy has no webhooks) | **RETIRED 2026-07-06** (#756, R21 kill list #8) | Standing public FunctionURL served nothing since Hevy publishes no webhooks. Lambda + FunctionURL + error alarm + IAM policy (`ingestion_hevy_webhook`) removed from `ingestion_stack.py`/`role_policies.py`. Handler source (`lambdas/ingestion/hevy_webhook_lambda.py`) kept in git (moved to `ci/lambda_map.json` `skip_deploy`) for revival if Hevy ever ships webhooks. `hevy-backfill` remains the sole ingestion path. **LifePlatformIngestion needs a deploy** to drop the live function + FunctionURL |
...
**Executed — 2026-07-07 (COST-01, #790).** The sanctioned follow-up shipped. **Correction to the premise:** the 2026-07-05 verification that "only 1 of 32 compute and 1 of 17 email lambdas route to a DLQ" was a *live-AWS* undercount — in CDK the `dlq=local_dlq` wiring has been in the `shared` dict of both `compute_stack.py` and `email_stack.py` since those stacks were first written (v3.2.9). `cdk synth --all` confirms **32/32 compute + 17/17 email app functions already carry a `DeadLetterConfig` → `life-platform-ingestion-dlq` with a per-role `sqs:SendMessage` grant** (the helper's `dead_letter_queue=` constructor auto-grant, one least-privilege policy per role). The DLQ path was therefore already covering terminal async failures; the only remaining step was retiring the now-redundant per-lambda alarms. Both `shared` dicts now set `error_alarm=False`, which drops all **48** `ingestion-error-*` alarms on the compute (32) + email (16) fleet in favour of the DLQ digest (`life-platform-ingestion-dlq-messages` + `life-platform-dlq-depth-warning` + the DLQ-consumer digest). **Auto-discovered `alarm_count`: 113 → 65** (`deploy/sync_doc_metadata.py`, AST parse == synth grep of `AWS::CloudWatch::Alarm`). daily-brief is unaffected (it already opted out with `alerts_topic=None` and keeps its dedicated `MonitoringStack` alarms). Estimated saving **~$4.80/mo** (48 × $0.10). Requires `cdk deploy` of Compute + Email (alarm deletes); Core/Monitoring/Operational unchanged by this diff. The residual failure mode dropped is a single transient throw that self-heals on retry — the noise class this consolidation targets. The audit §2 orphan cleanup (~$1.80/mo, `deploy/cloudwatch_retire_orphans.sh`) is a separate live-AWS op, unchanged here.
...
## ADR-123: Source-of-truth reconciliation generalized to Whoop; Garmin deliberately NOT reconciled (TR-07, #415)

**Date:** 2026-07-05 · **Status:** Accepted · **Story:** #415 (epic #348, "Deepen the N=1 instrument")

**Context.** Every freshness/liveness check in the platform reads only DynamoDB, so it can see only the *high-water mark* — it is structurally blind to a **silent drop**, where a record the provider API holds never lands in the store behind the latest date. The platform once lost five afternoon walks with no alarm for exactly this reason (the Jun-2026 evening-walk gap). The fix that caught it — DI-2 reconciliation on Strava (`strava_lambda._reconcile`) — periodically asks the provider what exists and diffs it against what we stored, emitting `LifePlatform/IngestReconciliation::MissingActivityCount{Source=strava}` with a digest alarm + heartbeat. TR-07 asked whether that pattern should be extended to the other wearable pipelines (Whoop, Garmin).

**Decision.**

1. **Whoop gets opt-in provider-diff reconciliation, mirroring Strava.** The Whoop ingestion Lambda, invoked daily with `{"reconcile": true}` (EventBridge, 18:20 UTC fixed = 11:20 AM PT, clear of its own :00 hourly + :30 recovery slots under reserved-concurrency=1), pulls a trailing 14-day set of sleeps + workouts from the Whoop v2 API (`_fetch_all_records`, paginating past the 25-record page via `next_token`) and diffs it against the store. Two record classes are checked: each **SCORED main sleep** anchors a UTC day that must have a stored `DATE#{day}` record (dropped-night class); each **workout id** on day D must have a stored `DATE#{D}#WORKOUT#{id}` sub-record (the late-syncing-workout class the `refresh_trailing_days=2` heal is blind to verifying). Emits the same metric with `Source=whoop`, alarmed by `ingest-reconciliation-whoop` + an absence heartbeat in `monitoring_stack`. **READ-ONLY** — it reports gaps; healing stays the ingestion job's role. **Dedup-aware:** multiple sleep/recovery records for one day collapse to a single expected `DATE#`, and near-simultaneous workout twins (GPS-drop / double-log) collapse via `_dedup_workouts` + a ±120s tolerance match against stored workout starts, so a known twin never raises a false gap. Opt-in is the `provider_reconcile` facet in `source_registry.py` (`provider_reconcile_source_ids()`); no new IAM (the reconcile invocation reuses the ingestion role's DDB-Query + PutMetricData + secret grants).

2. **Garmin is deliberately NOT reconciled.** Garmin is paused (ADR-074 — vendor anti-automation 429-blocks server-side OAuth refresh from datacenter IPs) and, even when live, is capped at **4x/day** under the OAuth rate limit and classified `best_effort` (evaluated + logged, excluded from `UnhealthySourceCount` so its accepted-degraded state can't mask a real death). A reconciler would spend that scarce request budget re-listing exactly what ingestion already cannot reliably fetch, and would false-alarm continuously on the accepted-degraded state — noise on the one alarm class that must stay trustworthy. The honest answer is **don't reconcile**: the cost is real and the signal would be untrustworthy. Recorded as the deliberate absence of the `provider_reconcile` facet on garmin, with the reasoning inline in `source_registry.py`. **Revisit only if** Garmin ingestion is restored to a healthy, non-rate-limited cadence — at which point Garmin would opt in by mirroring Whoop (respecting the request budget).

**Consequences.** A silent drop in the Whoop pipeline now pages (digest) the day after it appears, instead of hiding behind a green high-water mark — the Strava guarantee, extended. No layer bump is required for the frontier of TR-07 itself (the reconcile code ships bundled in the Whoop Lambda), but `source_registry.py` is a shared-layer module, so the facet addition rides the next layer rebuild for the layer-resident consumers (MCP, site-api) — those consumers don't read the new facet, so a lagging layer copy is harmless. Garmin's non-reconciliation is a documented, revisitable decision rather than an assumed omission.
...
**The options, graded on friction / honesty / privacy:**

**A — State of Mind check-ins → the existing HAE webhook (build: ~zero).** The iOS State of Mind gesture (or How We Feel → HealthKit; optionally fronted by a voice-memo Shortcut that files a check-in) rides the pipeline ADR-121 just repaired end-to-end: HAE webhook → `som_avg_valence`/`som_check_in_count`/labels → nine consumers already keyed correctly. *Friction:* low-medium — a 15-second phone gesture, but this exact habit already failed once (ADR-121's retirement trigger stands at 2026-Q4). *Honesty:* high — structured valence stamped at the moment of feeling, no retro-narrative; absence stays visible as absence (ADR-104). *Privacy:* labels/associations can name people and work — only the valence aggregate + check-in count may ever publish, never labels or associations.

**B — Passive proxies (calendar / comms / activity exhaust).** Infer connection from calendar events, message or call cadence, shared-activity detection. *Friction:* zero ongoing — its whole appeal. *Honesty:* low-medium — attendance is not connection; a proxy laundered as the construct is precisely the ADR-105 failure mode, and calibrating proxy→fulfillment needs ground truth that only options A/C can supply. *Privacy:* highest risk of the three — calendars and comms carry names, places, and third parties who never consented; publishing anything derived from them needs an aggregation layer aggressive enough to gut the signal. *Build:* M–L of new ingestion + IAM surface against the ADR-103 subtract-more-than-add posture. **Not viable as the primary channel; viable later as annotation** (context on top of a real signal), never as the signal.
...
**Context.** The coach-v2 pipeline (`ai_calls._run_coach_v2_pipeline`) regenerates each coach's daily narrative from scratch every cycle: a full Sonnet call plus the ADR-104 grounding gate and the ADR-108 quality gate (each of which can trigger a corrective regeneration). On a **quiet day** — no new weigh-in, no new sleep record synced, no stance change, no logging — the generation *brief* the orchestrator assembles is near-identical to yesterday's, so the pipeline pays full price to produce essentially the same paragraphs. AI is the single largest cost line (COST-02) and scheduled content is the largest bucket within it; re-saying the same silence is pure waste, and it's the most frequent case exactly when Matthew is disengaged and nothing is changing.

**Decision.** Fingerprint the generation brief; on an exact match with the last gate-passed run, reuse the stored output and skip the whole generation path (Sonnet + both gates). A new shared-layer module `generation_cache.py`:

- **`brief_fingerprint(*parts)`** — SHA-256 over the canonicalized semantic inputs. In the pipeline the parts are `system_prompt` + `user_message`, which together carry every semantic input: the authoritative canonical facts, the model-forecast expectations, the orchestrator brief (stance, open threads, engagement/presence signal, evidence ceiling), and the coach's domain data.
- **`canonicalize()`** strips only pure bookkeeping before hashing: keys starting with `_` (the orchestrator's own documented internal-bookkeeping convention — `_generated_at`, `_fallback`, `grounding_flag`) and a small explicit timestamp denylist (`as_of`, `generated_at`, `created_at`, …). Decimals are folded to float so a value read back from DDB fingerprints the same as the freshly-computed float.
- The cache is one DDB row per `(coach_id, output_type)` under `USER#matthew#SOURCE#coach_gen_cache`, holding the fingerprint, the stored output, and `first_generated` (the unchanged-since date) + `reuse_count`. On a hit the pipeline records the reuse, emits `LifePlatform/AI::GenerationSkippedUnchanged{Coach}`, still invokes the state-updater with the reused text (so today's coach record + thread state exist exactly as after a real generation — only the Bedrock generation is skipped), and returns. On a miss it generates as before and stores the fresh output under its new fingerprint.

**The honesty invariant (load-bearing).** The fingerprint covers *every* semantic input, so any change busts the cache — a new vitals number, a stance edit, and explicitly **a staleness day-count ticking up** (`gap_days` lives in the brief and is not stripped). There is no path by which reuse serves stale-but-claiming-fresh output; that is asserted directly in the tests. The strip list is deliberately conservative because the failure modes are asymmetric: a *missed* strip merely regenerates (no savings, no harm), while an *over-eager* strip could serve stale output as fresh — so when unsure, the field stays in the hash. Reuse is also entirely fail-soft: any error in the cache read/write path falls through to a normal generation.

**Metric.** `GenerationSkippedUnchanged` (count/day, per coach) makes the saving falsifiable in the spend attribution — the "regenerations-skipped/day" the story asks for.

**Consequences.** `generation_cache.py` is registered in `build_layer.sh` and rides a layer bump → `LifePlatformCore` publish → coach-consumer redeploy (CONVENTIONS §1); missing it from the layer would silently no-op the reuse path (the personal_baselines-layer-outage failure class). No change to the state-updater contract (the added `unchanged_since` payload field is optional and ignored by its `.get()` parsing). No schema migration — the cache partition is self-healing (a fingerprint mismatch just regenerates). The grounding + quality gates are not weakened: a reused output was already gate-passed for byte-identical inputs, and re-running a partly non-deterministic gate on known-good text could only introduce spurious failures. Pairs with ADR-125 (audience-ordered tiers) and #409 (batch pricing) under epic #719.

## ADR-127: Make "0 flags" falsifiable — a golden-brief eval harness with seeded-fault canaries (E6.1, #742)

**Date:** 2026-07-06 · **Status:** Accepted · **Story:** #742 (epic #720, R21 definitive review — the falsifiable honesty layer)

**Context.** The platform advertises "N checks, 0 flags" on every AI narrative surface — the grounded-generation number gate (ADR-104), the tight vital-contradiction detector (`grounding_guard`, SS-10), and the coach quality gate (ADR-108). All three are *runtime* gates that fire during generation and then discard their work. Nothing ever proved they could catch anything: a "0 flags" report was indistinguishable from a gate that was silently disabled (a failed `grounding_guard` import fails *open* — the number gate still runs, but the contradiction check just returns nothing). "0 flags" was therefore **unfalsifiable** — the exact weakness the R21 review flagged: a moat you can't demonstrate is a moat you can't safely change.

**Decision.** A frozen golden set replayed through the *same* deterministic gate the live pipeline uses, asserting two properties a real gate must have — and a mutation-testing canary suite that proves the flags can fire on demand.

- **`tests/golden_brief_eval.py`** — the harness. ~30 frozen `(coach_id, authoritative_facts, generation_brief, reference_output)` fixtures spanning all 8 coaches (`tests/fixtures/golden_briefs/golden.json`), each a known-good, fully-grounded output. It runs `grounded_generation.grounding_findings` (contradiction + evidence-ceiling number gate) plus a deterministic anti-pattern check (each coach's own `phrase_blacklist` + a universal vendor/fourth-wall guardrail) over every fixture and asserts **zero** findings — the gate must not false-flag honest voice.
- **Seeded-fault canaries** (`canaries.json`) — five fixtures that each *induce* a fault: a fabricated vital (HRV 58 where none exists), a hard contradiction (recovery 30% vs canonical 64%), an invented bodyweight, a two-point fabricated trend, and blacklisted anti-pattern phrases. The harness asserts each is **caught by the expected check**. A canary the gate fails to catch fails the whole run — this is what converts "0 flags" from a claim into a tested property. The five span all three deterministic dimensions so no check can rot undetected.
- **Cross-coach distinctiveness** — a deterministic content-word Jaccard over the golden outputs flags any different-coach pair above 0.55, so the voices can't silently converge into one generic voice (the real set peaks at 0.20).
- **Advisory Haiku voice rubric** (`--judge`) — trends one weekly `mean_voice_score`; **advisory only**, mirroring `ai_quality_canary_lambda` / ADR-076: the deterministic checks alone drive the pass/fail verdict, the LLM judge never gates.

**Gating.** `tests/test_golden_brief_eval.py` carries the `deploy_critical` marker (ADR-117), so the deterministic verdict runs in the main pipeline's deploy-critical lane and **blocks the deploy** on any golden false-positive or canary miss — a prompt/model/gate change now replays the golden set before it can ship. A separate weekly workflow (`.github/workflows/golden-brief-eval.yml`) re-runs it on a schedule to catch drift in quiet weeks and surfaces the ops line in the run summary.
...
## ADR-131: One code-distribution channel — retire the shared layer, bundle the whole tree (#781, 2026-07-06)

**Context.** Shared code shipped over THREE channels simultaneously: (1) every CDK function bundled the entire `lambdas/` tree as its code asset, (2) the same modules were also published as the pinned `life-platform-shared-utils` layer (a hand-curated ~60-module allowlist in `build_layer.sh`), and (3) per-script zips (site-api's partial `web/` zip, the MCP staging, CI's single-file `deploy_lambda.sh`) re-implemented packaging a third way. Since `/var/task` precedes `/opt` on `sys.path`, the bundled copy shadowed the layer at runtime for CDK-deployed functions — the pin was not the runtime truth — while script-deployed functions genuinely depended on the layer. Verified fleet state 2026-07-06: v115×13 / v116×1 / v118×~70 / none×11 against a pin of 118. Four incident classes trace to the channel multiplicity: the 2026-03-09 stale-layer P2, the #697 `personal_baselines` missing-from-allowlist outage, the #535/#538 site-api partial-zip import breaks, and the recurring MCP zip missing `reading/` (CI's own MCP step still shipped without it).

**Decision.** Collapse to ONE channel: the staged full-tree bundle, built by the single implementation `deploy/build_bundle.py` (the whole `lambdas/` tree + `config/food_vocabulary.json` at the root; the MCP shape adds `mcp_server.py` + `mcp/`). Every deploy path stages through it: the CDK asset (`lambda_helpers.staged_tree_asset()`), `deploy_lambda.sh` (now always full-bundle; `--extra-files` obsolete; MCP no longer rejected), the new `deploy_fleet.sh` (one zip → S3 → every function, with a live-handler-resolves-in-bundle guard), and `deploy_site_api.sh` (no more layer attach). The layer resource, `SHARED_LAYER_VERSION`, `build_layer.sh`, the `lambda_map.json` `shared_layer` block, and the CONVENTIONS §1 rebuild→publish→deploy ritual are retired. CI: the layer-consistency checks invert to a single invariant — **zero functions reference `life-platform-shared-utils`** (plan job + `test_i2_shared_layer_retired` + `session_postflight`) — and a changed `lambdas/` file with no per-function mapping triggers an automatic fleet deploy (closing the old silent gap where unmapped helpers deployed to nobody). Dependency layers with real third-party packages (garth, pillow) are explicitly NOT the shared layer and remain.

**Consequences.** Layer-version drift is structurally impossible (there is no version to drift); the four incident classes above die with it, as do the `cdk_only`-for-imports CI skips and the curated consumers list. Cost: every code deploy uploads ~1.5 MB zipped (unchanged for CDK functions, which always bundled the tree); a shared-module change now redeploys ~90 functions (~one S3 upload + N API calls) instead of relying on a consumers list that under-counted. Old layer versions remain published in AWS (the resource had `RemovalPolicy.RETAIN`) but nothing references them. Follow-ups: `lambda_map.json` `cdk_only` flags now mean "config changes need cdk deploy" only; #791's drift-reconciler scopes down to alarms + doc literals; #792 (fix the I2 layer-drift test) closes as superseded; #794 closes the residual site-api "dual ownership" doc/comment drift (stale pre-#781 comments in `operational_stack.py` and ADR-112 still described the script as the sole owner) and adds `tests/test_deploy_bundle_paths.py` to enforce structurally that CDK and `deploy_site_api.sh` stay on the one bundle channel.

## ADR-132: Batch inference for scheduled content — evaluated, deferred; the seam ships dormant behind the 100-record floor (#409, epic #344)

**Date:** 2026-07-07 · **Status:** Accepted (evaluated-deferred) · **Story:** #409 · **Epic:** #344 (the budget serves readers)

**Context.** AWS Bedrock prices batch inference (`CreateModelInvocationJob`) at ~50% of on-demand for the same Claude models, and most of our AI spend is scheduled, non-latency-sensitive content generation (coach narratives, daily-brief extraction, digests, podcast) — exactly the shape batch is built for. #409 proposed routing those producers through batch to halve the largest cost line. The #808 spend attribution (`scripts/ai_spend_attribution.py`) had correctly aimed #409 at the real top bucket — the coach pipeline + daily-brief extraction (Haiku), not the small standalone structured tools. Before building the pipeline we measured feasibility against three hard constraints:

1. **The floor.** Bedrock's `Minimum number of records per batch inference job` is **100, per job, per model** for every current Claude model (Haiku 4.5, Sonnet 4.6, Opus 4.x) — verified for this account via `aws service-quotas list-service-quotas --service-code bedrock`. It is not adjustable downward; a job with fewer than 100 records is rejected.
2. **The volume.** 30-day production telemetry (`LifePlatform/AI` `EstimatedCostUSD` SampleCount per producer, the metrics `bedrock_client._emit_usage_metrics` already emits) shows **~62 model calls/day across ALL scheduled producers combined, mixed across models**. Per model: Haiku ~31/day, everything else (mixed) ~28/day. The single largest producer (ai-expert-analyzer) is ~19.5/day; the daily brief ~10/day; coach narratives ~4.3/day. **No single model — not even the whole fleet's daily output for one model — reaches the 100-record floor.** Reaching it would require accumulating 3–4 days of a model's scheduled content into one job. Note this defeats even the #808-identified target directly: the Haiku bucket (coach pipeline + daily-brief extraction, the correct place to aim) totals only ~31 calls/day across every Haiku producer combined — a third of the floor — so batching *exactly* the bucket #808 flagged still cannot form a legal job on a daily cadence.
3. **The deadline.** Batch completes in "up to 24h" with no faster SLA. The overnight compute → 11 AM daily-brief chain cannot wait on a job that may not finish, so the deadline-critical producers would need a real-time fallback that, on any day it triggers, runs the full-price path anyway — negating the discount exactly when it is needed.

Multiplying it out (ADR-103 complexity posture): the machinery to batch honestly (rolling cross-producer accumulation queue, S3 input/output staging, job orchestration + polling, unordered-result reassembly, a Bedrock batch service role, and a real-time fallback) would earn **~$0/mo today** because no model crosses the floor, against a real carrying cost. The savings are not available at our volume; building the pipeline now would be unexercised machinery.

**Decision.** Do **not** wire batch into any producer. Ship the *mechanism* as a latent, fully-tested capability at the chokepoint — `lambdas/bedrock_batch.py` (`build_jsonl_record` / `batch_preflight` / `submit_batch` / `poll_batch` / `wait_for_batch` / `run_or_fallback` / `retrieve_results`) — whose load-bearing part today is the **eligibility floor**: `batch_preflight()` returns not-eligible below 100 records, and `run_or_fallback()` makes the real-time fallback a first-class, tested behavior. Each record is the same Messages body the real-time path would send (`_prepare_model_input` mirrors `invoke()`), so when the path lights up it passes through the identical budget-tier gate (tier-3 hard stop) and every grounding/quality gate — the cheaper path skips nothing (ADR-104/108). The interactive ask endpoints stay real-time by construction (they are excluded from the feasibility accounting and never call the batch seam). `scripts/batch_feasibility.py` makes the verdict a **living trip-wire**: it recomputes per-model calls/day vs the floor from live CloudWatch and prints FEASIBLE/INFEASIBLE.

**Trip-wire (enablement).** When `scripts/batch_feasibility.py` reports a single model's scheduled calls/day clearing ~120 (headroom over the 100 floor) — e.g. reader-facing content cadence scales, multi-user, or a new high-volume producer — enable batch for that producer by: (1) granting a Bedrock batch service role `s3:GetObject`/`s3:PutObject` on a dedicated batch prefix plus the caller `bedrock:CreateModelInvocationJob` + `iam:PassRole` (added deliberately at enablement, not speculatively — no unused grants ship now), (2) completing the batch branch in `run_or_fallback` (`submit_batch` → `wait_for_batch(deadline)` → `retrieve_results`, with the real-time fallback on deadline-miss), and (3) adopting `run_or_fallback` in the qualifying non-deadline producer. Until then the seam is dormant and costs nothing.

**Consequences.** #409 is answered with evidence rather than a pipeline that would save nothing at our scale. The batch seam and its floor are in the tree, tested (`tests/test_bedrock_batch.py`), and re-checkable (`scripts/batch_feasibility.py`) — so the day volume warrants it, enablement is a small, well-scoped change, not a from-scratch build. No new IAM, no new CDK, no producer behavior change ships with this ADR. Revisit is a one-command check, not a memory.

## ADR-133: Surge-mode ceiling — the budget floats to $100 on real reader traffic, never on spend creep; base raised $75 → $85 (E5.4, #739)

**Date:** 2026-07-08 · **Status:** Accepted · **Story:** #739 (epic #719, R21 definitive review) · **Amends:** ADR-063 (the monthly ceiling), ADR-125 (audience-based degradation ladder)

**Context.** The $75/mo ceiling (ADR-063) is calibrated for near-zero reader traffic. At any real traffic spike (e.g. a Reddit hit), the ask-endpoint + narrative AI spend that traffic itself generates would trip tier 2/3 and outage the reader-facing AI at the exact moment the platform succeeds — the opposite of what a success case should do. Pre-deciding the rule now (rather than improvising under a live spike) also keeps it separable from ADR-125's audience ladder: that ADR is about *what* pauses; this one is about *how much room* exists before anything has to.

**Threshold derivation.** `traffic_digest_lambda` already computes trailing-7-day unique visitors weekly (privacy-clean, IP-hashed-then-discarded) and, as of this story, also emits it as `LifePlatform/Traffic::UniqueVisitors7d` (CloudWatch). Real production readings at the time of this decision: **165 uniques** (week ending 2026-06-29) and **288 uniques** (week ending 2026-07-06, the most recent) — median of the two non-zero weeks ≈ 227. **SURGE_UNIQUES_THRESHOLD = 900** (env-overridable in `cost_governor_lambda.py`) is ~4x that median and ~3.1x the latest reading — clearly above organic levels at today's traffic, but low enough to catch a genuine surge (a single viral link routinely produces low-thousands of uniques/week) well before it collides with the ask-endpoint's own AI spend. Matthew's call on the ceiling itself: **SURGE_CEILING_USD = $100** (both are one-line env-var edits in `cdk/stacks/operational_stack.py`'s `CostGovernor` environment, or the code defaults in `cost_governor_lambda.py`, if the traffic baseline moves).

**Decision.** `cost_governor_lambda._effective_ceiling(recent_uniques)` returns `($100, True)` when the latest `UniqueVisitors7d` reading is ≥ 900, else `(base, False)` — including when the metric is missing/unreadable (fails closed to the normal base ceiling, never surge). `_tier_for`/`_decide_tier` now take the effective ceiling and scale the tier thresholds (calibrated against the original $75 in `_TIER_THRESHOLDS`, i.e. fixed bands of ≈73%/87%/97% of ceiling) proportionally, so a ceiling change doesn't change the *shape* of the degradation ladder, only the dollar amounts that trip it. Surge state is edge-triggered (SSM `/life-platform/surge-active`, mirroring the existing tier-alert pattern) so Matthew is emailed exactly on engage/disengage, not every 8h run, via the same `life-platform-alerts` SNS topic the tier-change alert already uses. The effective ceiling, `surge_active`, and `recent_uniques` are added to the existing `/life-platform/budget-breakdown` JSON (#822) so `budget_guard.format_headroom_line()` — the daily brief's one-line headroom readout — says so explicitly. **Isolation from spend creep (the scope's hard constraint):** surge is a pure function of `UniqueVisitors7d` alone; it is computed independently of `mtd`/`projected` and never reads them, so a dev-session or internal-AI spend spike cannot engage it — the ceiling stays at the base regardless of projection when uniques are below threshold.

**Amendment (2026-07-08, same session — base ceiling $75 → $85).** Decided by Matthew in response to the live tier-1 degradation caused by internal spend creep ($79.27 projected month-end vs the old $75 ceiling — dev-session burn, zero reader traffic involved). The BASE monthly ceiling is raised to **$85**; the surge ceiling stays **$100**. `MONTHLY_CEILING_USD` defaults to 85 in `cost_governor_lambda.py`; the AWS Budgets backstop (`cdk/stacks/core_stack.py`) is bumped to $85 — its name `life-platform-monthly-75` is now **historical** and deliberately NOT renamed (`budget_name` is the CfnBudget replacement key; renaming would delete + recreate the budget). Tier thresholds re-derive automatically as fractions of the ceiling: at $85 they trip at $62.33 / $73.67 / $82.73. **Honest note:** this raise does NOT by itself clear a tier-1 state driven by a ~$79 projection — tier 1 trips at ~73% of ceiling ($62.33), not at the ceiling; the tier clears when the trailing-7d burn rate decays, by design (the bands exist to degrade *before* the ceiling is hit).

**Consequences.** No IAM changes beyond one added SSM resource ARN (`role_policies.operational_cost_governor`, `ssm:GetParameter`/`PutParameter` on `/life-platform/surge-active`) — `cloudwatch:GetMetricStatistics` was already unscoped (`resources=["*"]`) so reading the new `LifePlatform/Traffic` namespace needed no policy change. `traffic_digest_lambda` already had `cloudwatch:PutMetricData` unscoped, so emitting the new metric needed no policy change either. Every existing call site of `_tier_for`/`_decide_tier`/`_alert`/`_write_breakdown` keeps working via a runtime-defaulted `ceiling` argument — the surge path stays dormant until reader traffic actually crosses 900 uniques/7d. Revisit the threshold as the baseline traffic grows — it is one env var, not a code change.
