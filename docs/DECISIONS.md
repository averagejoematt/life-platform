...
> 143 ADRs total (ADR-001 → ADR-145). (Some numbers were skipped/merged, so the record count and the max number diverge — see #817.)
...
- **Retention:** `journal_quotes` is RAW_TIMESERIES in `lambdas/phase_taxonomy.py` — consent artifacts are kept across resets like the journal they excerpt; they leave the surface only by explicit unmark.
