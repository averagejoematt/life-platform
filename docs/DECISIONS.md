...
5. **Telegram routing is registry data**: `telegram_route` on the persona (`persona_for_telegram_route()`), never `f"{route}_coach"` string surgery (eli_marsh broke it). The `training` route retires with the seat.
