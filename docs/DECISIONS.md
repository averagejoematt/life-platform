...
**Honest note on the first run after deploy.** The live reading at the time of writing is 1011 with surge ACTIVE. 1011 < 1188, and 1011 < 0.8·1188 = 950.4 is false — so hysteresis does **not** hold it: the first enforcement run after the deploy disengages surge, reverting the effective ceiling $252 → $215, emits one `SurgeFlip`, and (because that is the ≥2nd flip in 30 days) sends the flapping notice. That is the correct outcome — the current surge was minted by the mis-set bar — but it is a real ceiling tightening and is recorded here so it is not read later as an incident.
