...
| ADR-108 coach quality gate — scope (#968) | **Load-bearing on the two reader-facing coach-voiced surfaces; deliberately NOT extended further** | Added 2026-07-11 (#968): the gate covers the daily brief (regenerate-or-hold, ADR-108) and now the public board `/api/board_ask` initial + follow-up (regenerate-once-then-fail-open under a hard time budget — see the ADR-108 scope-extension note). `inter_coach_dialogue` + `coach_memoir` stay grounding-only ON PURPOSE: internal/narrative surfaces, lower stakes, and ADR-108's 10.2% failure rate was measured on daily outputs — extrapolating it to those voices is unmeasured. `/api/ask` + `/api/explain` are narrator-voiced (no coach_id/voice spec), so a per-coach voice-fidelity gate cannot apply there; their ADR-104 grounding gates stand. Re-open only with a measured failure rate on the excluded voices |
