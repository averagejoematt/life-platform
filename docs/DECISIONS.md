...
**Context.** The 2026-07-27 amendment named the precondition for this flip: `scripts/check_backlog_hygiene.py` could only move from advisory to blocking once #1868's backfill brought the open corpus to zero violations and that state held. Measured immediately before attempting the flip: `python3 scripts/check_backlog_hygiene.py --advisory` → `OK — 56 open issue(s) satisfy the ADR-099 filing contract.` (56 open issues that session — down from the 68 the 2026-07-27 amendment measured and the 60 recorded earlier the same day; issues filed and closed in the interim moved the count, never the zero-violation result). This mirrors the ADR-108 promotion pattern: flip on a measured signal, not the calendar.
