...
**Consequences.** `log_evening_intake` gains only bridging hardening under #1484 (Pacific date keying to match the signed-link write path; observable idempotency via `previous_count`), never new fields. The unified evening flow (`.claude/commands/journal-interview.md`, evening variant) bridges the existing surfaces — it does not create new ones.
