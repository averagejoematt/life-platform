...
|------|----------|-------------|---------|
| 2026-09-01 | **The two daily prose-truth judges diffed and BOTH KEPT — complementarity measured, not assumed (#3369)**: the nightly qa-smoke leg (`qa_check_reader_truth.py`, Aug $3.33) and the daily visual-qa CI leg (`--reader-truth`, Aug $4.65) share one rubric (`reader_truth_qa.py`) but their surfaced claim sets over 10 paired days (2026-08-22 → 08-31, n=10 runs per leg, zero budget pauses in either) overlap at 3 of 30 day-level (page, category) claims (10%) on the 4 pages both judge — and on note inspection all 3 nominal matches are different sentences (the CI leg flagged the home "logs gone quiet N days" arithmetic; the Lambda flagged the "Day-1 weigh-in / 90 days" design-copy class), so substantive claim overlap in the window is 0. The divergence is structural, not stochastic: 6/10 of the Lambda leg's claims sit on surfaces the CI leg cannot see (5 `/api/*` payloads + the frozen journal artifacts), 258/287 (90%) of the CI leg's sit on the ~64 rendered pages the Lambda never fetches, the text sources differ (static-HTML tag-strip vs Playwright innerText — JS-rendered content is CI-only), and the channels differ (nightly alarm/digest between deploys vs CI advisory issue) | $0 (declined the ≤$3.33/mo merge — the diff shows ~$8/mo buying measured non-duplication) | Do not re-flag the pair as duplication in a future cost review without a NEW claim-set diff showing material overlap. Evidence: the #3369 PR (10× visual-qa `report.json` `truth_findings` vs qa-smoke CloudWatch reader-truth lines, same window). |
