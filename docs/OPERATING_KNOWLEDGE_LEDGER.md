...
**Files in the memory index snapshot: 489** — feedback 25 · reference 286 · security 1 · project 173 · user 1 · index 3

**Rule-class files (feedback + reference + security): 312** — homed-here 82 · already-homed 193 · superseded 7 · narrative 21 · off-repo 9

**Program/session files (project): 173** — already-homed 14 · superseded 1 · narrative 155 · off-repo 2 · index 1
...
project_session_ar_2026_09_23.md
...
reference_a_ten_minute_bash_cap_kills_the_pr_watcher.md
...
reference_a_ttl_zero_cache_policy_cannot_carry_a_key_entry.md
...
reference_agent_commit_reverts_platform_counts_even_when_the_branch_owns_it.md
...
reference_an_md5_on_the_source_is_not_proof_the_mutation_ran.md
...
reference_auto_mode_escalates_private_s3_config_reads.md
...
reference_dispatch_pat_expiry.md
...
reference_unattended_lambda_invoke_stall.md
...
| `reference_a_ttl_zero_cache_policy_cannot_carry_a_key_entry.md` | reference | `tests/test_cloudfront_viewer_address_policies_1221.py` (`test_a_cache_policy_with_a_key_entry_has_caching_nominally_enabled` — every CachePolicy in `cdk/stacks/web_cloudfront_policies.py`: a key entry needs `max_ttl >= 1`, a `max_ttl 0` needs an empty key; the CloudFront constraint #3760's first Web deploy found by rolling back) | homed-here |
...
| `project_session_al_2026_09_19.md` | project | session record — the Fable drain's numbers, the merge posture's two reds, the audit's 194 → 35 split; the RULES it produced are homed in their own reference rows | narrative |
| `reference_a_pr_green_is_not_a_main_green_when_a_gate_keys_on_the_ci_event.md` | reference | `tests/test_doc_drift_date_stamp_2649.py` (`_gate_env()` builds the gate subprocess's env explicitly — `GITHUB_REF` removed, `GITHUB_EVENT_NAME` pinned — and one test asserts the push-to-main branch on purpose) | already-homed |
| `reference_a_visual_gate_failure_silently_reverts_every_site_deploy.md` | reference | incident narrative (09-16 → 09-19: three days of site merges published then auto-rolled-back by the #3652 scope check working as designed); the rule — read the LAST site-deploy's conclusion before merging `site/**` — is a driver reflex the `/land` skill's verify step already states, so this file keeps only the measurement | narrative |
| `reference_agent_commit_reverts_platform_counts_even_when_the_branch_owns_it.md` | reference | `deploy/agent_commit.sh` (the restore block's own comment: platform_counts.py is refused outright and diffed against the merge-base — the sync output is committed with a plain `git commit`, hook intact) | already-homed |
...
| `INDEX_deployment.md` | index | — the memory index itself (the Deployment section's roll-up annex) | index |
...
| `reference_dispatch_pat_expiry.md` | reference | `docs/RUNBOOK.md` (the PAT rotation section: scopes, the 2027-09-23 expiry, 401 = expired vs 403 = scope, the silent-read recipe — PR #4145) | already-homed |
| `reference_unattended_lambda_invoke_stall.md` | reference | — off-repo: the cause is an unanswered Claude Code permission prompt; the event is the 2026-09-24 row in `docs/INCIDENT_LOG.md` | off-repo |
