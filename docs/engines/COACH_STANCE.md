...
N-06 #390). The `coach-quality-gate` Lambda (`lambda_handler`, :929-1060) is **no longer a pure
...
verdict). The Haiku rubric (`QUALITY_GATE_SYSTEM_PROMPT`, :608-674) carries five criteria:
...
**The four ways a draft can fail** (`_run_quality_gate`, :853-921), in the order they apply:

| # | mechanism | source | binding? |
|---|---|---|---|
| 1 | `_apply_number_grounding_verdict` (:821-850) forces `passed=False` | deterministic (#2573) | **structural — the model cannot overrule it** |
...
| 4 | the judge itself cannot run — `_build_fallback_report` (:789-813) | infrastructure | yes — **fail-closed since #3083** (owner decision 2026-08-29) |
...
- **Fabricated numbers — BLOCKING** (`_number_grounding_report`, :457-511). Consumes
  ...
  **That last clause was false until #3202, and the gap held two reader-facing coaches dark for
  ...
  `grounding_findings=[<type>: <detail>]` (`_grounding_findings_summary`, :514-540) on a hold and
  ...
  **How the allow-list gets here.** The gate cannot derive it (that is built from
  ...
  (`_number_grounding_block`, :543-569: *"This verdict is computed by deterministic code, not by
...
(`_NUMERIC_FINDING_TYPES`, :818). The blocking rule itself is untouched: any finding still sets
