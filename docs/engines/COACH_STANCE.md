...
artifacts, never raw physiological values** (`_RAW_VITAL_RE`, :1131-1136):
...
2. Scored track record from `LEARNING#` verdicts + per-subdomain `CONFIDENCE#` records
   (`_summarize_track_record`, :1230-1236 — a thin binder since the #2428 split; the rollup
...
One Haiku call (`STANCE_SYSTEM_PROMPT`, :1146-1182) emits JSON: `headline_read`,
...
1. **Raw-vitals regex** (`_RAW_VITAL_RE`, :1131-1136): a stance must never cite numbers (bpm, ms,
...
2. **Change-claim sanitizer** (`_sanitize_stance`, :1291-1308): `how_my_read_changed` is blanked
   unless grounded in a logged correction or a real stage shift vs the prior stance; first run ⇒
   always blank.
3. **ADR-104 grounded-generation gate** (#534, `_apply_grounding_gate`, :1331-1402): the shared
   ...
   **#2195, the behavioral class.** This was the one surface #2056's census left deferred: the
   ...
   `STATE#current` singleton, hoisted above the 8-coach loop (`_presence_signal`, :348-370), so
...
`_write_stance` (:1469-1476): `pk COACH#<coach_id>` / `sk STANCE#<date>` (immutable history)
...
N-06 #390). The `coach-quality-gate` Lambda (`lambda_handler`, :819-944) is **no longer a pure
...
verdict). The Haiku rubric (`QUALITY_GATE_SYSTEM_PROMPT`, :508-574) carries five criteria:
...
**The three ways a draft can fail** (`_run_quality_gate`, :744-811), in the order they apply:

| # | mechanism | source | binding? |
|---|---|---|---|
| 1 | `_apply_number_grounding_verdict` (:712-741) forces `passed=False` | deterministic (#2573) | **structural — the model cannot overrule it** |
...
substitutes `_build_fallback_report` (:689-704) — `passed: True`, `score: 50`, one `suggestions`
...
- **Fabricated numbers — BLOCKING** (`_number_grounding_report`, :357-411). Consumes
  ...
  **That last clause was false until #3202, and the gap held two reader-facing coaches dark for
  ...
  `grounding_findings=[<type>: <detail>]` (`_grounding_findings_summary`, :414-440) on a hold and
  ...
  **How the allow-list gets here.** The gate cannot derive it (that is built from
  ...
  (`_number_grounding_block`, :443-469: *"This verdict is computed by deterministic code, not by
...
- **Self-repetition — advisory** (`_self_repetition_report`, :302-354, #2350). Delegates to
...
# ai_calls.py:1123, conversion at :1957): a deliberate hold is TERMINAL for the domain —
...
report's `suggestions`, which `_quality_gate_correction_note` (:1216-1249) already walks — so the
...
(`_NUMERIC_FINDING_TYPES`, :709). The blocking rule itself is untouched: any finding still sets
...
> **Verified against `lambdas/coach/coach_history_summarizer.py`, `lambdas/coach/coach_stance.py`, `lambdas/ai/ai_calls.py`, `lambdas/coach/coach_quality_gate.py` (plus `lambdas/ai/quality_gate_contract.py` and `lambdas/coach/coach_summarizer_support.py`, cited but not gate-tracked) @ git `868e009dd` on 2026-08-27 (#3085 — one comment-only change to `coach_quality_gate.py`, not material; prior `cd5af8005` on 2026-08-24, #2893 lineage). Every line span above was re-derived from the AST of the file at that sha, not carried forward from the prior stamp — **all 27 citations were checked and 18 had drifted**: eight pre-existing (`coach_history_summarizer.py`, uniform +1 from #2811's import at :162, missed by the 08-25 stamp) and ten from this change (`coach_quality_gate.py`, uniform +8 below :175). All are corrected; the checker reports zero remaining drift. This footer was itself two verifies stale (it still named the 08-24 sha after the 08-25 and 08-26 entries landed) and is brought forward here.**
