...
N-06 #390). The `coach-quality-gate` Lambda (`lambda_handler`, :842-967) is **no longer a pure
...
verdict). The Haiku rubric (`QUALITY_GATE_SYSTEM_PROMPT`, :521-587) carries five criteria:
...
**The four ways a draft can fail** (`_run_quality_gate`, :766-834), in the order they apply:

| # | mechanism | source | binding? |
|---|---|---|---|
| 1 | `_apply_number_grounding_verdict` (:734-763) forces `passed=False` | deterministic (#2573) | **structural — the model cannot overrule it** |
| 2 | `score < PASS_SCORE_THRESHOLD` (= 60, coach_quality_gate.py:98) | the judge's number | yes |
| 3 | the judge's own `passed: false` at a score ≥ 60 | the model's boolean | yes |
| 4 | the judge itself cannot run — `_build_fallback_report` (:702-726) | infrastructure | yes — **fail-closed since #3083** (owner decision 2026-08-29) |
...
removed that trigger by raising the cap to `QUALITY_GATE_MAX_TOKENS` (:114) and metering
...
— `VOICE_DISTINCTIVENESS_MINIMUM`, :99 — it does not fail a draft.)
...
- **Fabricated numbers — BLOCKING** (`_number_grounding_report`, :370-424). Consumes
  ...
  **That last clause was false until #3202, and the gap held two reader-facing coaches dark for
  ...
  `grounding_findings=[<type>: <detail>]` (`_grounding_findings_summary`, :427-453) on a hold and
  ...
  **How the allow-list gets here.** The gate cannot derive it (that is built from
  ...
  `generation_brief` via `ai.quality_gate_contract.brief_with_grounding` (:106-133), keeping the
  ...
  (`_number_grounding_block`, :456-482: *"This verdict is computed by deterministic code, not by
...
- **Self-repetition — advisory** (`_self_repetition_report`, :315-367, #2350). Delegates to
...
(`_NUMERIC_FINDING_TYPES`, :731). The blocking rule itself is untouched: any finding still sets
