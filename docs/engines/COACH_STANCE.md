...
verdict). The Haiku rubric (`QUALITY_GATE_SYSTEM_PROMPT`, :469-535) carries five criteria:
...
**The three ways a draft can fail** (`_run_quality_gate`, :694-761), in the order they apply:

| # | mechanism | source | binding? |
|---|---|---|---|
| 1 | `_apply_number_grounding_verdict` (:668-691) forces `passed=False` | deterministic (#2573) | **structural — the model cannot overrule it** |
...
substitutes `_build_fallback_report` (:650-665) — `passed: True`, `score: 50`, one `suggestions`
...
- **Fabricated numbers — BLOCKING** (`_number_grounding_report`, :347-401). Consumes
  ...
  `generation_brief` via `ai.quality_gate_contract.brief_with_grounding` (:77-104), keeping the
  ...
  (`_number_grounding_block`, :404-430: *"This verdict is computed by deterministic code, not by
...
- **Self-repetition — advisory** (`_self_repetition_report`, :294-344, #2350). Delegates to
...
**Regenerate-or-hold** (`ai_calls._enforce_quality_gate`, :1421-1486):
...
# ai_calls.py:1254, conversion at :2135): a deliberate hold is TERMINAL for the domain —
...
> **Verified against `lambdas/coach/coach_history_summarizer.py`, `lambdas/coach/coach_stance.py`, `lambdas/ai/ai_calls.py`, `lambdas/coach/coach_quality_gate.py` (plus `lambdas/ai/quality_gate_contract.py` and `lambdas/coach/coach_summarizer_support.py`, cited but not gate-tracked) @ git `b8b29416a` on 2026-08-23 (#2893; prior `b46dd4c98` on 2026-08-15, #2668). Every line span above was re-derived from the AST of the file at that sha, not carried forward from the prior stamp — seven of them had drifted and are corrected.**
