...
artifacts, never raw physiological values** (`_RAW_VITAL_RE`, :1132-1137):
...
2. Scored track record from `LEARNING#` verdicts + per-subdomain `CONFIDENCE#` records
   (`_summarize_track_record`, :1231-1237 — a thin binder since the #2428 split; the rollup
...
One Haiku call (`STANCE_SYSTEM_PROMPT`, :1147-1183) emits JSON: `headline_read`,
...
1. **Raw-vitals regex** (`_RAW_VITAL_RE`, :1132-1137): a stance must never cite numbers (bpm, ms,
...
2. **Change-claim sanitizer** (`_sanitize_stance`, :1292-1309): `how_my_read_changed` is blanked
   unless grounded in a logged correction or a real stage shift vs the prior stance; first run ⇒
   always blank.
3. **ADR-104 grounded-generation gate** (#534, `_apply_grounding_gate`, :1332-1403): the shared
   ...
   **#2195, the behavioral class.** This was the one surface #2056's census left deferred: the
   ...
   `STATE#current` singleton, hoisted above the 8-coach loop (`_presence_signal`, :349-371), so
...
`_write_stance` (:1470-1477): `pk COACH#<coach_id>` / `sk STANCE#<date>` (immutable history)
...
N-06 #390). The `coach-quality-gate` Lambda (`lambda_handler`, :952-1084) is **no longer a pure
...
verdict). The Haiku rubric (`QUALITY_GATE_SYSTEM_PROMPT`, :630-696) carries five criteria:
...
**The four ways a draft can fail** (`_run_quality_gate`, :876-944), in the order they apply:

| # | mechanism | source | binding? |
|---|---|---|---|
| 1 | `_apply_number_grounding_verdict` (:843-873) forces `passed=False` | deterministic (#2573; + the labs-draw class, #4134) | **structural — the model cannot overrule it** |
| 2 | `score < PASS_SCORE_THRESHOLD` (= 60, coach_quality_gate.py:109) | the judge's number | yes |
| 3 | the judge's own `passed: false` at a score ≥ 60 | the model's boolean | yes |
| 4 | the judge itself cannot run — `_build_fallback_report` (:811-835) | infrastructure | yes — **fail-closed since #3083** (owner decision 2026-08-29) |

**Mechanism 4 was a fail-open until #3083** (#2893, 2026-08-23). When the Haiku reply will not
parse as JSON (or the call raises), `_call_haiku`'s output never becomes a verdict and
`_run_quality_gate` substitutes `_build_fallback_report` — which returned `passed: True`, so the
ADR-108 blocking gate green-lit a draft it never evaluated, silently (the Lambda's `Errors`
metric stays at 0.0). **Measured over the 30 days to 2026-08-23: 84 of 484 metered calls
(17.4%), on 24 of the 30 days.** The cause was truncation — `AnthropicOutputTokens` for
`coach-quality-gate` had `Maximum == 800.0` exactly, its own `max_tokens` cap — and #2893
removed that trigger by raising the cap to `QUALITY_GATE_MAX_TOKENS` (:125) and metering
`stop_reason == "max_tokens"` at the Bedrock chokepoint (`LifePlatform/AI TruncatedResponses`).
Re-measured after that fix (#3083's Session-F measurement): **0 of 42 evaluations fell back**
(rule-of-three ceiling ≤7.1%), which is what made fail-closed cheap enough to choose. Since
#3083 the fallback returns `passed: False`, `score: 50`, `_fallback: true`, and an honest
`reason` naming the infrastructure failure; the unjudged draft rides the SAME
regenerate-or-hold path as a failing verdict — one regeneration gives the judge a second chance
on a fresh draft, and a hold darkens only that coach's section (`CoachHold` is terminal
per-domain, #966), never the brief. Still fail-OPEN, deliberately and out of #3083's scope: the
caller's transport layer (`ai_calls._invoke_quality_gate_sync` — an unreachable gate Lambda
never judged anything) and the handler's own crash-path 500 (`passed: True`, a failure *around*
the gate rather than of its judge).
...
— `VOICE_DISTINCTIVENESS_MINIMUM`, :110 — it does not fail a draft.)

**The three deterministic detectors** (all run in `lambda_handler` before the Haiku call; the
first two report *honest absence* rather than a green verdict when they cannot run):

- **Fabricated numbers — BLOCKING** (`_number_grounding_report`, :457-511). Consumes
  ...
  **That last clause was false until #3202, and the gap held two reader-facing coaches dark for
  three cycles.** Same function, different *arguments*: this surface spreads
  `**cycle_gate_params(generation_date)`, arming the cycle-freshness classes
  (`stale_phase`/`stale_baseline`/`experiment_span`); the coach-v2 corrective-rewrite loop in
  `ai_calls._run_coach_v2_pipeline` did not. A freshness finding therefore met the BLOCKING gate
  having never been offered a rewrite — and `ai_calls`' own `baseline_freshness_findings` call is
  advisory and sits *after* the gate, so a HELD draft never reaches it either. #3202 spreads the
  same params into that loop, which is what makes the sentence above true rather than aspirational.
  Second consequence of the same asymmetry, worth naming because the report's field is called
  `number_grounding_violations`: **the blocking finding is frequently not a number.** On the
  2026-08-26 17:00Z brief the real held drafts (replayed offline out of `EVALRET#coach_brief`)
  returned `findings[*].type == ['stale_phase']` for `mind_coach` on both attempts and for
  `nutrition_coach` on its second — a phase-framing claim, not a digit.

  Observability, also #3202: the gate previously logged only the verdict word, so a hold could
  not be root-caused from CloudWatch without a re-run. The `COMPLETE` line now appends
  `grounding_findings=[<type>: <detail>]` (`_grounding_findings_summary`, :536-562) on a hold and
  names a non-`measured` status explicitly; a clean verdict appends nothing, so the line is
  byte-identical to its pre-#3202 form on a passing draft. It rides `COMPLETE` because that is the
  one exit every path reaches — both `_run_quality_gate` fallbacks return into `lambda_handler`.
  `_retain_coach_brief_flag` (moved to `ai/coach_brief_retention.py` by the same change) now retains
  the grounding findings, the allow-list and the canonical facts alongside the draft; it had been
  keeping the text without the reason, which is precisely why the drafts had to be reconstructed.

  **How the allow-list gets here.** The gate cannot derive it (that is built from
  ...
  `generation_brief` via `ai.quality_gate_contract.brief_with_grounding` (:161-188), keeping the
  ...
  (`_number_grounding_block`, :565-591: *"This verdict is computed by deterministic code, not by
...
- **Labs coach arranging a draw — BLOCKING** (`_labs_draw_findings`, :514-533, #4134). For
  `labs_coach` only: every sentence that arranges a lab draw, or instructs ahead of one, without
  naming a FUTURE panel is a `past_draw_scheduling` finding. The detector is
  `health.labs_draw_claims.schedules_a_past_draw`, the same function the nightly
  `coach_labs:truth` check runs (it was moved out of `operational.qa_check_coach_labs` so the two
  cannot drift). It needs no allow-list, so `_apply_number_grounding_verdict` blocks on
  `grounding["labs_draw_findings"]` whatever the numeric check's status; the finding lands in
  `suggestions` as `Grounding violation (past_draw_scheduling)`, and the regenerate-or-hold loop
  below is unchanged. Why: on 2026-09-23 the stored labs narrative said "Schedule the draw" a day
  after the owner ruled no draw is booked (#4052), and only the nightly, after publish, saw it.
- **Self-repetition — advisory** (`_self_repetition_report`, :402-454, #2350). Delegates to
...
**Regenerate-or-hold** (`ai_calls._enforce_quality_gate`, :1269-1339):

```
report = sync invoke coach-quality-gate      # fails OPEN on TRANSPORT errors only (#3083:
                                             #  a responding gate whose judge failed now
                                             #  reports passed=False and is enforced below)
                                             # (#2573: the brief now carries the caller's
                                             #  allow-list + canonical facts — ai_calls.py:1964)
while not passed and attempts < 1:           # _QUALITY_GATE_MAX_REGENERATIONS = 1
    regenerate with a corrective note built from the report's findings
                                             # (#3516: `regenerate_fn` is now the ONE shared
                                             #  `_regen_fn` closure every gate arm is handed —
                                             #  same user turn, same max_tokens=1000, same
                                             #  system slot. The state machine is unchanged;
                                             #  what changed is that five hand-copied argument
                                             #  lists became one.)
    re-score
...
# ai_calls.py:1131, conversion at :1971): a deliberate hold is TERMINAL for the domain —
...
report's `suggestions`, which `_quality_gate_correction_note` (:1226-1259) already walks — so the
corrective rewrite gets the specific number complaint without `ai_calls` learning a new field.

**What that channel actually said, until #3202.** `_apply_number_grounding_verdict` rendered
*every* finding as `Ungrounded number (<type>): …`, including the freshness and behavioural
classes — so a coach whose phase framing was wrong was told to fix a figure, and each rewrite went
hunting a number that was never wrong. Non-numeric classes now read `Grounding violation (<type>)`
(`_NUMERIC_FINDING_TYPES`, :840). The blocking rule itself is untouched: any finding still sets
`passed=False` before the score threshold, and #2573/ADR-104's "the model cannot overrule it" is
pinned by `tests/test_coach_quality_gate_number_grounding_2573.py`.

**The loop can fail to converge, and did.** `_QUALITY_GATE_MAX_REGENERATIONS = 1` assumes the
correction is satisfiable. Before #3202, `stale_phase` was not a self-location check but a "Day N"
token ban — it flagged every `day <digits>` token with no framing requirement, unlike its sibling
`stale_baseline`, which requires baseline framing nearby. So the note said "use Day 10", the model
complied, and the *comparative* clause it wrote next ("matters more at Day 10 than it did at Day
1") re-tripped the identical finding: attempt 2 reproduced attempt 1's verdict exactly, every
cycle. `stale_phase` is now scoped to self-location claims by two structural exclusions — the
correct day also being cited (for *earlier* tokens only; a wrong FUTURE day is never a
back-reference and still fires), and a span/comparison anchor governing the token. A **backward**
mis-location co-occurring with the correct day stays uncovered on purpose: it is ambiguous with a
legitimate back-reference, and ADR-105 sends a semantic question to the judge, not to a regex.
...
Five zero-AI checks run over a generated coach section *in `ai_calls` itself*, before the draft
is published. (The count read "three" until #1973's row was added without it, corrected
2026-08-13; it read "four" until #3516's row was added, corrected 2026-09-06. The heading's
"before the ADR-108 scorer" is loose and always was: #1896 and #3516 run at Steps 6.4 and 6.3,
i.e. just AFTER the scorer call — what they all share is that they are deterministic, live in
`ai_calls`, and gate the draft before a reader sees it.) They are cheap, never budget-paused (no tokens), and each answers a question that
...
| **paused-source attribution (#3516)** | **does the draft blame a sync failure for a source the REGISTRY says is paused or lag-by-design?** | **BLOCKING** |

**paused-source attribution** (`coach_brief_input_gate.enforce_source_facet_attribution`, wired
at Step 6.3 of `ai_calls._run_coach_v2_pipeline`, :1992-1999). Regenerate once, then HOLD — the
fifth `CoachHold` reason, `source_facet_misattribution`. The TRIGGER is structural and is the
whole point: only a source whose `ingestion.source_registry` row says `paused` (Garmin, ADR-074)
or carries a `stale_hours` override past the platform default (MacroFactor, 96h) can fire it, so
it cannot be armed by a phrase and cannot fire on a live source however the sentence is worded.
The detection INSIDE that trigger is a small sync-failure vocabulary and is not claimed to be
exhaustive; its positive controls are the two sentences that actually shipped on Day 0 of cycle
16 — "Garmin step data isn't syncing to my dashboard yet" and "MacroFactor isn't syncing yet …
it needs to be running by tomorrow" — for a source that cannot report at all and one that is
~24h behind by design. The prompt states the rule too (`coach_brief_input_gate.INVENTORY_RULE`,
carried in the same bytes as the inventory per #3107), so the gate is the backstop for an
instruction the model was given, not the only thing standing between a paused source and an
invented outage.
...
> **Verified against `lambdas/coach/coach_history_summarizer.py`, `lambdas/coach/coach_stance.py`, `lambdas/ai/ai_calls.py`, `lambdas/coach/coach_quality_gate.py` (plus `lambdas/ai/quality_gate_contract.py` and `lambdas/coach/coach_summarizer_support.py`, cited but not gate-tracked) @ `origin/main` `bcc0f766a` + the #3083 change itself, on 2026-08-29 (#3083 — `_build_fallback_report` fail-open → fail-closed, MATERIAL, ADR-108 amendment; prior `868e009dd` on 2026-08-27, #3085 lineage). Every line span above was re-derived from the AST of the file in this tree, not carried forward from the prior stamp — **all citations were checked and 24 had drifted**: sixteen pre-existing from the three post-stamp commits (`6a6ef3a1f` +7 at the top of `coach_quality_gate.py`; `9e2dd694f` +1 in BOTH `coach_quality_gate.py` :~155 and `coach_history_summarizer.py` :~257 — the 08-27 stamp's summarizer corrections went stale the same day; `05188e1ef` +8 in `ai_calls.py` :~190) and the rest from this change (`coach_quality_gate.py` net +15, `ai_calls.py` net +6, both itemized in the header). All are corrected; details and the old→new table are in the 2026-08-29 header entry.**
