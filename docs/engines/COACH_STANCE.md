...
N-06 #390). The `coach-quality-gate` Lambda (`lambda_handler`, :811-936) is **no longer a pure
...
verdict). The Haiku rubric (`QUALITY_GATE_SYSTEM_PROMPT`, :500-566) carries five criteria:
...
**The three ways a draft can fail** (`_run_quality_gate`, :736-803), in the order they apply:

| # | mechanism | source | binding? |
|---|---|---|---|
| 1 | `_apply_number_grounding_verdict` (:704-733) forces `passed=False` | deterministic (#2573) | **structural — the model cannot overrule it** |
...
substitutes `_build_fallback_report` (:681-696) — `passed: True`, `score: 50`, one `suggestions`
...
- **Fabricated numbers — BLOCKING** (`_number_grounding_report`, :349-403). Consumes
  ...
  **That last clause was false until #3202, and the gap held two reader-facing coaches dark for
  ...
  `grounding_findings=[<type>: <detail>]` (`_grounding_findings_summary`, :406-432) on a hold and
  ...
  **How the allow-list gets here.** The gate cannot derive it (that is built from
  ...
  (`_number_grounding_block`, :435-461: *"This verdict is computed by deterministic code, not by
...
(`_NUMERIC_FINDING_TYPES`, :701). The blocking rule itself is untouched: any finding still sets
