...
notion, habitify, eightsleep`) flattened to day-rows by `build_data_narrative` (:374-476).
...
hypotheses without a parseable spec are rejected (`validate_test_spec`, :578-625):

- `condition_metric`, `outcome_metric` — both from the fixed `SPEC_METRICS` vocabulary
  (:155-193, exactly what `build_data_narrative` emits — a spec can only reference values the
...
Other creation gates (`validate_hypothesis`, :490-542): required fields incl. confidence ∈
...
## The deterministic check (`evaluate_test_spec`, :628-731)
...
## Lifecycle (status machine, in `check_pending_hypotheses`, :1141-1216)
...
anything non-terminal older than 30 days ──▶ archived (hard expiry, `enforce_hard_expiry`, :545-569)
```
...
- `USER#matthew#SOURCE#hypotheses / HYPOTHESIS#<ISO-ts>` — the hypothesis + frozen spec +
  ...
  `deterministic_verdict`; written by `update_hypothesis_status`, :335-371). EXPERIMENT_SCOPED
  (wiped at reset).
- `USER#matthew#SOURCE#calibration / CALIB#<date>#<hypothesis_id>` — one row per resolution with
  stated confidence, outcome, effect + CI (`build_calibration_item`, :762-787). **CROSS_PHASE**:
...
- **#1411 (quarterly, piggybacked on this cron):** `refit_cross_pillar_effects` (:1318-1350)
...
`S3_BUCKET`. Generation/check thresholds are module constants (:115-197), most sourced from the
