...
### S-07 — daily-brief cache-write waste — ⚠️ touches shared layer (not a casual edit)
- See D-01: daily-brief writes prompt-cache blocks it never reads (once/day vs 5-min TTL). **Caveat:** caching is auto-applied to ALL system messages by `ai_calls.py:1171` (`lambdas/ai_calls.py`, a SHARED LAYER module). Suppressing it for daily-brief alone means adding a per-call opt-out param to `ai_calls` → blast radius across every AI caller + a layer rebuild (`bash deploy/build_layer.sh`) + `cdk deploy`. Worth it (recurring waste on the top-2 spender) but needs a deliberate layer-change session, not a one-liner.
