...
> Last update: 2026-03-12 (v3.7.4 — 150 MCP tools, 42 Lambdas, 31 modules, 19 data sources, 9 secrets, 42 alarms, 8 CDK stacks)

---

## Current State

- **Platform version:** v3.7.4
- **MCP Server:** 150 tools across 31-module package, serving health data through Claude Desktop + claude.ai + Claude mobile (1024 MB, 12 tools pre-cached nightly)
...
- **Lambdas:** 42 (13 ingestion + 1 webhook + 2 enrichment + 7 email/digest [incl. monday-compass] + 1 dropbox-poll + 1 inbound-email + 1 key-rotator + 1 character-sheet-compute + 1 adaptive-mode-compute + 1 daily-metrics-compute + 1 daily-insight-compute + 1 hypothesis-engine + 1 dashboard-refresh + 1 data-export + 1 qa-smoke + 1 data-reconciliation + 1 pip-audit + **1 dlq-consumer + 1 canary**)
- **Cost:** ~$10/month
- **Secrets Manager:** 9 secrets (`life-platform/whoop, garmin, withings, strava, eightsleep, ai-keys, todoist, notion, habitify`); `life-platform/api-keys` pending deletion ~2026-04-07
...
| Anomaly Detector v2.3 | Every day 9:05 AM | ✅ Live |
...
| COST-A | **CloudWatch alarm audit & pruning** | Audit all ~47 alarms for redundancy. Currently 3 overlapping alarm families exist per Lambda (e.g. `character-sheet-compute-errors` + `ingestion-error-character-sheet-compute` + `life-platform-character-sheet-compute-errors`). Target: consolidate from ~47 to ~30–35 without losing coverage by eliminating the `ingestion-error-*` duplicate layer. Potential savings: ~$1–2/month at current count; scales as more Lambdas are added. Also: 27+ alarms currently in ALARM state — resolve or silence stale ones to reduce alert fatigue. Context: analysis done 2026-03-09. | 1-2 hr | -$1–2/mo savings |
| COST-B | **Secrets Manager consolidation review** | Review whether the 3 low-sensitivity ingestion secrets (`life-platform/todoist`, `life-platform/notion`, `life-platform/dropbox`) can be merged into the existing `life-platform/ingestion-keys` secret. Each secret costs $0.40/month; consolidating 3→1 saves ~$0.80/month. Must validate security boundary acceptability given the P0 split done 2026-03-08. Do NOT consolidate OAuth secrets (Whoop, Withings, Strava, Garmin, Eight Sleep) — these require separate rotation. Context: analysis done 2026-03-09. | 1 hr | -$0.80/mo savings |
...
| Version | What | Date |
|---------|------|------|
| v3.5.2 | IC-19 complete — slow drift detection, sustained anomaly tracking, hypothesis-experiment bridge. 4 files upgraded, all 19 board conditions fulfilled. All deployed. | 2026-03-11 |
| v3.5.1 | Secrets hygiene (habitify restored), Partner email deploy, CDK deploy | 2026-03-11 |
...
| v2.77.1 | Housekeeping: State of Mind resolved (iPhone permissions toggle), Phase 4 reward machinery confirmed complete, reward ideation menu generated for Matthew + Partner. No code deployed. | 2026-03-05 |
...
| TB7-2 | **Partner email — verify real address is set** (was cutover in v3.5.7 per changelog; handover still says pending). | Sarah | 2 min | 🔴 |
