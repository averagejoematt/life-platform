...
| LifePlatformEmail | 8 Lambdas | partner-weekly, daily-brief, monday-compass, monthly-digest, nutrition-review, wednesday-chronicle, weekly-digest, weekly-plate |
...
### Option A — Partner Weekly Email (fully unblocked)
`partner-weekly-email` Lambda already exists in AWS. Start here:
```bash
aws lambda get-function-configuration --function-name partner-weekly-email \
  --region us-west-2 \
