...
- Progress photos (#3757): `USER#matthew#SOURCE#progress_photos` (index rows) over `s3://matthew-life-platform/raw/matthew/progress_photos/YYYY/MM/YYYY-MM-DD-<pose>.jpg` — front / side / back body photos captured from the phone. **Owner-only, with no public projection of any kind.** Unlike `hevy` or `macrofactor`, which are owner-only rows with a deliberate Tier-0 aggregate surface above them, nothing derived from a progress photo is served publicly — there is no aggregate, no thumbnail and no count on any page. The coaches read them for body-composition analysis through owner-only paths. Classed `CROSS_PHASE` (ADR-077): a reset must never wipe them, because a before/after spanning attempts is the entire point and the images are unrecomputable. Viewing is a dedicated Lambda (`progress-viewer`, its own role — no `site_api*` role may name `raw/`, #3757) behind a 24-hour single-use link (#3760) that the head-coach bot texts on `/progress view`; the link is exchanged once for a 12-hour signed cookie, and the page (`/progress-photos/`, Lambda-rendered — never a `site/**` object, since every byte there is anonymously readable at the S3 origin) issues 10-minute presigned GETs. There is no CloudFront route to the prefix itself, the route is in no sitemap, feed, nav or QA page registry (`tests/test_progress_viewer_privacy_3760.py` is what holds that), and the bucket policy's `raw/*` delete-protection covers it. Replicated cross-region (DIL-027) for the same reason as the rest of `raw/`: it cannot be recomputed from anything.
- Tape measurements (#3719): `USER#matthew#SOURCE#measurements` — waist, chest, arm and thigh circumferences, served on the physical surface. **Owner-published by the ADR-155 consent, dated 2026-09-13.** This row exists because they were already being served publicly with no tier declared at all — publication by omission, which is precisely what this registry exists to make impossible. The owner's ruling was to keep publishing them, so what changed is the stamp and not the surface.
- Owner pain-flag dismissals (#4036): `USER#matthew#SOURCE#training_constraints` / `DISMISSAL#<site>#<date>` — Matthew's own statement that a flagged body site is resolved, stored with the site he named, the date he said it and **his verbatim words**, plus the flag instance it dismisses (note date + movement). **Owner-only, with no public projection of any kind** — it is a sentence about his body and an injury history, and the only readers are the owner-facing planning surfaces (`plan_next_session`'s tripwire and the joints/tendons critic). Declared here before the first record exists, per ADR-154's ordering, so there is no window in which an owner statement about an injury is stored under no tier. Classed `CROSS_PHASE` (ADR-077): a reset must never wipe it, because wiping it would silently re-arm every site he has already said is resolved, and the input cannot be recomputed from anything.
- Named-human contact state (#4063): `USER#matthew#SOURCE#named_human_contact` / `STATE#current` — the episode de-dup and cooldown row for the disengagement check-in path (v0.3 §9/§13.3): the date Matthew last used the platform (the episode anchor), how many days quiet, and on which dates a check-in went out and in which mode (preview to the owner, or armed to the contact). **Owner-only, no public projection of any kind.** It carries no identity: the contact's name and address live ONLY in the private S3 object `config/coaching/named_human.json`, are read at send time, and are never written to DynamoDB, logs or this repo (`tests/test_named_human_contact_4063.py` greps the tree for the field shape). The email itself carries no health data — only that he has gone quiet and a nudge to check in. Declared before the first row exists (ADR-154's ordering). Classed `SYSTEM_STATE` (ADR-077): a reset must not wipe it, or the contact would be re-mailed for a stretch they were already told about.
- Chat pending-writes queue (#4078): `USER#matthew#SOURCE#pending_writes` / `PENDING#<ts>-<hash8>` — writes a chat coaching session proposed and Matthew has not yet approved or discarded: the target write tool's name, its **exact arguments** (stored as JSON text), a one-line summary and optional context. **Owner-only, with no public projection of any kind** — a queued row can carry anything a write tool accepts (journal-grade text, a statement about his body, a meal), so it takes the highest tier of what it may hold. The only readers are MCP (`manage_pending_writes`, `get_capture_queues`) and the nightly qa-smoke age check, which reports ids, tools and ages, never the arguments. Declared before the first row exists (ADR-154's ordering). Classed `SYSTEM_STATE` (ADR-077): a workflow buffer — an approved write lands in its target partition under that partition's own class. Open rows never expire; resolved rows carry a 90-day `ttl`.
...
| `reader_input/` (**third-party PII** — the two moderated reader-input doors, `/api/board_question` → `reader_input/board_questions/{id}.json` and `/api/submit_finding` → `reader_input/findings/{id}.json`; each record carries the reader's OPTIONAL `email` verbatim and an unsalted `ip_hash` (sha256(ip)[:16]) beside the text; #3559 / SEC-1) | **UNSIGNED — forever pending the owner's window.** No lifecycle rule ON PURPOSE: a moderation queue must not self-destruct under the owner (`uploads/`' 30-day expiry is why that prefix was not used). Not delete-protected, so the owner can purge a moderated record by hand; nothing is anonymously readable (no public-read bucket-policy statement, no CloudFront behaviour — `tests/test_reader_input_prefix_3559.py` derives both from `deploy/bucket_policy.json`). Until 2026-09-05 these records lived under `generated/` (anonymous `GetObject`, key derivable from the public answers feed); the five live objects were redacted in place that day and the write site moved. Open: salting `ip_hash` (a secret the doors do not yet hold) and a signed retention window | none (deliberate; see retention) |
...
| `deploys/` (Lambda deploy artifacts) | **Current: 30 days; non-current: 1 day (keep 1)** | Pre-existing (Expiration) + #2642 (`NoncurrentVersionExpiration` at 7d) + #3368 (tightened 7d → 1d) — `expire-lambda-deploy-artifacts` |
...
| 2026-09-14 | `recap/` added: the daily recap card moved OFF `generated/*` (anonymously readable under `PublicReadGenerated` at the S3 origin, with or without CloudFront). Eight daily cards and the weekly shipped there on deploy and were verified world-readable — http 200, byte-identical — at keys derivable from a date, with the prefix named in the public repo. Cards carry weight, cumulative loss, daily grade and habit misses by name. The exposed objects could not be DELETED (`ProtectDataFromDeployScripts` denies DeleteObject on `generated/*`) and were overwritten in place; the prefix moved, and `tests/test_public_write_prefix_registry_3741.py` now guards the SET — every write grant under an anonymously-readable prefix must declare that intent | #3741 |
| 2026-09-05 | `reader_input/` added (third-party PII, UNSIGNED): the two reader-input doors' moderation records moved OFF `generated/*` (anonymously readable under `PublicReadGenerated`; records carried `email` + `ip_hash`). Write site `web/site_api_capture_store.capture_key()`, serve-role grant, moderation script and the guard test moved as one set; existing objects are the owner's `aws s3 mv` | #3559 |
...
| 2026-08-31 | #3368 (fin-diligence): `deploys/` noncurrent-version expiry tightened 7d → 1d (keep-newest-1 retained). ~72GB of noncurrent deploy bundles measured under the 7d window (one-bundle fleet sweeps mint ~1.7GB each; 14 sweeps on 08-31 alone) and nothing reads them — the scripted rollback (`rollback_lambda.sh`, CI auto-rollback, `deploy_fleet.sh::seed_artifacts`) reads only the CURRENT `deploys/<fn>/previous.zip`; repo sweep found zero `list-object-versions`/VersionId S3 reads. Trade-off accepted: the MANUAL deep-rollback window (older versions via S3 versioning) shrinks to ~1 day + the 1 kept newest noncurrent generation; beyond that, re-deploy from a git ref | #3368 |
