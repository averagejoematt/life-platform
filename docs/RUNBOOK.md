...
Last updated: 2026-08-11 (v8.6.0 — 76 MCP tools, 38-module package, 103 Lambdas, 20 data sources)

**Ground truth (point-in-time values are drift — run the command instead):**
- Lambda functions defined (CDK): 103 — re-derive via `python3 deploy/sync_doc_metadata.py` (AST discoverers)
...
15. `restart_verify_rendered.py --old-genesis <outgoing>` — hard gate over the 96-URL v4 surface (apply mode only)
