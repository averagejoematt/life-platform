...
Last updated: 2026-09-24 (v8.6.0 — 85 MCP tools, 59-module package, 106 Lambdas, 20 data sources)
...
15. `restart_verify_rendered.py --old-genesis <outgoing>` — hard gate over the 97-URL v4 surface (apply mode only)
...
1. Filters to urgent alarms (substrings: `canary`, `dlq-depth`, `site-api-error`, `budget-tier`, `bedrock-throttle`, `slo-`). Routine ingestion-source errors stay non-urgent — the Mon/Wed/Fri ~10:35 PT sweep handles them.
...
# 4. Sync static configs to S3. The repo copy of all four is the authority, so pushing
#    it is a publish: three are hand-owned, and movement_catalog.json is repo-generated by
#    deploy/build_movement_catalog.py (#4108 — rebuild it there, never hand-grow it). A merge
#    that touches config/** also publishes them through site-deploy's config-twin step.
