...
permission regression. Partner weekly email unblocked and confirmed sending.
...
- **TB7-2 CLOSED** — Partner weekly email fully unblocked: set `PARTNER_EMAIL=[partner-address-redacted]`
...
- Partner email next scheduled run: Sunday 2026-03-15 at 09:30 PT.
...
## v3.6.4 — 2026-03-11: anomaly_detector v2.4.0 — HRV log-transform + doc corrections
...
## v3.6.3 — 2026-03-11: INTELLIGENCE_LAYER.md — Known Statistical Limitations section
...
### Changes
- **INCIDENT_LOG.md** — updated to v3.6.0 (12 versions behind); added 2 new incidents: Partner email zip-path-prefix bug (P2) + EmailStack missing layer v4 (P3); patterns section updated
...
## v3.5.7 — 2026-03-11: Partner email production cutover complete

### Partner Weekly Email — production cutover ✅
- `PARTNER_EMAIL` updated in `cdk/stacks/email_stack.py` to his partner's real address
...
## v3.5.5 — 2026-03-11: Partner email live; email stack layer v4 fix

### Partner Weekly Email — now live
- First successful invocation confirmed: `Matthew's Week · 2026-03-10` sent to `awsdev@mattsusername.com`
- Root cause of prior failures: (1) `deploy_obs1_ai3_apikeys.sh` used inline zip logic with path prefix — module landed at `lambdas/partner_email_lambda.py` inside zip instead of root; (2) EmailStack had no layer reference — all 8 email Lambdas were on layer v2 (missing `set_date`)
...
### Deployed
- `bash deploy/deploy_obs1_ai3_apikeys.sh` — all 5 Lambda code changes live (partner-weekly-email, journal-enrichment, habitify-data-ingestion, notion-journal-ingestion, todoist-data-ingestion)
...
- **NEW:** `partner_email_lambda.py` — added `get_logger("partner-weekly")` + `logger.set_date()` in handler (OBS-1 complete across all email Lambdas)

### AI-3 — ai_output_validator rollout status
- **Confirmed wired** in all email Lambdas (prior sessions)
- **NEW:** `partner_email_lambda.py` — added `validate_ai_output(commentary, AIOutputType.WEEKLY_DIGEST)` with `was_replaced` logging (AI-3 complete across all email Lambdas)
...
## v3.5.1 — 2026-03-11: Secrets hygiene, Partner email deploy, CDK deploy, sick day
...
### Partner email
- `cdk/stacks/email_stack.py`: `PARTNER_EMAIL` placeholder → `awsdev@mattsusername.com`
...
- `deploy/cdk_env_diff.sh`: two-direction diff before every CDK deploy
  ...
  - High-sensitivity vars (`EMAIL_RECIPIENT`, `ANTHROPIC_SECRET`, `AI_MODEL`, `PARTNER_EMAIL`, etc.) flagged explicitly
...
- Top items: wire ai_output_validator to all email Lambdas, OBS-1 logger rollout, ship Partner email, unit tests for shared modules
...
- 9 email methods: `_email_base()` helper + `email_daily_brief`, `email_weekly_digest`, `email_monthly_digest`, `email_nutrition_review`, `email_wednesday_chronicle`, `email_weekly_plate`, `email_monday_compass`, `email_partner`
...
- Email rules recovered: `daily-brief-schedule`, `weekly-digest-sunday`, `monthly-digest-schedule`, `nutrition-review-saturday`, `wednesday-chronicle-schedule`, `weekly-plate`, `monday-compass`, `partner-weekly-email-schedule`
...
- Key finding: `partner-weekly-email` Lambda already exists in AWS
...
- `shared_no_dlq` used for: `anomaly-detector`, `character-sheet-compute`, `dashboard-refresh`, `partner-weekly-email`
...
- `cdk/stacks/email_stack.py` — 8 email/digest Lambdas:
  ...
  - `monday-compass` (Mon 7 AM), `partner-weekly-email` (Sun 9:30 AM)
...
- `partner-weekly-email` used `lambda-weekly-digest-role` at create time
...
- `deploy/p3_attach_shared_utils_layer.sh` updated: 11 → 16 Lambdas (`+partner-weekly-email`, `+daily-metrics-compute`, `+adaptive-mode-compute`, `+dashboard-refresh`)
...
- `partner-weekly-email` → `life-platform-email-role`
...
- Privacy redactions: habit substance rows (shot02/05), weight numbers (shot06/16/18), Partner vulnerable content (shot07)
...
## v2.80.0 — 2026-03-06 — Partner Email Fixes & Design Refinements
...
## v2.79.0 — 2026-03-06 — Partner Weekly Email

### New Lambda: `partner-weekly-email` (30th Lambda)
- Weekly partner-focused email sent to Partner every Sunday 9:30 AM PT
...
- **The Chair**: His Body This Week — physical health board synthesis for Partner
...
- Model: Sonnet 4.6 · Schedule: Sunday 17:30 UTC (9:30 AM PT) · Recipient: `PARTNER_EMAIL` env var
- Warm, partner-focused HTML design (distinct from clinical weekly digest)

### Stats
- Lambdas: 29 → 30
- Files: `lambdas/partner_email_lambda.py`, `deploy/deploy_v2.79.0.sh`
...
- **Content:** Hand-written by Elena (not AI-generated), covers backstory (mum, Jo, reorg, Rolex), the "I don't know what I want" moment, Partner showing up with meals, Maslow's hierarchy framing, onboarding metaphor
...
**The Weekly Plate** — A personalized Friday evening food email (6:00 PM PT). Magazine-style couch read built from 14 days of MacroFactor data, designed to be enjoyable for both Matthew and Partner.
...
- **Bio fix:** Age corrected to 37, added Partner as girlfriend
...
- **Issue:** Prologue stated "thirty-five" and "lives alone" — should be "thirty-seven" and "lives with his girlfriend, Partner"
