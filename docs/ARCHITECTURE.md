...
Last updated: 2026-09-19 (v8.6.0 — 84 tools, 50-module MCP package, 20 data sources, 105 Lambdas, 26 secrets, 127 alarms, 10 CDK stacks deployed).
...
┌────────────────────────▼────────────────────────────────────┐
│  SERVE LAYER                                                │
│  MCP Server Lambda (84 tools, 768 MB) + Lambda Function URL │
...
CloudWatch carries **~127 metric alarms** (CDK-declared; the count is auto-discovered by `deploy/sync_doc_metadata.py` and the inventory is regenerated into `docs/MONITORING.md`).
...
**Lambda:** `life-platform-mcp` | **Tools:** 84 | **Memory:** 768 MB | **Runtime:** python3.12 | **Modules:** 50 (`mcp/tools_*.py` + helpers)
...
~/dev/life-platform/
  ...
  mcp/                            ← MCP server package (32 tool modules + helpers)
