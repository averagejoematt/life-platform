...
Last updated: 2026-09-15 (v8.6.0 — 83 tools, 47-module MCP package, 20 data sources, 105 Lambdas, 26 secrets, 125 alarms, 10 CDK stacks deployed).
...
┌────────────────────────▼────────────────────────────────────┐
│  SERVE LAYER                                                │
│  MCP Server Lambda (83 tools, 768 MB) + Lambda Function URL │
...
CloudWatch carries **~125 metric alarms** (CDK-declared; the count is auto-discovered by `deploy/sync_doc_metadata.py` and the inventory is regenerated into `docs/MONITORING.md`).
...
**Lambda:** `life-platform-mcp` | **Tools:** 83 | **Memory:** 768 MB | **Runtime:** python3.12 | **Modules:** 47 (`mcp/tools_*.py` + helpers)
...
~/dev/life-platform/
  ...
  mcp/                            ← MCP server package (30 tool modules + helpers)
