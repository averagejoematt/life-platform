...
Last updated: 2026-09-14 (v8.6.0 — 82 tools, 46-module MCP package, 20 data sources, 105 Lambdas, 26 secrets, 124 alarms, 10 CDK stacks deployed).
...
┌────────────────────────▼────────────────────────────────────┐
│  SERVE LAYER                                                │
│  MCP Server Lambda (82 tools, 768 MB) + Lambda Function URL │
...
CloudWatch carries **~124 metric alarms** (CDK-declared; the count is auto-discovered by `deploy/sync_doc_metadata.py` and the inventory is regenerated into `docs/MONITORING.md`).
...
**Lambda:** `life-platform-mcp` | **Tools:** 82 | **Memory:** 768 MB | **Runtime:** python3.12 | **Modules:** 46 (`mcp/tools_*.py` + helpers)
...
~/dev/life-platform/
  ...
  mcp/                            ← MCP server package (29 tool modules + helpers)
