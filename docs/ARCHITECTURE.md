...
┌────────────────────────▼────────────────────────────────────┐
│  SERVE LAYER                                                │
│  MCP Server Lambda (76 tools, 768 MB) + Lambda Function URL │
...
CloudWatch carries **~117 metric alarms** (CDK-declared; the count is auto-discovered by `deploy/sync_doc_metadata.py` and the inventory is regenerated into `docs/MONITORING.md`).
...
**Lambda:** `life-platform-mcp` | **Tools:** 76 | **Memory:** 768 MB | **Runtime:** python3.12 | **Modules:** 39 (`mcp/tools_*.py` + helpers)
