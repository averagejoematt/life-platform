...
┌────────────────────────▼────────────────────────────────────┐
│  SERVE LAYER                                                │
│  MCP Server Lambda (86 tools, 768 MB) + Lambda Function URL │
...
| CloudWatch | Alarms + logs | **~129 metric alarms**. Per-Lambda `ingestion-error-*` first-error alarms are retired across ingestion (2026-05-29), compute + email (#790/ADR-116, 2026-07-07 — 48 alarms) in favour of the shared `life-platform-ingestion-dlq` digest path (`life-platform-ingestion-dlq-messages` + `life-platform-dlq-depth-warning`). |
| CDK | Infrastructure as Code | `cdk/` — 10 stacks. CDK owns all Lambda IAM roles + 90 EventBridge schedule rules (`model/platform_model.json` `meta.counts.schedules`, guarded by `check_doc_facts.py`; the live account holds more rules than CDK declares). Stacks: `core_stack`, `ingestion_stack`, `email_stack`, `compute_stack`, `mcp_stack`, `operational_stack`, `serve_stack` (public serving path: site-api + site-api-ai — #793, split via `cdk refactor` 2026-07-08), `web_stack`, `monitoring_stack`, `backup_stack` (DIL-027/#3042 — the `raw/` cross-region replica bucket + replication role, **us-east-2**; the only stack outside us-west-2 other than `web_stack`'s us-east-1). |
...
**Prompt caching (COST-OPT-2, ADR-049):** Both `ai_calls.py` and `retry_utils.py` auto-wrap system messages as cached content blocks (`anthropic-beta: prompt-caching-2024-07-31`). 90% discount on repeated system prompt tokens — **but only above the model's minimum cacheable prefix, and a `cache_control` block is not evidence of caching**: below the floor the API accepts the marker and silently ignores it (no error, no warning, `cache_read_input_tokens` stays 0). Floors are per-model, non-monotonic across generations, and identical on Bedrock and the first-party API — **Haiku 4.5's is 4,096 tok, 4x Sonnet 4.6's 1,024**, so the cheap tier model tiering routes structured tasks to is also the hardest to cache. The registry, and the per-caller record of which callers cache vs. were measured and deliberately declined, is `lambdas/ai/prompt_cache.py` (`MIN_CACHEABLE_PREFIX_TOKENS`, `CACHING_DECISIONS`). CloudWatch metrics: `AnthropicCacheWriteTokens`, `AnthropicCacheReadTokens`, plus `PromptCacheNoOp` (#2888 — asked for caching, wire reported none). Model tiering: structured/templated tasks use Haiku (`AI_MODEL` env var), narrative content stays on Sonnet. All model assignments are env-var configurable for instant rollback.

| Source | Lambda | Schedule | Type |
|---|---|---|---|
| Whoop | `whoop-data-ingestion` | Hourly (active hours) | API pull |
| Garmin | `garmin-data-ingestion` | Paused (ADR-074) — no live rule | API pull |
...
> **SIMP-2 cohort (8 of 15 ingestion Lambdas, ADR-056):** `whoop`, `garmin`, `strava`, `withings`, `eightsleep`, `habitify`, `todoist`, `weather`. All `import from ingestion_framework`. The 7 pattern-exempt ingestion functions are: `notion`, `macrofactor`, `dropbox-poll`, `food-delivery-ingestion`, `health-auto-export-webhook` (HAE near-real-time), `measurements-ingestion` (a separate Lambda from the HAE webhook), and `hevy-backfill` (hourly, all 24 hours — ADR-060; #3720 widened it from 12–23 UTC). (`apple_health` direct ingestion is RETIRED — ADR-103/#474; HAE is the Apple Health path now. Not all 15 are EventBridge-scheduled: macrofactor/food-delivery-ingestion/measurements-ingestion are S3-triggered, HAE is webhook-triggered, and garmin — though SIMP-2 — is currently PAUSED with no live rule, ADR-074. Live cadence + paused state per source: `lambdas/ingestion/source_registry.py`.)
>
...
CloudWatch carries **~129 metric alarms** (CDK-declared; the count is auto-discovered by `deploy/sync_doc_metadata.py` and the inventory is regenerated into `docs/MONITORING.md`).
...
**Lambda:** `life-platform-mcp` | **Tools:** 86 | **Memory:** 768 MB | **Runtime:** python3.12 | **Modules:** 62 (`mcp/tools_*.py` + helpers)
...
**Rate limiting:** DynamoDB-backed per-IP atomic counters (`lambdas/common/rate_limiter.py`, since Phase 2.1 — survives warm-container distribution). The module-level dicts `_ask_rate_store`/`_board_rate_store` are the in-memory fail-open fallback only (used if the DDB limiter is unavailable). Vote/follow rate limits also use DynamoDB atomic counters with TTL. Role is primarily read-only with limited writes for interactive features (votes, follows, checkins).
...
~/dev/life-platform/
  ...
  mcp/                            ← MCP server package (34 tool modules + helpers)
