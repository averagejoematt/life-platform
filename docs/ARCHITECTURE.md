...
Last updated: 2026-09-24 (v8.6.0 — 85 tools, 59-module MCP package, 20 data sources, 106 Lambdas, 26 secrets, 128 alarms, 10 CDK stacks deployed).
...
┌────────────────────────▼────────────────────────────────────┐
│  SERVE LAYER                                                │
│  MCP Server Lambda (85 tools, 768 MB) + Lambda Function URL │
...
CloudWatch carries **~128 metric alarms** (CDK-declared; the count is auto-discovered by `deploy/sync_doc_metadata.py` and the inventory is regenerated into `docs/MONITORING.md`).
...
**Lambda:** `life-platform-mcp` | **Tools:** 85 | **Memory:** 768 MB | **Runtime:** python3.12 | **Modules:** 59 (`mcp/tools_*.py` + helpers)
...
~/dev/life-platform/
  ...
  mcp/                            ← MCP server package (33 tool modules + helpers)
